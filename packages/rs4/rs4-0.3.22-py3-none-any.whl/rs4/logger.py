...
USERLIB_DIR = os.path.dirname (os.path.dirname (asyncore.__file__))
if USERLIB_DIR.endswith ("rs4"):
    USERLIB_DIR = os.path.dirname (USERLIB_DIR)
LIB_DIR = os.path.dirname (codecs.__file__)
LIB_SITE_DIR = os.path.join (os.path.dirname (codecs.__file__), "site-packages")

def traceback (multirows = False):
    (file, fun, line), t, v, tbinfo = asyncore.compact_traceback ()
    try: v = str (v)
    except:    v = repr (v)
    # if ISATTY and len (tbinfo) > 1000:
    #     tbinfo = tbinfo [:960] + '...\n  - ... (further details are omitted)\n'
    if multirows:
        c = "%s %s Traceback: %s" % (t, v, tbinfo)
        c = c.replace ("] [", "\n  - ").replace ("Traceback: [", "\n  -----------\n  + Traceback\n  ===========\n  - ").replace (LIB_SITE_DIR, '${SPLIB}').replace (USERLIB_DIR, '${LCLIB}').replace (LIB_DIR, '${LIB}')
        c = c [:-1] + "\n  -----------"
        c += '\n  * LCLIB=%s' % USERLIB_DIR
        c += '\n  * SPLIB=%s' % LIB_SITE_DIR
        c += '\n  * LIB=%s' % LIB_DIR
    else:
        c = "%s %s Traceback: %s" % (t, v, tbinfo)
    return c
...
class base_logger:
    ...
    def traceback (self, name = ''):
        return self.log (traceback (True), "expt", name)
    trace = traceback
...
class screen_logger (base_logger):
    def __init__ (self, cacheline = 200, flushnow = 1, colored = True):
        ...
        self.colored = sys.stdout.isatty() and colored
        self.hide_date = os.environ.get ("DAEMONIZER") == 'systemd'
    ...
    def log (self, line, type = "info", name = ""):
        ...
        if not self.colored:
            if self.hide_date:
                line = "{}{}\n".format (self.tag (type, name), line)
            else:
