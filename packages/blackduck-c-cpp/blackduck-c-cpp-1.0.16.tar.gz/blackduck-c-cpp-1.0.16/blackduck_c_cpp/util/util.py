...
def get_absolute_path(relative_path):
    try:
        ...
        if absolute_path != relative_path:
            logging.warning("{} appears to be a relative path. Using extracted absolute path {} instead.".format(relative_path, absolute_path))
        return absolute_path
    except:
        logging.error("Could not get absolute path for {}. Will attempt to proceed with relative path.".format(relative_path))
