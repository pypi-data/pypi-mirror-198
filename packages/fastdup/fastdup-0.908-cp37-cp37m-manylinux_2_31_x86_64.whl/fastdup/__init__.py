...
def run(input_dir='',             
    ...
    if bounding_box == 'face' or bounding_box == 'yolov5s' or bounding_box == 'rotated':
        ...
        try:
            ...
            shutil.move(os.path.join(work_dir, 'atrain_stats.csv'), backup_dir)
            shutil.move(os.path.join(work_dir, 'outliers.csv'), backup_dir)
            shutil.move(os.path.join(work_dir, 'similarity.csv'), backup_dir)
            shutil.move(os.path.join(work_dir, 'connected_components.csv'), backup_dir)
            os.unlink(os.path.join(work_dir, 'atrain_features.dat'))
        except Exception as ex:
        ...
        if bounding_box in ['face', 'yolov5s','rotated']:
            if bounding_box in ['face', 'yolov5s']:
                out_df = pd.read_csv(os.path.join(work_dir, 'atrain_crops.csv'))[[
                    'index', 'filename', 'col_x', 'row_y', 'width', 'height']]
            elif bounding_box == 'rotated':
                out_df = pd.read_csv(os.path.join(work_dir, 'atrain_crops.csv'))[[
                    'index', 'filename']]
            out_len = len(out_df)
            max_index = out_df['index'].max()
            # handle the case some crops are missing, we need a placeholder not to change the offsets
            out_df.sort_values('index').to_csv(input_dir, index=False)
...
def delete_or_retag_stats_outliers(stats_file, metric, filename_col = 'filename', label_col=None, lower_percentile=None, upper_percentile=None,
    ...
    try:
        ...
        if isinstance(stats_file, pd.DataFrame):
            ...
            df = load_dataframe(work_dir, "outliers", "", work_dir, {}, ["from","to","distance"])
