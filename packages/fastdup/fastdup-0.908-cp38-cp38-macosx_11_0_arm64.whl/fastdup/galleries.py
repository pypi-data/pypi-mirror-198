    ...
    if (get_label_func is not None):
        if isinstance(get_label_func, str):
            ...
            if 'id_to_filename_func' in kwargs:
                df[out_col] = df['__id' if '__id' in df.columns else in_col].apply(lambda x: get_label_func(x))
            else:
...
def do_create_duplicates_gallery(similarity_file, save_path, num_images=20, descending=True,
    ...
    save_dir = os.path.dirname(save_path) if save_path.endswith(".html") else save_path

    df = similarity_file
    if df['from'].dtype in [int, np.int64] and not v1:
    ...
    if get_label_func is not None:
        subdf = find_label(get_label_func, subdf, 'from', 'From', kwargs)
        subdf = find_label(get_label_func, subdf, 'to', 'To', kwargs)
    else:
...
def load_one_image_for_outliers(args):
    ...
    try:
        ...
        assert img is not None, "Failed to read image from " + impath1
        img = plot_bounding_box(img, get_bounding_box_func, outlier_id)
        assert img is not None, "Failed to read image from " + impath1
        img = my_resize(img, max_width=max_width)
        assert img is not None, "Failed to resize image from " + impath1
...
def load_one_image(args):
    try:
        ...
        assert f is not None, f"Got None image name {fid} {input_dir}"
...
def do_create_components_gallery(work_dir, save_path, num_images=20, lazy_load=False, get_label_func=None,
    ...
    if callable(get_extra_col_func):
        ret['files'] = subdf['files'].apply(lambda x: get_extra_col_func(x))
...
def do_create_similarity_gallery(similarity_file, save_path, num_images=20, lazy_load=False, get_label_func=None,
    ...
    df = similarity_file
    get_bounding_box_func = get_bounding_box_func_helper(get_bounding_box_func)
    filename_transform_func = kwargs.get('id_to_filename_func', lambda x: x)
    reformat_disp_path = kwargs.get('get_display_filename_func', lambda x: x)
    load_crops = kwargs.get('load_crops', False)
    if load_crops:
        assert not "filename" in df.columns
        filenames = load_filenames(work_dir, kwargs)
        ...
        df = df.merge(filenames["index", "filename"], left_on="from", right_on="index")
        df = df.rename(columns={"filename":"from_filename"})
        df = df.merge(filenames['index', "filename"], left_on="to", right_on="index")
        df = df.rename(columns={"filename": "to_filename"})
    ...
    top_to_filename = None
    top_from_filename = None
    if "from_filename" in df.columns and "to_filename" in df.columns:
        top_to_filename = df.groupby('from')["to_filename"].apply(list)
        top_from_filename = df.groupby('from')["from_filename"].apply(set)
    ...
    if "from_filename" in df.columns and "to_filename" in df.columns:
        subdf["to_filename"] = top_to_filename
        subdf["from_filename"] = top_from_filename
    ...
    if slice is None or slice != 'label_score':
        ...
        for i, row in tqdm(subdf.iterrows(), total=len(subdf)):
            id_from = row['from']
            if "from_filename" in row:
                filename = str(row["from_filename"])
            elif not isinstance(id_from, str):
                filename = filename_transform_func(id_from)
                assert isinstance(filename, str), f"Failed to convert {id_from} to image filename {row}"
            else:
                filename = id_from
    ...
    if 'id_to_filename_func' in kwargs and  "from_filename" not in subdf.columns:
        id_to_file = kwargs['id_to_filename_func']
        df2['from'] = df2['from'].apply(lambda x: id_to_file(x))
        df2['to'] = df2['to'].apply(lambda x: [id_to_file(y) for y in x])
    ...
    for i, row in tqdm(subdf.iterrows(), total=min(num_images, len(subdf))):
        try:
            ...
            if "from_filename" in row:
                ...
                filename = filename_transform_func(id_from)
                assert isinstance(filename, str), f"Failed to convert filename {row['from']}"
            else:
                filename = id_from
            ...
            image_suffix = f'_{id_from}' if 'id_to_filename_func' in kwargs else ''
            save_dir = save_path
            ...
            assert id_from not in to_ids, f"Found duplicate {id_from} in {to_ids}"

            if "to_filenames" in row:
                to_impaths_ = row["to_fienames"]
            else:
            ...
            keep_aspect_ratio = kwargs is not None and 'keep_aspect_ratio' in kwargs and kwargs['keep_aspect_ratio']
            h = []
            w = []
            for im in imgs:
                if im is not None:
            ...
            to_impaths = []
            for im, imgpath2, to_id in zip(img2, to_impaths_, to_ids):
                assert imgpath2 != imgpath, f"Found duplicate image {imgpath} {imgpath2}"
                image_suffix = f'_{to_id}' if 'id_to_filename_func' in kwargs else ''
...
if __name__ == "__main__":
    ...
    if False:
        ...
        #       verbose=False, ccthreshold=0.8, num_images=200)    #ret = fd.vis.similarity_gallery(load_crops=True,slice='label_score',ascending=True,num_images=100,enhance_image=True,keep_aspect_ratio=True)
        ...
        fd = fastdup.create(input_dir='/mnt/data/sku110k', work_dir='output3')
