...
class FastdupController:
    ...
    def similarity(self, data: bool = True, split: Union[str, List[str]] = None,
                   include_unannotated: bool = False) -> pd.DataFrame:
        """
    ...
    def _set_fastdup_input(self):
        ...
        ***************************************************************************************************************
        ...
        # image data type - return input dir or subset (and edit annot by intersecting over subset)
            ...
            assert False, f"Wrong input type {self._dtype} please file a github issue"
    ...
    def _merge_df_with_annot(self, df: pd.DataFrame, left_on: List[str],
        ...
        assert len(df), "Failed to merge df with annotations"
        return df
        ...
        if self._df_annot is None:
            if df_bad_files is not None and not df_bad_files.empty:
                df_bad_files = df_bad_files[['index', 'filename']]\
                    .rename({FD.BAD_FD_ID: FD.ANNOT_FD_ID, FD.BAD_FILENAME: FD.ANNOT_FILENAME}, axis=1)
            ...
            if df_bad_files is not None and not df_bad_files.empty:
                df_bad_files = df_bad_files.rename({FD.BAD_FILENAME: FD.ANNOT_FILENAME},
                                                   axis=1)[[FD.ANNOT_FILENAME, FD.BAD_FD_ID]]
                self._df_annot = pd.merge(self._df_annot, df_bad_files, on=FD.ANNOT_FILENAME, how='left')
                mask = ~self._df_annot[FD.BAD_FD_ID].isna()
                self._df_annot[FD.ANNOT_FD_ID][mask] = self._df_annot[FD.BAD_FD_ID][mask].tolist()
                self._df_annot = self._df_annot[list(set(self._df_annot.columns) - set([FD.BAD_FD_ID]))]
    ...
    def _expand_annot_df(self):
        ...
        if self._run_mode == FD.MODE_CROP or self._run_mode == FD.MODE_ROTATED_BBOX:
            ...
            df_crops_annot = self._fetch_df(FD.CROPS_CSV)
            ...
            self._df_annot.rename({FD.MAP_FILENAME: FD.ANNOT_CROP_FILENAME,
...
def verify_input_dirs_or_s3_paths_exist(input_dir_or_list):
    def verify_single_path(path):
        if path is None:
            pass
        elif str(path).startswith('minio://') or str(path).startswith('smb://'):
            pass
