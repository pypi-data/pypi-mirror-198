    ...
    if (get_label_func is not None):
        if isinstance(get_label_func, str):
            ...
            if 'id_to_filename_func' in kwargs:
                df[out_col] = df['__id' if '__id' in df.columns else ('fastdup_id' if 'fastdup_id' in df.columns else in_col)].apply(lambda x: get_label_func(x))
            else:
...
def read_components_from_file(work_dir, get_label_func, kwargs):
    ...
    if load_crops: # crop filename contains both crop filename and original image filename, if we want to load_crop then we need to pick the later
...
if __name__ == "__main__":
    ...
    if True:
        ...
        ret= fd.vis.stats_gallery(load_crops=False)
