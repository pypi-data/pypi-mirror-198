...
class FastdupController:
    ...
    def similarity(self, data: bool = True, split: Union[str, List[str]] = None,
        ...
        assert df is not None and not df.empty, f'No similarity file found {FD.SIMILARITY_CSV}'
    ...
    def outliers(self, data: bool = True, split: Union[str, List[str]] = None,
        ...
        assert df is not None and not df.empty, f'No outliers found {FD.OUTLIERS_CSV}'
        df = df.rename({
    ...
    def connected_components(self, data: bool = True, split: Union[str, List[str]] = None,
        ...
        assert df_cc is not None and not df_cc.empty, f'No connected components found {FD.CC_CSV}'
    ...
    def _expand_annot_df(self):
        ...
        df_mapping = self._fetch_df(FD.MAPPING_CSV)
        assert df_mapping is not None and not df_mapping.empty,  f"Failed to find {FD.MAPPING_CS} in work_dir"
