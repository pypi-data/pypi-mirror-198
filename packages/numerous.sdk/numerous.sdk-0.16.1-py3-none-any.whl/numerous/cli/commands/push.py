...
def command_push(log: Logger, repo_path: Optional[Path], comment: str) -> None:
    ...
    directory = Dir(str(path))
    for subdir in directory.itersubdirs(pattern="*bin", abspath=True):
        files = directory.files(abspath=True)
        if (
            os.path.join(subdir, "activate") in files
            and os.path.join(subdir, "python") in files
        ):
            print_push_error(
    ...
    if repo.snapshot == directory.hash(index_func=filehash):
