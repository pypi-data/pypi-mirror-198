...
def publish(media_type, post_text, ossurl, last):
    type = 0
    if media_type == "video":
        type = 2
...
def download(name, media_type, post_text, media_resource_url, audio_resource_url, last):
    ext = ".mp4"
    if media_type == "image":
        ext = ".jpg"
    elif media_type == "audio":
    ...
    s.close()
    os.remove(savePath)
    publish(media_type, post_text, ossurl, last)
...
def processPosts(uuid, data):
    ...
    for it in medias:
        ...
        lastItem = (idx == (len(medias) - 1))
        download(f"{uuid}_{idx}", media_type, post_text, media_resource_url, audio_resource_url, lastItem)
        idx += 1
...
def main():
    ...
    if sys.argv[1] == "test":
        curGroupId = 0
        aaaapp(False, "https://youtube.com/shorts/u85_CKdhhpw?feature=share", "", 0)
        return
    if sys.argv[1] == "test1":
        utils.ffmpegTest()
        return
