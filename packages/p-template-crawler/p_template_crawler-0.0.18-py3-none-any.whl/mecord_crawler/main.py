...
def pathWithSize(path):
    if "http" in path:
        w,h = utils.getOssImageSize(path)
    else:
        w,h = utils.getLocalImageSize(path)
...
def download(name, media_type, post_text, media_resource_url, audio_resource_url):
    ...
    if os.path.exists(coverPath):
        ...
        cover_url = pathWithSize(coverossurl)
    #publish
...
logging.basicConfig(filename=f"{thisFileDir}/mecord_crawler_{d}.log", 
                        ...
                        encoding="utf-8",
                        level=logging.DEBUG)
aaaapp(False, "https://www.tiktok.com/@letizia.melissa/video/7211596389841784070", "", 0)