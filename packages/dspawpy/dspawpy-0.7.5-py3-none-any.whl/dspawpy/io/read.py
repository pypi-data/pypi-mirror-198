...
def load_h5_todict(dir_h5: str) -> Dict:
    """Combine keys such as /Structures/Step-1/* and /Structures/Step-2/*
    Still return a dictionary
    """
...
def get_band_data_json(band: Dict, iwan=False):
    ...
    if (
        band[f"{bd}"]["SpinType"] == "none"
        or band[f"{bd}"]["SpinType"] == "non-collinear"
    ):
        number_of_spin = 1
    else:
        number_of_spin = 2

    symmmetry_kpoints = band[f"{bd}"]["SymmetryKPoints"]
    symmetry_kPoints_index = band[f"{bd}"]["SymmetryKPointsIndex"]
