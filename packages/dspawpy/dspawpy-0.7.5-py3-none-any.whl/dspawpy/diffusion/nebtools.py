...
def set_pbc(spo: np.ndarray or list):
    ...
    """
    # wrap into [-0.5, 0.5)
    pbc_spo = np.mod(spo + 0.5, 1.0) - 0.5
...
def get_distance(
    ...
    >>> # 请根据实际情况修改 structure01.as 和 structure02.as 结构文件的路径
...
def plot_neb_converge(
    ...
    >>> neb_dir = "./neb"
    >>> plot_neb_converge(neb_dir, image_key='01', show=False, image_name="neb_conv.png")
    # 生成 neb_conv.png 图片
    """
    if os.path.exists(os.path.join(neb_dir, 'neb.h5')):
        ...
        print(f"请检查{os.path.join(neb_dir, 'neb.h5')}或{os.path.join(neb_dir, 'neb.h5')}是否都存在！")
    ...
    for i, folder in enumerate(subfolders):
        if i == 0: # 初末态在NEB计算过程中不会优化结构
            ...
            if os.path.exists(neb_js_path): # 优先读取neb计算得到的system00.json
                ...
                raise FileNotFoundError(f'{os.path.abspath(js_path)}和{os.path.abspath(neb_js_path)}均不存在！')
...
def plot_barrier(datafile:str='neb.h5',
                 ri: float = None,
                 rf: float = None,
                 ei: float = None,
                 ef: float = None,
                 ...
                 show:bool=True,
                 **kwargs):
    """调用scipy.interpolate.interp1d插值算法，拟合NEB能垒
    ...
    try:
        interpolate_method = import_from('scipy.interpolate', method)
    except:
...
def summary(directory: str = ".",
    ...
    print("\n--> 2. 绘制能垒图...")
    plot_barrier(directory=directory, **kwargs)
