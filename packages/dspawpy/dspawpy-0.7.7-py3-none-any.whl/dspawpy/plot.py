...
def plot_polarization_figure(
    directory: str, repetition:int = 2, annotation: bool = False, show: bool = True, fig_name: str = "pol.png"
):
    """绘制铁电极化结果图
    ...
    for j in range(3): # x, y, z
        ...
        axes[j].plot(subfolders, ys, ".") # plot
        ...
        if annotation:
            # if j != 0:
            #     axes[j].set_yticks(ys[0,:])
            # axes[j].twinx().set_yticks(ys[-1,:])
...
def plot_potential_along_axis(
    datafile: str, task:str='potential', axis=2, smooth=False, smooth_frac=0.8, **kwargs
):
    """绘制沿着某个轴向的物理量平均值曲线
...
def plot_phonon_thermal(
    datafile: str, figname: str=None, show: bool = True
):
    """绘制声子热力学性质
