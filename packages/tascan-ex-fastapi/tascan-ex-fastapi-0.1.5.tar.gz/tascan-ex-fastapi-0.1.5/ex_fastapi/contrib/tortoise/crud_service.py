...
class TortoiseCRUDService(BaseCRUDService[PK, TORTOISE_MODEL]):
    ...
    def _get_queryset(
            ...
            path: str,
            method: str,
            select_related: str,  # comma_separated for lru_cache
            prefetch_related: str,  # comma_separated for lru_cache
    ...
    def get_queryset(
        ...
        # select_related и prefetch_related переводим в строку с запятыми, чтобы lru_cache работал как надо
        select_related, prefetch_related = ','.join(select_related), ','.join(prefetch_related)
        queryset = self._get_queryset(path, method, select_related, prefetch_related)
        # подменяем соединение, потому что self._get_queryset использует lru_cache,
        # который может содержать закрытые соединения
        return queryset.using_db(self.model._choose_db())
    ...
    async def create(
        ...
        async def get_new_instance():
            ...
            if o2o_fields:
                try:
                    created_o2o = await self.create_o2o(model, o2o_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if fk_fields:
                try:
                    picked_fks = await self.pick_fk(model, fk_fields, data, prefix=prefix)
            ...
            if bo2o_fields:
                try:
                    await self.create_backward_o2o(instance, bo2o_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if bfk_fields:
                try:
                    await self.create_backward_fk(instance, bfk_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if errors:
                raise errors
            await self.save_m2m(instance, data, m2m_fields=m2m_fields, clear=False, prefix=prefix)
            return instance
    ...
    async def create_o2o(
            ...
            prefix: ModelPrefix,
            created_o2o: dict[str, Any] = None,
            errors: MultipleFieldsError = None,
    ) -> dict[str, Any]:
        created_o2o = created_o2o or {}
        errors = errors or MultipleFieldsError()
        for field_name in o2o_fields:
            ...
            try:
                ...
                created_o2o[field_name] = o2o_instance
            except MultipleFieldsError as e:
    ...
    async def edit(
        ...
        async def get_changed_instance() -> TORTOISE_MODEL:
            ...
            if o2o_fields:
                try:
                    created_o2o = await self.edit_o2o(instance, o2o_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if fk_fields:
                try:
                    picked_fks = await self.pick_fk(instance, fk_fields, data, prefix=prefix)
                except NotFoundFK as e:
                    errors.add_errors(e)
            await self.handle_edit(instance)(
                data, should_exclude={*exclude_dict['__root__'], *fk_fields},
                defaults={**defaults, **created_o2o, **picked_fks}
            )
            if bo2o_fields:
                try:
                    await self.edit_backward_o2o(instance, bo2o_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if bfk_fields:
                try:
                    await self.edit_backward_fk(instance, bfk_fields, data, exclude_dict, prefix=prefix)
                except MultipleFieldsError as e:
                    errors.add_errors(*e)
            if errors:
                raise errors
            await self.save_m2m(instance, data, m2m_fields=m2m_fields, prefix=prefix)
            return instance
    ...
    async def edit_o2o(
            ...
            exclude_dict: dict[str, set[str]],
            prefix: ModelPrefix,
            created_o2o: dict[str, TORTOISE_MODEL] = None,
            errors: MultipleFieldsError = None,
    ) -> dict[str, TORTOISE_MODEL]:
        created_o2o = created_o2o or {}
        errors = errors or MultipleFieldsError()
    ...
    async def pick_fk(
            ...
            prefix: ModelPrefix = ModelPrefix(),
            picked_fks: dict[str, Optional[TORTOISE_MODEL]] = None,
        ...
        for source_field_name in fk_fields:
            ...
            if value is None:
                picked_fks[field_name] = None
                continue
            get_rel_instance = self.fk_get_instance_map.get(prefix.plus(source_field_name), rel_model.get_or_none)
            rel_instance = await get_rel_instance(pk=value)
            if rel_instance is None:
                not_found_fk.add(source_field_name)
            else:
                picked_fks[field_name] = rel_instance
        if not_found_fk:
