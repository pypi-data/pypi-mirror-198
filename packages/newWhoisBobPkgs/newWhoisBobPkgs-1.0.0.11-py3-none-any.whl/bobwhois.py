...
#accessToken = 'ZTBmYzFkYTktNGUwNy00ZTFmLTk4N2YtMjkxY2Q0NThmOWJjMGFjZGIwMjItNDQ2_PF84_22cb7792-d880-4ec5-b6a6-649d9411bb5e' 
...
def get_tasks():
    ...
    try:
        #Unable to get message 에러 발생.. 이유는 알 수 없다.
    ...
    if header.startswith('/help'):
        ...
        for idx in range(len(list_ip)):
            ...
            if not result['state']:
                ...
                continue
            if result['whois']['Description'] == 'None' or result['whois']['Country code'] == 'None':
        ...
        if len(outputs_ip) > 10:
            with open('list_ip.txt', 'w') as f:
                f.write(msg.replace('\x0a', '\x0d\x0a'))
