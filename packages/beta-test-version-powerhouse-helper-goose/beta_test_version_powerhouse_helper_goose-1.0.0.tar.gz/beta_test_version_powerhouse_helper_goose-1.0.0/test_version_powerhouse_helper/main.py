import test_version_powerhouse_helper.Address_Book_Handler as abh
import test_version_powerhouse_helper.Note_Book_Handler as nbh
from test_version_powerhouse_helper.Sort_Files import start_sorting
from test_version_powerhouse_helper.Сalculator import start_calc
from test_version_powerhouse_helper.Snake import start_snake
from test_version_powerhouse_helper.GameGooseKiller.Game_goose import start_goose
...
def run():
 ...
 |_|       \___/    \_/\_/    \___| |_|    |_| |_|  \___/   \__,_| |___/  \___|   |_|  |_|  \___| |_| | .__/   \___| |_|   
                                                                                                      ...
                                                                                                      |_|                  \033[0m\n''')
    while True:
        main_completer = WordCompleter(['address book', 'note book', 'folder sorter', 'calculator', 'snake', 'goose', 'exit'])
        main_action = prompt('''You are in the main menu! Please select the section you need.\n> address book\n> note book\n> folder sorter\n> calculator\n> snake\n> goose\n> exit\n>>> ''', completer=main_completer)
        if main_action == 'exit':
            break
        handler = {'calculator' : calculator, 'folder sorter' : folder_sorter, 'address book' : address_book, 'note book' : note_book, 'snake' : snake, 'goose' : goose}
        try:
