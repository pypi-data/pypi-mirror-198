...
def chambolle_pock(impl_f,impl_gs,τ_f,τ_gs,K,x,y=None,
	...
	Inputs : 
	- impl_f : a tuple (f,f_star,prox_f) implementing the function f, the 
	...
	- impl_gs : a tuple (g_star,g,prox_g_star), as above.
	- K : linear operator. Applies either as K(x) or K*x.
	...
	- E_rtol (optional) : two possibilities
		- (positive float) : algorithm stops when (E_primal-E_dual)/abs(E_primal) < E_rtol,
...
#	'rgap':2*(primal_values-dual_values)/(np.abs(primal_values)+np.abs(dual_values)),
	'ops':{'K':K,'KT':KT,'E_primal':E_primal,'E_dual':E_dual},
