...
class Fluid:
    ...
    def __init__(self):
        self.id_usuario = os.getenv('fluid_cod_user')
    ...
    def protocolar_processo_fluid(self, cod_processo: str, id_tipo_processo: str,  mensagem: str, id_empresa_origem='1', id_empresa_destino='1', nome_do_nodo ='selecionado_proximo_nodo', abertura_de_processo= False, acao = 0, usuario_destino=2735):
        ''' 
...
class Dados:
    ...
    def __init__(self):
        ...
        self.ip_grafana = str(os.getenv('ip_grafana'))
        self.user_grafana = str(os.getenv('user_grafana'))
        self.pw_bd_grafana = str(urllib.parse.quote_plus(os.getenv('pw_bd_grafana')))
        
        self.ip_sistema_senhas = str(os.getenv('ip_sistema_senhas'))
        self.user_sistema_senhas = str(os.getenv('user_sistema_senhas'))
        self.database_sistema_senhas = str(os.getenv('database_sistema_senhas'))
        self.pw_bd_sistema_senhas = str(urllib.parse.quote_plus(os.getenv('pw_bd_sistema_senhas')))
        
        self.ip_denodo = str(os.getenv('ip_denodo'))
        self.user_denodo = str(os.getenv('user_denodo'))
        self.pw_denodo = str(urllib.parse.quote_plus(os.getenv('pw_denodo')))
...
class Gerador:
    ...
    def pwd(self, palavra_chave:str, mostrar_todas_credenciais=False):
        '''
        res = gerador.pwd('denodo')\n
        print(res)\n
        print(res.get('usuario'))\n
        '''
        import os
        import socket
        ...
        dados = Dados()
        conex = dados.criar_engine('fluid')
        
        if mostrar_todas_credenciais:
            sql = f"SELECT * FROM rpa_cofre_senhas"
            credenciais = dados.consultar_banco_dados(conex, sql)
            ...
            sql = f"SELECT * FROM rpa_cofre_senhas WHERE palavra_chave like '%{palavra_chave}%'"
            df = dados.consultar_banco_dados(conex, sql)
            # print(df)
...
class Monitorar:
    import socket
    data_ini = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d') + ' 17:30:00'
    data_fim = datetime.now().strftime('%Y-%m-%d') + ' 17:30:00'
    usuario = os.getenv('user_monitora_vm')
    pwd = os.getenv('pw_monitora_vm')
    ...
    def check_vms_running(self, vms:list):
        for vm in vms: 
            try:
                ...
                continue
            if ('UiPath.Executor.exe' in processos) or ('python.exe' in processos): 
                status_vm = 'ok - python em execução'
                print('{:<20} {:<8}'.format(vm, status_vm))
            else:
                status_vm = 'com erro'
                print('{:<20} {:<8}'.format(vm, status_vm))
    ...
    def check_status_rpa(self, tipo_status):
        dados = Dados()
        engine = dados.criar_engine('cronos')
        df = dados.consultar_banco_dados(engine, f"SELECT * FROM rpa_fila WHERE status_rpa='{tipo_status}'")
        print(df)
...
class Escrever_por_extenso:  
    ...
    def numero_por_extenso(self, valor:str):
        ...
        '''
        gerador = Gerador()
        res = gerador.pwd('extenso')
        url_api_escreve_por_extenso = str(res.get('senha'))
        ...
        url = url_api_escreve_por_extenso   
