...
@run.command()
@click.option(
    "--url", help="The fakts url for connection", default="http://localhost:8000"
)
@click.option(
    "--public_url",
    help="The fakts public_url for connection",
    default="http://localhost:8000",
)
@click.option(
    "--instance_id", "-i", help="The fakts instance_id for connection", default="main"
)
...
def port(url, token):
    ...
    asyncio.run(
        run_port(manifest.entrypoint, manifest.identifier, manifest.version, url, token)
    )
...
@click.option(
    "--runtime", help="The runtime to use", type=click.Choice(compile_runtimes())
)
@click.option("--nodefs", help="Do not inspect definitions", is_flag=True)
@click.option(
    "--builder", help="The builder to use", type=str, default="arkitekt.builder.port"
)
...
def port(version, dockerfile, nopush, tag, runtime, nodefs, builder, entrypoint):
    ...
    if not os.path.exists("Dockerfile"):
        if click.confirm(
            "Dockerfile does not exists. Do you want to generate a template Dockerfile?"
        ):
            with open("Dockerfile", "w") as f:
                f.write(default_docker_file)
                console.print(
                    "Dockerfile generated. Please edit it to your needs. And restart the deployment."
                )
                return

    md = Panel(
        "Deploying to Port", subtitle="This may take a while...", subtitle_align="right"
    )
    console.print(md)
    ...
    if version == "dev":
        raise click.ClickException(
            "You cannot deploy a dev version. Please change the version in your manifest. Or use the --version flag."
        )
    ...
    tag = tag or click.prompt(
        "The tag to use", default=f"{username}/{manifest.identifier}:{version}"
    )
    runtime = runtime or click.prompt(
        "The runtime to use", type=click.Choice(compile_runtimes()), default="standard"
    )
    ...
    docker_run = subprocess.run(
        ["docker", "build", "-t", tag, "-f", dockerfile or "Dockerfile", "."]
    )
    ...
    generate_deployment(
        ...
        entrypoint,
        deployed,
        "arkitekt.deployers.port.dockerbuild",
        manifest.scopes,
        with_definitions=not nodefs,
        builder=builder,
    )
...
def init(identifier, version, author, entrypoint, builder, scopes, template):
    ...
    manifest = Manifest(
        ...
        or click.prompt(
            " Where should we search for the entrypoint?",
            default=getattr(oldmanifest, "entrypoint", "app"),
        ),
        builder=builder
        or click.prompt(
            " Which builder do you want to use?",
            default=getattr(oldmanifest, "builder", "arkitekt.builders.easy"),
        ),
