...
from PySide6.QtWidgets import (QSplashScreen, QMainWindow, QApplication, QMessageBox, QWidget)
...
if __name__ == '__main__':
    ...
    for module,name in modules.items():
        ...
        if module[0] == '.':
            if name == '':
                exec(f'from . import {module[1:]}')
            else:
                exec(f'from . import {module[1:]} as {name}')
        else:
            if name == '':
