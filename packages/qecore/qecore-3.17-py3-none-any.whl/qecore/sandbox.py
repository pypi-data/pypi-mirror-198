...
class TestSandbox:
    ...
    def _attach_abrt_link_to_report(self, context) -> None:
        ...
        abrt_directories = run_verbose("sudo ls /var/spool/abrt/ | grep ccpp-")
        ...
        for abrt_directory in abrt_directories_as_list:
            ...
            try:
                log.debug(f"_attach_abrt_link_to_report: reading file '{reason_file}'")
                abrt_faf_reason_run = run_verbose(f"sudo cat '{reason_file}'")

                log.debug(f"_attach_abrt_link_to_report: reading file '{reported_to_file}'")
                abrt_faf_hyperlink_run = run_verbose(f"sudo cat '{reported_to_file}'")
                ...
                if abrt_faf_reason_run[1] == 0 and abrt_faf_hyperlink_run[1] == 0:
                    ...
                    log.debug("_attach_abrt_link_to_report: non-zero return code while reading file.")
                    log.debug(f"_attach_abrt_link_to_report: abrt_faf_reason_run '{abrt_faf_reason_run}'")
                    log.debug(
                        "".join(
                            (
                                "_attach_abrt_link_to_report: ",
                                f"abrt_faf_hyperlink_run '{abrt_faf_hyperlink_run}'",
                            )
                        )
                    )

            except Exception as error:  # pylint: disable=broad-except
                log.debug(f"_attach_abrt_link_to_report: Exception caught: {error}'")
