...
class GetNode:
    ...
    def __init__(self, context, **kwargs) -> None:
        ...
        from dogtail.tree import root  # pylint: disable=import-error
    ...
    def __exit__(self, exc_type, exc_value, trcb) -> bool:
        if exc_type is not None:
            return False
        return True

    def format_string(self, string, bold=False) -> str:
        """
        Colour string based on environemnt.
        ...
        if bold:
            return (
                colored(string, "yellow", attrs=["bold"]) if QE_DEVELOPMENT else string
            )
        return colored(string, "yellow") if QE_DEVELOPMENT else string

    def get_formated_duplicates(self, list_size, list_of_duplicates, attr) -> str:
        """
                ...
                sorted(
                    {
                        "".join(
                            (
                                "\t",
                                f"{self.format_string('name')}: '{repr(node.name)}' ",
                                f"{self.format_string('roleName')}: '{node.roleName}' ",
                                f"{self.format_string('position')}: '{node.position}' ",
                                f"{self.format_string('size')}: '{node.size}' ",
                                f"{self.format_string(attr if attr else 'attribute')}: ",
    ...
    def get_formated_error_message(
        ...
        return "".join(
            (
                "\n\n",
                f"{self.format_string('Item was not found', bold=True)}: {error}",
                "\n\n",
                f"{self.format_string('Items with name', bold=True)}: {node_name}:",
                "\n",
                same_name_items,
                "\n",
                f"{self.format_string('Items with roleName', bold=True)}: {node_role_name}:",
                "\n",
    ...
    def get_error_message(self, node, error) -> str:
        ...
        nodes_with_name_formatted = self.get_formated_duplicates(
        ...
        nodes_with_role_name_formatted = self.get_formated_duplicates(
            nodes_with_role_name_size, nodes_with_role_name, node.attr
        )

        return self.get_formated_error_message(
            error,
            name,
