...
class GeneratingCommand(BaseCommand):
    ...
    def run_on_temp_dir(self, temp_dir):
        ...
        self.generate_files()
        logger.info("Generated %s temporary Sphinx files to %s", self.generated_count, self.temp_dir)
...
class Command(GeneratingCommand):
    ...
    def actor_overview(self, rpt):
        ...
        s += rstgen.ul([self.action2par(ba) for ba in rpt._actions_list if ba.action.show_in_toolbar])
        if rpt.parameters:
