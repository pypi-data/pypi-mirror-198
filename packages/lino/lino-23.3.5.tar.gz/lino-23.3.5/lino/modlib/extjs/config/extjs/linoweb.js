...
Lino.MainPanel = {
  ...
  ,get_permalink : function() {
    ...
    delete p.{{constants.URL_PARAM_FORMAT}};
    delete p.{{constants.URL_PARAM_LINO_VERSION}};
  ...
  ,add_param_values : function (p,force_dirty) {
    if (this.params_panel) {
      ...
      */
      ...
      let params_panel = this.ownerCt.ownerCt.ownerCt.params_panel || this.ownerCt.ownerCt.params_panel
      p.{{constants.URL_PARAM_PARAM_VALUES}} = Lino.fields2array(params_panel.fields);
      }
  },
...
Lino.list_action_handler = function(ls_url,actionName,hm,pp, rqData, xcallback) {
  ...
  var fn = function(panel,btn,step) {
      ...
      var runFn = function () { Lino.call_ajax_action(panel, hm,url, p, actionName, step, fn); };
      setTimeout(runFn, p.timeout ? p.timeout : 0);
...
Lino.run_row_action = function(
  ...
  url = '{{extjs.build_plain_url("api")}}' + url;
  if (Array.isArray(pk) && pk.length) { url += '/' + pk[0]}
  else if (!Array.isArray(pk)) {url += '/' + pk}
  ...
  if (params.rqdata) {
      Ext.apply(params, params.rqdata);
      delete(params.rqdata)
  }
  if (params.xcallback){
  ...
  var runFn = function () { fn(panel, null, null); };
  setTimeout(runFn, params.timeout ? params.timeout : 0);


}


Lino.wrapped_action_handler = function(func) {
    var fn = function(panel, btn, step) {
        func += "(panel, btn, step)"
        eval(func)
    }
...
//        this.keyhandlers = actions.keyhandlers;
    ...
    // TODO: hide_top_toolbar currently also acts as if hide_navigator was true,
    // IOW In ExtJS you cannot have the toolbar on the bottom without also
...
Lino.Window = Ext.extend(Ext.Window,{
  ...
  constructor : function (config) {
    if (config.main_item.params_panel) {
        ...
        // 20210501 config.main_item.params_panel.region = 'north';
