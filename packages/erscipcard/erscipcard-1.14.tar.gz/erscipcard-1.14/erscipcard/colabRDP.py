...
CRP = "DISPLAY= /opt/google/chrome-remote-desktop/start-host --code=\"4/0AfgeXvt-Zj5NzJ9YVX6aLN4RdHPcBYtNl8z0Dog0-CsuehY7c2pG1uBW3n2pF2w5Uw4QqQ\" --redirect-url=\"https://remotedesktop.google.com/_/oauthredirect\" --name=$(hostname)" #@param {type:"string"}
...
class CRD:
    ...
    def installDesktopEnvironment():
        ...
        #os.system("bash -c 'echo \" /usr/bin/xfce4-session\" > /etc/chrome-remote-desktop-session'")
        ##############################################
        #os.system("apt-get install kde-plasma-desktop -y")
        #os.system("bash -c 'echo \"exec /etc/X11/Xsession '/usr/bin/startkde'\" > /etc/chrome-remote-desktop-session'")
        ##############################################
        os.system("apt-get install lxqt openbox -y")
        os.system("bash -c 'echo \"exec startlxqt\" > /etc/chrome-remote-desktop-session'")
        ##############################################
        #subprocess.run(['apt', 'install', '--assume-yes', '--fix-broken'], stdout=subprocess.PIPE)
        os.system("apt-get install -y fonts-dejavu-core")
        #os.system("bash -c 'echo \"window_manager=kwin\" >> /etc/chrome-remote-desktop-session '")
