...
def networklink_kml(name, link_path, write_path, description='', refresh_interval=-1, view_refresh=-1, collapsed=True):
    """
       Creates a KML string.
       ...
       INPUTS:
           ...
           refresh_interval (Int) [Optional]:
               Number of seconds between file refreshes. (-1 = no refresh)
           view_refresh (Int) [Optional]:
               Number of seconds after camera movement stops before area is refreshed. (-1 = no refresh)
           collapsed (Bool) [Optional]:
                   True = Root folder is collapsed.
                   False = Root folder is open/expanded.
