...
class MlFoundryRun:
        ...
        try:
            ...
            for key in metric_dict.keys():
                ...
                if not self.VALID_PARAM_AND_METRIC_NAMES.match(key):
                    logger.warning(
                        f"Invalid metric name: {key}. Names may only contain alphanumerics, underscores (_), dashes (-), periods (.), spaces ( ), and slashes (/). "
        ...
        try:
            ...
            for param_key in param_dict.keys():
                if len(str(param_key)) > 250 or len(str(param_dict[param_key])) > 250:
                    logger.warning(
                        f"MlFoundry can't log parmeters with length greater than 250 characters. Discarding {param_key}:{param_dict[param_key]}."
                    )
                    continue
                if not self.VALID_PARAM_AND_METRIC_NAMES.match(param_key):
                    logger.warning(
                        f"Invalid param name: {param_key}. Names may only contain alphanumerics, underscores (_), dashes (-), periods (.), spaces ( ), and slashes (/). "
        ...
        *,
        ...
        Args:
            ...
            model_schema (Optional[Union[Dict[str, Any], ModelSchema]], optional): instance of `mlfoundry.ModelSchema`.
                ...
                This field can be updated at any time after logging. Defaults to `None`
            custom_metrics: (Optional[Union[List[Dict[str, Any]], CustomMetric]], optional) = instance of list of `mlfoundry.CustomMetric`
    ...
    def use_model(self, fqn: str) -> ModelVersion:
        ...
        Examples:
            ...
            client = mlfoundry.get_client()
            run = client.create_run(project_name="my-new-project")
