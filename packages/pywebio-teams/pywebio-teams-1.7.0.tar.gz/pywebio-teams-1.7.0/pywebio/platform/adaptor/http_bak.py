...
class HttpHandler:
    ...
    def handle_request_context(self, context: HttpContext):
        ...
        if context.request_method() == 'POST':  # client push event
            event = context.request_json()
            event_seq = int(context.request_url_parameter('event', 0))
            event_ack_id = cls._webio_last_event_id.get(webio_session_id, 0)
            if event is not None and event_seq == event_ack_id + 1:
                cls._webio_last_event_id[webio_session_id] = event_seq
                webio_session.send_client_event(event)
                yield cls.WAIT_MS_ON_POST / 1000.0  # <--- <--- <--- <--- <--- <--- <--- <--- <--- <--- <--- <---
