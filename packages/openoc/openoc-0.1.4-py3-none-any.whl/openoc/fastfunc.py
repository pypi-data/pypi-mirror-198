...
def _deco_basic(num_free_symbol: int, parallel: bool, fastmath: bool) -> str:
    ...
    tail = ''
    # maybe numba bugs, do not compile if set
    # if parallel:
    #     tail += ', target="parallel"'
    if fastmath:
        tail += ', fastmath=True'
    return '@numba.vectorize("float64({})"{})\n'.format(', '.join(['float64'] * num_free_symbol), tail)


@functools.lru_cache
def _deco_F(parallel: bool, fastmath: bool) -> str:
    """Decorator for aggregate function F (value)."""
    tail = ''
    if parallel:
        tail += ', parallel=True'
    if fastmath:
...
def _deco_GH(parallel: bool, fastmath: bool) -> str:
    """Decorator for aggregate functions G (gradient) & H (hessian)."""
    tail = ''
    if parallel:
        tail += ', parallel=True'
    if fastmath:
...
class FastFunc:
    """FastFunc take a sympy expression and a list of sympy symbols as function arguments,
    and generate JITed, vectorized functions for value, gradient and hessian.
    Gradient and hessian are generated by automatic differentiation and in sparse format,
    i.e., only non-zero elements are computed. The index of non-zero elements are stored in
    G_index and H_index, respectively.

    Both F, G, and H take a second argument for length of a single variable, the allows the
    user change the discretization scheme without recompiling the functions.
    """

    def __init__(self, function: Union[int, float, sp.Expr], args: List[sp.Symbol], /,
                 simplify: bool = False, parallel: bool = False, fastmath: bool = False) -> None:
        self._function = sp.sympify(function)
        self._args = args

        # random valid python name to avoid conflict
        self._valid_args = [sp.Symbol('xbN4dRhrnN_{}'.format(i)) for i in range(len(args))]
        ...
        self.H_index = self._gen_H_index()
    ...
    def _gen_basic_funcs(self) -> str:
        ...
        result_str += deco_str + 'def f({}):\n'.format(param_str) + '    return ' + func_str + '\n\n'

        for j in free_symbols_value:  # row in hess
            # gradient
            ...
            deco_str = _deco_basic(len(free_symbols_grad), self._parallel, self._fastmath)
            grad_str = lambdarepr(self.__expand_opt(grad)).replace('math.', '')
            param_str = ', '.join([self._args[k].name for k in free_symbols_grad])
            result_str += deco_str + 'def df_d{}({}):\n'.format(j, param_str) \
                          + '    return ' + grad_str + '\n\n'

            # hessian
            for k in free_symbols_value:  # col
                if j < k:
                    break
    ...
    def _gen_aggregate_funcs(self) -> str:
        ...
        if self._value_consts is not None:
            ...
            param_str = ', '.join([f'x[{j} * n:{j + 1} * n]' for j in self._value_index])
            result_str += '    r = f({})\n'.format(param_str)
        result_str += '    return r\n\n'
        ...
        for row, (x, y, fs) in enumerate(self._hess_index):
            if (x, y) in self._hess_consts:
                ...
                param_str = ', '.join([f'x[{j} * n:{j + 1} * n]' for j in fs])
                result_str += '    r[{}] = d2f_d{}d{}({})\n'.format(row, x, y, param_str)
        result_str += '    return r\n\n'

        return result_str

    def _gen_G_index(self) -> NDArray[np.int32]:
        return np.array([x for x, _ in self._grad_index], dtype=np.int32)

    def _gen_H_index(self) -> Tuple[NDArray[np.int32], NDArray[np.int32]]:
        return np.array([x for x, _, _ in self._hess_index], dtype=np.int32), \
            np.array([y for _, y, _ in self._hess_index], dtype=np.int32)
