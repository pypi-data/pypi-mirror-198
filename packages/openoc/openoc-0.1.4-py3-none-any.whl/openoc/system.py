...
class System:
    def __init__(self, static_parameter: Union[int, List[str]], /, simplify: bool = False, parallel: bool = False,
        ...
        self._basic_value_hash = None
        self._basic_value_cache = None
    ...
    def _basic_value(self, x):
        hash_x = xxhash.xxh3_64_hexdigest(x)
        if self._basic_value_hash != hash_x:
            ...
            self._basic_value_hash = hash_x
            self._basic_value_cache = sp, V, SP, MT, DT, T, M, I, v_s
        return self._basic_value_cache
    ...
    def _basic_gradient_integral(self, p_, I_, suffix=''):
        ...
        for row, n_ in enumerate(p.F_I[I_].G_index):
            if n_ < p.n:
                if p.c[n_]:
                    grad.append((f'p_gi{suffix}.f_nc2c(g_gi{suffix}[{row}])',
                                 np.arange(p.l_v[n_], p.r_v[n_], dtype=np.int32) + self.l_p[p_]))
                else:
                    grad.append((f'g_gi{suffix}[{row}]',
                                 np.arange(p.l_v[n_], p.r_v[n_], dtype=np.int32) + self.l_p[p_]))
            elif n_ < p.n + self.n_s:
                grad.append((f'np.array([np.sum(g_gi{suffix}[{row}])], dtype=np.float64)',
                             np.array([self.l_s + n_ - p.n], dtype=np.int32)))
            else:
                grad.append((f'np.array([g_gi{suffix}[{row}] @ (1 - p_gi{suffix}.t_m),'
                             f' g_gi{suffix}[{row}] @ p_gi{suffix}.t_m])',
                             np.array([self.r_p[p_] - 2, self.r_p[p_] - 1], dtype=np.int32)))
            ...
            if n_ < p.n:
                if p.c[n_]:
                    v_index = np.concatenate(
                        [np.arange(p.l_c[i], p.r_c[i], dtype=np.int32) + p.l_v[n_] + self.l_p[p_] for i in range(p.N)])
                    grad.append((f'g_gpf{suffix}[{row}]', v_index))
                else:
                    grad.append(
                        (f'g_gpf{suffix}[{row}]', np.arange(p.l_v[n_], p.r_v[n_], dtype=np.int32) + self.l_p[p_]))
            elif n_ < p.n + self.n_s:
                grad.append((f'g_gpf{suffix}[{row}]', np.full(p.l0_m, self.l_s + n_ - p.n, dtype=np.int32)))
            else:
                grad.append((f'(g_gpf{suffix}[{row}] * (1 - p_gpf{suffix}.t_m))',
                             np.full(p.l0_m, self.r_p[p_] - 2, dtype=np.int32)))
                grad.append((f'(g_gpf{suffix}[{row}] * p_gpf{suffix}.t_m)',
                             np.full(p.l0_m, self.r_p[p_] - 1, dtype=np.int32)))
        return init, grad
    ...
    def constraints(self, x):
        ...
        translation_variable_dynamics = np.concatenate([p.T_x @ v[:p.L_x] for p, v in zip(self.p, V)])
        integration_function_dynamics = np.concatenate([p.I_nc @ f_d.F(m, p.l0_m) * dt
                                                        for p, m, dt in zip(self.p, M, DT) for f_d in p.F_d])
        c_dynamics = translation_variable_dynamics - integration_function_dynamics

        c_phase = [f_c.F(m, p.l0_m) for p, m in zip(self.p, M) for f_c in p.F_c]
        # guard against empty list
        if c_phase:
            c_phase = np.concatenate(c_phase)
    ...
    def _update_jacobian_structure(self):
        ...
        for p_, p in enumerate(self._phase):
            # dynamics constraint: translation variable
            Jac_code += f'p = self.p[{p_}]\n'
            Jac_code += f'jac_data[{len(Jac_row)}:{len(Jac_row) + p.T_x_coo.nnz}] = p.T_x_coo.data\n'
            Jac_row += [c_base_ + r for r in p.T_x_coo.row]
            Jac_col += [self.l_p[p_] + c for c in p.T_x_coo.col]

            for d_, f_d in enumerate(p.F_d):
                init, grad = self._basic_gradient_phase_func(p_, f'F_d[{d_}]', suffix=f'_p{p_}d{d_}')
                Jac_code += init
                for c, i in grad:
                    Jac_code += f'jac_data[{len(Jac_row)}:{len(Jac_row) + p.I_nc_coo.nnz}] = - p.I_nc_coo.data ' \
                                f'* DT[{p_}] * ({c})[p.I_nc_coo.col]\n'
                    Jac_row += [c_base_ + d_ * p.l0_m + r for r in p.I_nc_coo.row]
                    Jac_col.extend(i[p.I_nc_coo.col])

                Jac_code += f'grad_t = - p.I_nc @ p.F_d[{d_}].F(M[{p_}], p.l0_m)\n'
                Jac_code += f'jac_data[{len(Jac_row)}:{len(Jac_row) + p.l0_m}] = -grad_t\n'
                Jac_row += [c_base_ + d_ * p.l0_m + r for r in range(p.l0_m)]
                Jac_col += [self.r_p[p_] - 2] * p.l0_m
                Jac_code += f'jac_data[{len(Jac_row)}:{len(Jac_row) + p.l0_m}] = grad_t\n'
                Jac_row += [c_base_ + d_ * p.l0_m + r for r in range(p.l0_m)]
                Jac_col += [self.r_p[p_] - 1] * p.l0_m
    ...
    def _update_hessian_structure(self):
        ...
        for p_, p in enumerate(self._phase):
            ...
            for d_ in range(p.n_x):
                init, hess = self._basic_hessian_phase_func(p_, f'F_d[{d_}]', suffix=f'_p{p_}d{d_}')
                Hess_code += init
                for code, row, col in hess:
                    Hess_code += f'hess_data[{len(Hess_row)}:{len(Hess_row) + p.I_nc_coo.nnz}] ' \
                                 f'= - p.I_nc_coo.data * ({code})[p.I_nc_coo.col] * DT[{p_}] ' \
                                 f'* fct_c[{c_base_ + d_ * p.l0_m} + p.I_nc_coo.row]\n'
                    Hess_row.extend(row[p.I_nc_coo.col])
                    Hess_col.extend(col[p.I_nc_coo.col])

                init, grad = self._basic_gradient_phase_func(p_, f'F_d[{d_}]', suffix=f'_p{p_}d{d_}')
                Hess_code += init
                for c, i in grad:
                    row_ = np.full_like(p.I_nc_coo.col, self.r_p[p_] - 2, dtype=np.int32)
                    col_ = i[p.I_nc_coo.col]
                    index, row2_, col2_ = _rotate_rc(row_, col_)
                    index_str = 'np.array([' + ', '.join(map(str, index)) + '], dtype=np.int32)'
                    Hess_code += f'hess_data[{len(Hess_row)}:{len(Hess_row) + len(index)}] = (p.I_nc_coo.data  * ({c})' \
                                 f'[p.I_nc_coo.col] * fct_c[{c_base_ + d_ * p.l0_m} + p.I_nc_coo.row])[{index_str}]\n'
                    Hess_row.extend(row2_)
                    Hess_col.extend(col2_)

                    row_ = np.full_like(p.I_nc_coo.col, self.r_p[p_] - 1, dtype=np.int32)
                    col_ = i[p.I_nc_coo.col]
                    index, row2_, col2_ = _rotate_rc(row_, col_)
                    index_str = 'np.array([' + ', '.join(map(str, index)) + '], dtype=np.int32)'
                    Hess_code += f'hess_data[{len(Hess_row)}:{len(Hess_row) + len(index)}] = (- p.I_nc_coo.data * ({c})' \
    ...
    def check(self, value: Union[Variable, List[Union[Variable, Iterable[float]]]], /,
        ...
        value_is_variable = isinstance(value, Variable)
        if value_is_variable:
            value = [value]

        if not self.n_s and len(value) != self.n_p:
            raise ValueError("len(value) must be equal to the number of phases")
        elif self.n_s and len(value) != self.n_p + 1:
            raise ValueError("len(value) must be equal to the number of phases + 1 (for static variables)")

        ok = []
        if not self.n_s:
            for p_, v_ in zip(self.p, value):
                ok.append(p_.check(v_, absolute_tolerance_continuous=absolute_tolerance_continuous,
                                   relative_tolerance_continuous=relative_tolerance_continuous,
                                   absolute_tolerance_discontinuous=absolute_tolerance_discontinuous,
                                   relative_tolerance_discontinuous=relative_tolerance_discontinuous,
                                   tolerance_mesh=tolerance_mesh))
        else:
            for p_, v_ in zip(self.p, value[:-1]):
                ok.append(p_.check(v_, np.array(list(value[-1]), dtype=np.float64),
    ...
    def refine(self, value: Union[Variable, List[Union[Variable, Iterable[float]]]], /,
               num_point_min: int = 5, num_point_max: int = 10,
               absolute_tolerance_continuous: float = 1.e-8, relative_tolerance_continuous: float = 1.e-8,
               absolute_tolerance_discontinuous: float = 1.e-5, relative_tolerance_discontinuous: float = 1.e-2,
               tolerance_mesh: float = 1.e-4):
        if not self.ok:
        ...
        if self.check(value, absolute_tolerance_continuous=absolute_tolerance_continuous,
                      relative_tolerance_continuous=relative_tolerance_continuous,
                      absolute_tolerance_discontinuous=absolute_tolerance_discontinuous,
                      relative_tolerance_discontinuous=relative_tolerance_discontinuous,
                      tolerance_mesh=tolerance_mesh):
        ...
        if not self.n_s:
            for p_, v_ in zip(self.p, value):
                p_.refine(v_, num_point_min=num_point_min, num_point_max=num_point_max,
                          absolute_tolerance_continuous=absolute_tolerance_continuous,
                          relative_tolerance_continuous=relative_tolerance_continuous,
                          absolute_tolerance_discontinuous=absolute_tolerance_discontinuous,
                          relative_tolerance_discontinuous=relative_tolerance_discontinuous,
            ...
            for p_, v_ in zip(self.p, value[:-1]):
                p_.refine(v_, np.array(list(value[-1]), dtype=np.float64),
                          num_point_min=num_point_min, num_point_max=num_point_max,
                          absolute_tolerance_continuous=absolute_tolerance_continuous,
                          relative_tolerance_continuous=relative_tolerance_continuous,
                          absolute_tolerance_discontinuous=absolute_tolerance_discontinuous,
                          relative_tolerance_discontinuous=relative_tolerance_discontinuous,
...
@nb.njit("Tuple((int32[:], int32[:], int32[:]))(int32[:], int32[:])")
