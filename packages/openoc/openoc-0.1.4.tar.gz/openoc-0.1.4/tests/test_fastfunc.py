...
def test_constant_function():
    ...
    assert np.allclose(ff.H_index[0], np.zeros(0, dtype=np.float64))
    assert np.allclose(ff.H_index[1], np.zeros(0, dtype=np.float64))
...
def test_hessian():
    ...
    assert np.allclose(ff.G_index, np.array([0, 1]))
    assert np.allclose(ff.H_index[0], np.array([1, 1]))
    assert np.allclose(ff.H_index[1], np.array([0, 1]))  # (r, c) must in order
    ...
    assert np.allclose(ff.G_index, np.array([0, 1]))
    assert np.allclose(ff.H_index[0], np.array([0, 1, 1]))
    assert np.allclose(ff.H_index[1], np.array([0, 0, 1]))  # (r, c) must in order
