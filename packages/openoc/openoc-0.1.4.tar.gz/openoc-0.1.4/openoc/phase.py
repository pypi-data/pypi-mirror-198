...
class Phase:
    ...
    def set_phase_constraint(self, phase_constraint: List[Union[float, sp.Expr]],
                             ...
                             bang_bang: Union[bool, Iterable[bool]] = False):
        ...
        Args:
            ...
            lower_bound (Iterable[Union[float]]): List of lower bounds of phase constraints
            upper_bound (Iterable[Union[float]]): List of upper bounds of phase constraints
            bang_bang (Union[bool, Iterable[bool]], optional): Whether the corresponding phase constraint is a bang-bang constraint.
        """
        ...
        # scale to [0, 1]
        if isinstance(bang_bang, bool):
            if bang_bang:
                self._func_bang_bang = [
                    FastFunc((expr - lb) / (rb - lb), self._symbols, self._simplify, self._parallel, self._fastmath)
                    for expr, lb, rb in zip(phase_constraint, lower_bound, upper_bound)]
            else:
                self._func_bang_bang = []
        else:
            self._func_bang_bang = [
                FastFunc((expr - lb) / (rb - lb), self._symbols, self._simplify, self._parallel, self._fastmath)
                for expr, lb, rb, bb in zip(phase_constraint, lower_bound, upper_bound, bang_bang) if bb]
    ...
    def set_discretization(self, mesh: Union[int, Iterable[float]], num_point: Union[int, Iterable[int]],
        ...
        if isinstance(state_continuity, bool):
            self._continuity_state = np.full(self._num_state, state_continuity, dtype=bool)
        else:
            self._continuity_state = np.array(list(state_continuity), dtype=bool)

        if isinstance(control_continuity, bool):
            self._continuity_control = np.full(self._num_control, control_continuity, dtype=bool)
        else:
            self._continuity_control = np.array(list(control_continuity), dtype=bool)
        ...
        self.T_c = T_c(self._mesh, self._num_point)
        self.T_nc = T_nc(self._mesh, self._num_point)
        self.I_c = I_c(self._mesh, self._num_point)
        self.I_nc = I_nc(self._mesh, self._num_point)
        ...
        self.T_x = T_x(self._mesh, self._num_point, self._continuity_state)
        self.I_x = I_x(self._mesh, self._num_point, self._continuity_state)

        self.T_x_coo = self.T_x.tocoo()  # cache to speed up jacobian & hessian calculation
        self.T_x_coo.eliminate_zeros()
        self.I_nc_coo = self.I_nc.tocoo()  # cache to speed up jacobian & hessian calculation
        self.I_nc_coo.eliminate_zeros()

        # data for error estimation and refining mesh
        self._l_nc_aug, self._r_nc_aug = lr_nc(self._num_point + 1)
        self._V_xu_aug = V_xu_aug(self._mesh, self._num_point, self._continuity)
        self._V_x_aug = V_x_aug(self._mesh, self._num_point, self._continuity_state)
        self._T_x_aug = T_x_aug(self._mesh, self._num_point, self._continuity_state)
        self._I_nc_aug = I_nc(self._mesh, self._num_point + 1)
        self._t_c_aug, _ = xw_c(self._mesh, self._num_point + 1)
        self._t_nc_aug, _ = xw_nc(self._mesh, self._num_point + 1)
    ...
    def _m(self, variable, static_parameter: Iterable[float] = None):
        if not self._discretization_set:
            raise ValueError('Discretization scheme not set')
        mt = (variable.t_f + variable.t_0) / 2
        dt = variable.t_f - variable.t_0
        if not self.n_s:
            s = np.array([], dtype=np.float64)
        else:
            s = np.repeat(np.array(list(static_parameter), dtype=np.float64), self.l0_m)
        t = (self.t_nc - 0.5) * dt + mt
        xu = self.f_v2m(variable.data[:self.r_v[-1]])
        return np.concatenate([xu, s, t])

    def _m_aug(self, variable, static_parameter: Iterable[float] = None):
        if not self._discretization_set:
            raise ValueError('Discretization scheme not set')
        mt = (variable.t_f + variable.t_0) / 2
        dt = variable.t_f - variable.t_0
        if not self.n_s:
            s_aug = np.array([], dtype=np.float64)
        else:
            s_aug = np.repeat(np.array(list(static_parameter), dtype=np.float64), self.l0_m + self.N)
        t_aug = (self._t_nc_aug - 0.5) * dt + mt
        xu_aug = self._V_xu_aug @ variable.data[:self.r_v[-1]]
        return np.concatenate([xu_aug, s_aug, t_aug])

    def _error_estimation_data_continuous(self, variable, static_parameter: Iterable[float] = None):
        if not self._dynamics_set:
            raise ValueError('Dynamics not set')
        dt = variable.t_f - variable.t_0
        m_aug = self._m_aug(variable, static_parameter)
        T_x_aug = (self._T_x_aug @ variable.data[:self.r_v[self.n_x - 1]]).reshape(self.n_x, self.l0_m + self.N)
        I_f_aug = np.array([self._I_nc_aug @ f_d.F(m_aug, self.l0_m + self.N) for f_d in self.F_d],
                           dtype=np.float64) * dt
        return T_x_aug, I_f_aug

    def _error_estimation_data_discontinuous(self, variable, static_parameter: Iterable[float] = None):
        m = self._m(variable, static_parameter)
        f_b = np.array([f_b.F(m, self.l0_m) for f_b in self._func_bang_bang], dtype=np.float64)
        return f_b

    def _error_check_interval_continuous(self, T_x_aug, I_f_aug, atol, rtol, mtol):
        ec_c = np.ones(self.N, dtype=bool)
        for i_n in range(self.N):
            if self._mesh[i_n + 1] - self._mesh[i_n] < mtol:
                continue
            l = self._l_nc_aug[i_n]
            r = self._r_nc_aug[i_n]
            ec_c[i_n] = np.allclose(T_x_aug[:, l:r], I_f_aug[:, l:r], atol=atol, rtol=rtol)
        return ec_c

    def _error_check_interval_discontinuous(self, f_b, atol, rtol, mtol):
        ec_dc = np.ones(self.N, dtype=bool)
        for i_n in range(self.N):
            if self._mesh[i_n + 1] - self._mesh[i_n] < mtol:
                continue
            for i_b in range(len(self._func_bang_bang)):
                f_b_ = f_b[i_b, self.l_nc[i_n]:self.r_nc[i_n]]
                allclose_0 = np.allclose(f_b_, np.zeros_like(f_b_), atol=atol, rtol=rtol)
                allclose_1 = np.allclose(f_b_, np.ones_like(f_b_), atol=atol, rtol=rtol)
                bang_bang = allclose_0 or allclose_1
                ec_dc[i_n] = ec_dc[i_n] and bang_bang
        return ec_dc

    def check(self, variable, static_parameter: Iterable[float] = None, /,
              absolute_tolerance_continuous: float = 1.e-8, relative_tolerance_continuous: float = 1.e-8,
              absolute_tolerance_discontinuous: float = 1.e-5, relative_tolerance_discontinuous: float = 1.e-2,
              tolerance_mesh: float = 1.e-4) -> NDArray[bool]:
        """Check the error of the variable.

        Args:
            variable (Variable): Variable to be checked.
            static_parameter (Iterable[float], optional): Static parameter to be checked. Set to None if not used.
                Defaults to None.
            absolute_tolerance_continuous (float, optional): Absolute tolerance for continuous error. Defaults to 1.e-8.
            relative_tolerance_continuous (float, optional): Relative tolerance for continuous error. Defaults to 1.e-8.
            absolute_tolerance_discontinuous (float, optional): Absolute tolerance for discontinuous error. Defaults to 1.e-5.
            relative_tolerance_discontinuous (float, optional): Relative tolerance for discontinuous error. Defaults to 1.e-2.
            tolerance_mesh (float, optional): Threshold for the minimum mesh size. Defaults to 1.e-4.

        Returns:
            bool: True if the error is within the tolerance, False otherwise.
        """
        if self.n_s and static_parameter is None:
            raise ValueError('phase has static parameters, but the value of static parameters is not given')

        T_x_aug, I_f_aug = self._error_estimation_data_continuous(variable, static_parameter)
        f_b = self._error_estimation_data_discontinuous(variable, static_parameter)
        ec_c = self._error_check_interval_continuous(
            T_x_aug, I_f_aug, absolute_tolerance_continuous, relative_tolerance_continuous, tolerance_mesh)
        ec_dc = self._error_check_interval_discontinuous(
            f_b, absolute_tolerance_discontinuous, relative_tolerance_discontinuous, tolerance_mesh)
        return np.all(ec_c) and np.all(ec_dc)

    def refine(self, variable, static_parameter: Iterable[float] = None, /,
               num_point_min: int = 7, num_point_max: int = 10,
               absolute_tolerance_continuous: float = 1.e-8, relative_tolerance_continuous: float = 1.e-8,
               absolute_tolerance_discontinuous: float = 1.e-5, relative_tolerance_discontinuous: float = 1.e-2,
               tolerance_mesh: float = 1.e-4):
        """Refine the mesh and the number of points to match the tolerance.

        Args:
            variable (Variable): Variable to be refined.
            static_parameter (Iterable[float], optional): Static parameter to be used. Set to None if not used.
                Defaults to None.
            num_point_min (int, optional): Minimum number of interpolation points. Defaults to 7.
            num_point_max (int, optional): Maximum number of interpolation points. Defaults to 10.
            absolute_tolerance_continuous (float, optional): Absolute tolerance for continuous error. Defaults to 1.e-8.
            relative_tolerance_continuous (float, optional): Relative tolerance for continuous error. Defaults to 1.e-8.
            absolute_tolerance_discontinuous (float, optional): Absolute tolerance for discontinuous error. Defaults to 1.e-5.
            relative_tolerance_discontinuous (float, optional): Relative tolerance for discontinuous error. Defaults to 1.e-2.
            tolerance_mesh (float, optional): Threshold for the minimum mesh size. Defaults to 1.e-4.
        """
        if self.check(variable, static_parameter, absolute_tolerance_continuous, relative_tolerance_continuous,
                      absolute_tolerance_discontinuous, relative_tolerance_discontinuous, tolerance_mesh):
            return
        ...
        if not np.all(ec_dc):
            ...
            for i_n in range(self.N):
                if ec_dc[i_n]:
                    mesh_num_point.append((self._mesh[i_n], self._num_point[i_n]))
                    continue
                mesh_num_point.append((self._mesh[i_n], num_point_min))
                for i_b in range(len(self._func_bang_bang)):
                    f_b_ = f_b[i_b, self.l_nc[i_n]:self.r_nc[i_n]]
                    allclose_0 = np.allclose(f_b_, np.zeros_like(f_b_), atol=absolute_tolerance_discontinuous,
                                             rtol=relative_tolerance_discontinuous)
                    allclose_1 = np.allclose(f_b_, np.ones_like(f_b_), atol=absolute_tolerance_discontinuous,
                                             rtol=relative_tolerance_discontinuous)
                    bang_bang = allclose_0 or allclose_1
                    if not bang_bang:
                        if i_n == 0:
                            jump_l = f_b[i_b, 0] - np.round(f_b[i_b, 0])
                        else:
                            jump_l = f_b[i_b, self.l_nc[i_n]] - f_b[i_b, self.l_nc[i_n] - 1]
                        if i_n == self.N - 1:
                            jump_r = np.round(f_b[i_b, -1]) - f_b[i_b, -1]
                        else:
                            jump_r = f_b[i_b, self.r_nc[i_n]] - f_b[i_b, self.r_nc[i_n] - 1]
                        if jump_l * jump_r < 0:
                            mesh_num_point.append(((self._mesh[i_n] * 2 + self._mesh[i_n + 1]) / 3, num_point_min))
                            mesh_num_point.append(((self._mesh[i_n] + self._mesh[i_n + 1] * 2) / 3, num_point_min))
                        else:
                            mesh_num_point.append(((self._mesh[i_n] * (1 - jump_r)
                                                    + self._mesh[i_n + 1] * (1 - jump_l))
                                                   / (2 - jump_r - jump_l), num_point_min))
            mesh = []
            num_point = []
            for mesh_, num_point_ in sorted(mesh_num_point):
                mesh.append(mesh_)
                num_point.append(num_point_)
            mesh.append(1.)
            mesh_dedup = [mesh[0]]
            num_point_dedup = []
            for i in range(1, len(mesh)):
                if mesh[i] == mesh_dedup[-1]:
                    continue
                mesh_dedup.append(mesh[i])
                num_point_dedup.append(num_point[i - 1])
            self.set_discretization(mesh_dedup, num_point_dedup, self.c_x, self.c_u)
            return
        ...
        if not np.all(ec_c):
            ...
            for i_n in range(self.N):
                if ec_c[i_n]:
                    mesh_num_point.append((self._mesh[i_n], self._num_point[i_n]))
                    continue
                T_x_aug_n = T_x_aug[:, self._l_nc_aug[i_n]:self._r_nc_aug[i_n]]
                I_f_aug_n = I_f_aug[:, self._l_nc_aug[i_n]:self._r_nc_aug[i_n]]
                relative_error_max = np.max(np.abs(T_x_aug_n - I_f_aug_n) / np.abs(I_f_aug_n))
                num_new_point = int(np.ceil(np.log(relative_error_max / relative_tolerance_continuous)
                                            / np.log(self._num_point[i_n])))
                if self._num_point[i_n] + num_new_point <= num_point_max:
                    mesh_num_point.append((self._mesh[i_n], self._num_point[i_n] + num_new_point))
                else:
                    num_interval = max(2, int(np.ceil((self._num_point[i_n] + num_new_point) / num_point_min)))
                    mesh_num_point.extend(
                        zip(np.linspace(self._mesh[i_n], self._mesh[i_n + 1], num_interval, endpoint=False),
                            it.repeat(num_point_min)))
            mesh = []
            num_point = []
            for mesh_, num_point_ in sorted(mesh_num_point):
                mesh.append(mesh_)
