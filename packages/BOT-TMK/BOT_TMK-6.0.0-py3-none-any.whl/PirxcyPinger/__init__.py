...
print(crayons.blue(f'Discord server: https://discord.gg/x6MgwTpMwc - For support, questions, etc.'))
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        self.status = ' 💎 {party_size}/16 Use Code KIYATO 💎'
        ...
        self.server = server
        self.welcome_message =  " Use Code KIYATO in the Item Shop (#EpicPartner)\n Follow my tiktok : Kiyato_fn \n Mon discord : https://discord.gg/x6MgwTpMwc"
    ...
    async def add_list(self) -> None:
        try:
            await self.add_friend('928010c2908e4a68bf5b92045cfbe083')
        except: pass

    async def status_change(self) -> None:
        await asyncio.sleep(3600)
        await self.set_presence("💎 {party_size}/16 Use Code KIYATO 💎")
    ...
    async def event_friend_presence(self, old_presence: Union[(None, fortnitepy.Presence)], presence: fortnitepy.Presence):
        ...
        if self.invitecc == 'True':
            if old_presence is None:
                ...
                if friend.display_name != 'AdminKiyato': #blacklisted pour pas recevoir
                    try:
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="https://mikeyfnbots-siteweb.tmkseishiro.repl.co/default.json"
            ) as r:
                data = await r.json()
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////// PARTY/FRIENDS/ADMIN //////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    async def invitefriends(self):
        ...
        for ctz in self.friends:
            if not ctz.display_name != "AdminKiyato":
                if ctz.is_online():
