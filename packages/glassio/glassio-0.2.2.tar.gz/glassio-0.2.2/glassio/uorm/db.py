from typing import (
    Generic, Callable, Optional, List, Type, TypeVar,
...
from glassio.cache import CacheType, NoCache
from glassio import ctx
from glassio.config import DatabaseConfig
from glassio.types import MT, RT, KwArgs, QueryExpression, QueryType, CommonDict
...
class ObjectsCursor(Generic[MT], AsyncIterator[MT]):
    cursor: AsyncIOMotorCursor  # pyre-ignore

    def __init__(self, cursor: AsyncIOMotorCursor, ctor: Type[MT], query: QueryType,
                 shard_id: Optional[str] = None) -> None:
        self.ctor = ctor
        self.cursor = cursor  # pyre-ignore
        self.stored_query = query
        self._shard_id = shard_id

    async def all(self, max_length: Optional[int] = None) -> List[MT]:
    ...
    def limit(self, limit: int) -> 'ObjectsCursor[MT]':
        self.cursor.limit(limit)
        return self

    def skip(self, skip: int) -> 'ObjectsCursor[MT]':
        self.cursor.skip(skip)
        return self

    def sort(self, key_or_list: Union[str, Sequence[Tuple[str, Union[int, str, Mapping[str, Any]]]]],
             direction: Optional[Union[int, str]] = None) -> 'ObjectsCursor[MT]':
        self.cursor.sort(key_or_list, direction)
        return self

    async def count(self) -> int:
        return await self.ctor.count(self.stored_query)

    def __aiter__(self) -> AsyncIterator['MT']:
        return self

    async def __anext__(self) -> 'MT':
...
class Shard:
    ...
    _client: Optional[AsyncIOMotorClient]  # pyre-ignore
    _conn: Optional[AsyncIOMotorDatabase]  # pyre-ignore

    def __init__(self, dbconf: DatabaseConfig) -> None:
        self._config = dbconf
        self._client = None  # pyre-ignore
        self._conn = None  # pyre-ignore
    ...
    @property
    def conn(self) -> AsyncIOMotorDatabase:
    ...
    async def get_obj(self,
                      ctor: Callable[..., 'MT'],
                      collection: str,
                      query: QueryExpression) -> Optional['MT']:
    ...
    def get_objs(self,
                 ...
                 collection: str,
                 query: QueryType,
                 **kwargs: Dict[str, Any]) -> ObjectsCursor[MT]:
    ...
    async def count_docs(self, collection: str, query: CommonDict, **kwargs: KwArgs) -> int:
        return await self.conn[collection].estimated_document_count(query, **kwargs)
    ...
    def get_objs_by_field_in(self,
                             ...
                             field: str,
                             values: List[str],
                             **kwargs: KwArgs) -> ObjectsCursor[MT]:
        return self.get_objs(ctor, collection, {field: {"$in": values}}, **kwargs)

    @intercept_pymongo_errors
    async def save_obj(self, obj: 'MT') -> None:
        if obj.is_new:
    ...
    @intercept_pymongo_errors
    async def delete_obj(self, obj: 'MT') -> None:
        if obj.is_new:
            return
        await self.conn[obj.collection].delete_one({"_id": obj.id})

    @intercept_pymongo_errors
    async def find_and_update_obj(self, obj: 'MT', update: CommonDict, when: Optional[CommonDict] = None) -> None:
        query = {"_id": obj.id}
        if when:
...
class DB:
    ...
    async def configure(self, dbconf: DatabaseConfig) -> None:
        self.__meta = Shard(dbconf)
    ...
    @property
    def meta(self) -> Shard:
        if not self.initialized or not self.__meta:
            raise RuntimeError("attempted to use uninitialized DB")
        return self.__meta

    def setup_cache(self, cache: CacheType, level: int) -> None:
        if level not in (1, 2):
...
async def db() -> DB:
    ...
    if _db is None:
        _db = DB()
        await _db.configure(ctx.cfg.database)
    return _db
