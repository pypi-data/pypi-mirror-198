...
class MigrationLog(LoggerBuilder):
    ...
    def log_exception(exception):
        MigrationLog.logger.exception(exception)

    @staticmethod
    def log_no_raw_processed_product(survey):
        MigrationLog.logger.warning(f"No raw, processed, or product dataset found for survey {survey}. Survey will not be migrated.")

    @staticmethod
    def log_no_file_label_error(mb_file: MbFile):
        MigrationLog.logger.error(f"no valid file category identified for file {mb_file.data_file} with ngdc_id {mb_file.ngdc_id}")
