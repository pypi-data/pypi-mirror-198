...
class Loader:
    def __init__(self, message, bar_length=30):
    ...
    def progress_bar(self):
        percent = int((self.progress / self.bar_length) * 100)
        filled = int(self.progress / self.bar_length * self.bar_length)
    ...
    def animate(self):
        ...
        idx = 0
        while not self.stop_flag:
            sys.stdout.write(f"\r{self.message} [{Fore.YELLOW}{animation[idx]}{Style.RESET_ALL}]")
            sys.stdout.flush()
            idx = (idx + 1) % len(animation)
            time.sleep(0.1)

        sys.stdout.write('\r' + ' ' * (len(self.message) + self.bar_length + 4) + '\r')
        sys.stdout.flush()
