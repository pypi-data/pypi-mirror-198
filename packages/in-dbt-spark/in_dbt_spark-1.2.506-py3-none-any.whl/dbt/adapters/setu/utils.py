...
def get_grestin_certs() -> Optional[Tuple[str, str]]:
    certs = None
    if os.environ.get(PLATFORM_KEY) == DARWIN_PLATFORM:
        grestin_certs_dir = os.getenv(GRESTIN_DIR_PATH_KEY, None)
        logger.info("PLATFORM : {platform}".format(platform=DARWIN_PLATFORM))
        logger.info(
            "grestin_certs_dir : {grestin_certs_dir}".format(grestin_certs_dir=grestin_certs_dir)
        )
        if grestin_certs_dir:
            certs = (
                grestin_certs_dir + "identity.cert",
                grestin_certs_dir + "identity.key",
            )
            ...
            raise Exception(
                ...
                "{platform}".format(env_var=GRESTIN_DIR_PATH_KEY, platform=DARWIN_PLATFORM)
            )
    return certs
        ...
        if datavault_token_path:
            ...
            raise Exception(
                "Please set path for datavault using env var : {env_var} for platform {platform}".format(
                    env_var=DATAVAULT_TOKEN_PATH_KEY, platform=DARWIN_PLATFORM
                )
            )
