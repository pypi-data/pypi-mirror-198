...
from pypgx._utils.error_messages import INVALID_OPTION, PROPERTY_NOT_FOUND, UNHASHABLE_TYPE
...
from pypgx.api.mllib._edge_combination_method import ConcatEdgeCombinationMethod
...
class Analyst:
    ...
    def deepwalk_builder(
        ...
        if validation_fraction is not None:
            warnings.warn(
                "validation_fraction is deprecated since 22.4",
                DeprecationWarning
            )
            java_handler(builder.setValidationFraction, [validation_fraction])
        if seed is not None:
    ...
    def supervised_graphwise_builder(
        ...
        for c_layer_config in conv_layer_config:
            if not isinstance(c_layer_config, GraphWiseConvLayerConfig):
        ...
        if batch_gen not in pgx_types.BATCH_GENERATORS.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='batch_gen',
                    opts=', '.join(pgx_types.BATCH_GENERATORS.keys())
                )
            )
        java_handler(
            builder.setBatchGenerator, [pgx_types.BATCH_GENERATORS[batch_gen](*batch_gen_params)]
        )
    ...
    def supervised_edgewise_builder(
        ...
        *,
        ...
        standardize: bool = False,
        normalize: bool = False,
        edge_combination_method: Optional[Union[ConcatEdgeCombinationMethod]] = None,
    ) -> SupervisedEdgeWiseModel:
    ...
    def unsupervised_graphwise_builder(
        ...
        if loss_fn not in pgx_types.UNSUPERVISED_LOSS_FUNCTIONS.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='loss_fn',
                    opts=', '.join(pgx_types.UNSUPERVISED_LOSS_FUNCTIONS.keys())
                )
            )
        java_handler(builder.setLossFunction, [pgx_types.UNSUPERVISED_LOSS_FUNCTIONS[loss_fn]])
    ...
    def graphwise_pred_layer_config(
        ...
        if activation_fn not in pgx_types.ACTIVATION_FUNCTIONS.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='activation_fn',
                    opts=', '.join(pgx_types.ACTIVATION_FUNCTIONS.keys())
                )
            )
        java_handler(builder.setActivationFunction, [pgx_types.ACTIVATION_FUNCTIONS[activation_fn]])

        if weight_init_scheme not in pgx_types.WEIGHT_INIT_SCHEMES.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='weight_init_scheme',
                    opts=', '.join(pgx_types.WEIGHT_INIT_SCHEMES.keys())
                )
            )
        java_handler(
            builder.setWeightInitScheme, [pgx_types.WEIGHT_INIT_SCHEMES[weight_init_scheme]]
        )
    ...
    def graphwise_dgi_layer_config(
        ...
        if readout_function not in pgx_types.READOUT_FUNCTIONS.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='readout_function',
                    opts=', '.join(pgx_types.READOUT_FUNCTIONS.keys())
                )
            )
        java_handler(builder.setReadoutFunction, [pgx_types.READOUT_FUNCTIONS[readout_function]])

        if discriminator not in pgx_types.DISCRIMINATOR_FUNCTIONS.keys():
            raise ValueError(
                INVALID_OPTION.format(
                    var='discriminator',
                    opts=', '.join(pgx_types.DISCRIMINATOR_FUNCTIONS.keys())
                )
            )
        java_handler(builder.setDiscriminator, [pgx_types.DISCRIMINATOR_FUNCTIONS[discriminator]])
    ...
    def weighted_pagerank(
        ...
        :example:
            ...
            .. code-block:: python
                ...
                graph = ....
                cost = graph.get_or_create_edge_property(
                    name="cost", data_type='double', dim=0)
    ...
    def louvain(
        ...
        :example:
            ...
            .. code-block:: python
                ...
                graph = ....
                weight = graph.get_or_create_edge_property(
                    name="cost", data_type='double', dim=0)
                partition = analyst.louvain(
                    graph, weight, community='community')
                result_set = graph.query_pgql(
    ...
    def shortest_path_bidirectional_dijkstra(
        ...
        :example:
            ...
            .. code-block:: python
                    ...
                    src = graph.get_vertex("1")
                    dst = graph.get_vertex("2")
                    cost = graph.graph.get_or_create_edge_property(
                        name="cost", data_type='double', dim=0)
                    filter = EdgeFilter("edge.cost > 5")
    ...
    def filtered_bfs(
        ...
        :example:
            ...
            .. code-block:: python
                ...
                graph = ....
                prop1 = graph.get_or_create_vertex_property(
                    name="prop1", data_type='double', dim=0)
                vertex = graph.get_vertex("1")
                navigator = VertexFilter("vertex.prop1 < 2")
        ...
        :example:
            ...
            .. code-block:: python
                ...
                is_left = graph.get_or_create_vertex_property(
                    name="is_left", data_type='boolean', dim=0)
