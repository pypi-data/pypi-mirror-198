...
class JobMonitor():
    def __init__(self, progress_check_duration):
        ...
        self.child_job_timeout = timedelta(seconds=progress_check_duration)
    ...
    def handle_stalled_job(self, job_doc):
        ...
        if self.get_tries_left(number_of_tries, max_number_of_tries) > 0:
            #Requeue
            print("Requeued child job id" + str(job_doc["_id"]))
            return self.requeue(job_doc, number_of_tries+1)
        else:
            print("Failed stalled child job id" + str(job_doc["_id"]))
