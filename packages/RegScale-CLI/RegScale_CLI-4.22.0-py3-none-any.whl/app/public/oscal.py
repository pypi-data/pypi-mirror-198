...
def convert(input: click.Path, output: click.Path, command_type: str, file_type: str):
...
def oscal_convert(
    input: click.Path, output: click.Path, command_type: str, file_type: str
) -> None:
    """
...
def upload_catalog(
    ...
    with job_progress:
        ...
        if len(new_regscale_controls) > 0:
            ...
            obj_parts = [part for part in parts if part["objectiveType"] == "objective"]
            if obj_parts:
                # log the information
                logger.debug(
...
def create_objectives(args: Tuple, thread: int) -> None:
    ...
    obj_parts = [obj for obj in objectives if obj["objectiveType"] == "objective"]
