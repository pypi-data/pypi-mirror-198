...
from typing import Any, Callable, Dict, Iterable, Optional, Sized, Tuple, Union
...
class Menu(BaseMenu):
    ...
    async def get_content(
    ...
    ) -> Tuple[Optional[str], Optional[InputMedia]]:
        ...
        if isinstance(content, Sized) and len(content) == 2:
            return content
        elif isinstance(content, str):
            return (content, None)
        elif isinstance(content, InputMedia):
            return (None, content)
        else:
            raise ValueError("content must be of type InputMedia or str")
    ...
    async def call_preliminary(
        ...
        if self.preliminary:
            preliminary = [self.preliminary] if not isinstance(self.preliminary, Iterable) else self.preliminary
            for func in preliminary:
                if iscoroutinefunction(func):
                    await func(handler, client, context, parameters)
                elif callable(func):
                    func(handler, client, context, parameters)
    ...
    async def on_callback(
        ...
        await self.call_preliminary(handler, client, callback, parameters)

        text, media = await self.get_content(handler, client, callback, parameters)
        ...
        if media:
            ...
            await callback.edit_message_media(
                media,
                reply_markup=self.keyboard(handler, client, callback, parameters),
                **self.kwargs
            )
        elif text:
    ...
    async def on_message(
        ...
        await self.call_preliminary(handler, client, message, parameters)

        text, media = await self.get_content(handler, client, message, parameters)

        if media:
            await message.reply_cached_media(
                file_id=media.media,
                caption=text or media.caption,
                reply_markup=self.keyboard(handler, client, message, parameters),
                **self.kwargs
            )
        elif text:
    ...
    def keyboard(
        ...
        keyboard = self.style.generate(handler, client, context, parameters, self)
