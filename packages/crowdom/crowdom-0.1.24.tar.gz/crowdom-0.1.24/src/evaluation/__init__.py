...
def collect_evaluations_from_check_assignments(
    ...
    for task_labels_probas, worker_labels in classification.collect_labels_probas_from_assignments(
        ...
        evaluations.append(
            SolutionEvaluation(
                ok=confidence >= confidence_threshold, confidence=confidence, worker_labels=worker_labels
            )
        )
...
# Задание финализируется, если соблюдено хотя бы одно из условий:
# 1) У него есть решение приемлемого качества.
# 2) Число попыток исчерпано.
# 3) Мы проверяем не все решения со страниц, задание со страницы не проверено,
#    но точность страницы достаточно высокая.
