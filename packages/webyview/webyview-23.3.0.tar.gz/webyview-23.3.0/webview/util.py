...
def js_bridge_call(window, func_name, param, value_id):
    def _call():
        try:
            ...
            code = 'window.webyview._returnValues["{0}"]["{1}"] = {{value: \'{2}\'}}'.format(func_name, value_id, result)
        except Exception as e:
            error = {
                'message': str(e),
            ...
            code = 'window.webyview._returnValues["{0}"]["{1}"] = {{isError: true, value: \'{2}\'}}'.format(func_name, value_id, result)
