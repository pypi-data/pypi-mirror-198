...
def _api_call(function, event_type):
    """
    Decorator to call a webyview API, checking for _webview_ready and raisings
...
class Window:
    ...
    def get_elements(self, selector):
        ...
        code = """
            ...
            for (var i = 0; i < elements.length; i++) {
                var node = webyview.domJSON.toJSON(elements[i], {
                    metadata: false,
    ...
    def evaluate_js(self, script, callback=None):
        ...
        if callback:
            escaped_script = """
                var value = eval("{0}");
                if (webyview._isPromise(value)) {{
                    value.then(function evaluate_async(result) {{
                        webyview._asyncCallback(JSON.stringify(result), "{1}")
                    }});
                    "true";
                }} else {{ {2} }}
    ...
    def expose(self, *functions):
        ...
        if self.events.loaded.is_set():
            self.evaluate_js('window.webyview._createApi(%s)' % func_list)
