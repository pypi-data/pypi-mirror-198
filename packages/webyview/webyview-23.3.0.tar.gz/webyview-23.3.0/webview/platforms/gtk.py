...
logger = logging.getLogger('webyview')
...
class BrowserView:
    ...
    def __init__(self, window):
        ...
        storage_path = _storage_path or os.path.join(os.path.expanduser('~'), '.webyview')
    ...
    def close_window(self, *data):
        should_cancel = self.webyview_window.events.closing.set()
        ...
        if self.webyview_window in windows:
            windows.remove(self.webyview_window)

        self.webyview_window.events.closed.set()
    ...
    def on_window_state_change(self, window, window_state):
        if window_state.changed_mask == Gdk.WindowState.ICONIFIED:
            ...
            if Gdk.WindowState.ICONIFIED & window_state.new_window_state == Gdk.WindowState.ICONIFIED:
                self.webyview_window.events.minimized.set()
            else:
                self.webyview_window.events.restored.set()
            ...
            if Gdk.WindowState.MAXIMIZED & window_state.new_window_state == Gdk.WindowState.MAXIMIZED:
                self.webyview_window.events.maximized.set()
            else:
                self.webyview_window.events.restored.set()
    ...
    def on_window_resize(self, window, allocation):
        ...
        if allocation.width != self._last_width or allocation.height != self._last_height:
            ...
            self.webyview_window.events.resized.set(allocation.width, allocation.height)

    def on_window_configure(self, window, event):
        self.webyview_window.events.moved.set(event.x, event.y)
    ...
    def on_js_callback(self, js_data):
        try:
            if 'type' not in js_data:
                ...
                # code = 'webyview._bridge.return_val = "{0}";'.format(escape_string(str(return_val)))
                return return_val
    ...
    def show(self):
        ...
        if gtk.main_level() == 0:
            if self.webyview_window.hidden:
                self.window.hide()
        else:
    ...
    def _set_js_api(self):
        def create_bridge():
            self.webview.run_javascript(parse_api_js(self.js_bridge.window, 'gtk', uid=self.webyview_window.uid))
            self.loaded.set()
...
def get_active_window():
    ...
    for uid, browser_view_instance in BrowserView.instances.items():
        if browser_view_instance.window.get_id() == active_window_number:
            return browser_view_instance.webyview_window
