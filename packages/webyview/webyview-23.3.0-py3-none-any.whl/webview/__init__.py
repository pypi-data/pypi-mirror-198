...
formatter = logging.Formatter('[webyview] %(message)s')
handler.setFormatter(formatter)
logger.addHandler(handler)

log_level = logging.DEBUG if os.environ.get('webyview_LOG') == 'debug' else logging.INFO
...
DRAG_REGION_SELECTOR = '.webyview-drag-region'
...
def start(func=None, args=None, localization={}, gui=None, debug=False, http_server=False,
    ...
    if threading.current_thread().name != 'MainThread':
        raise WebViewException('webyview must be run on a main thread.')
