...
_profile_storage_path = _storage_path or os.path.join(os.path.expanduser('~'), '.webyview')
...
class BrowserView(QMainWindow):
    ...
    def __init__(self, window):
        ...
        if is_webengine:
            if _private_mode:
                ...
                self.profile = QWebEngineProfile('webyview')
    ...
    def resizeEvent(self, e):
        if self.webyview_window.initial_width != self.width() or \
           self.webyview_window.initial_height != self.height():
            self.webyview_window.events.resized.set(self.width(), self.height())

    def eventFilter(self, object, event):
        if event.type() == QtCore.QEvent.Move:
            self.webyview_window.events.moved.set(self.x(), self.y())
...
def get_active_window():
    ...
    if active_window:
        return active_window.webyview_window
