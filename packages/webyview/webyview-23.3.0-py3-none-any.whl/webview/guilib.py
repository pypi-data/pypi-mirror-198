...
def initialize(forced_gui=None):
    ...
    if not forced_gui:
        ...
        forced_gui = os.environ['webyview_GUI'].lower() \
            if 'webyview_GUI' in os.environ and os.environ['webyview_GUI'].lower() in ['qt', 'gtk', 'cef', 'mshtml', 'edgechromium'] \
            else forced_gui
    ...
    if platform.system() == 'Darwin':
        ...
        if not try_import(guis):
            raise WebViewException('You must have either PyObjC (for Cocoa support) or Qt with Python bindings installed in order to use webyview.')
        ...
        if not try_import(guis):
            raise WebViewException('You must have either QT or GTK with Python extensions installed in order to use webyview.')
        ...
        if not try_import(guis):
            raise WebViewException('You must have pythonnet installed in order to use webyview.')
    else:
