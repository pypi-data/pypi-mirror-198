...
def createDemoWidget():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "demo")
...
def createWidget(root):
    ...
    addWidget(os.path.join(root, script_name), False)
    print("create widget success")

def CheckWidgetDataInPath(path, silence):
...
def addWidget(root, silence):
    if CheckWidgetDataInPath(root, silence) == False:
        return
...
def publishWidget(root):
    if CheckWidgetDataInPath(root, False) == False:
        return
    #h5 folder
    package_folder = ""
    if folderIsH5(root):
