...
try:
    ...
    logging.warning(e)
    subprocess.run("{} -m pip install retrying".format(sys.executable))
...
def _run_cmd(cmd):
    ...
    if not completed_p.returncode:
        logging.info("The dependent library is already installed")
    else:
...
def get_pubnet_ip():
    try:
        ...
        logging.warning("Installing dependent libraries: requests")
        if OS_NAME == "Windows":
