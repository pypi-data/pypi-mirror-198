...
ROOT_DIR = f"{os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))}/codeformer"
WEIGHTS_DIR = os.path.join(ROOT_DIR, "weights/dlib")
...
class FaceRestoreHelper(object):
    ...
    def init_dlib(self, detection_path, landmark5_path):
        ...
        detection_path = load_file_from_url(
            url=detection_path, model_dir=WEIGHTS_DIR, progress=True, file_name=None
        )
        landmark5_path = load_file_from_url(
            url=landmark5_path, model_dir=WEIGHTS_DIR, progress=True, file_name=None
        )
