...
from test5_powerhouse_helper.AddressBook.addressbook_data import *

logger = logging.getLogger("ab_log")
logger.setLevel(logging.INFO)
formatter = logging.Formatter("[%(asctime)s]   %(message)s")
fh = logging.FileHandler("test5_powerhouse_helper\\AddressBook\\address_saves\\address_book.log")
...
class AddressBook:
    ...
    def save(self):
        with open('test5_powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin', "wb") as fd:
            pickle.dump(self.data, fd)
            logger.info(f"Address Book has been saved")
            return f"Address Book has been saved"

    def load(self):
        with open('test5_powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin', "rb") as fd:
            if not os.stat('test5_powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin').st_size == 0:
                self.data = pickle.load(fd)
                logger.info(f"Address Book has been loaded")
                return f"Address Book has been loaded from auto save file"
    ...
    def delete(self, contact_info_to_remove):
        try:
            ...
            return f"Contact {current_contact_name} has been deleted"
        except IndexError:
            return "There is no such contact!"
