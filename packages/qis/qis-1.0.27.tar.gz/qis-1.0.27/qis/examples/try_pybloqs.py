...
def generate_performances(prices: pd.DataFrame,
    ...
    fig1, ax = plt.subplots(1, 1, figsize=(12, 4), tight_layout=True)
    ...
    r = p.VStack([p.Block("Hello World!", title="Announcement"),
              p.Block(fig1), p.Block(fig2), p.Block(fig3), p.Block(fig4), p.Block(fig5)])
    r.save('this.pdf')
