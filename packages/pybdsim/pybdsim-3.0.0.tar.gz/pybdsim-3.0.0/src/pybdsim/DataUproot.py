...
class Histogram4D:
    ...
    def plot_spectrum(self, x=0, y=0, z=0, xlim=[1e-10, 300], ylim=[1e-15, 1e-7], log=True, error_bar=True, **kwargs):
        ...
        bins = bh.axes[3].edges
        centers = bh.axes[3].centers
        widths = bh.axes[3].widths

        values = bh[x, y, z, :].to_numpy()[0]
        ...
        for j in range(len(bins)):
            ...
            if j == 0:
                energies.append(bins[j] * scale)
                spectrum.append(values[j])
            if j == len(values):
                energies.append(bins[j] * scale)
                spectrum.append(values[j - 1])
            else:
                energies.append(bins[j] * scale)
                spectrum.append(values[j - 1])
        ...
        ax.plot(energies, spectrum, **kwargs)

        if error_bar:
            ax.bar(centers * scale, bh[x, y, z, :].to_numpy()[0], widths, yerr=bh_err[x, y, z, :].to_numpy()[0],
                   color='white', edgecolor='white', error_kw=dict(ecolor='black', lw=1))

        ax.set_xlim(xlim[0], xlim[1])

        ax.set_xlabel("Energy (MeV)", fontsize=15, fontweight='bold')
        ax.set_ylabel("Flux (cm$^{-2}$ primary$^{-1}$)", fontsize=15, fontweight='bold')
    ...
    def get_position_in_space(self, x=0, y=0, z=0, origin=[0, 0, 0]):
        ...
        pos_x = origin[0] + centers[0][x]
        pos_y = origin[1] + centers[1][y]
    ...
    @property
    def bins_volume(self):
        return _np.diff(self.bh.axes[0].edges)[0] * \
               _np.diff(self.bh.axes[1].edges)[0] * \
               _np.diff(self.bh.axes[2].edges)[0]
    ...
    def edges(self):
        ...
        edges_e = self.bh.axes[3].edges
        return _np.array([edges_x, edges_y, edges_z, edges_e], dtype=object)

    @property
    def centers(self):
        centers_x = self.bh.axes[0].centers
        centers_y = self.bh.axes[1].centers
        centers_z = self.bh.axes[2].centers
