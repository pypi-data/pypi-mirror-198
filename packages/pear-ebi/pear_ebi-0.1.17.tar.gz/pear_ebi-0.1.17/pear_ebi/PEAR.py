...
try:
    ...
    # ─── Normal Pear ──────────────────────────────────────────────────────
        ...
        if config["dir"] is not None:
            ...
            pattern = (
                config["dir"]["pattern"] if config["dir"]["pattern"] is not None else "*"
            )
        ...
        distance_matrix = (
            args.distance_matrix if args.distance_matrix is not None else distance_matrix
        )
        ...
        if config["highlight"] is not None and config["trees"] is not None:
            ...
            for set_trees in SET.data.keys():
                if config["highlight"][set_trees] is not None:
                    values = np.array([0 for i in range(SET.data[set_trees]["n_trees"])])
                    ...
                    if config["highlight"][f"trace{n_key_tree_config[idx]}"] is not None:
                        values = np.array(
                            [0 for i in range(SET.data[set_trees]["n_trees"])]
                        )
                        values[config["highlight"][f"trace{n_key_tree_config[idx]}"]] = 1
                        highlight.extend(values.tolist())
