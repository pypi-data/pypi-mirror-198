        ...
        if self.get_bank_id() in [super().EQUITY]:
            if self.get_operation() not in [super().MOBILE_WALLET]:
                ...
                if self.get_bank_id() == super().EQUITY:
                    ...
                    if request.get("businessid", None) is not None:
                        self.callback_payload = {
                            'businessid': request.get("businessid", None)
                        }
            ...
            if forward:
                if callback is not None:
                    ...
                    if callback.business_id is not None:
                        self.callback_payload = {
                            'businessid': str(callback.business_id)
                        }
            ...
            if forward:
                if ipn is not None:
                    ...
                    if ipn.business_id is not None:
                        self.callback_payload = {
                            'businessid': str(ipn.business_id)
                        }
    ...
    def register_callback(self):
        ...
        if save_callback:
            ...
            if reference != '' and callback != '':
                Callbacks.objects.create(reference=reference, tenant_id=request.get("__tenant", None), business_id=request.get("businessid", None),
                                         callback=callback,
