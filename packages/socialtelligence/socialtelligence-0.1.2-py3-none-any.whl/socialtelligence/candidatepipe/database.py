...
class Connection:
    ...
    def update(self, table_name, d):
        if "id" not in d:
            raise pymysql.err.OperationalError(0, "Parameter 'id' is missing")
        if len(d) == 1:
            raise pymysql.err.OperationalError(0, "Parameter 'id' can not be changed")
        sets = ""
        args = []
        ...
        return self.sql(sql, args=tuple(args))