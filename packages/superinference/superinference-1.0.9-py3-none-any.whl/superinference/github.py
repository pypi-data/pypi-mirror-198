...
class GithubProfile:
    ...
    def _process_api_response(self, url, item_name="items", json_key=None):
        item_list = []
        message = None
        while url:
    ...
    def _repository_inference(self, top_repo_n=3, include_private=False):
        ...
        repos, repo_message = self._process_api_response(repo_url, "repos")
    ...
    def _contribution_inference(self, include_private=False):
        ...
        issue_data, issue_message = self._process_api_response(issue_url, "issues", "items")
        pr_data, pr_message = self._process_api_response(pr_url, "PRs", "items")
        issues, prs = [], []
    ...
    def _activity_inference(self, repos, top_repo_n=3, include_private=False):
        if include_private:
            self._username_token_check()
        ...
        commit_data, commit_message = self._process_api_response(commit_url, "commits", "items")
    ...
    def perform_inference(self, top_repo_n=3, top_language_n=3, include_private=False):
        ...
        skill = self._skill_inference(profile['bio'], repository['original_repos'], repository['repo_api_message'], top_language_n=top_language_n)
        activity = self._activity_inference(repos= repository['original_repos'], top_repo_n=top_repo_n, include_private=include_private)
