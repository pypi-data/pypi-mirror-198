...
from ._base_monitor import CoreMonitorConfig, CoreMonitor, MonitorCliCommand, CoreMonitorSettings, \
    GenericRenderer
from ..shared import SocketMessage, Styles
from ..shared.dto import FanInfo
...
class FanSpeedMonitor(CoreMonitor[FanInfo, CoreMonitorConfig]):
    ...
    def _format_data_impl(self, msg: SocketMessage[FanInfo]) -> pt.RT | list[pt.RT]:
        ...
        if self._state.is_alt_mode:
            ...
            pref_st = pt.Style(pref_st, italic=True)
            int_st = pt.Style(int_st, italic=True)
            frac_st = pt.Style(frac_st, italic=True)
        else:
            label = " "
