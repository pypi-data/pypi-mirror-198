...
class OptionsCliCommand(CliCommand):
    ...
    +==================================================================+
    \b
    Also see{ES7S_TRACE}below.
    """,
        ),
    ]
    ENVIRONMENT = [
        ("{ES7S_CLI}", "Contains path to CLI entrypoint of 'es7s' system. Read by tmux."),
        ("",),
        (
            "{ES7S_TRACE}",
            "Non-empty string increases the verbosity to max level (same as "
            "'--trace' or '-vvv') and enables extended output of 'pytermor' "
            "(formatting library) with{PYTERMOR_TRACE_RENDERS.} Works in [CLI], "
            "[DAEMON] and [GTK] domains, whereas '-v' or '--trace' are recognized "
            "only in CLI mode.",
        ),
        ("",),
        (
