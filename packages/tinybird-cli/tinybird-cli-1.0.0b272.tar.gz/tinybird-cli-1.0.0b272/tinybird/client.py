...
class TinyB(object):
    ...
    async def branch_regression_tests(self, branch_id: str, pipes: Optional[List[str]], only_response_times: bool,
                                      limit: int, sample_by_params: int, match: List[str],
                                      failfast: bool, ignore_order: bool, validate_processed_bytes: bool):
        params = {'only_response_times': only_response_times,
                  'limit': limit,
                  'sample_by_params': sample_by_params,
                  'failfast': failfast,
                  'ignore_order': ignore_order,
                  'validate_processed_bytes': validate_processed_bytes}
        if match:  # to avoid sending '()'
            params.update({'match': match})  # type: ignore
        data = json.dumps({'pipes': list(pipes) if pipes else []})
        url = f"/v0/branches/{branch_id}/regression?{urlencode(params)}"
