...
from tinybird.tb_cli_modules.common import CLIException, CatchAuthExceptions, coro, create_tb_client, folder_init, folder_pull, \
...
async def cli(
    ...
    ctx.ensure_object(dict)['client'] = TinyB(config.get('token', None), config['host'], version=VERSION, send_telemetry=True)
