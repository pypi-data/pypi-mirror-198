...
class Agent(object):
    ...
    def make_environ(
        ...
        PATH_INFO = str_to_environ(unquote(PATH_INFO))
        SCRIPT_NAME = str_to_environ(unquote(SCRIPT_NAME))
    ...
    def _wsgi_request(
        ...
        if check_status and response.status[0] not in "23":
            raise BadStatusError(
                "%s %r returned HTTP status %r"
                % (environ["REQUEST_METHOD"], path, response.status)
            )
    ...
    def _request(
        ...
        **kwargs
        ...
        if self.request:
            PATH_INFO = url_join_same_server(self.request.url, PATH_INFO)
        else:
        ...
        if json is not _marker:
            data = json_dumps(json, separators=(",", ":")).encode(charset)
        ...
        if data is not None:
            ...
            envargs = {
                ...
                "CONTENT_LENGTH": str_to_environ(str(len(data))),
            }
    ...
    def post_json(self, path, data, ajax=False, *args, **kwargs):
        ...
        """
        return self._request_json("POST", path, data, ajax, *args, **kwargs)

    def put_json(self, path, data, ajax=False, *args, **kwargs):
        """
        ...
        :param ajax: if True, an 'X-Requested-With: XMLHttpRequest'
                     header will be added
        """
        return self._request_json("PUT", path, data, ajax, *args, **kwargs)

    def _request_json(self, method, path, data, ajax=False, *args, **kwargs):
        if kwargs.pop("ajax", True):
            kwargs["HTTP_X_REQUESTED_WITH"] = "XMLHttpRequest"
        charset = kwargs.pop("charset", self.charset)
        encoded = json_dumps(data, separators=(",", ":")).encode(charset)
        return self._request(
            path,
            method=method,
            data=encoded,
    ...
    def reload(self, follow=True, check_status=True):
        ...
        if self._lxml is not None:
            for src, dst in zip(
                ...
                if src.tag == "input" and src.attrib.get("type") in (
                    "radio",
                    "checkbox",
                ):
    ...
    def json(self):
        ...
        if self.content_type != "application/json":
            raise ValueError("Content-Type not application/json")
