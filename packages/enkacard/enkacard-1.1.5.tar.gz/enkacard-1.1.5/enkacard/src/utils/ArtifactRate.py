...
async def generator_artifact(name,raiting,name_set,name_charter,icon,substats,props,imageStats,stats,lvl,CRIT_DMG,CRIT_RATE,imageCharter):
...
async def get_artifact_rate(info, teample, artType,imageCharter):
    ...
    for key in filter(lambda x: x.type == EquipmentsType.ARTIFACT, info.equipments):
        if str(key.detail.artifact_type.value) in artType:
            ...
            if key.detail.mainstats.prop_id == "FIGHT_PROP_CRITICAL_HURT":
                CRIT_DMG += key.detail.mainstats.value
            if key.detail.mainstats.prop_id == "FIGHT_PROP_CRITICAL":
                CRIT_RATE += key.detail.mainstats.value
            if teample == 1:
                artImg = await generator_artifact(name,raiting,name_set,info.name,icon,key.detail.substats, key.props,imageStats,stat,lvl,CRIT_DMG,CRIT_RATE,imageCharter)
