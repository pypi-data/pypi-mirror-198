...
class Edit(Command):
    def processing(self, data_container: AddressBot):
        category_completer = WordCompleter('name', 'phone', 'address', 'email', 'birthday')
        contact_data = input("Enter any data about contact that you want to change: ")
        new_category = prompt("Enter a category (name/phone/address/email/birthday): ", completer=category_completer).strip().lower().capitalize()
        data_container.book.save()
