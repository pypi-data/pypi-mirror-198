...
def run_experiment_no_fune_tuning(params=None, offline=False, validate=True, splits=None):
    ...
    for i, record in training_dataset.iterrows():
        ...
        template = f"The stance of '{sentence}' on the topic '{topic}' is {stance}.\n"
    ...
    for i, record in test_dataset.iterrows():
        ...
        template_no_stance = f"The stance of '{sentence}' on the topic '{topic}' is"
        prompt_to_predict = prompt + template_no_stance
        seq = tokenizer.encode(prompt_to_predict, return_tensors="pt")
        generated = model.generate(seq, max_length=5, do_sample=True)

        resulting_string = tokenizer.decode(generated.tolist()[0])
        stance = resulting_string[-3:]
        #print(stance)
        if stance.lower() == "pro" :
    ...
    print(f"size of test labels is {len(labels)}")
