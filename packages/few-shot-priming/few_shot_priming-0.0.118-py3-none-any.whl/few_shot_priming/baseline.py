...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,df, labels, path=None):
        ...
        for i, record in df.iterrows():
            self.texts.append(tokenizer(record['claims.claimCorrectedText'], record["topicText"], padding="max_length", max_length=512,
...
def random_baseline(params, offline=True, validate=True):
    splits = load_splits()
    if validate:
...
if __name__ == "__main__":
    ...
    params = get_baseline_params()
    if args.random:
        accuracy = random_baseline(params, offline=args.offline, validate=args.validate)
