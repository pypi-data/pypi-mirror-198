...
from BukePypiProjectVersion2.indicator import sma, ibs, pivot_standard, stochastic_fast_k, stochastic_fast_d, \
    pivot_standard_for_sellprice
from BukePypiProjectVersion2.function import rank
from BukePypiProjectVersion2.parameter import Market, MovingAverage
from BukePypiProjectVersion2.tools import get_stock_exchange_tax, get_ask_price, get_bid_price
...
def read_trading_days(start_date: date, end_date: date):

    df = pd.read_csv('data_file/korea_trading_days.csv', index_col=0)
...
def read_day_price(start_date: date, end_date: date):
    df = pd.read_csv(f"data_file/수급데이터_2022_to_{end_date.year}.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
    # try:
...
def read_index_day_price(start_date: date, end_date: date):

    df = pd.read_csv(f"data_file/MSCI_한국_ETF.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
    return df


def read_index_history(start_date: date, end_date: date):

    ks_history = pd.read_csv(f"data_file/코스피_종목리스트_2022_to_{end_date.year}.csv", index_col=0)
    ks_history['날짜'] = ks_history['날짜'].apply(lambda day: str_to_date(day))
    ks_history = ks_history.set_index('날짜')

    kq_history = pd.read_csv(f"data_file/코스닥_종목리스트_2022_to_{end_date.year}.csv")
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['스토캐스틱_slow'] = stochastic_fast_d(group['고가'], group['저가'], group['종가'], 20,
                                                     5, MovingAverage.sma)
    ...
    for key, group in grouped:
        ...
        increase_condition1 = group['종가_이평선_5'] > group['종가_이평선_5'].shift(5)
        increase_condition2 = group['거래대금_이평선_10'] > group['거래대금_이평선_10'].shift(10)
        increase_condition3 = group['종가'] / group['종가_이평선_3'] < 1
        increase_condition4 = group['종가'] / group['종가_이평선_20'] > 1.05
        increase_condition5 = group['ibs'] < 0.7

        increase_condition = increase_condition1 & increase_condition2 & increase_condition3 & increase_condition4 & \
                             increase_condition5

        liquidity_condition1 = group['거래대금_이평선_5_순위'] > 0.5
        liquidity_condition2 = group['시가총액_순위'] > 0.3
        liquidity_condition3 = group['순거래대금_이평선_5_순위'] > 0.5

        liquidity_condition = liquidity_condition1 & liquidity_condition2 & liquidity_condition3
...
def back_testing(start_date, end_date):
    ...
    # 1. 거래일 가져오기
    trading_days_list = read_trading_days(start_date, end_date)

    # 2. 일봉 가져오기
    day_price = read_day_price(start_date, end_date)

    index_day_price = read_index_day_price(start_date, end_date)

    # 3. 지수 종목 구성 내역 가져오기
    kospi_history, kosdaq_history = read_index_history(start_date, end_date)
...
def Buke_S002_sell_list(today):


    path = f"trade_list/{today}"
    ...
    basket = back_testing(real_start_date, real_end_date)
    ...
    sell_basket.to_csv(f"{path}/{file_name}")