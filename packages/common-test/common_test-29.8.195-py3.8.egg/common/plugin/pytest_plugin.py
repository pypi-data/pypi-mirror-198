...
class PytestPlugin(object):
    ...
    def runSceneCase(self, _caseNameList):
        ...
        for case in _caseNameList:
            ...
            if caseStatus == '准备执行':
                ...
                logger.info(f"------用例编号：{caseId}  用例名称:{caseName}  运行ID: {caserunId} 脚本路径:{scriptURL}  开始执行----")
                if scriptURL != '':
    ...
    def initRunCyleCase(self):
        if DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)):
            MysqlPlatForm.update_autoTest_run(get_system_key(Constant.TEST_SRTCYCLE_ID), Constant.STATUS_PRE)
    ...
    def send_case_result(self, item: Item, result):
        ...
        logger.info(f"---用例列表：{caseInfoList}  脚本路径:{_caseurl} 执行状态:{result}")
