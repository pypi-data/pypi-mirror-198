...
def check_excel_data(testdata:dict, _mysql, _filter):
    ...
    if DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)) and _mysql is not None:
        ...
        if len(_info) > 0:
            logger.info(f'执行参数化用例:{casename} 执行周期：{cycleId} ')
            print_info(f'执行参数化用例:{casename} 测试数据: {testdata}')
            return True
        else:
            _sql1 = f"select * from `traffic_test`.`test_autotest_run` where `status` = '{Constant.STATUS_PRE}' and cycleId='{cycleId}' and caseid='{caseNo}'"
            _info1 = _mysql.execute(_sql).fetchall()
            if len(_info1) > 0:
                logger.info(f'执行参数化用例编号:{caseNo} 执行周期：{cycleId} ')
                print_info(f'执行参数化用例编号:{caseNo} 测试数据: {testdata}')
                return True
            else:
                return False
...
if __name__ == '__main__':
    ...
    # print(readList)
    _str='/app/jenkins-local/workspace/ibsapi-test-api-jira/data/case_data.xls'
    print(_str.find('/'))
