...
class PyMyGekkoApiClient:
    ...
    async def read_data(self) -> None:
        if self._demo_mode:
            varResponseFile = open("PyMyGekko/api_var_demo_data.json")
            self._resources = json.loads(varResponseFile.read())
            statusResponseFile = open("PyMyGekko/api_var_status_demo_data.json")
            self._status = json.loads(statusResponseFile.read())
            return
        else:
            async with self._session.get(
