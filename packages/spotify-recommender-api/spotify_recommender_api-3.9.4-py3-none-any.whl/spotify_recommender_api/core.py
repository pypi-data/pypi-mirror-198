...
import spotify_recommender_api.request_handler as request_handler
sns.set()
...
def create_playlist(type: str, headers: dict, user_id: str, base_playlist_name: str, additional_info: str = None, _update_created_playlists: bool = False) -> str:
    ...
    if playlist_id_found := util.playlist_exists(name=playlist_name, base_playlist_name=base_playlist_name, headers=headers, _update_created_playlists=_update_created_playlists):
        ...
        playlist_tracks = list(map(lambda track: {'uri': track['track']['uri']}, request_handler.get_request(url=f'https://api.spotify.com/v1/playlists/{new_id}/tracks', headers=headers).json()['items']))

        delete_json = request_handler.delete_request(url=f'https://api.spotify.com/v1/playlists/{new_id}/tracks', headers=headers, data={"tracks": playlist_tracks}).json()
        ...
        if _update_created_playlists:
            ...
            update_playlist_details = request_handler.put_request(url=f'https://api.spotify.com/v1/playlists/{new_id}', headers=headers, data=data)
        ...
        playlist_creation = request_handler.post_request(url=f'https://api.spotify.com/v1/users/{user_id}/playlists', headers=headers, data=data)
