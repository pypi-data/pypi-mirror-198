...
class Dijkstra:
    ...
    def diaporama(self, filename="index"):
        ...
        directory = str(PyGraph.__path__[0])
                ...
                if neighbor not in self.__locked:
                    ...
                    if path < self.__dist[neighbor]:
                        ...
                        if neighbor not in self.__visited:  
                            if len(self.__priority_queue.queue) != 0:
...
class Coloring:
    """Pour explorer la coloration de graphes"""
    ...
    def dsat(self, node_id):
        g = self.graph
        return len({g.node_view(v).color_id for v in g.neighbors(node_id) if g.node_view(v).color_id != -1})

    def dsatur(self):
        """implémente l'algorithme du même nom"""
    
        def smallest_color_id(colors_seen):
            color_id = 0
            while color_id in colors_seen:
                color_id += 1
            return color_id
    
        def select_node():
            g = self.graph
            return max((node_id for node_id in g.node_ids() if g.node_view(node_id).color_id == -1),
                       key=lambda s: (g.dsat(node_id), g.degree(s)))

        for _ in range(self.graph.number_of_nodes()):
            node_id = select_node()
            vues = {self.graph.node_view(node).color_id for node in self.graph.neighbors(node_id)}
            color_id = smallest_color_id(vues)
            self.graph.node_view(node_id).color_id = color_id
        self.graph.color_on()