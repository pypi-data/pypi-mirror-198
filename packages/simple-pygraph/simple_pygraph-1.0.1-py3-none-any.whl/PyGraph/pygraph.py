...
Auteurs : Sébastien Hoarau, Donovan Roullan
Date    : 2021-08
...
class EdgeView:
    ...
    def __init__(self, gv, node_src, node_dst, weight='', color_id=BLACK):        
        self.__gv = gv
    ...
    def color_off(self):
        self.__gv.edge(str(self.edge[0]), str(self.edge[1]), str(self.weight), style='filled', color=COLORS[BLACK])
...
class Graph:
    ...
    def add_edge(self, s1, s2, weight=None):
        if weight is not None:
            ...
            self.model.add_edge(s1, s2, view=EdgeView(self.view, s1, s2))            
    ...
    def save_json(self, filename, encoding='utf-8'):
        ...
        with io.open(filename, 'w', encoding=encoding) as outfile:
            str_data = json.dumps(data, indent=4, sort_keys=True, ensure_ascii=False)
