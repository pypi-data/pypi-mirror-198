...
class EnrollRecordServices:
    ...
    def appoint(enroll_id, record_id, subitem_id):
        ...
        try:
            ...
            # TODO 这里的咯及有问题，如果多份则不是相乘的关系，是累加的关系
            ...
            if again_enrolled_count >= need_count:
                ...
                # 获取到所有的追加价格列表，并进行求和
                initiator_again_price_list = list(EnrollSubitemRecord.objects.annotate(
                    r_enroll_id=F("enroll_record__enroll_id"),
                    r_initiator_again_price=F("enroll_record__initiator_again_price")
                ).filter(r_enroll_id=enroll_id).values("r_initiator_again_price"))
                try:
                    sum_initiator_again_price = sum([round(float(i["r_initiator_again_price"]), 2) for i in initiator_again_price_list])
                except Exception as e:
                    write_to_log(prefix="指派计算追加价格失败", err_obj=e)
                ...
                if sum_initiator_again_price == 0.0:
                    ...
                    Enroll.objects.filter(id=enroll_id).update(
                        enroll_status_code=make_up_difference_code,
