...
class Intents:
    """
    Creates chatbot intents from Q&A databank

    Includes Feedback chips, Back to Menu and recommendations
    """
    ...
    def getIntentTemplate():
        """
        Dialogflow's intent json template with different placeholders.
        Focus on updating: Name, Response's text, image link and external link
        :return: json with corresponding placeholders values from QAfile
        """
        return {
                                    ...
                                    [
                                        {
                                            "type": "image",
                                            "rawUrl": ""
                                            # ["responses"][0]["messages"][0]["payload"][0]["richContent"][0]["rawUrl"] = row["Image Link"]
                                        },
                                        {
                                            "icon": {
                                                "type": "chevron_right",
                                            ...
                                            "type": "button",
                                            "link": "",
                                            # ["responses"][0]["messages"][0]["payload"][0]["richContent"]["link"] = row["External Link"]
                                            "text": ""
                                            # ["responses"][0]["messages"][0]["payload"][0]["richContent"]["text"] = row["Response 1"]
                                        }
                                    ]
                                ]
                            },
    ...
    def getResponse2():
        """
        Dialogflow's json template as a placeholder for 2nd response in the case of long response from QA
        :return: JSON with the 2nd repsonse as text from QAfile
        """
    ...
    def getFeedbackChips():
        """
        Dialogflow's chips template for upvote/downvote to have student's feedback
        :return: JSON of chips response consisting icon for votes
        """
        return {
            "type": "4",
            "title": "",
    ...
    def getTrainingPhrases():
        """
        Dialogflow's training phrases json template
        All the paraphrased texts are stored for chatbot taining/matching purposes.
        :return: JSON with the different training phrases as text from QAfile
        """
        return {
    ...
    def getIntents(df):
        ...
        for index, rows in df.iterrows():
            ...
            intent_name = str(rows['Main Label']) + '.' + str(rows['Intent Label']) + ' ' + rows['Main Topic'] + ' - ' + rows['Name']
            ...
            Intents.write_file_json("./Chatbot/intents/{}.json".format(intent_name), intentResponse1)
            Intents.write_file_json("./Chatbot/intents/{}_usersays_en.json".format(intent_name), trainingPhrases)
            
            recIntents.clear()  # clear list for next intent

        print("Intents Successfully Created!\n")
