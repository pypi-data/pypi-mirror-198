...
class Menu():
    ...
    def createLearnMenu(df):
        ...
        for i in range(1, maxi + 1):
            chipsTemplate = Menu.getChipsPayload()
            teleTemplate = Menu.getTelePayload()
        ...
        for phrase in defaultPhrases:
            ...
            trainingPayload = Menu.getTrainingTemplate()
            trainingPayload["data"][0]["text"] = phrase
            trainingIntent.append(trainingPayload)

            Menu.write_file_json("./Chatbot/intents/Menu - Learn.json", intent)
            Menu.write_file_json("./Chatbot/intents/Menu - Learn_usersays_en.json", trainingIntent)
    ...
    def createMainTopicMenu3(df):
        ...
        while intentCount < len(numOfIntentsMain):
            ...
            intent = Menu.getMainTopicTemplate()
            intent["name"] = "{}".format(rows['Main Topic'])
            intent["responses"][0]["messages"][1]["payload"]["richContent"][0][0]["options"] = payload
            intent["responses"][0]["messages"][0]["payload"]["telegram"]["reply_markup"]["keyboard"] = telePayload
 
            Menu.write_file_json("./Chatbot/intents/{}.json".format(rows['Main Topic']), intent)
            ...
            for phrase in defaultPhrases:
                ...
                trainingPayload["data"][0]["text"] = phrase
                trainingIntent.append(trainingPayload)
                Menu.write_file_json("./Chatbot/intents/{}_usersays_en.json".format(rows['Main Topic']), trainingIntent)
        ...
        while intentCount < len(numOfIntentsMain):
            ...
            for n in range (1, numOfIntentsSub+1):
                ...
                for k, rows in intentGroupSub.iterrows():
                    chipsTemplate = Menu.getChipsPayload()
                    teleTemplate = Menu.getTelePayload()
                    intent = Menu.getSubTopicTemplate()
                ...
                Menu.write_file_json("./Chatbot/intents/{} - {}.json".format(rows['Main Topic'], rows['Sub Topic']), intent)
        ...
        for i in range(len(listOfSubTopicGrouping)):
            trainingTemplate = Menu.getTrainingTemplate()
            trainingTemplate["data"][0]["text"] = listOfSubTopicNames[i]
            trainingPayload.append(trainingTemplate)

            Menu.write_file_json("./Chatbot/intents/{} - {}_usersays_en.json".format(listOfMainTopics[i], listOfSubTopics[i]), trainingPayload)
    ...
    def createMainTopicMenu2(df):
        ...
        while countOfIntents < len(numOfIntentMain):
            ...
            Menu.write_file_json("./Chatbot/intents/{}_usersays_en.json".format(rows['Main Topic']), trainingPayload)
