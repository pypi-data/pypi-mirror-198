...
from mojo.taskplus.execution import execute_workflow
...
def run_workflow_entrypoint(workflow_file: str, workflow_info: dict):
    ...
    if "environment" not in workflow_info:
        error_msg = "The work packet file must have an 'workpacket->environment section. file=%s" % workflow_file
        raise SyntaxError(error_msg)

    if "parameters" not in workflow_info:
        error_msg = "The work packet file must have an 'workpacket->parameters section. file=%s" % workflow_file
        raise SyntaxError(error_msg)

    if "workflow" not in workflow_info:
        error_msg = "The work packet file must have an 'workpacket->workflow section. file=%s" % workflow_file
        raise SyntaxError(error_msg)
