...
from gantry.dataset.constants import (
    DATASET_FEATURES_KEY,
    DATASET_FEEDBACK_KEY,
    JOIN_KEY,
    UserDtype,
)
from gantry.dataset.utils import get_inferred_type
...
from gantry.utils import (
    _download_file_to_IO,
    _upload_file_from_IO,
    compute_feedback_id,
    download_file_from_url,
    get_files_checksum,
    get_s3_sha256_checksum,
    list_all_files,
    parse_s3_path,
    read_lines_from_url,
    upload_file_to_url,
...
def _ensure_not_in_local_mode(func: Callable) -> Callable:
    """
    Decorator to disable operations when API client not initialized
    """
    ...
    def wrapper(self, *args, **kwargs):
        if not self._api_client:
            logger.error(
                "This operation is not available in local mode, to use it please reinitialize"
                " the client with a proper API key."
            )
            return None
        return func(self, *args, **kwargs)
...
class GantryDataset:
    ...
    def __iter__(self):
        return iter(self.pull_huggingface_dataset_by_version())

    @_ensure_not_in_local_mode
    def list_versions(self) -> List[Dict[str, Any]]:
        ...
        """
        response = self._api_client.request(  # type: ignore
    ...
    def pull(
        ...
        """
        Pull a spicific version of the dataset from dataset server to local working directory.

        If version ID is provided, this method will pull the specific snapshot of your dataset
    ...
    def push_version(self, message: str) -> Dict[str, Any]:
        ...
        resp = self._api_client.request(  # type: ignore
            "GET", f"/api/v1/datasets/{self._dataset_id}/commits", raise_for_status=True
        )
    ...
    def delete(self) -> None:
        ...
        """
        if not self._api_client:
            shutil.rmtree(Path(self.workspace) / self.dataset_name)
            logger.info(
                f"Local mode dataset {self.dataset_name} has been removed from the local workspace."
            )
        else:
    ...
    def _add_single_file(
        ...
        try:
            file.seek(0)
            file_sha256 = get_s3_sha256_checksum(file.read())
        except TypeError:
            file.seek(0)
            file_sha256 = get_s3_sha256_checksum(bytes(file.read(), encoding="utf-8"))
        file.seek(0)

        # get file directory
        file_dir = constants.ARTIFACTS
        if len(filename) > 4 and filename[-4:] == constants.CSV_SUFFIX:
            file_dir = constants.TABULAR_MANIFESTS

        # get put urls:
        upload_file_info = DatasetFileInfo(
            file_name=f"{file_dir}/{filename}",
            url=self._get_s3_url(f"{file_dir}/{filename}", commit_id),
            sha256=file_sha256,
        )
        manifest_file_info = DatasetFileInfo(
            file_name=constants.DATASET_MANIFEST_FILE,
            url=self._get_s3_url(constants.DATASET_MANIFEST_FILE),
        )
        urls = self._generate_put_presign_url([upload_file_info, manifest_file_info])

        # upload added file to s3
        upload_file_info.version_id = _upload_file_from_IO(
            urls[self._get_obj_key(upload_file_info.url)], file
        )

        # download manifest file
        url = self._generate_get_presign_url([manifest_file_info])[
            self._get_obj_key(manifest_file_info.url)
        ]
        manifest_file = io.BytesIO()
        _download_file_to_IO(url, manifest_file)
        # update manifest file
        manifest_file.write(bytes(upload_file_info.to_jsonl(), encoding="utf-8"))
        manifest_file.seek(0)
        # upload new manifest file
        version_id = _upload_file_from_IO(
            urls[self._get_obj_key(manifest_file_info.url)], manifest_file
        )
    ...
    def _get_dataset_config(self) -> Dict[str, Any]:
        """
        Read dataset config from dataset_config.yaml
        Returns:
            A key value pair of Dict where key is the column name and value is huggingface
            feature type
        """
    ...
    def _update_dataset_config(self, config: Dict[str, Any]):
        """
        Update dataset config
        Args:
            config (Dict[str, Any]): dataset config
        """
        yaml.safe_dump(
            config,
            (Path(self.workspace) / self.dataset_name / constants.DATASET_CONFIG_FILE).open(
    ...
    def get_eval_result(self, target_tags: Optional[List[str]] = None) -> pd.DataFrame:
        """
        This function will show evaluation result for the test run.
        ...
        .. code-block:: python
            ...
            # show the llm evaluation result for the test run
            df = dataset.get_llm_eval_result(test_run_name)

        Args:
            test_run_name (str): test run name to show the llm evaluation result.
        """

        if not target_tags:
            target_tags = ["latest"]

        results_path = Path(f"{self.workspace}/{self.dataset_name}/artifacts/eval/")

        with (results_path / "tags.json").open() as f:
            eval_run_tags = json.loads(f.read())

        runs = {
            tag: results_path / eval_run_tags[tag] for tag in eval_run_tags if tag in target_tags
        }

        # if any(not run.exists() for run in run_list):
        #     raise ValueError(f"Run {run_list} does not exist")

        df = self.get_huggingface_dataset().to_pandas()
        eval_scores: Dict[str, Any] = collections.defaultdict(dict)
        # get a list of run result folders, each folder contain a run with the same hyperparameters
        for tag, run in runs.items():
            eval_records = {}
            with (run / "outputs.jsonl").open() as f:
                for line in f.readlines():
                    data = json.loads(line)
                    eval_records.update(data)

            df[f"{run.name}: {tag}"] = df.apply(
                lambda x: eval_records[x["join_key"]]["output"].strip(), axis=1
            )
            if self._label_column:
                labels = df[self._label_column].tolist()
                predictions = df[f"{run.name}: {tag}"].tolist()

                print("*** Evaluation results for run: ", run.name)
                eval_scores[f"{run.name}: {tag}"]["exact_match"] = evaluate.load(
                    "exact_match"
                ).compute(
                    references=labels,
        ...
        return df, eval_scores

    def list_runs(self):
        """
        List all the runs for the dataset.
        Currently assumes all folders inside artifacts/eval are runs.
        """
        results_path = Path(f"{self.workspace}/{self.dataset_name}/artifacts/eval/")
        run_list = [run.name for run in results_path.iterdir() if run.is_dir()]
        # Append tags to run names
        run_tags = {}
        with (results_path / "tags.json").open() as f:
            tags = {}
            tags_text = f.read()
            ...
            for tag, run in tags.items():
                if run not in run_tags:
                    run_tags[run] = []
                run_tags[run] += [tag]
        for i, run in enumerate(run_list):
            if run in run_tags:
                run_list[i] = f"{run} ({', '.join(run_tags[run])})"
        return run_list
    ...
    def sync_data(
        ...
        """
        This function will sync data to the dataset.
        ...
        .. code-block:: python
            ...
            # sync data to the dataset
            dataset.sync_data(file_path, version_message)

        Args:
            file_path (Path): file path to sync data to the dataset.
            version_message (str): version message for the sync data.
        """
        if label_column:
            self._label_column = label_column
        ...
        if not version_message:
            raise ValueError("version message is required")

        if self._api_client:
            self.pull(forced=forced)
        if file_path.suffix == ".json":
            ...
            raise ValueError(f"Unsupported file type {file_path.suffix}, only support json and csv")

        feature_to_type = {}
        labels_to_type = {}
        for col in df.columns:
            if col == label_column:
                labels_to_type[col] = get_inferred_type(df[col])
            else:
                feature_to_type[col] = get_inferred_type(df[col])

        config = self._get_dataset_config()

        if JOIN_KEY not in df.columns:
            feature_to_type[JOIN_KEY] = UserDtype.TEXT.value
            df[JOIN_KEY] = df.apply(lambda x: compute_feedback_id(x.to_dict()), axis=1)

        config[DATASET_FEATURES_KEY] = feature_to_type
        if labels_to_type:
            config[DATASET_FEEDBACK_KEY] = labels_to_type

        self._update_dataset_config(config)
        self._clean_tabular_data()

        df.to_csv(
            Path(self.workspace)
            / self.dataset_name
            / constants.TABULAR_MANIFESTS
            / f"{file_path.stem}.csv",
            index=False,
        )
        if self._api_client:
            self.push_version(version_message)

    def set_eval_run_tag(self, run_id, tag):
        results_path = Path(f"{self.workspace}/{self.dataset_name}/artifacts/eval")

        with (results_path / "tags.json").open("r") as f:
            eval_run_tags = json.loads(f.read())

        with (results_path / "tags.json").open("w") as f:
            eval_run_tags[tag] = run_id
            json.dump(eval_run_tags, f)
