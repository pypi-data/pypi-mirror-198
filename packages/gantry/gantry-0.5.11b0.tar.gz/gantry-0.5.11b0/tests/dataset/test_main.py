...
@pytest.mark.skip(reason="currently we support only local datasets")
...
@pytest.mark.parametrize(
    ("is_git_repo", "expected_working_dir"),
    [
        (False, "working_dir"),
        (True, "working_dir/.gantry"),
    ],
)
@mock.patch("gantry.dataset.client.is_git_repo")
def test_init(mock_is_git_repo, is_git_repo, expected_working_dir):
    """Test init"""
    passed_api_key = "some_key"
    mock_is_git_repo.return_value = is_git_repo
    with mock.patch.dict(
        ...
        assert main._DATASET.working_directory == expected_working_dir
