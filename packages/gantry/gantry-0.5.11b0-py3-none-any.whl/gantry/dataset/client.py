...
from gantry.dataset.utils import create_local_dataset, get_creation_time, is_git_repo
...
class GantryDatasetClient:
    def __init__(
        ...
        if is_git_repo(working_directory):
            self._workspace = str(Path(working_directory) / ".gantry")
            # TODO:// add .ganty to gitignore
        else:
    ...
    def create_dataset(
        ...
        if not self._api_client:
            # return a local dataset object
            create_local_dataset(self._workspace, name)
    ...
    def get_dataset(self, name: str) -> GantryDataset:
        ...
        """
        if not self._api_client:
            dataset_folder = Path(self.working_directory) / name
            if not dataset_folder.exists():
                raise DatasetNotFoundError(f'Could not find dataset with name "{name}"')
    ...
    def list_dataset_versions(self, dataset_name: str) -> List[Dict[str, Any]]:
        ...
        if not self._api_client:
            logger.warning(
                "API client not initialized, this operation is not available in local mode."
            )
            return []
    ...
    def list_datasets(self, include_deleted: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        if not self._api_client:
            dataset_dirs = [f for f in Path(self.working_directory).iterdir() if f.is_dir()]
            return [
                {"name": ds.name, "dataset_id": "", "created_at": get_creation_time(ds)}
    ...
    def ping(self) -> bool:
        ...
        if not self._api_client:
            logger.error(
                "API client not initialized, please provide API key if"
                "you want to interact with Gantry service."
            )
            return False
        try:
    ...
    def sync_dataset_data(
        ...
        """
        Sync the dataset data from a local file
        ...
        .. code-block:: python
            ...
            import gantry.dataset as gdataset
            gdataset.sync_dataset_data("path/to/file", "dataset_name")

        Args:
            file_path (str): the path to the local file.
            dataset_name (str): the name of the dataset.
            version_message (str): the commit message for the new dataset version.
        ...
        if not version_message:
            version_message = f"Sync data from {file_path.name}"

        try:
            dataset = self.get_dataset(dataset_name)
        except DatasetNotFoundError:
            logger.info(f"Dataset {dataset_name} does not exist, creating a new one")
            dataset = self.create_dataset(dataset_name, label_column=label_column)

        dataset.sync_data(file_path, version_message, label_column=label_column)
        return dataset
