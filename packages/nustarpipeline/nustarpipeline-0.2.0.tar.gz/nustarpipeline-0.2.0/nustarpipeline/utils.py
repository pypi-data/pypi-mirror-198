...
def rebin_matrix(e_min, e_max, pp_input, dpp_input, min_s_n = 50, only_pulsed=False, use_counts=False, background=None):
    '''
    ...
    ;param background a couple (background, background_uncertainty)
    :return:
    ...
    dnew_background rebinned 2-d array with background pulse profiles uncertainties) - returned only is background is not None 
    '''
...
def pulsed_fraction_area(c, dc, background=None, background_error=None): 
    ...
    return np.sum(c-np.min(c)+a0)/np.sum(c+a0)
...
def pulse_fraction_from_data_rms(counts, counts_err, n_harm=-1, background=None, background_error=None, plot=False, label='', verbose=False):
    ...
    if plot:
        ...
        if n_harm > 1:
            plt.errorbar(f, counts, yerr=counts_err, marker='.', label='data %s' % label, linestyle='')
        else:
            plt.plot(np.arange(1,K+1), cumulative_sum - m4, label='data %s' % label)
        #plt.plot(f, y, label='%d harmonics' % n_harm, linestyle='--')
        plt.xlabel('Phase')
        plt.legend()
        if label != '':
...
def subtract_background(a0, background, background_error):
    ...
    if np.sum(ind)>0:
        ...
        if a0 == 0:
            logger.warning('continuum level is zero')
            #raise Exception
    return a0_out
...
def fft_pulsed_fraction(x, dx, level=0.1, n_harm_min=2, n_harm_max=-1, plot=False, verbose=False, label='', 
    ...
    if plot:
        ...
        #plt.figure()
        f = np.linspace(0, 1, n)
        plt.errorbar(f, x, yerr=dx, marker='.', label='data %s' % label, linestyle='')
        plt.plot(f, y, label='%d harmonics' % n_harm, linestyle='--')
        plt.xlabel('Phase')
        plt.legend()
        if label != '':
