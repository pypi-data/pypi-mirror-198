...
def before_all():
    ...
    # Get the path of the directory that contains the calling file
    calling_file_path = os.path.dirname(inspect.getframeinfo(inspect.currentframe()).filename)

    # Construct the path to the file you want to access
    file_path = os.path.join(calling_file_path, 'app-config.json')
