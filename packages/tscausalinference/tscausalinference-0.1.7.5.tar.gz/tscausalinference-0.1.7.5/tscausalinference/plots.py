...
def plot_diagnostic(data = DataFrame(), figsize = (15, 10), intervention = []):
    data = data.copy()
    training = data[data.ds <= (pd.to_datetime(intervention[0]) - pd.Timedelta(days=1))].copy()
    test = data[(data.ds >= pd.to_datetime(intervention[0]))&(data.ds <= pd.to_datetime(intervention[1]))].copy()
    ...
    axes[0].text(1.0, 0.4, f'R2: {r_train:.2f}', fontsize = 14, bbox = bbox_props)

    scatter_test = sns.scatterplot(data=test, x="yhat", y="y", ax=axes[0][1])
    scatter_test.set_title('Model fit in Test')

    axes[1].text(1.0, 0.4, f'R2: {r_test:.2f}', fontsize = 14, bbox = bbox_props)
    ...
    fig.set_title('Model Diagnostics')
