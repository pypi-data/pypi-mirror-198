...
class EyeQ5(Component):
    ...
    def copy_from(
        ...
        """
        return self._scp(f"{self._ssh_root}:{src}", str(dst), timeout, recurcive)
    ...
    def copy_to(
        ...
        Returns:
            CompletedProcess[str]: the process
        """
        return self._scp(str(src), f"{self._ssh_root}:{dst}", timeout, recurcive)
    ...
    def is_alive(self) -> bool:
        ...
        return self.is_mest_alive() and self.wait_for_frames(timeout=1)
    ...
    def wait_for_core_write(self, core_type: CoreType, timeout: float = 180) -> bool:
    ...
    def delete_crash_folder(self) -> CompletedProcess[str]:
        ...
        return self.run_ssh_cmd(f"rm -rf {self.crash_folder}")
