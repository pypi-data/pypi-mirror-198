...
def genTrain(net:AdvNet, xArr:np.ndarray, yArr:np.ndarray, iterations:int = 1000, 
                    batchSize:int = 10, weightSelection:str = None, R2Goal = 0.999,
                    Silent:bool = False):
    """
    Returns a net trained to the yArr data in a more thorough training method than Train().
    ...
    iterations:
        - Amount of attempts at imrpoving the net's fit to the data.
    ...
    weightSelection: (all, gates, middle, None)
        ...
        - The default method (called by None), alternates between training
          the gates and middle arrays every other iteration.
    ...
    for I in range(iterations):
        ...
        twk = 2 ** (-I / 50)        ## ~30 seems to be a good scale for this (found via Train())
...
def thinData(xData, yData, numPoints:int):
    """
    Returns three arrays for AdvNet curve fitting / plotting from the 1D array of given data. This is 
    primarily meant for curve fitting using ATAN AdvNets, as fitting to all the data tends to be 
    more than what is neccessary.
    
    Array 1 is the x-data for the net input (slices of the x-data).

    Array 2 is the thinned y-data for the net validation data. This array will have a number of data points
    equal to numPoints that are seperated evenly.

    Array 3 is the indicies sliced at -- this is useful for plotting the yThin data (array 2).
