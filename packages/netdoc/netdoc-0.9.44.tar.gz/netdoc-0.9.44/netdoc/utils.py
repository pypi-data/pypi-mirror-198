...
DRAWIO_ROLE_MAP = {
    ...
    "router": {
        "style": "sketch=0;points=[[0.5,0,0],[1,0.5,0],[0.5,1,0],[0,0.5,0],[0.145,0.145,0],[0.8555,0.145,0],[0.855,0.8555,0],[0.145,0.855,0]];verticalLabelPosition=bottom;html=1;verticalAlign=top;aspect=fixed;align=center;pointerEvents=1;shape=mxgraph.cisco19.rect;prIcon=router;fillColor=#FAFAFA;strokeColor=#005073;",
        "width": 50,
        "height": 50,
    },
    "firewall": {
...
def append_nornir_task(task, commands, enable=True, template=None, order=128, supported=True):
...
def log_ingest(log):
    ...
    if log.order > 0 and not log.discoverable.device:
        ...
        raise ValueError("Device not attached to the Discoverable")
...
def normalize_route_type(
    ...
    if route_type in ["ex"]:
        # EIGRP External
        return "ex"
    if route_type in ["o", "ospf", "o_intra"]:
        # OSPF Inter Area
        return "oia"
    if route_type in ["n1"]:
        # OSPF NSSA Type 1
