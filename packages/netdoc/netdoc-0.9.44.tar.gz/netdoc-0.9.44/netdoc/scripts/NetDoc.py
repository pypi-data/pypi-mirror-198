...
from dcim.models import Site, DeviceRole, Interface
...
from netdoc.utils import log_ingest, normalize_interface_label
...
class FixInterfaceLabels(Script):
    """Fix Interface labels from Interface name."""

    class Meta:
        """Script metadata."""

        name = "Fix interface labels"
        description = "Fix Interface labels from Interface name."
        commit_default = True
    ...
    def run(self, data, commit):
        ...
        for interface_o in interface_qs:
            ...
            if interface_o.label == new_label:
                ...
                self.log_warning(f"Renaming label {interface_o.label} to {new_label} for interface {interface_o}")
