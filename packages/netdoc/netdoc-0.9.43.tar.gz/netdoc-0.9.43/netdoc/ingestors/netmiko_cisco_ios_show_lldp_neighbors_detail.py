...
def ingest(log):
    ...
    for item in log.parsed_output:
        ...
        remote_management_ip = utils.normalize_ip_address_or_none(item.get("management_ip"))
        remote_name = (
            item.get("neighbor") if item.get("neighbor") else item.get("chassis_id")
        )
        remote_name = utils.normalize_hostname(remote_name)
        remote_interface_name = (
            item.get("neighbor_interface")
            if item.get("neighbor_interface")
            else item.get("neighbor_port_id")
        )
