...
from xiot import App, MQTTDTU, DTU, __version__
import time
...
class ShellDTU(DTU):
    def __init__(self, shell):
        self.shell = shell
        DTU.__init__(self, app=self.shell.app, dtu_id=self.shell.gw_id)
        ...
        def run():
            ...
            def notify(finish=False):
                if needret:
                    return
                text = ''.join(outs)
                dtu.sendNotify(dev_id, {'type': 'result', 'text': text, 'value': value, 'finish': finish, 'sid': sid, 'mid': mid})
                del outs[:]
            ...
            try:
                ...
                # subprocess.call()
                p = subprocess.Popen(args, shell=cmdd.get('shell'), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=os.getcwd(), bufsize=1, env=env)
                pipe = Pipe(p.stdout)
                last = time.time()
                start = time.time()
                outs.append('\n>run start...\n\n')
                ...
                while self.running:
                    ...
                    if timeoff > 1 and outs or timeoff > 5:
                        last = time.time()
                        notify()
                # print("self.running out", self.running, p.returncode)
                code = p.returncode
                ctime = int((time.time() - start) * 1000)
                outs.append('\n>run end, exit code=%s, time %dms!\n' % (code, ctime))
                notify(True)
                print('%s exit code=%s, time %dms!' % (action, code, ctime))
                try:
                    pipe.close()
                except:
                    pass
...
if __name__ == '__main__':
    ...
    try:
        ...
        sh = Shell(servurl=args.server, appid=args.appid, appkey=args.appkey, mqtturi=args.mqtt, gateway=args.gateway, device=args.device, runner=runner, name=args.name, interval=int(args.interval))
        sh.start()
        while sh.running:
            time.sleep(1)
            # print('sleepping...')
