...
# :Copyright: © 2008, 2009, 2010, 2013-2020, 2021, 2022 Lele Gaifax
#
...
class Player(GloballyUnique, Base):
    ...
    def sendTrainingURL(self, request, match, cnum, opponent):
        ...
        if opponent is not None:
            ...
            if match.tourney.socialurl:
                details = "\n" + t(_('Do not forget to upload your video on ${socialurl}.',
            ...
            oname = t(_("Phantom"))
            body = t(TRAINING_PHANTOM_MESSAGE).format(firstname=self.firstname,
                                                      tourney=match.tourney.description,
                                                      opponent=oname)
            url = None
    ...
    def sendTrainingURLs(self, request, matches):
        ...
        urls = '\n  '.join(t(_('${board}. ${url} against ${opponent}',
                               mapping=dict(board=board, url=url, opponent=oname)))
                           for board, (oname, url) in enumerate(urls, 1))

        if match.tourney.socialurl:
            details = "\n" + t(_('Do not forget to upload your videos on ${socialurl}.',
                                 mapping=dict(socialurl=match.tourney.socialurl))) + "\n"
        else:  # pragma: nocover
            details = ""
        nboards = matches[0][0].tourney.championship.trainingboards
        body = t(TRAINING_URLS_MESSAGE).format(firstname=self.firstname,
                                               tourney=match.tourney.description,
                                               urls=urls, details=details,
                                               nmatches=len(matches), nboards=nboards)
