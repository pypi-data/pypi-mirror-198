...
from jwpackage.authentication.exceptions import AuthorizationFieldError,JWSecretsError
...
class JWTAuthenticate:
    ...
    async def authenticate_middleware(self,request: Request, call_next):
        try:
            start_time = time.time()
            ...
            if "JWSecret" in request.headers:
                JWSecret = request.headers.get("JWSecret")
                if JWSecret != os.getenv("JW_SECRET"):
                    raise JWSecretsError()
            else:
                ...
                if bearer != "Bearer":
                    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={"message": "Unauthorized"})
                auth = Authenticator(self.public_key,token)
                if not auth.is_authenticated():
                    return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={"message": "Unauthorized"})
            response = await call_next(request)
            process_time = time.time() - start_time
