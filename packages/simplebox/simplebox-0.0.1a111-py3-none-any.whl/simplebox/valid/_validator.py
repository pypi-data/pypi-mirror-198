...
class CompareValidator(Validator):
    ...
    def validate(self, value, message: str = None):
        for operate, obj in self.__operators.items():
            ...
            _check(type(value) != type(obj), message, TypeError(
                f"Valid error: '{type(value).__name__}' and '{type(obj).__name__}' cannot be '{symbol}'"))
...
class StringValidator(Validator):
    ...
    def validate(self, value, message: str = None):
        ...
        _check(self.__min is not None and len(value) <= self.__min, message, ValidatorException(
            f'Valid error: Expected {value!r} to be no smaller than {self.__min!r}'
        ))
        _check(self.__max is not None and len(value) >= self.__max, message, ValidatorException(
...
class TypeValidator(Validator):
    ...
    def validate(self, value, message: str = None):
        ...
        _check(not issubclass(value_type, tuple(self.__types)) or (self.__can_none and value is not None), message,
