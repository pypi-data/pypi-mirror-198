...
def basic():
	global proceed
	proceed = False
	print("Property Lister v1.3 ( github.com/ivan-sincek/property-lister )")
	print("")
...
def check_directory_files(directory):
	...
	for file in os.listdir(directory):
		file = directory + file
		if os.access(file, os.R_OK) and os.stat(file).st_size > 0:
			tmp.append(file)
	return tmp
...
def validate(key, value):
	...
	if len(value) > 0:
		if key == "-db" and args["database"] is None:
			...
			args["out"] = os.path.abspath(value) + os.path.sep
	...
	for db in database:
		...
		if data:
			...
			if blobs:
				for blob in blobs:
					count += 1
					open(out + os.path.basename(db) + "." + str(count) + ext["blob"], "wb").write(binascii.unhexlify(blob))
...
def extract(file, out, external = True):
	try:
		...
		if external:
			...
			shutil.copyfile(file, out + os.path.basename(file))
			file = out + os.path.basename(file)
	...
	if proceed and check_directory(args["out"]):
		...
		if args["database"]:
			...
			for file in os.listdir(args["out"]):
				extract(args["out"] + file, args["out"], False)
			for file in os.listdir(args["out"]):
				if file.lower().endswith(ext["plist"]):
					convert(args["out"] + file, args["out"])
		elif args["plist"]:
