...
from blackdark_nautobot_secrets_providers.providers import (
...
class HashiCorpVaultSecretsProviderTestCase(SecretsProviderTestCase):
    ...
    def test_get_client_k8s(self, requests_mocker, mock_file):
        ...
        new_plugins_config = {
            "blackdark_nautobot_secrets_providers": {
                "hashicorp_vault": {
                    "url": vault_url,
        ...
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["role_name"] = "some_role"
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            # Test Valid Response
    ...
    def test_valid_settings(self):
        ...
        self.assertEqual(returned_settings, settings.PLUGINS_CONFIG["blackdark_nautobot_secrets_providers"]["hashicorp_vault"])

        # No blackdark_nautobot_secrets_providers
        with self.settings(PLUGINS_CONFIG={"blackdark_nautobot_secrets_providers": {}}):
            with self.assertRaises(exceptions.SecretProviderError) as err:
        ...
        # invalid auth_method
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["url"] = vault_url
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["auth_method"] = "invalid"
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            with self.assertRaises(exceptions.SecretProviderError) as err:
                self.provider.get_client(self.secret)
        self.assertEqual(
        ...
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["auth_method"] = "token"
        del new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["token"]
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            with self.assertRaises(exceptions.SecretProviderError) as err:
                self.provider.get_client(self.secret)
        self.assertEqual(
        ...
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["auth_method"] = "kubernetes"
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            with self.assertRaises(exceptions.SecretProviderError) as err:
                self.provider.get_client(self.secret)
        self.assertEqual(
        ...
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["auth_method"] = "approle"
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["role_id"] = "asdf"
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            with self.assertRaises(exceptions.SecretProviderError) as err:
                self.provider.get_client(self.secret)
        self.assertEqual(
        ...
        del new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["role_id"]
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["auth_method"] = "approle"
        new_plugins_config["blackdark_nautobot_secrets_providers"]["hashicorp_vault"]["secret_id"] = "asdf"  # nosec B105
        with self.settings(PLUGINS_CONFIG=new_plugins_config):
            with self.assertRaises(exceptions.SecretProviderError) as err:
                self.provider.get_client(self.secret)
        self.assertEqual(
