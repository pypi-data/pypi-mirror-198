...
import mlopsdna.evidently as evidently.utils.data_operations
from mlopsdna.evidently.analyzers.classification_performance_analyzer import ClassificationPerformanceAnalyzer
from mlopsdna.evidently.analyzers.classification_performance_analyzer import ClassificationPerformanceMetrics
from mlopsdna.evidently.model_monitoring.monitoring import MetricsType
from mlopsdna.evidently.model_monitoring.monitoring import ModelMonitor
from mlopsdna.evidently.model_monitoring.monitoring import ModelMonitoringMetric
    ...
    def _yield_metrics(
        ...
        for idx, class_x_name in enumerate(metrics.confusion_matrix.labels):
            yield ClassificationPerformanceMonitorMetricsMonitor.class_representation.create(
                ...
                dict(dataset=dataset, class_name=str(class_x_name), type="target"),
            )
            yield ClassificationPerformanceMonitorMetricsMonitor.class_representation.create(
                sum([i[idx] for i in metrics.confusion_matrix.values]),
                dict(dataset=dataset, class_name=str(class_x_name), type="prediction"),
            )
