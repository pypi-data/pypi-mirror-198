...
try:
    ...
    raise ImportError("Datashader or seaborn not installed, but can be installed by running "
                      "`pip install hiveplotlib[datashader]`")


def datashade_edges_mpl(instance: Union[HivePlot, P2CP],
                        tag: Optional[Hashable] = None,
                        cmap: Union[str, colors.ListedColormap] = sns.color_palette("ch:start=.2,rot=-.3",
                                                                                    as_cmap=True),
                        ...
                        figsize: Tuple[float, float] = (10, 10), dpi: int = 300, mpl_axes_off: bool = True,
...
def datashade_hive_plot_mpl(instance: Union[HivePlot, P2CP],
                            tag: Optional[Hashable] = None,
                            cmap_edges: Union[str, colors.ListedColormap] = sns.color_palette("ch:start=.2,rot=-.3",
                                                                                              as_cmap=True),
                            cmap_nodes: Union[str, colors.ListedColormap] = "copper",
                            vmin_nodes: float = 1, vmax_nodes: Optional[float] = None,
                            vmin_edges: float = 1, vmax_edges: Optional[float] = None,
                            log_cmap: bool = True,
                            ...
                            axes_labels_fontsize: int = 16, axes_kwargs: Optional[dict] = None,
                            text_kwargs: Optional[dict] = None,
                            fig_kwargs: Optional[dict] = None,
                            **im_kwargs) -> Tuple[plt.Figure, plt.Axes, AxesImage, AxesImage]:
    """
    ...
    fig, ax, im_edges = datashade_edges_mpl(instance=instance, tag=tag, fig=fig, ax=ax, buffer=buffer,
                                            cmap=cmap_edges, vmin=vmin_edges, vmax=vmax_edges, log_cmap=log_cmap,
                                            reduction=reduction, pixel_spread=pixel_spread_edges, figsize=figsize,
                                            dpi=dpi, mpl_axes_off=mpl_axes_off,
                                            fig_kwargs=fig_kwargs, **im_kwargs)

    axes_viz(instance=instance, fig=fig, ax=ax,
             ...
             text_kwargs=text_kwargs, **axes_kwargs)

    fig, ax, im_nodes = datashade_nodes_mpl(instance=instance, fig=fig, ax=ax, buffer=buffer,
                                            cmap=cmap_nodes, vmin=vmin_nodes, vmax=vmax_nodes, log_cmap=log_cmap,
                                            reduction=reduction, pixel_spread=pixel_spread_nodes,
                                            mpl_axes_off=mpl_axes_off, **im_kwargs,
                                            zorder=2)
