...
from qrunner.utils.exceptions import ScreenFailException, PkgIsNull, DeviceNotFoundException
from qrunner.utils.log import logger
from qrunner.core.sonic.sib_util import SibUtil
from qrunner.core.ios.common import TideviceUtil
...
class IosDriver(object):
    ...
    def __init__(self, device=None, pkg_name=None):
        logger.info(f'init ios driver.')

        self.pkg_name = pkg_name
        if not self.pkg_name:
            raise PkgIsNull('应用bundle_id不能为空')
        if isinstance(device, str):
            self.flag = 'local'
            self.device_id = device
            if not self.device_id:
                raise DeviceNotFoundException('udid不能为空')
            ...
            sib_host = device['sib_host']
            sib_port = device['sib_port']

            # 本地需要部署sib环境，参考：https://github.com/SonicCloudOrg/sonic-ios-bridge
            self.sib = SibUtil(sib_host, sib_port)
            self.device_id = self.sib.connect()
            self.d = wda.Client(wda_url)
        else:
            raise DeviceNotFoundException('异常格式的device参数')
    ...
    def install_app(self, ipa_url, new=True, pkg_name=None):
        ...
        if self.flag == 'local':
            ...
            self.sib.install_app(self.device_id, ipa_url)
    ...
    def uninstall_app(self, pkg_name=None):
        ...
        if self.flag == 'local':
            ...
            self.sib.uninstall_app(self.device_id, pkg_name)
    ...
    def screenshot(self, file_name):
        ...
        img_dir = os.path.join(os.getcwd(), "screenshot")
