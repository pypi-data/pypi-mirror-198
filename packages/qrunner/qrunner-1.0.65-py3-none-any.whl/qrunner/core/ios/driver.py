...
from qrunner.utils.exceptions import ScreenFailException, ConnectInfoIsNull, PkgIsNull
...
class IosDriver(object):
    ...
    def __init__(self, device_id=None, pkg_name=None, remote_addr=None):
        self.pkg_name = pkg_name
        if not self.pkg_name:
            raise PkgIsNull('应用包名不能为空')

        if device_id:
            logger.info(f'init ios driver: {device_id}')
            self.device_id = check_device(device_id)
            self.port = get_tcp_port(self.device_id)
            self.remote_addr = f"http://localhost:{self.port}"
        elif remote_addr:
            logger.info(f'init ios driver: {remote_addr}')
            self.remote_addr = remote_addr
        else:
            raise ConnectInfoIsNull('device_id和remote_addr不能都为空')

        self.d = wda.Client(self.remote_addr)
