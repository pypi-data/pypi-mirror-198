...
def is_url_accessible(url: str) -> bool:
    ...
    if logger:
        logger.log(message=f"Checking if API URL is reachable: {url}", log_level="info", to_terminal=False)
    
    if not isinstance(url, str):
        if logger:
            logger.log(message=f"Input must be a string.", log_level="error", to_terminal=True)
    ...
    try:
        ...
        if logger:
            logger.log(message=f"Error while accessing URL: {e}", log_level="error", to_terminal=False)
        return False
