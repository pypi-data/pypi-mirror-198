...
class ConnectionModelType(sgqlc.types.Enum):
    ...
    * `FIVETRAN`: Fivetran
    '''
    __schema__ = schema
    __choices__ = ('ATHENA', 'BIGQUERY', 'DATABRICKS_DELTA', 'DATABRICKS_METASTORE', 'DATABRICKS_SQL_WAREHOUSE', 'DBT_CLOUD', 'DBT_CLOUD_V2', 'DBT_CORE', 'FIVETRAN', 'GLUE', 'HIVE', 'HIVE_MYSQL', 'HIVE_S3', 'LOOKER', 'LOOKER_GIT', 'LOOKER_GIT_CLONE', 'LOOKER_GIT_SSH', 'POWER_BI', 'PRESTO', 'PRESTO_S3', 'REDSHIFT', 'S3', 'S3_AIRFLOW_LOG_EVENTS', 'S3_METADATA_EVENTS', 'S3_QL_EVENTS', 'SNOWFLAKE', 'SPARK', 'TABLEAU')
...
class IncidentCategory(sgqlc.types.Enum):
    ...
    * `schema`None
    '''
    __schema__ = schema
    __choices__ = ('dimension', 'field_health', 'field_quality_rule', 'freshness', 'schema', 'sql_rule', 'volume')
...
class IntegrationKeyScope(sgqlc.types.Enum):
    ...
    * `S3PresignedUrl`None
    '''
    __schema__ = schema
    __choices__ = ('CircuitBreaker', 'DatabricksMetadata', 'S3PresignedUrl', 'Spark')
...
class WarehouseModelConnectionType(sgqlc.types.Enum):
    ...
    * `S3_METADATA_EVENTS`: S3 Metadata Events
    '''
    __schema__ = schema
    __choices__ = ('AIRFLOW_LOGS_S3', 'BIGQUERY', 'DATA_LAKE', 'REDSHIFT', 'S3_METADATA_EVENTS', 'SNOWFLAKE')
...
class FieldMetricInput(sgqlc.types.Input):
    __schema__ = schema
    __field_names__ = ('table_mcon', 'table_mcons', 'field_name', 'field_names', 'metric_type', 'filters')
...
class NotificationRoutingRules(sgqlc.types.Input):
    __schema__ = schema
    __field_names__ = ('project_names', 'dataset_ids', 'full_table_ids', 'rule_ids', 'domain_ids', 'tag_keys', 'tag_key_values', 'all_tag_key_values', 'table_stats_rules', 'monitor_labels', 'exclude_project_names', 'exclude_dataset_ids', 'exclude_full_table_ids', 'exclude_tag_keys', 'exclude_tag_key_values', 'exclude_all_tag_key_values', 'table_regex')
...
class Connection(sgqlc.types.relay.Connection):
    __schema__ = schema
    __field_names__ = ('id', 'uuid', 'type', 'account', 'warehouse', 'bi_container', 'etl_container', 'job_types', 'credentials_s3_key', 'integration_gateway_credentials_key', 'data', 'created_on', 'updated_on', 'dbt_projects', 'connection_identifier', 'connection_identifiers', 'job_errors')
    id = sgqlc.types.Field(sgqlc.types.non_null(ID), graphql_name='id')
...
class CreateOrUpdateUserSettings(sgqlc.types.Type):
    ...
    __schema__ = schema
    __field_names__ = ('response',)
    response = sgqlc.types.Field('CreateOrUpdateUserSettingsResponse', graphql_name='response')
    '''Response'''



class CreateOrUpdateUserSettingsResponse(sgqlc.types.Type):
    '''Create or Update User Settings Response'''
    __schema__ = schema
    __field_names__ = ('key', 'value', 'description')
    key = sgqlc.types.Field(String, graphql_name='key')
    '''Key for the user settings'''

    value = sgqlc.types.Field(JSONString, graphql_name='value')
    '''User settings'''

    description = sgqlc.types.Field(String, graphql_name='description')
    '''A short description about this user-specific setting and what it
    does.
    '''
...
class Freshness(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('status', 'expected', 'breach', 'reason', 'last_update', 'detector_threshold')
...
class IMonitor(sgqlc.types.Interface):
    __schema__ = schema
    __field_names__ = ('uuid', 'monitor_type', 'created_time', 'last_update_time', 'creator_id', 'updater_id', 'resource_id', 'entities', 'entity_mcons', 'schedule_type', 'name', 'rule_name', 'description', 'notes', 'labels', 'severity', 'notification_settings', 'is_snoozeable', 'is_paused', 'is_template_managed', 'namespace', 'next_execution_time', 'prev_execution_time', 'is_transitioning_data_provider', 'schedule_config', 'seven_days_incident_count', 'incident_count_history', 'weekly_incident_count_change_rate')
    uuid = sgqlc.types.Field(sgqlc.types.non_null(UUID), graphql_name='uuid')
...
class LineageGraphNode(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('mcon', 'display_name', 'has_downstream_nodes', 'has_upstream_nodes', 'object_type', 'is_custom')
    mcon = sgqlc.types.Field(sgqlc.types.non_null(String), graphql_name='mcon')
    '''Monte Carlo full identifier for an entity'''
...
class MonitorLabel(sgqlc.types.Type):
    ...
    label = sgqlc.types.Field(String, graphql_name='label')
    '''Monitor label created on account'''
...
class Mutation(sgqlc.types.Type):
    ...
    * `doc_mcon` (`String`): mcon of Doc, required if updating
      existing doc
    * `parent_doc_mcon` (`String`): mcon of parent Doc
    ...
    * `field_metric` (`FieldMetricInput`): Field metric parameters (if
      query generated by createFieldMetricParameters)
    * `interval_minutes` (`Int`): How often to run scheduled custom
      rule check (DEPRECATED, use schedule instead)
    * `labels` (`[String]`): The monitor labels
    * `notes` (`String`): Additional context for the rule
    ...
    * `id` (`Int!`): ID of existing data maintenance entry for
    ...
    create_or_update_user_settings = sgqlc.types.Field(CreateOrUpdateUserSettings, graphql_name='createOrUpdateUserSettings', args=sgqlc.types.ArgDict((
        ...
        ('value', sgqlc.types.Arg(sgqlc.types.non_null(JSONString), graphql_name='value', default=None)),
))
    )
    '''Create a new user-specific setting
    ...
    * `auth_mode` (`PowerBIAuthModeEnum!`): Authentication mode. We
      support two values here [service_principal, master_user]
    ...
    * `connection_options` (`ConnectionTestOptions`): Common options
      for integration tests.
    * `password` (`String`): Password when auth as a master user.
      Required if auth_mode is master_user.
    * `tenant_id` (`String!`): Azure power bi tenant uuid
    * `username` (`String`): Username when auth as a master user.
      Required if auth_mode is master_user.
    '''
...
class Query(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('get_collection_block_list', 'get_fivetran_connectors', 'get_pii_filtering_preferences', 'get_pii_filters', 'get_pii_filter_metrics', 'get_dbt_connections', 'get_dbt_projects', 'get_dbt_jobs', 'get_dbt_nodes', 'get_dbt_runs', 'get_dbt_models', 'get_dbt_model_results', 'get_dbt_model_results_count', 'get_dbt_test_results_count', 'get_dbt_test_results', 'get_dbt_upload_url', 'get_exec_dashboard_metrics', 'get_exec_dashboard_tables', 'get_custom_users', 'get_docs', 'get_unified_users', 'get_unified_user_assignments', 'get_monte_carlo_config_templates', 'export_monte_carlo_config_templates', 'get_correlation_sampling_metadata', 'detect_time_axis_intrinsic_delta', 'perform_correlation_sampling', 'get_rca_result', 'get_rca_job_result', 'get_sensitivity', 'thresholds', 'get_thresholds', 'get_table_columns_lineage', 'get_derived_tables_partial_lineage', 'get_parsed_query', 'get_job_execution_history_logs', 'get_dimension_tracking_monitor_suggestions', 'get_field_health_monitor_suggestions', 'monitor_labels', 'get_account_monitor_labels', 'get_monitors', 'get_all_user_defined_monitors_v2', 'get_all_user_defined_monitors', 'get_custom_metrics', 'get_custom_rule', 'get_custom_rules', 'get_generated_rules', 'get_circuit_breaker_rule_state', 'get_circuit_breaker_rule_state_v2', 'get_run_sql_rule_state', 'get_tables_for_sql', 'get_notification_settings_for_rules_with', 'get_field_metric_query', 'get_custom_rule_execution_analytics', 'get_insights', 'get_insight', 'get_reports', 'get_report_url', 'get_lineage_node', 'get_lineage_edge', 'get_lineage_node_block_pattern', 'get_lineage_node_block_patterns', 'get_lineage_node_replacement_rule', 'get_lineage_node_replacement_rules', 'simulate_lineage_node_replacement_rule', 'get_catalog_object_metadata', 'get_catalog_nav_level_nodes', 'get_catalog_nav_grouped_nodes', 'get_object_properties', 'get_object_property_name_values', 'get_object_property_names', 'get_object_property_values', 'get_active_monitors', 'get_monitor_summary', 'get_monitors_by_type', 'get_monitor', 'get_monitor_configuration', 'get_monitor_scheduling_configuration', 'get_time_axis_sql_expressions', 'get_notification_settings_for_monitors_with', 'get_data_assets_dashboard', 'get_incident_dashboard_data', 'get_incident_data_weekly', 'get_monitor_dashboard_data', 'get_blast_radius_direct_users', 'get_blast_radius_direct_users_v2', 'get_blast_radius_direct_queries', 'get_blast_radius_direct_queries_v2', 'get_blast_radius_direct_queries_summary', 'get_incident_tables', 'get_incident_warehouse_tables', 'get_direct_blast_radius_counts', 'get_blast_radius_direct_queries_for_user', 'get_airflow_tasks', 'get_airflow_task_attempts', 'get_airflow_task_logs', 'get_events', 'get_comments_for_monitor_incidents', 'get_event', 'get_event_comments', 'get_event_type_summary', 'get_incidents', 'get_incident_reaction', 'get_incident_summaries', 'get_incident_type_summary', 'get_incident_notification_settings_used', 'get_slack_messages_for_incident', 'get_slack_engagements_for_incident', 'get_all_domains', 'get_domain', 'get_account_roles', 'get_authorization_groups', 'get_user_authorization', 'search', 'get_object', 'get_metadata', 'get_metrics_v3', 'get_non_table_metrics', 'get_aggregated_metrics', 'get_latest_table_access_timestamp_metrics', 'get_top_category_labels', 'get_segmented_where_condition_labels', 'get_first_seen_dimensions_by_labels', 'get_first_and_last_seen_dimensions_by_labels', 'get_direct_lineage', 'get_downstream_bi', 'get_downstream_impact_radius_summary', 'get_downstream_reports', 'get_downstream_report_owners', 'get_downstream_report_types', 'get_table_lineage', 'get_external_source_paths_sample', 'get_tableau_workbook_count', 'get_query_list', 'get_query_by_id', 'get_query_by_query_hash', 'get_query_data_by_query_hash', 'get_query_data', 'get_query_log_hashes_that_affect_these_tables', 'get_query_log_hashes_on_these_tables', 'get_related_users', 'get_lineage_node_properties', 'get_recent_timestamp', 'get_hourly_row_counts', 'get_digraph', 'get_pipeline_freshness_v2', 'get_custom_sql_output_sample', 'get_metric_sampling', 'get_fh_sampling', 'get_dt_sampling', 'get_fh_reproduction_query', 'get_dt_reproduction_query', 'run_custom_query', 'test_sql_query_part', 'test_sql_query_where_expression', 'get_table_stats', 'get_resource', 'get_resources', 'get_table_fields_importance', 'get_data_maintenance_entries', 'get_wildcard_templates', 'get_common_fields', 'get_user_settings', 'get_user', 'get_user_by_id', 'get_warehouse', 'get_collection_properties', 'get_table', 'get_tables', 'get_tables_health', 'get_bq_projects', 'get_slack_oauth_url', 'get_slack_channels', 'get_slack_channels_v2', 'get_projects', 'get_datasets', 'get_field_bi_lineage', 'get_event_muting_rules', 'get_users_in_account', 'get_invites_in_account', 'get_token_metadata', 'get_integration_keys', 'test_existing_connection', 'test_telnet_connection', 'test_tcp_open_connection', 'test_notification_integration', 'get_databricks_cluster_info', 'get_databricks_notebook_link', 'get_databricks_metadata_job_info', 'get_current_databricks_notebook_version', 'validate_connection_type', 'get_event_onboarding_data', 'get_etl_containers')
    ...
    * `mcons` (`[String]`): The tables to inspect. All tables must
    ...
    get_user_settings = sgqlc.types.Field(JSONString, graphql_name='getUserSettings', args=sgqlc.types.ArgDict((
        ('key', sgqlc.types.Arg(sgqlc.types.non_null(String), graphql_name='key', default=None)),
))
    )
    '''Get user-specific settings. Returns null when no setting is
    defined.

    Arguments:

    * `key` (`String!`): User setting's key
    '''
...
class Size(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('metric', 'ucs_upper', 'ucs_lower', 'ucs_min_size_change', 'ucs_reason', 'ucs_status', 'sd_upper', 'sd_lower', 'sd_reason', 'sd_status', 'last_size_change', 'detector_threshold', 'sd_upper_high', 'sd_lower_high', 'sd_upper_medium', 'sd_lower_medium', 'sd_upper_low', 'sd_lower_low')
    metric = sgqlc.types.Field(String, graphql_name='metric')
...
class TableCapabilitiesResponse(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('supports_freshness', 'supports_bytes', 'supports_rows', 'supports_volume_slo', 'supports_write_throughput', 'supports_query_logs', 'supports_lineage', 'supports_lineage_pipeline', 'supports_field_lineage', 'supports_objects_deleted', 'supports_total_row_count_change', 'supports_dynamic_schedule', 'has_bytes', 'has_freshness', 'has_objects_deleted', 'has_rows', 'has_total_row_count_change', 'has_write_throughput')
...
class Warehouse(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('id', 'uuid', 'name', 'connection_type', 'credentials_s3_key', 'bq_project_id', 'account', 'data_collector', 'created_on', 'config', 'connections', 'tables', 'incidents', 'events', 'datasets', 'mute_rule', 'fivetran_destinations', 'data_sampling_enabled', 'value_ingestion_enabled', 'value_based_threshold_enabled', 'custom_sql_sampling_supported', 'custom_sql_sampling_enabled', 'supports_reproduction_queries', 'supports_sampling')
    id = sgqlc.types.Field(sgqlc.types.non_null(ID), graphql_name='id')
...
class AuthUser(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('cognito_user_id', 'email', 'first_name', 'last_name', 'display_name', 'state', 'created_on', 'is_sso', 'sso_groups', 'sso_groups_updated_at', 'token_id', 'notification_settings_added', 'notification_settings_modified', 'invitees', 'eventmodel_set', 'incident_reactions_created', 'incident_reactions_modified', 'user_comments', 'creator', 'metricmonitoringmodel_set', 'combinedtablestatsmodel_set', 'object_properties', 'catalog_object_metadata', 'docs', 'doc_authors', 'lineage_nodes', 'lineage_edges', 'resources', 'lineage_block_patterns', 'lineage_repl_rules', 'monte_carlo_config_templates', 'domain_created_by', 'slack_credentials_v2', 'custom_users', 'unified_users', 'last_updated_unified_users', 'collection_preference_created_by', 'collection_preference_last_updated_by', 'collection_preference_deleted_by')
    ...
    sso_groups = sgqlc.types.Field(sgqlc.types.list_of(String), graphql_name='ssoGroups')
...
class CustomRule(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('uuid', 'account_uuid', 'projects', 'datasets', 'description', 'notes', 'labels', 'is_template_managed', 'namespace', 'severity', 'rule_type', 'warehouse_uuid', 'comparisons', 'interval_minutes', 'interval_crontab', 'start_time', 'timezone', 'creator_id', 'updater_id', 'prev_execution_time', 'next_execution_time', 'last_check_timestamp', 'created_time', 'updated_time', 'is_deleted', 'snooze_until_time', 'slack_snooze_user', 'conditional_snooze', 'dc_schedule_uuid', 'data_collection_dc_schedule_uuid', 'custom_sql', 'override', 'variables', 'generated_by', 'entities', 'rule_name', 'sequence_number', 'system_snooze_until_time', 'query_result_type', 'custom_sampling_sql', 'generated_rules', 'entity_mcons', 'rendered_custom_sql', 'schedule_config', 'data_collection_schedule_config', 'notification_settings', 'is_snoozed', 'field_metric')
    uuid = sgqlc.types.Field(sgqlc.types.non_null(UUID), graphql_name='uuid')
...
class DbtProject(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'connection', 'project_id', 'project_name', 'source', 'generates_incidents', 'last_model_import', 'last_test_import', 'dbt_jobs', 'dbt_nodes', 'dbt_edges', 'dbt_runs')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class DbtRun(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'dbt_project', 'dbt_job_definition_id', 'job', 'dbt_run_id', 'git_branch', 'run_logs', 'created_at', 'finished_at', 'duration', 'queued_duration', 'run_duration', 'generated_at', 'started_at', 'in_progress', 'is_complete', 'is_success', 'is_error', 'is_cancelled', 'status_raw', 'status_humanized', 'status_message', 'command', 'dbt_run_steps')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class DbtRunStep(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'account_id', 'status', 'started_at', 'completed_at', 'thread_id', 'execution_time', 'message', 'raw_json', 'dbt_run', 'node_unique_id', 'table')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class FivetranConnector(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('created_time', 'updated_time', 'uuid', 'mcon', 'account_id', 'fivetran_destination', 'fivetran_id', 'service', 'schema', 'source_sync_details', 'created_at', 'succeeded_at', 'failed_at', 'sync_frequency', 'setup_state', 'sync_state', 'update_state', 'tables', 'fivetran_status', 'fivetran_url')
    created_time = sgqlc.types.Field(sgqlc.types.non_null(DateTime), graphql_name='createdTime')
...
class Incident(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('uuid', 'warehouse', 'created_time', 'updated_time', 'owner', 'severity', 'system_set_severity', 'feedback', 'feedback_time', 'last_update_user', 'project', 'dataset', 'incident_type', 'incident_sub_types', 'incident_time', 'lock_incident', 'events', 'incident_reaction', 'slack_msg_details', 'reaction_type', 'summary', 'suggested_owner', 'topology')
    uuid = sgqlc.types.Field(sgqlc.types.non_null(UUID), graphql_name='uuid')
    ...
    lock_incident = sgqlc.types.Field(sgqlc.types.non_null(Boolean), graphql_name='lockIncident')
...
class TablePartitionKeys(sgqlc.types.Type, Node):
    ...
    template_keys = sgqlc.types.Field(sgqlc.types.list_of(String), graphql_name='templateKeys')
...
class User(sgqlc.types.Type, Node):
    ...
    * `last` (`Int`)None
    ...
    '''Only used to identify system users. Auth groups supersede this.'''
...
class WarehouseTableHealth(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('table_id', 'full_table_id', 'warehouse', 'discovered_time', 'friendly_name', 'description', 'location', 'project_name', 'dataset', 'table_type', 'is_encrypted', 'created_time', 'last_modified', 'view_query', 'labels', 'path', 'priority', 'tracked', 'status', 'freshness_anomaly', 'size_anomaly', 'freshness_size_anomaly', 'metric_anomaly', 'dynamic_table', 'is_deleted', 'last_observed', 'data_provider', 'mcon', 'is_important', 'importance_score', 'tags', 'categories_with_monitors', 'incidents')
    table_id = sgqlc.types.Field(String, graphql_name='tableId')
