...
def gamma_DNV205(hs, tp):
    ...
    if tp_hs <= 3.6: gamma = 5
    elif tp_hs >= 5: gamma = 1
    else: gamma = np.exp(5.75-1.15*tp_hs)
    return gamma


def jonswap(f:np.ndarray, Hm0:float, Tp:float, gamma:float=None, sigma_low:float=.07, sigma_high:float=.09,
            g:float=9.80665, method:str='yamaguchi', normalize:bool=False):
    ...
    gamma : float
        JONSWAP peak-enhancement factor (default: DNV-RP-205)
    sigma_low : float
    ...
    Returns
    -------
    E : numpy.ndarray with wave energy densities
    f : frequency vector (clipped version of inputted if f=0 present)
    '''

    gamma = gamma_DNV205(Hm0, Tp) if gamma is None else gamma
    
    f = np.clip(f, 1e-8, np.max(f))  
...
#    """
#    # Peak shape parameter.
#    gamma = gamma_DNV205(h_s, t_p) if gamma is None else gamma
#   
#    # Normalizing factor.
