...
class ImagePipeline(OPipeline[OlympImage]):
    ...
    def explode(self, explode_function: Callable[[OlympImage], List[OlympImage]]):
        ...
        output = ImagePipeline()
        output.pipeline = self.pipeline.explode(explode_function)
    ...
    def filter(self, filter_function: Callable[[OlympImage], bool]) -> "ImagePipeline":
        ...
        output = ImagePipeline()
        output.pipeline = self.pipeline.filter(filter_function)
    ...
    def class_task(
        ...
        output.pipeline = self.pipeline.class_task(
    ...
    def batch(self, batch_size: int) -> "BatchPipeline":
        output = BatchPipeline()
        if self.pipeline is None:
            raise Exception("Undefined pipeline")

        output.pipeline = self.pipeline.batch(batch_size).task(OlympBatch.from_images)

        return output

    # def limit(self, count: int) -> "ImagePipeline":
    #     """Limits the number of packets to 'count'

    #     Args:
    #         count (int): maximum of packets that went through
    #     """
...
class BatchPipeline(OPipeline[OlympBatch]):
    def __init__(self):
        self.pipeline: Optional[Pipeline[OlympBatch]] = None
    ...
    def to_images(self) -> "ImagePipeline":
        ...
        output.pipeline = self.pipeline.explode(OlympBatch.to_images)
