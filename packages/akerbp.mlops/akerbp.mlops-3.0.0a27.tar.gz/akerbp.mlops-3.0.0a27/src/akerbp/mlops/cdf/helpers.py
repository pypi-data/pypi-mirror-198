...
def get_client(
    client_id: str,
    client_secret: str,
    tenant_id: str = client_secrets["tenant-id"],
    base_url: str = client_secrets["base-url"],
) -> CogniteClient:
    """
...
def call_function(function_name: str, data: Dict) -> Any:
    ...
    time.sleep(15)  # Give cognite some time to collect the logs
    logs = call.get_logs()
    for log in logs:
        logger.info(
...
def garbage_collection(
    ...
    if models_to_keep is not None and models_to_keep >= len(version_ids):
        ...
        logger.info(
            f"Keeping all model versions ({len(version_ids)}) alive in {env} as specified in the settings file"
        )
    else:
        logger.info(
