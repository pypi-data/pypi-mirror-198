...
def setup_yams_tenant(command):
    commands = command.add_subcommands('action', [
        ('list', "list tenants (only Edge team)", ""),
        ('info', "get tenant", ""),
        ('update', "update tenant (only Edge team)",
         "--sdrn sdrn:company:team:name --admin-email admin@email.com"),
        ('delete', "delete tenant", ""),
    ])

    commands['update'].add_argument('--sdrn', required=False, default=None, help="sdrn associated with this tenant")
    commands['update'].add_argument('--admin-email', required=False, default=None, help="admin email")
...
def setup_yams_bucket(command):
    commands = command.add_subcommands('action', [
        ...
        ('create', "create a bucket",
            "--domain-id 5adcb26c-92fe-4f83-afb5-bf2322c0bfca --bucket-name my-new-bucket --bucket-continent europe"),
    ...
    commands['create'].add_argument(
        '--bucket-continent', required=True, choices=["europe", "america"], help="AWS continent of the bucket")
...
def setup_yams_image(command):
    commands = command.add_subcommands('action', [
        ('fetch', "get image applying a transformation rule",
            ...
            "\n  %(prog)s" +
            " --domain-id 5adcb26c-92fe-4f83-afb5-bf2322c0bfca --bucket-id 04760f77-da25-4de1-adde-bdfe62337c3c" +
            " --object-id document.pdf --page 2 --rule-json " +
            '\'{"format":"png","name":"my-rule-2","actions":' +
            '[{"resize":{"width":200,"fit":{"type":"clip"}}}],"version":"2017-06","quality":90}\'' +
             "\n  %(prog)s" +
    ...
    commands['fetch'].add_argument_as_dict(object_id)
    commands['fetch'].add_argument_as_dict(page)
...
def setup_yams_job(command):
    commands = command.add_subcommands('action', [
        ('list', "list all existing jobs for a bucket in this tenant",
         "--domain-id 5adcb26c-92fe-4f83-afb5-bf2322c0bfca --bucket-id 04760f77-da25-4de1-adde-bdfe62337c3c"),
        ('info', "show the details of a job",
         "--domain-id 5adcb26c-92fe-4f83-afb5-bf2322c0bfca --bucket-id 04760f77-da25-4de1-adde-bdfe62337c3c "
         + "--job-id 71e20289-c0d9-487b-86ee-87829e97afef"),
        ('create', "create a new job in a bucket of this tenant",
         "--domain-id 5adcb26c-92fe-4f83-afb5-bf2322c0bfca --bucket-id 04760f77-da25-4de1-adde-bdfe62337c3c --job {\"operation\": "
         "\"document-concatenation\", \"source_object_ids\": [\"object-id-1\", \"object-id-2\"]}"),
    ])
    ...
    job = {
        'args': ['--job'],
        'required': True,
        'help': "job configuration as JSON string",
    }
    ...
    commands['info'].add_argument_as_dict(domain_id)
    commands['info'].add_argument_as_dict(bucket_id)
    commands['info'].add_argument_as_dict(job_id)

    commands['create'].add_argument_as_dict(domain_id)
    commands['create'].add_argument_as_dict(bucket_id)
    commands['create'].add_argument_as_dict(job)
