...
class _TwoNameUid(_Card):
    ...
    def __init__(self, bline):
        ...
        try:
            ...
            # 对可以调换节点12的卡规定顺序
            if self.values[self.field_index[CARD_TYPE]][0] == 'L':  # L, L+, LM, LD, LZ
                if self.values[n1] > self.values[n2] or (self.values[n1] == self.values[n2] and self.values[b1] > self.values[b2]):  # 规则：要小的在前面. 处理：直接value换位，避免后续影响
                    self.values[n1], self.values[n2] = self.values[n2], self.values[n1]
                    self.values[b1], self.values[b2] = self.values[b2], self.values[b1]
...
class _LineCard(_TwoNameUid):
    def get_y(self):
        ...
        return y11 + y10, -y12, -y21, y22 + y20