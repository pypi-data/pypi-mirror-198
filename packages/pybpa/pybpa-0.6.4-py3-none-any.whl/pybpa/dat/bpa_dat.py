...
class DAT:
    ...
    *_indexs: {uid_str: _Cards}
    ...
    bus_card_set = {'B ', 'BV', 'BM', 'BQ', 'BS', 'BT', 'BE', 'BF', 'BJ', 'BK', 'BL', 'BX', 'BD', '+', 'X ','BA','BZ','BC'}
    ...
    def __init__(self, *p):
        ...
        self.gen, self.gen_indexs, self.gen_order, self.gen_name, self.gen_bus_order,self.zone_exchange,self.bus_zone,self.gen_zone,self.bus_zone_type,self.gen_zone_type= self.get_data_struct()
        self.gen_info = None
    ...
    def describe(self):
        ...
        """
        # 统计各电压等级节点数
        vol_level = [800, 500, 220, 110, 10, 0.48, 0.4, 0]
        ...
        for v in vol_count.keys():
            for ll, n in vol_bases:
                if ll > v:
                    vol_count[v] += n
        print([str(k) + 'kV: ' + str(v) for k, v in vol_count.items()])
        # 统计线路数
        branch_type = Counter([b[0].get_value('TYPE') for b in self.branch_indexs.values()])
    ...
    def get_data_struct(self):
        ...
        return bus, bus_indexs, bus_order, bus_name, \
               ...
               gen, gen_indexs, gen_order, gen_name, gen_bus_order,zone_exchange,bus_zone,gen_zone,bus_zone_type,gen_zone_type
    ...
    def read_pfo(self, dat_path, pu=False, type ='BPA'):
        ...
        if type=='BPA':
            ...
            # 标幺值转换
            if pu:
                df[[GEN_P, GEN_Q, 'PF', LOAD_P, LOAD_Q, 'SHUNTQ1', 'SHUNTQ2', 'SHUNTQ3']] /= self.mva_base
                # todo pfo中的pf是什么，为什么要标幺化？
                df['SHUNTB'] = df['SHUNTQ1'].copy().add(df['SHUNTQ3']).div(df[VOLTAGE]).div(df[VOLTAGE])
            else:
                print('dat.read_pfo:pu==False,没有计算SHUNTB，可能出错。')
