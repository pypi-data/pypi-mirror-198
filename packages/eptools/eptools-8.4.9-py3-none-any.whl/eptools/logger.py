...
def createSlackLogger(name, logger = None):
    ...
    slacker = SlackFactory(config_path=None, logger=logger)
    return slacker
...
class EasyPostLogger():
    ...
    def __init__(self, name: str=None, logpath: str=None) -> None:
        ...
        self.slack_logger = createSlackLogger(self.name,logger=self.file_logger)
    ...
    def __overall__(self, msg: str, level: int):
        ...
        if self.slack_logger:
            if self.slacker_level <= level:
                self.slack_logger.log(self.name, message=full_msg, error=msg.split('\n')[0])
