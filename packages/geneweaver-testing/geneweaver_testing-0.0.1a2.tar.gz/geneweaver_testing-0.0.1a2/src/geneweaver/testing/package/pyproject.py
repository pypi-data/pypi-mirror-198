from .utils import find_root_dir, get_pyproject_toml_contents
...
def test_has_pyproject_toml():
    """Test that the pyproject.toml file exists."""
    assert (find_root_dir() / "pyproject.toml").is_file(), (
...
def test_has_tool_poetry_section():
    ...
    assert "tool" in pyproject, error_msg
    assert "poetry" in pyproject["tool"], error_msg


def test_pyproject_has_package_name():
...
def test_pyproject_has_version():
    ...
    assert (
        "version" in pyproject["tool"]["poetry"]
...
def test_pyproject_has_description():
    ...
    assert pyproject["tool"]["poetry"]["description"] != "", (
...
def test_pyproject_has_authors():
    ...
    pyproject = get_pyproject_toml_contents()
    assert (
        "authors" in pyproject["tool"]["poetry"]
    ), "pyproject.toml file does not have an authors section"
    assert (
        len(pyproject["tool"]["poetry"]["authors"]) > 0
...
def test_pyproject_has_license():
    ...
    pyproject = get_pyproject_toml_contents()
    assert (
        "license" in pyproject["tool"]["poetry"]
    ...
    assert (
        pyproject["tool"]["poetry"]["license"] == "Apache-2.0"
...
def test_pyproject_has_readme():
    ...
    pyproject = get_pyproject_toml_contents()
    assert (
        "readme" in pyproject["tool"]["poetry"]
...
def test_poetry_has_packages_definition():
    ...
    pyproject = get_pyproject_toml_contents()
    assert (
        "packages" in pyproject["tool"]["poetry"]
    ), "pyproject.toml file does not have a packages section"
    assert (
        len(pyproject["tool"]["poetry"]["packages"]) > 0
...
def test_poetry_build_system_definition():
    ...
    assert "build-system" in pyproject, error_msg
    assert "requires" in pyproject["build-system"], error_msg
    assert "poetry-core" == pyproject["build-system"]["requires"], error_msg
    assert "build-backend" in pyproject["build-system"], error_msg
    assert "poetry.masonry.api" == pyproject["build-system"]["build-backend"], error_msg
