...
class Wire(BaseModel):
    ...
    direction: Optional[StrictStr] = Field(None, description="Indicates whether the wire was sent (`OUT`) or received (`IN`)")
    ...
    suspense_reason: Optional[StrictStr] = Field(None, description="Description of the reason the transfer is currently in suspense")
    transaction_id: StrictStr = Field(..., description="ID of the resulting transaction resource")
    __properties = ["amount", "bank_message", "creation_time", "currency", "customer_id", "direction", "effective_date", "id", "last_updated_time", "originating_account_id", "receiving_account_id", "recipient_message", "sender_reference_id", "status", "suspense_reason", "transaction_id"]

    @validator('direction')
    def direction_validate_enum(cls, v):
        if v is None:
            return v

        if v not in ('IN', 'OUT'):
            raise ValueError("must validate the enum values ('IN', 'OUT')")
        return v

    @validator('status')
    def status_validate_enum(cls, v):
        if v not in ('PENDING', 'COMPLETED', 'CANCELED', 'DECLINED', 'SUSPENSE'):
            raise ValueError("must validate the enum values ('PENDING', 'COMPLETED', 'CANCELED', 'DECLINED', 'SUSPENSE')")
        return v
    ...
    def from_dict(cls, obj: dict) -> Wire:
        ...
        _obj = Wire.parse_obj({
            ...
            "suspense_reason": obj.get("suspense_reason"),
