...
def format_games(games):
    ...
    col_renamer.update({col: col.lstrip('L') + '_OPP' for col in games.columns if col.startswith('L')})
    ...
    games['_id'] = games[['Season', 'DayNum', 'TeamID', 'TeamID_OPP']].agg(lambda x: '.'.join(map(str, x)), axis=1)
