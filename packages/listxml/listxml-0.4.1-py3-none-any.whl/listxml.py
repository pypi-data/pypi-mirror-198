...
def _search_for_path(topels, els, l, with_element):
    ...
    #print("_path: els={}  l={}  atts={}  body={}".format(els, l, atts, body))
            ...
            if len(els[1]) == 1:
                ...
                if a in atts and els[1][1] == atts[a]:
                    ...
                    if with_element and not els[2:]:
                        ...
                        return _search_for_path(topels, [els[0], *els[2:]], l, with_element)
                    else:
                ...
                raise ValueError("search_for_path: paths which end in a list must have either one or two values, not {}".format(topels))
        else:
...
def search_for_path(xp, l, with_element=False):
    ...
    return _search_for_path(els_list, els_list, l, with_element)

_xptrans1_atts = re.compile(r'(?:(\w+)|@(\w+)(?:=(\w+))?)$')
def _parse_xpath1(xp):
    def xptrans(el):
        if not el:
            raise ValueError("Malformed xpath (empty element): {}".format(xp))
        ...
        if m:
            ...
            raise ValueError("Malformed xpath: {}".format(xp))
    if xp:
        return [xptrans(x) for x in xp.split('/')]
    else:
        return []

_xptrans_atts = re.compile(r'(?:(\w+)(?:\[([^]]+)\])?|@(\w+))$')
_xptrans_attvalue = re.compile(r'(?:@(\w+)="(\w+)"|(\d+))$')
def _parse_xpath(xp):
    def xptrans(el):
        if not el:
            raise ValueError("Malformed xpath (empty element): {}".format(xp))
            ...
            if m.group(2):
                ...
                if m2:
                    if m2.group(2):
                        ...
                        # return (m.group(1), [int(m2.group(3))])
                        raise ValueError("Malformed xpath (bad predicate): {}".format(xp))
                else:
                    raise ValueError("Malformed xpath (bad predicate): {}".format(xp))
            elif m.group(1):
                # el
