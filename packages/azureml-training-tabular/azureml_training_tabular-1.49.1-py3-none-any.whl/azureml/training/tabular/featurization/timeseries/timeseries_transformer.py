...
from ..._constants import FeatureType, TimeSeries, TimeSeriesInternal, TransformerParams
...
from ..._types import FeaturizationSummaryType, GrainType
...
from ...timeseries.forecasting_verify import is_iterable_but_not_string
...
from .._featurization_config import FeaturizationConfig
from .._featurization_info_provider import FeaturizationInfoProvider
from .._operator_names import _OperatorNames, _TransformerOperatorMappings
from .._raw_feature_stats import TimeSeriesStat
from .._supported_transformers import SupportedTransformers
from ._distributed.aggregate_transformer import PerGrainAggregateTransformer
...
class TimeSeriesTransformer(AzureMLTransformer, FeaturizationInfoProvider):
    ...
    @staticmethod
    def _join_reusable_features_for_cv(
        ts_transformer_cv: "TimeSeriesTransformer",
        X_cv: pd.DataFrame,
        ...
        check_columns = hasattr(ts_transformer_full, "detected_column_purposes") and hasattr(
            ts_transformer_cv, "detected_column_purposes"
        )

        if check_columns:
            bad_column_sets = [
                (
                    set(ts_transformer_full.detected_column_purposes[purpose]).symmetric_difference(
                        ts_transformer_cv.detected_column_purposes[purpose]
                    )
                )
            ...
            if len(bad_columns) > 0:
                cv_only_drop = set(ts_transformer_cv.drop_column_names).difference(
        ...
        Contract.assert_true(
            ...
            (
                "CV pipeline transforms must be a subset of " "FULL pipeline transforms. CV-unique transforms: {}"
            ).format(cv_unique_transforms),
            log_safe=True,
        )
        ...
        X_cv_new = X_cv.drop(columns=list(cols_drop_cv), errors="ignore").merge(
            X_temp, how="inner", left_index=True, right_index=True, sort=True
        )
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.use_stl = kwargs.get(TimeSeries.USE_STL, TimeSeriesInternal.USE_STL_DEFAULT)
        self.seasonality = kwargs.get(TimeSeries.SEASONALITY, TimeSeriesInternal.SEASONALITY_VALUE_DEFAULT)
        self.force_time_index_features = kwargs.get(
            TimeSeriesInternal.FORCE_TIME_INDEX_FEATURES_NAME, TimeSeriesInternal.FORCE_TIME_INDEX_FEATURES_DEFAULT
        )
        ...
        if self._featurization_config.drop_columns is not None and len(self._featurization_config.drop_columns) > 0:
        ...
        self.origin_column_name = kwargs.get(
        ...
        self.target_imputation_marker_column_name = MissingDummiesTransformer.get_column_name(self.target_column_name)
    ...
    def _init_missing_y(self) -> MissingDummiesTransformer:
        ...
        out = MissingDummiesTransformer([self.target_column_name])
        return out
    ...
    # TODO: Implement this outside of this package
    """
    def __repr__(self):
    ...
    def __setstate__(self, state):
        ...
        if pipeline is not None and pipeline.__class__.__name__ == "AzureMLForecastPipeline":
        ...
        try:
            if series_stats_dict:
                self._series_stats = TimeSeriesStat(
                    series_stats_dict.get(TimeSeries.SERIES_COLUMN_COUNT, 0),
                    series_stats_dict.get(TimeSeries.SERIES_COUNT, 0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_MIN, 0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_MAX, 0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_AVG, 0.0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_PERC_25, 0.0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_PERC_50, 0.0),
                    series_stats_dict.get(TimeSeries.SERIES_LEN_PERC_75, 0.0),
                )
        except ImportError:
            pass
    ...
    def _create_feature_transformer_graph(
        self, X: Union[pd.DataFrame, TimeSeriesDataSet], y: Optional[np.ndarray] = None
    ) -> None:
        """
        No-op.

        TODO: Move all transformer graph/engineered feature names code out
        """
        self._feature_graph = None

    def _create_feature_transformer_graph_if_not_set(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> None:
    ...
    def _get_features_by_transform(self) -> DefaultDict[str, List[str]]:
        ...
        for feature in cast(Dict[str, Any], self._feature_graph):
            # Get the last transform in the list for this feature - we assume other transforms are intermediate
            _, trans = cast(Dict[str, Any], self._feature_graph)[feature][-1]
    ...
    def _get_json_str_for_engineered_feature_name(self, engineered_feature_name: str) -> str:
        """
        ...
        if engineered_feature_name not in self._engineered_feature_name_objects:
            ...
            engineered_feature_name_json_obj = cast(
                _FeatureTransformersAsJSONObject, self._engineered_feature_name_objects[engineered_feature_name]
            )._entire_transformation_json_data
            # Convert JSON into string and return
            return json.dumps(engineered_feature_name_json_obj)

    def get_json_strs_for_engineered_feature_names(
        self, engi_feature_name_list: Optional[List[str]] = None
    ) -> List[str]:
        """
        ...
        for engineered_feature_name in cast(List[str], engi_feature_name_list):
            json_str = self._get_json_str_for_engineered_feature_name(engineered_feature_name)
    ...
    def _select_known_before_date(self, X: pd.DataFrame, date: pd.Timestamp, freq: pd.DateOffset) -> pd.DataFrame:
        """
        ...
        Contract.assert_true(
            self.origin_column_name in X.index.names, "X doesn't contain origin times.", log_safe=True
        )
        ...
        if len(self._transforms) == 1 and TimeSeriesInternal.LAG_LEAD_OPERATOR in self._transforms:
            lag_lead_operator = cast(LagLeadOperator, self._transforms[TimeSeriesInternal.LAG_LEAD_OPERATOR])
            lag_setting = lag_lead_operator.lags_to_construct

            # Lag orders may be ints or list of ints. Get orders as a list of lists so we can safely iterate
            lag_orders_list = [[cast(int, lag)] if not is_iterable_but_not_string(lag) else cast(
                List[int], lag) for lag in lag_setting.values()]
    ...
    def _get_fit_column_order_after_transform(self, df_transformed: pd.DataFrame) -> List[str]:
        ...
        if not self._keep_missing_dummies_on_target:
        ...
        fit_column_order = [nm for nm in df_transformed.columns if nm not in exclude_col_list]

        return cast(List[str], fit_column_order)

    def _transform_prep_common(
        self, df: pd.DataFrame, y: Optional[np.ndarray] = None
        ...
        """
        Validation.validate_type(df, "X", pd.DataFrame, reference_code=ReferenceCodes._TST_TRANSFORM_ARG_WRONG_TYPE)
        ...
        if self._keep_missing_dummies_on_target:
        ...
        if y is not None:
            try:
                ...
                raise AzureMLError.create(
                    AutoMLInternal,
                    error_details="Unable to append target column to input DataFrame.",
                    inner_exception=e,
                )
    ...
    def _transform_finalize_common(self, tsds: TimeSeriesDataSet, transformed_data: TimeSeriesDataSet) -> pd.DataFrame:
        """
        ...
        if self._keep_missing_dummies_on_target:
            ...
            Contract.assert_true(
                self.original_order_column in transformed_data.data.columns,
        ...
        stl = forecasting_utilities.get_pipeline_step(self.pipeline, TimeSeriesInternal.MAKE_SEASONALITY_AND_TREND)
        if stl:
            if isinstance(stl, PerGrainAggregateTransformer):
        ...
        if self.pipeline_type is TimeSeriesPipelineType.FULL:
            ...
            if TimeSeriesInternal.ORIGIN_TIME_OCCURRENCE_COLUMN_NAME in transformed_data_df.columns:
                ...
                Contract.assert_true(
                    self.origin_column_name in old_idx_names,
                ...
                transformed_data_df.set_index(
                    TimeSeriesInternal.ORIGIN_TIME_OCCURRENCE_COLUMN_NAME, append=True, inplace=True
                )
    ...
    @function_debug_log_wrapped(logging.INFO)
    def transform(self, df: pd.DataFrame, y: Optional[np.ndarray] = None) -> pd.DataFrame:
        """
        ...
        df_fixed = fix_df_frequency(
            df, self.time_column_name, self.grain_column_names, self.dict_latest_date, self.freq_offset
        )
    ...
    @function_debug_log_wrapped(logging.INFO)
    def fit_transform(self, df: pd.DataFrame, y: np.ndarray) -> pd.DataFrame:
        """
    ...
    def _save_convert_grain_types(self, df: pd.DataFrame) -> None:
        ...
        for grain in self.grain_column_names:
            if (
                self._featurization_config.column_purposes is not None
                    and self._featurization_config.column_purposes.get(grain) is not None
            ):
                continue
    ...
    def _convert_grain_type_safe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for grain in self.grain_column_names:
            if grain not in self._detected_grain_types:
                continue
            try:
                if self._detected_grain_types[grain] == "str" and any(not isinstance(val, str) for val in df[grain]):
                    warnings.warn(
                        "Mixed type time series identifiers " "were detected and were converted to string type."
                    )
    ...
    def _internal_fit(self, df, y):
        ...
        for grain, df_one in tsds.groupby_time_series_id():
            ...
            self._y_imputers[grain] = _get_target_imputer(
                self.target_column_name, self.freq_offset, self._featurization_config, tsds=tsds_one
            )
        ...
        self.detected_column_purposes = {
            FeatureType.Numeric: _get_numerical_columns(
                tsds.data, self.target_column_name, self.drop_column_names, self._featurization_config
            ),
            FeatureType.Categorical: _get_categorical_columns(
                tsds.data, self.target_column_name, self.drop_column_names, self._featurization_config
            ),
            FeatureType.DateTime: _get_date_columns(tsds.data, self.drop_column_names, self._featurization_config),
    ...
    def remove_rows_with_imputed_target(self, X: pd.DataFrame, y: np.ndarray) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        Contract.assert_true(
            self.target_column_name not in X.columns,
        ...
        try:
            ...
            raise AzureMLError.create(
                AutoMLInternal, error_details="Unable to append target column to input DataFrame.", inner_exception=e
            )
    ...
    def _remove_nans_from_look_back_features(self, X: pd.DataFrame, y: np.ndarray) -> Tuple[pd.DataFrame, np.ndarray]:
        Contract.assert_type(X, "X", pd.DataFrame)
    ...
    def _raise_no_fit_exception_maybe(self, reference_code: str) -> None:
        ...
        if not self.pipeline:
            raise AzureMLError.create(FitNotCalled, reference_code=reference_code)

    def _check_phase(
        self, scoring_tsds: TimeSeriesDataSet, scoring_freq: DateOffset, freq_diff_exception: str, has_pii: bool = True
    ) -> None:
        """
        ...
        **Note:** This method is not used anymore. It is retained for backward
        ...
        for grain, df_one in scoring_tsds.groupby_time_series_id():
            ...
            if date_before is None:
                raise AzureMLError.create(
                    TimeseriesGrainAbsent, target="grain", reference_code=ReferenceCodes._TST_CHECK_PHASE_NO_GRAIN
                )
            time_index = df_one.index.get_level_values(scoring_tsds.time_column_name)
            # Create a date grid.
            date_grid = pd.date_range(start=date_before, end=time_index.max(), freq=self.freq_offset)
    ...
    def _impute_target_value(self, tsds: TimeSeriesDataSet) -> TimeSeriesDataSet:
        ...
        if self._is_using_customized_target_imputer():
            target_imputer = _get_target_imputer(
                self.target_column_name, self.freq_offset, self._featurization_config, tsds
            )
        else:
            target_imputer = _get_target_imputer(self.target_column_name, self.freq_offset, self._featurization_config)
    ...
    def _is_using_customized_target_imputer(self) -> bool:
        ...
        if self._featurization_config.transformer_params is not None:
            imputer_settings = self._featurization_config.transformer_params.get(
                SupportedTransformers.Imputer, zip([], {})
            )
    ...
    def unique_target_grain_dropper(self) -> UniqueTargetGrainDropperBase:
        ...
        return cast(
            ...
            forecasting_utilities.get_pipeline_step(pipeline, TimeSeriesInternal.UNIQUE_TARGET_GRAIN_DROPPER))
    ...
    def get_target_lags(self) -> List[int]:
        ...
        pipeline = cast(Pipeline, self.pipeline)
        lag_operator = forecasting_utilities.get_pipeline_step(pipeline, TimeSeriesInternal.LAG_LEAD_OPERATOR)
    ...
    def get_target_rolling_window_size(self) -> int:
        ...
        """
        self._raise_no_fit_exception_maybe(ReferenceCodes._TST_GET_RW_FIT_NOT_CALLED)
        # We know that the pipeline is not None, because otherwise
        # _raise_no_fit_exception_maybe will throw the error.
        # mypy do not see this assertion.
        pipeline = cast(Pipeline, self.pipeline)
        rolling_window = forecasting_utilities.get_pipeline_step(pipeline, TimeSeriesInternal.ROLLING_WINDOW_OPERATOR)
...
def get_boolean_col_names(X: pd.DataFrame) -> List[str]:
    return [colname for colname in filter(lambda col: any(isinstance(val, bool) for val in X[col]), X.columns.values)]


def _get_date_columns(
    df: pd.DataFrame, drop_column_names: List[str], featurization_config: FeaturizationConfig
) -> List[str]:
    """
...
def _get_numerical_columns(
    ...
    numerical_columns = _get_columns_by_type(
        df, [np.number], FeatureType.Numeric, drop_column_names, featurization_config
    )
...
def _get_categorical_columns(
    ...
    categorical_columns = _get_columns_by_type(
        df, ["object"], FeatureType.Categorical, drop_column_names, featurization_config
    )
...
def _get_columns_by_type(
    ...
    columns = [
        x for x in df.select_dtypes(selected_types).columns
        if (x not in (drop_column_names or [])
            and x not in exclude_cols and x not in include_cols)]
...
def _get_included_columns(
    df: pd.DataFrame, include_purpose: str, featurization_config: FeaturizationConfig
) -> Set[str]:
    """
...
def _get_excluded_columns(
    df: pd.DataFrame, exclude_purpose: str, featurization_config: FeaturizationConfig
) -> Set[str]:
    """
...
def _get_target_imputer(
    ...
    :param tsds: A timeseries dataset that contains target column for imputation.
    """
    imputer = None
    if tsds is not None:
            ...
            if target_column_name in cols:
                ...
                if strategy == TransformerParams.Imputer.Ffill:
                    ...
                    imputer = TimeSeriesImputer(
                        input_column=[target_column_name], option="fillna", value=fill_value, freq=freq_offset
                    )
                    return imputer
    return None
...
def _get_default_target_imputer(
    ...
    """
    if grain_df is None:
        return TimeSeriesImputer(input_column=[target_column_name], option="fillna", method="ffill", freq=freq_offset)
    else:
...
def _get_numerical_imputer_value(
    col: str, default_value: Union[int, float], tsds: TimeSeriesDataSet, transformer_params: Dict[str, Any]
) -> Union[int, float]:
    """
...
def _has_valid_customized_imputer(featurization_config: FeaturizationConfig) -> bool:
    ...
    """
    return (
        featurization_config is not None
        and featurization_config.transformer_params is not None
        and featurization_config.transformer_params.get(SupportedTransformers.Imputer) is not None
    )
