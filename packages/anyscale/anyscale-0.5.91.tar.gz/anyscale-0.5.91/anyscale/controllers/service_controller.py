...
class ServiceController(BaseController):
    ...
    def _get_cloud_s3_bucket_from_cluster(
        self, decorated_cluster: DecoratedSession
    ) -> Optional[str]:
        """
        ...
        cloud: CloudWithCloudResource = self.api_client.get_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_get(
            cloud_id
        ).result
    ...
    def _override_configs_in_workspaces(self, service_config: ServiceConfig) -> None:
        ...
        if (
            ...
            if not is_dir_remote_uri(working_dir) and upload_path is None:
                ...
                new_runtime_env = self._upload_and_rewrite_workspace_working_dir(
    ...
    def _upload_and_rewrite_workspace_working_dir(
        self,
        *,
        ...
        """
        decorated_cluster = self.api_client.get_decorated_cluster_api_v2_decorated_sessions_cluster_id_get(
            cluster_id
        ).result
        cloud_id = decorated_cluster.cloud.id

        s3_bucket_name = self._get_cloud_s3_bucket_from_cluster(decorated_cluster)
    ...
    def _get_organization_id(self):
        user_info: UserInfo = (
            self._api_client.get_user_info_api_v2_userinfo_get().result
        )
