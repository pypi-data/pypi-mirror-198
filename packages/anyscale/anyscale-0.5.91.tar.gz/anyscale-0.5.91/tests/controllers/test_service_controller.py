...
def test_get_s3_bucket_gcp(mock_auth_api_client):
    ...
    service_controller.api_client.get_cloud_with_cloud_resource_api_v2_clouds_with_cloud_resource_router_cloud_id_get.return_value = DecoratedsessionResponse(
        result=Mock(provider=CloudProviders.GCP),
    )

    with pytest.raises(click.ClickException) as e:
        service_controller._get_cloud_s3_bucket_from_cluster(cluster)
    assert (
...
def test_get_s3_bucket_aws(
    mock_auth_api_client, aws_s3_id: Optional[str], expected_bucket_name: Optional[str]
):
    mock_logger = Mock()
    service_controller = ServiceController(log=mock_logger)
...
def test_upload_and_rewrite_workspace_working_dir(
    ...
    with patch.multiple(
        ...
        service_controller._upload_and_rewrite_workspace_working_dir(
...
@pytest.mark.parametrize("override_canary_percent", [None, 40])
...
def test_service_rollout(
    ...
    service_v2_config = ApplyProductionServiceV2APIModel(
        ...
        canary_percent=override_canary_percent if override_canary_percent else 100,
        rollout_strategy=override_rollout_strategy
