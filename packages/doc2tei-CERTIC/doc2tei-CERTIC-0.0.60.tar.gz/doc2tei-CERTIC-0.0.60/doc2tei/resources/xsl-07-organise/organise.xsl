...
<xsl:variable name="titlePageNodes" as="node()*">
    <xsl:sequence select="//text:p[starts-with(@text:style-name,'TEI_author')] | 
...
<xsl:variable name="frontNodes" as="node()*">
    ...
    <xsl:sequence select="//text:p[starts-with(@text:style-name,'TEI_acknowledgment')] | 
                          ...
                          //text:p[starts-with(@text:style-name,'TEI_epigraph')][following::*:div[@type='section1']] | 
                          //text:p[starts-with(@text:style-name,'TEI_bibl_reference')][preceding-sibling::*:p[1][starts-with(@text:style-name,'TEI_epigraph') and following::*:div[@type='section1']]]"/>
</xsl:variable>
