...
def get_sat_weather(project_name:str, start:str|DateLike, end:str|DateLike,
    ...
    keys, (start, end) = _generate_keys(ccr_id, start, end, 'satellite')
    if len(keys) < 50:
        df_weather = _s3_pool(pool_size=pool_size, keys=keys)
        df_weather = df_weather.reindex(columns=['ghi', 'dni', 'dhi', 'air_temp', 'wind_speed_10m'])
    else:
