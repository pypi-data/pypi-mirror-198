...
class DashboardSession:
    ...
    def __init__(self,
        ...
        if project_list:
            if isinstance(project_list, (str, list)):
    ...
    def get_project(self, project_name: str|Project, get_neighbors: bool=True) -> Project:
        """
    ...
    def add_project(self, project_name:str|Project|list, get_neighbors:bool = True) -> Project:
        ...
        """
        # Type validation
        if not isinstance(project_name, (str, list, Project)):
        ...
        if isinstance(project_name, list):
            print(f"A list was supplied to the `add_project` method. The method will return the first project in the list that was successfully added to the `DashboardSession`.")
            project_list = project_name
            project = self.add_projects(project_list)
    ...
    def add_projects(self, project_list:list|str|Project, get_neighbors:bool = True) -> Project:
        ...
        if isinstance(project_list, (str, Project)):
            project = self.add_project(project_list, get_neighbors=get_neighbors)
            return project
        elif isinstance(project_list, list):
            successful_projects = []
            for project_name in project_list:
                try:
                    project = self.add_project(project_name, get_neighbors=get_neighbors)
            ...
            # Return the first successful project from the list
            project = successful_projects[0]
            return project
        else:
            raise TypeError("`project_list` must be a string, Project object, or Python list! Please reformat input and try again.")
                

    def __initialize_project(self, project_name:str, get_neighbors:bool = True):
        ...
        try:
            ...
            proj_init_dict['error_info'] = e
            project = Project(proj_init_dict)

        return project
    

    def __get_neighbors(self, project_name:str):
        ...
        if neighbor_list:
            for neighbor in neighbor_list.copy():
                try:
                    self.add_project(neighbor, get_neighbors=False)
                except:
    ...
    def process_project(self, project_name:str|Project|list, reprocess:bool = False,
                        data_sub:bool = False, use_solcast:bool = True) -> Project:
        """
        ...
        Args:
            ...
            data_sub (bool): Option to use automatic data substitution algorithm.
            use_solcast (bool): Option to use Solcast data in data sub process. `data_sub`
        ...
        if isinstance(project_name, list):
            ...
            project_list = project_name
            self.process_projects(project_list=project_list, method='series',
                                  reprocess=reprocess, data_sub=data_sub, use_solcast=use_solcast)
            ...
            if reprocess:
                project.last_update_config = 0
                project.last_update_bool = 0
                project.last_update_powertrack = 0
            if any([int(project.last_update_config) != int(last_update_config_file),
                    int(project.last_update_bool) != int(last_update_bool_file),
                    int(project.last_update_powertrack) != int(last_update_powertrack_file)]):
            ...
            try:
                project._process_data(neighbor_sensor_data=neighbor_sensor_data,
                                      data_sub=data_sub, use_solcast=use_solcast)
            except Exception:
                print('\n')
                print('###########################')
    ...
    def process_projects(self, project_list:str|Project|list, method:str = 'series',
                         reprocess:bool = False, data_sub:bool = False, use_solcast:bool = True):
        ...
        if isinstance(project_list, (str, Project)):
            ...
            project = self.process_project(project_name, reprocess, data_sub=data_sub, use_solcast=use_solcast)
    ...
    def process_all_projects(self, method:str = 'parallel', reprocess:bool = True,
                             data_sub:bool = False, use_solcast:bool = True):
        """
        ...
        self.process_projects(project_list, method=method, reprocess=reprocess, data_sub=data_sub, use_solcast=use_solcast)
    

    def export_project(self, project_name:str|list, reprocess:bool = False, use_solcast:bool = True,
                       server:str = 'dev', save_pickle:bool = True, dest:str|list = 'all') -> None:
        """
        Export project to desired location(s).

        Args:
            project_name (str or Project): Name of a project to export.
            use_solcast (bool): Option to use Solcast data in data sub process. `server`
                must be set to `datasub` for this option to take effect.
            server (str): SQL table to export to. Options are `prod` or `dev`, or `datasub`.
        ...
        if isinstance(project_name, list):
            ...
            if dest != 'bool':
                ...
                if server == 'datasub':
                    data_sub = True
                self.process_project(project_name, reprocess=reprocess, data_sub=data_sub, use_solcast=use_solcast)
    ...
    def export_projects(self, project_list:list|str, reprocess:bool = True, use_solcast:bool = True,
                        server:str = 'dev', save_pickle:bool = True, dest:str|list = 'all', method:str = 'series') -> None:
    ...
    def draw_plots(self, project_name:str|Project|list, plot_order=[], reprocess:bool = False,
                        data_sub:bool = False, use_solcast:bool = True, *args, **kwargs):
        ...
        if isinstance(project_name, (str, Project)):
            ...
            project = self.process_project(project_name, reprocess=reprocess, data_sub=data_sub, use_solcast=use_solcast)
            neighbor_sensor_data = {self.project_list[neighbor].project_name: self.project_list[neighbor].df_sensors_native_avg for neighbor in project.neighbor_list}
            try:
                if project.errored:
            ...
            try:
                ...
                plotter = Plotter(project, neighbor_sensor_data)
                self.plotters[project_name] = plotter

            plotter._update_plotter_neighbors(neighbor_sensor_data)
            ...
            if not redraw:
                ...
                if plotter.plot_list == plot_order:
                    ...
                    plotter.draw_plots(plot_order=plot_order, *args, **kwargs)
            else:
                plotter.draw_plots(plot_order=plot_order, *args, **kwargs)
            ...
            for i, project_nm in enumerate(project_list):
                project = self.get_project(project_nm, get_neighbors=False)
