...
from autoai_ts_libs.lale import (  # type: ignore # noqa; StandardRowMeanCenterMTS,; WindowTransformerMTS; DifferenceFlattenAutoEnsembler,; FlattenAutoEnsembler,; LocalizedFlattenAutoEnsembler,
    AutoaiTSPipeline,
...
class TestAutoaiTSLibs(unittest.TestCase):
    ...
    def test_pipeline_MT2RF(self):
        trainable = MT2RForecaster(target_columns=[0, 1, 2, 3, 4, 5, 6], feature_columns=None, trend="Linear")
        self.doTestPipeline(
...
class TestMT2RForecaster(unittest.TestCase):
    ...
    def test_fit(self):
        ...
        model = MT2RForecaster(target_columns=test_class.target_columns, feature_columns=None)
    ...
    def test_predict_multicols(self):
        ...
        model = MT2RForecaster(
            target_columns=test_class.target_columns, feature_columns=None, prediction_win=2
        )
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict()
        self.assertEqual(len(ypred), 2)
        model = MT2RForecaster(
            target_columns=test_class.target_columns, feature_columns=None, prediction_win=1
        )
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict()
        self.assertEqual(len(ypred), 1)
        model = MT2RForecaster(target_columns=test_class.target_columns, feature_columns=None, trend="Mean")
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict()
        self.assertEqual(len(ypred), 12)
        model = MT2RForecaster(target_columns=test_class.target_columns, feature_columns=None, trend="Poly")
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict()
        self.assertEqual(len(ypred), 12)
    ...
    def test_predict_prob(self):
        ...
        test_class = self.__class__
        model = MT2RForecaster(target_columns=[0], feature_columns=None, prediction_win=2)
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict_proba()
        assert ypred is not None
        self.assertEqual(ypred.shape, (2, 1, 2))
        model = MT2RForecaster(target_columns=[0, 1], feature_columns=None, prediction_win=2)
        fitted_model = model.fit(test_class.X)
        ypred = fitted_model.predict_proba()
...
class TestPrettyPrint(unittest.TestCase):
    @unittest.skip("Does not work for the new xgboost")
...
class TestWatForeForecasters(unittest.TestCase):
    ...
    def test_watfore_pickle_write(self):
        ...
        from autoai_ts_libs.lale import WatForeForecaster
...
def get_srom_time_series_estimators(
    ...
    """
    srom_estimators = []
    from autoai_ts_libs.lale import EnsembleRegressor
...
objs = [
    # "autoai_ts_libs.lale.AutoaiTSPipeline",  # does not work as steps can be None or empty
    "autoai_ts_libs.lale.AutoaiWindowedWrappedRegressor",
    "autoai_ts_libs.lale.AutoaiWindowTransformedTargetRegressor",
    "autoai_ts_libs.lale.MT2RForecaster",
    "autoai_ts_libs.lale.SmallDataWindowTargetTransformer",
    "autoai_ts_libs.lale.SmallDataWindowTransformer",
    "autoai_ts_libs.lale.StandardRowMeanCenter",
    "autoai_ts_libs.lale.T2RForecaster",
    "autoai_ts_libs.lale.WindowStandardRowMeanCenterMTS",
    "autoai_ts_libs.lale.cubic",
    "autoai_ts_libs.lale.flatten_iterative",
    "autoai_ts_libs.lale.linear",
    "autoai_ts_libs.lale.fill",
    "autoai_ts_libs.lale.previous",
    "autoai_ts_libs.lale.next",
    "autoai_ts_libs.lale.AutoRegression",
    "autoai_ts_libs.lale.EnsembleRegressor",
    "autoai_ts_libs.lale.WatForeForecaster",
]
for obj in objs:
    setattr(
        TestSchemas,
