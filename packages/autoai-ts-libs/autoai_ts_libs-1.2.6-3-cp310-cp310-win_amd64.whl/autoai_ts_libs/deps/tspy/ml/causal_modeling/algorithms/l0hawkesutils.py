...
'''
L0 Hawkes algorithm utility routines
'''
...
def show_causal_triggers_of(n,qself,qhist,timestamps,event_types, 
    ...
    try:
        ...
        print('{}. {}:\n{}'.format(error, 'Install by running', 'pip install matplotlib'))

    try:
        import seaborn as sb; sb.set()
    except ImportError as error:
        print('{}. {}:\n{}'.format(error, 'Install by running', 'pip install seaborn'))
    ...
    if num_bars > Nh + 1:
        ...
        num_bars = Nh + 1
    if n <= 0 or n > N:
        raise TSError('n={} is invalid. Must be in [1,N].'.format(n))
    elif N != len(qself)-1 or N != qhist.shape[0] -1:
        raise TSError('qself or qhist size inconsistent with timestamps')
    ...
    if time_unit == '':
        ...
        try:
            ...
            raise TSError('{}: invalid time_unit'.format(time_unit))
...
def show_model(result,note='',show_diagonal=False,shorten_mode=0):
    ...
    if any(ge not in result for ge in ['event_list','learned_params']):
        raise TSError('event_list or learned_params not found')
    ...
    if any(ge not in learned_params for ge in ['mu','beta','loglik','A']):
        raise TSError('mu/beta/loglik/A not found')
