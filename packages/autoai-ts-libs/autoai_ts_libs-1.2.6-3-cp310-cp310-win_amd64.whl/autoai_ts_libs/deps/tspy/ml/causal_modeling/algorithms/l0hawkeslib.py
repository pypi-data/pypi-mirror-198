...
'''
L0 Hawkes algorithm implementation routines
'''
...
def verify_input(decayfunc, sparse_level, prior, nu_mu, nu_beta, nu_A,
    ...
    '''
    from autoai_ts_libs.deps.tspy.exceptions import TSError        
    L2strengths = [nu_mu,nu_beta,nu_A]
    Gamma_shapes = [a_mu,a_beta]
    Gamma_rates = [b_mu, b_beta]       
        
    if not decayfunc.lower().startswith(('exp','pow')):
        raise TSError('{}:decayfunc invalid'.format(decayfunc))
    elif sparse_level < 0.5 or sparse_level >=1:
       raise TSError('{}:sparse_level must be in [0.5,1)'.format(sparse_level))
    elif not prior.lower().startswith(('gauss','gamma','gg')):
        raise TSError('{}:prior invalid'.format(prior))
    elif any(ge <=0 for ge in L2strengths):
       raise TSError('L2 strengths must be positive')
    elif any(ge < 1 for ge in Gamma_shapes):
       raise TSError('Gamma prior shape must not be less than 1')
    elif any(ge <= 0 for ge in Gamma_rates):
       raise TSError('Gamma prior rates must be positive')
...
def verify_timeorder(timestamps,event_types,Nh):
    ...
    if len(event_types) != len(timestamps):
        raise TSError('Length differs:timestamp and event_types')
    elif Nh > len(event_types) -1:
        raise TSError('Nh is too large (cannot be greater than N)')
    elif Nh < 0:
        raise TSError('Nh cannot be negative)')
    elif not isinstance(timestamps,np.ndarray):
        raise TSError('timestamps must be numpy 1D array')
    elif not isinstance(event_types, np.ndarray) :
       raise TSError('event_types must be numpy 1D array')
    ...
    if ~isTimeOrdered:
        raise TSError('timestamps must be time-ordered.')
...
def history_of(n,Nh):
    ...
    if n >= Nh:
        ...
        raise TSError('{}:Invalid index'.format(n))
...
def decay(beta,u,func,eta,**kwargs):
    ...
    **kwargs : TYPE
    ...
    if func.lower().startswith('exp'):
        ...
        raise TSError('{}:undefined decay func type'.format(func))
