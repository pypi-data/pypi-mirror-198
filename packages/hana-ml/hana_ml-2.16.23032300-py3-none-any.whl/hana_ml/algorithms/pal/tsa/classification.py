    ...
    **kwargs : keyword arguments
    ...
    def fit(self,
        ...
        data : DataFrame
            ...
            and that number is designated by the parameter ``data_dim`` when initialize a TimeSeriesClassification instance.

        label : DataFrame, optional
            The label of time series.
            If classification_method = "LogisticRegression" and transform_method is "MiniRocket"/"MultiRocket", label is a mandatory parameter.
    ...
    def predict(self, data, key=None, thread_ratio=None):
        ...
        data : DataFrame
            ...
            For univariate time series, each row represents one time series, while for multivariate time series, a fixed number of consecutive rows forms one time series, and that number is designated by the parameter ``data_dim`` when transform_method = "rocket" is used.
