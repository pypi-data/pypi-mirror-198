...
#pylint: disable=consider-using-f-string, raising-bad-type, consider-using-enumerate
import logging
import operator
import sys
import math
...
def plot_time_series_outlier(data,
    ...
    if ax is None:
        ax = plt.axes()
    outliers = result.loc[result["IS_OUTLIER"] == 1, ["RAW_DATA"]]

    ax.plot(result["RAW_DATA"], color='blue')
    ax.scatter(outliers.index, outliers["RAW_DATA"], color='red')
...
def plot_change_points(data,
    ...
    if isinstance(cp_object, BCPD):
        ...
        ax.plot(data.select(key).collect(), data.select(col).collect(), color='blue', label="Original Time Series")
        if title is None:
            title = "Change Points"
        ax.set_title(title)
        ax.set_xlabel(key)
...
def plot_moving_average(data, col, rolling_window, key=None, ax=None, compare=True):
    """
    Plot the rolling mean by given rolling window size.
