...
__all__ = ["acf", "pacf", "ccf", "lb", "lm", "bglm"]
...
def acf(
    ...
    !!! example "Examples"
        ...
        >>> from ts_stat_tests.correlation import acf
        >>> data = macrodata.load_pandas()
        >>> x = data.data["realgdp"]
        >>> res_acf, res_confint, res_qstat, res_pvalues = acf(data, nlags=40, qstat=True, alpha=0.05, fft=False)
        >>> pprint(res_acf[1:11])
        ...
        >>> from pprint import pprint
        >>> from ts_stat_tests.utils.get_data import load_airline
        >>> from ts_stat_tests.correlation import acf
        >>> data = load_airline()
...
def pacf(
    ...
    !!! example "Examples"
        ...
        >>> from pprint import pprint
        >>> from ts_stat_tests.utils.get_data import load_airline
        >>> from ts_stat_tests.correlation import pacf
        >>> data = load_airline()
...
def ccf(
    ...
    !!! example "Examples"
        ...
        >>> from pprint import pprint
        >>> from ts_stat_tests.utils.get_data import load_airline
        >>> from ts_stat_tests.correlation import ccf
        >>> data = load_airline()
...
def lb(
    x: array_like,
    lags: Union[int, array_like, None] = None,
    ...
    ??? tip "See Also"
        ...
        - [`ts_stat_tests.correlation.alb`][src.ts_stat_tests.algorithms.correlation.lb]: Ljung-Box test of autocorrelation in residuals.
        - [`ts_stat_tests.correlation.alm`][src.ts_stat_tests.algorithms.correlation.lm]: Lagrange Multiplier tests for autocorrelation.
        - [`ts_stat_tests.correlation.abg`][src.ts_stat_tests.algorithms.correlation.bglm]: Breusch-Godfrey Lagrange Multiplier tests for residual autocorrelation.
    """
    ...
    *,
    ...
    Params:
        ...
        autolag (str, optional):
            If `None`, then a fixed number of lags given by `maxlag` is used. Searching for model specification cannot control test size.<br>
            Defaults to `None`.
            !!! deprecation "Deprecation Warning"
                This parameter is deprecated and will be removed after `statsmodels` version `0.12`.
        store (bool, optional):
    ...
    !!! example "Examples"
        ...
        >>> from pprint import pprint
        >>> from ts_stat_tests.utils.get_data import load_airline
        >>> from ts_stat_tests.correlation import alm
        >>> data = load_airline()
        >>> res_lm, res_lmpval, res_fval, res_fpval = alm(data)
        >>> pprint(res_lm)
        128.09655717844828
        >>> pprint(res_lmpval)
...
def bglm(
    ...
    !!! example "Examples"
        ...
        >>> from ts_stat_tests.correlation import abg
        >>> y = sm.datasets.longley.load_pandas().endog
        >>> X = sm.datasets.longley.load_pandas().exog
        >>> X = sm.add_constant(X)
        >>> res_lm, res_lmpval, res_fval, res_fpval = abg(sm.OLS(y, X).fit())
        >>> print(res_lm)
        5.1409448555268185
        >>> print(res_lmpval)
        0.16176265367835008
