...
from quantizeml.layers import (QuantizedConv2D, QuantizedReLU,
...
def _set_conv2d_variables(layer_ak, layer_k):
    ...
    if layer_k.use_bias:
        ...
        if target_version == AkidaVersion.v1:
            ...
            layer_ak.variables["bias_shift"] = bias_shift
...
def convert_conv_block(model_ak, layers):
    ...
    if relu_layer:
        ...
        out_quantizer = getattr(relu_layer, "out_quantizer", None)

        if get_akida_version() == AkidaVersion.v2:
            # Set the optional activation variables
            set_relu_variables(conv_ak, relu_layer)
        elif out_quantizer is None:
            # The QuantizedReLU should include an output_quantizer with Akida V1
            raise ValueError(f"{conv.name} must include an OutputQuantizer.")
    else:
        # otherwise, output quantizer is in the conv layer
        out_quantizer = getattr(conv, "out_quantizer", None)
