...
def parse_relu(layer_k):
    ...
    bitwidth = out_quantizer.bitwidth if out_quantizer else None
    if get_akida_version() == AkidaVersion.v2:
        ...
        if bitwidth > 4 or bitwidth < 1:
            raise ValueError(f"{lname}: an output bitwidth between 1 and 4 must be specified.")
