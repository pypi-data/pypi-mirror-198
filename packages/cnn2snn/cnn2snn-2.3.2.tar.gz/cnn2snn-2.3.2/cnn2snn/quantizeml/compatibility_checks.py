...
skippable_layers = (layers.InputLayer, layers.Reshape, layers.Flatten, layers.Rescaling,
                    qlayers.Dequantizer, layers.Activation, layers.Softmax)
...
def _check_block_integrity(block):
    ...
    def _islast(layer):
        ...
        if len(outbound) == 1 and isinstance(outbound[0].outbound_layer, skippable_layers):
            return _islast(outbound[0].outbound_layer)
...
def _check_skippable_compatibility(skippables):
    """Check rules over skippable layers (if need to be checked)

    Args:
        skippables (list(tf.keras.Layer)): layers list to check.
    """
    endpoint_layers = (layers.Flatten, layers.Reshape, layers.Activation, layers.Softmax,
                       qlayers.Dequantizer)

    def _find_next_non_skippable_outbound(layer):
        outbound = layer.outbound_nodes
        if len(outbound) == 1 and isinstance(outbound[0].outbound_layer, skippable_layers):
            return _find_next_non_skippable_outbound(outbound[0].outbound_layer)
        return outbound
    ...
    for layer in skippables:
        ...
        outbound = _find_next_non_skippable_outbound(layer)
        if len(outbound) == 0:
            continue
            ...
            if len(outbound) > 1 or not isinstance(outbound_layer, qlayers.QuantizedDense):
                raise RuntimeError("A QuantizedFlatten layer is only supported at the end of "
                                   ...
                                   f"Layer: {layer.name}")
        elif isinstance(layer, endpoint_layers):
            raise RuntimeError(f"{layer.name} must be placed at the end of the model.")
