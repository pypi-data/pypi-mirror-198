...
from work_components.consts import DIRECTORY, DIRECTORY_DEBUG, FLAG_FILE, RC_FILE_PATH
...
class Work:
    ...
    def get_single_time(self, args) -> dt.datetime:
        ...
        allowed_names = [START_NAME, STOP_NAME, SWITCH_NAME]
        # Also add aliases for the modes, as they are not replaced on the command line.
        for mode_name in allowed_names.copy():  # List is modified in the loop => copy.
            allowed_names.extend(self.configuration.aliases[mode_name])

        if args.mode not in allowed_names:
    ...
    def _recess_add_vacation(self, vacation_period: List[str]) -> Tuple[Callable, str]:
        ...
        def action():
            self.recess_dao.add_vacation(first_day=first, last_day=last)
    ...
    def _see(self, s_target) -> None:
        ...
        if s_target == "dir":
            print(
                ...
                f"Configuration file:  {RC_FILE_PATH}\n"
