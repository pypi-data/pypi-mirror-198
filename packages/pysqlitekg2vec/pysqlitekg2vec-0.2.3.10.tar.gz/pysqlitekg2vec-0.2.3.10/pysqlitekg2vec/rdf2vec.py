...
from pysqlitekg2vec.embedders import Embedder, Word2Vec
from pysqlitekg2vec.graphs import KG
from pysqlitekg2vec.typings import Embeddings, Entities, Literals, SWalk
from pysqlitekg2vec.walkers import RandomWalker, Walker
from pysqlitekg2vec.walkers.vault import CorpusVault, CorpusVaultFactory, \
    InMemoryCorpusVaultFactory
...
class RDF2VecTransformer:
    ...
    Attributes:
        ...
        vault_factory: The vault factory to create vaults for storing generated
            walks.
            Defaults to [InMemoryCorpusVaultFactory()].
    ...
    vault_factory = attr.ib(
        factory=lambda: InMemoryCorpusVaultFactory(),
        type=CorpusVault,
        validator=attr.validators.instance_of(CorpusVaultFactory),
    )
    ...
    def fit(
        ...
        vault = self.get_walks(kg, entities)
        try:
            tic = time.perf_counter()
            self.embedder.fit(vault, is_update)
    ...
    def get_walks(self, kg: KG, entities: Entities) -> CorpusVault:
        ...
        vault = self.vault_factory.create()
        tic = time.perf_counter()
        with vault.walk_importer() as importer:
            for walker_id, walker in enumerate(self.walkers):
                importer.set_walker_id(walker_id)
                walker.extract(kg, importer, entities, self.verbose)
        toc = time.perf_counter()
