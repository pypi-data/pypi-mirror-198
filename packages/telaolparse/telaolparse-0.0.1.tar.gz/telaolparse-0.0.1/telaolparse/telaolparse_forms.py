...
def handler(event, context):
    ...
    if ed_id_list==[]:
        ...
        new_goals['created_at'] = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        new_obj['created_at'] = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        new_traits['created_at'] = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

        if len(archive_ids)>0:
            archiveRows(archive_ids, 'sat_criterion', 'criterion_id')
    ...
    def updateTable(table, raw_df, columns_dict={}, common_id = False, amount = 1000, nohub=False, ready=False, cursor=cursor, conn=conn):
        if not ready:
    ...
    def updateLink(table, raw_df, columns_dict, f_index_name, s_index_name, add_data=[], f_common_id=False, s_common_id=False, f_amount=1000, s_amount=1000, cursor=cursor, conn=conn):
        print(raw_df)
        df = pd.DataFrame(raw_df, columns=list(columns_dict.keys()), dtype=None, copy=None)
        df = df.rename(columns=columns_dict)
        index_name=table+'_id'
        ...
        if archive_id_list != []:
            ...
            archive_ids = ','.join(str(e) for e in archive_id_list)
            if len(archive_ids)>0:
                archiveRows(archive_ids, f'sat_{table}', index_name)
        print(new_id_list)
        if new_id_list != []:
        ...
        if add_data == []:
            ...
            print('comparison')
            print(comparison)
            updateTable(table, comparison, nohub=True, ready=True, common_id = True)

    updateTable('artifact', artifact, {"ArtifactID": "artifact_id", "Наименование замера": "artifact_name"})
    updateLink('course_artifact', artifact, {"ArtifactID": "artifact_id", "CourseID": "course_id"}, 'artifact_id', 'course_id')
    updateLink('artifact_criterion', lgo, {"ArtifactID": "artifact_id", "TraitID": "criterion_id"}, 'artifact_id', 'criterion_id')

    updateTable('student', student, {"student_id": "student_id"}, common_id=True)
    updateLink('student_major', student, {"student_id": "student_id", "major_id": "major_id"}, 'student_id', 'major_id', f_common_id=True, s_common_id=True)

    updateTable('major', major_df, {'major_id': 'major_id', 'degree': 'degree', 'program_name': 'program_name', 'major_name': 'major_name'}, common_id=True)
    
    updateTable('course', courses_df, {'Semester': 'semester', 'Course name': 'course_name', 'CourseID': 'course_id', 'Course status (core/elective)': 'is_elective', 'Elective pack code (troughout)': 'elective_pack_code'})
    updateLink('course_criterion', courses_obj, {"CourseID": "course_id", "level_1": "criterion_id", 0: 'value'}, 'course_id', 'criterion_id', add_data=['value'])

    courses_obj = courses_obj.drop(columns=["level_1", 0])
    courses_obj['major_id'] = major_id

    updateLink('major_course', courses_obj, {"CourseID": "course_id", "major_id": "major_id"}, 'course_id', 'major_id', s_common_id = True)
