...
def multi_scores(y_true: int, y_pred: float, threshold=0.5, show=True, show_index=True, abbr=False):
    ...
    TNR = np.round(TN/(TN+FP+1e-6), 5)                                     # specificity
    ...
    if show:
        if not abbr and show_index:
            print(
                "TP\tTN\tFP\tFN\tprecision\trecall\tspecificity\taccuracy\tmcc\tf1-score\tAUROC\tAUPRC")
...
if __name__ == "__main__":
    test = multi_scores(
        ...
        [0.001, 0.2, 0.4, 0.6, 0.8, 0.999, 0.001, 0.2, 0.4, 0.6, 0.8, 0.999], show=True,abbr=True)
