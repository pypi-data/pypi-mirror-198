...
def main(ip_str, target_name, regression, simulator, dry_mode):
    ...
    tests_yml_path = write_tests_yml(ip, target_name, test_suite, regression, tests, regression.suite.timestamp, dry_mode)
    ...
    if not dry_mode:
        regr_results = results.main(f"{ip.vendor}/{ip.name}", "", True, test_suite.filename, regression.name, test_suite.timestamp)
        cov_report_path = cov.gen_cov_report(f"{ip.vendor}/{ip.name}", True, test_suite.filename, regression.name, test_suite.timestamp)
...
def write_tests_yml(ip, target_name, test_suite, regression, tests, timestamp_start, dry_mode):
    ip_str = f"{ip.vendor}/{ip.name}"
    ip_dir_name = f"{ip.vendor}__{ip.name}"
    results_path = cfg.regr_results_dir + "/" + ip.name + "__" + regression.name + "__" + timestamp_start
    ...
    #tests_yml['waves'          ] = regression.set
    tests_yml['tests'] = {}
    for test in tests:
        cur_test = {}
        cur_test['id'        ] = test.id
...
def print_end_of_regression_msg(ip, results, cov_report_path, test_suite, regression, sim_job_list, timestamp_start, timestamp_end):
    ...
    results_path = cfg.regr_results_dir + "/" + ip.name + "__" + regression_name + "__" + test_suite.timestamp
