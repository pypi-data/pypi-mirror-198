...
class Base_Logging():
    ...
    def __init__(self, log_file_name=None, level="DEBUG",
                 format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {module} | {message}", product=""):
        "Constructor for the logging class"
        self.log_file_name = log_file_name
        self.log_file_dir = os.path.abspath(
            os.path.join(os.curdir, 'log', product))  # log file dir base on Base_Loggin.py
        self.level = level
        self.format = format
    ...
    def set_log(self, log_file_name, level, format, test_module_name=None):
        ...
        if log_file_name is None:
            log_file_name = self.log_file_dir + os.sep + test_module_name + '.loog'
        else:
            log_file_name = self.log_file_dir + os.sep + log_file_name
        logger.add(log_file_name, level=level, format=format,
                   rotation="30 days", filter=None, colorize=None, serialize=False, backtrace=True, enqueue=False,
                   catch=True)
