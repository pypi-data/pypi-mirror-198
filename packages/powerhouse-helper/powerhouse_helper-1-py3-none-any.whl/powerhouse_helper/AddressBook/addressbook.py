...
from AddressBook.addressbook_data import *

logger = logging.getLogger("ab_log")
logger.setLevel(logging.INFO)
formatter = logging.Formatter("[%(asctime)s]   %(message)s")
fh = logging.FileHandler("powerhouse_helper\\AddressBook\\address_saves\\address_book.log")
...
class AddressBook:
    ...
    def save(self):
        with open('powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin', "wb") as fd:
            pickle.dump(self.data, fd)
            logger.info(f"Address Book has been saved")
            return f"Address Book has been saved"

    def load(self):
        with open('powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin', "rb") as fd:
            if not os.stat('powerhouse_helper\\AddressBook\\address_saves\\address_book_auto_save.bin').st_size == 0:
