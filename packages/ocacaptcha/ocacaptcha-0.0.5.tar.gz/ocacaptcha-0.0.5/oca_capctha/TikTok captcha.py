...
if len(capctha_circle) > 0:
    for i in range(1, 30):
        ...
        if len(capctha_circle) > 0:
            ...
            print(full_screenshot_img_url_base64)
            ...
            print(small_screenshot_img_url_base64)
            time.sleep(100) 
            action_type = 'tiktokcircle'
            user_api_key = 'YOUR_API_KEY' #paste your api key in the field 

            multipart_form_data = {
                'FULL_IMG_MD5': (None, full_img_md5),
                'SMALL_IMG_MD5': (None, small_img_md5),
...
if len(capctha_3D) > 0:
    for i in range(1, 30):
        ...
        if len(capctha_3D) > 0:
            ...
            #start request solving capthca
            full_img_md5 = driver.find_element(By.XPATH, '//div[contains(@class,"captcha_verify_img")]/img').get_attribute("src") #get link full img
