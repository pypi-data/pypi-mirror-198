...
def check_if_cursor_is_in_bbox(img, line_thickness, alpha=1, beta=0.5, gamma=0, set_cls_trackbar=True):
    global IS_BBOX_SELECTED, SELECTED_BBOX, IMG_OBJECTS  

    # smallest bbox flag
    smallest_area = -1

    # at lease in one bbox
    at_least_in_1_box = False
    ...
    for idx, obj in enumerate(IMG_OBJECTS):
        ...
        if is_point_in_rect(MOUSE_X, MOUSE_Y, x1, y1, x2, y2):
            ...
            if tmp_area < smallest_area or smallest_area == -1:
                smallest_area = tmp_area
                SELECTED_BBOX = idx
                at_least_in_1_box = True    # at lease in one box
                IS_BBOX_SELECTED = True     # set bbox selected
            ...
            # highlight that bbox
            img = highlight_selected_bbox(img, line_thickness, alpha, beta, gamma)


    # not in any one of the bbox
    if at_least_in_1_box == False:
...
def mouse_listener(event, x, y, flags, param):
    ...
    global TRACKBAR_CLASS, WINDOW_NAME, LINE_THICKNESS  # , tmp_img


    # mark mode
    if MARK_MODE:
        # move:  tracking MOUSE_X  mouse_y
...
def inspect2( img_dir, 
    ...
    # cv2.setMouseCallback(WINDOW_NAME, mouse_listener, param={'tmp_img': tmp_img})
    ...
    while True:
        ...
        if MARK_MODE:
            ...
            # 03.15
            # when cursor move into bbox area, high-light that box
            # set_selected_bbox(set_cls_trackbar=True)
            # rich.print(f"IS_BBOX_SELECTED: {IS_BBOX_SELECTED}")
            # rich.print(f"SELECTED_BBOX: {SELECTED_BBOX}")
            # rich.print(f"IMG_OBJECTS: {IMG_OBJECTS}")
            ...
            # when cursor move out, de-seleted that bbox
            tmp_img = check_if_cursor_is_in_bbox(tmp_img, LINE_THICKNESS, set_cls_trackbar=True)
        ...
        # # ---------------------------------------
        # # e/E  =>  do examining bbox
        # # ---------------------------------------
        # elif pressed_key in (ord('e'), ord('E')):

        #     for bbox in tqdm(IMG_OBJECTS):
        #         print(bbox)
        #         # TODO: check if two box is highly occluded!
        #         # TODO: check if box is small enough or lay at the corner!  Dist < thresh!


        # # ---------------------------------------
        # # p/P  =>  image pre-processing
        # # ---------------------------------------
        # elif pressed_key in (ord('p'), ord('P')):
        #     # pass
