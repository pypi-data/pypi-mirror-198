...
from airvue_sg import label
from airvue_sg.attachment import Attachment
from airvue_sg.label import Label
from airvue_sg.message import Message
from airvue_sg.boto import get_file_contents_from_s3, put_json_object_in_s3
...
class Gmail(object):
    ...
    def __init__(
        ...
        try:
            ...
            json_token = get_file_contents_from_s3(s3_client, bucket_name, file_key)

            creds = client.Credentials.new_from_json(json_token)
            if creds.access_token_expired:
                creds.refresh(Http())
                if first_time:
                    put_json_object_in_s3(s3_client, bucket_name, file_key, creds.to_json())
    ...
    def _get_messages_from_refs(
        ...
        def thread_download_batch(thread_num):
            gmail = Gmail(_creds=self.creds, first_time=False)
