...
def __standardize_account(
    ...
    year = None
    if "doc_date" in df.columns and "year" in account_data.columns:
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                year = pd.to_datetime(df["doc_date"]).dt.year
        except Exception:
            pass
...
def __standardize_service(df, log_file, disable_service=False,
    ...
    # Test if year can be fetched based on the data
    year = None
    if "doc_date" in df.columns and "year" in service_data.columns:
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                year = pd.to_datetime(df["doc_date"]).dt.year
