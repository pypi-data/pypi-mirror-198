...
def clean_data(df, log_file=False, **args):
    ...
    # Check country data
    df = __standardize_country(df, log_file=log_file, **args)
    return df
...
def __standardize_country(df, log_file, disable_country=False,
...
def __standardize_service(df, log_file, disable_service=False,
        ...
        if any(res):
            ...
            if log_file:
                msg = ("The following organization data contains duplicated " +
                       "(e.g., equal business ID but different name) or " +
                       "empty values or business IDs are incorrect. Please " +
...
def __check_empty_observations(df, log_file, thresh=0, **args):
    ...
    if any(empty_values > 0):
        ...
        if log_file:
            msg = ("It seems that the data includes rows with up to " +
                   str(max_num) + " empty values. Please check them for " +
                   "errors.")
            utils.__log_to_file(
                df.loc[empty_values > 0, :], log_file, "vat_number.csv", msg)
        else:
            warnings.warn(
