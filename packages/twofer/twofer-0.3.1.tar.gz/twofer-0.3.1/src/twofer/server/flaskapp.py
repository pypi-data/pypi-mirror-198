...
def user_data_from_access_token_response(atr: AccessTokenResponse, /) -> UserInfo:
...
def ensure_unexpired_client(user_token: str, /) -> IClient:
    ...
    sec = timegm(gmtime())
    if expires_at <= sec:
        token_dict = authentication.refresh_token(refresh_token)
        user_info = user_data_from_access_token_response(token_dict)
        access_token: str = user_info.access_token
        client = Client(access_token)
...
def callback() -> Any:
    ...
    user_data = user_data_from_access_token_response(atr)
