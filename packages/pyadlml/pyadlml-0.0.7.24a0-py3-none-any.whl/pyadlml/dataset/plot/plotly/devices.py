...
def fraction(df_dev, height=350, order='alphabetical') -> Figure:
    ...
    dev_frontier = pd.Series(index=df[DEVICE].unique(), data=0)
    for cat in cats:
        mask_df_devs = df[VALUE] == cat
        devs = df.loc[mask_df_devs, 'device']
        base = dev_frontier[devs].copy()
        dev_frontier += df.loc[mask_df_devs].set_index(DEVICE)['frac']
    ...
    fig = px.bar(
        df, y=DEVICE, x='frac', orientation='h', color=VALUE,
