...
class GoddessService(BaseGoddessService):
    ...
    # send command text to 指定的 users
    async def send_command_text(self, ws, channel_id, to_user_ids, text, clear=False):
        await self._send_command_down(
            ws, 
            self.codes.COMMAND_DOWN_DISPLAY_TEXT,
            channel_id = channel_id, 
    ...
    # send message text to 指定的 users
    async def send_message_text(self, ws, channel_id, from_user_id, to_user_ids, text):
        await self._send_message_down(
            ws, 
            self.codes.MESSAGE_DOWN_TEXT,
            channel_id = channel_id, 
            from_user_id = from_user_id, 
            to_user_ids = to_user_ids, 
            origin = 'Goddess service', 
            temp_msg_id = uuid.uuid4().hex,
            msg_body = text
        )
