...
import socialization.ccs.goddess_service as gs
...
class Drawguess:
    ...
    def __init__(self, service):
        ...
        self.drawguess_finished = []   # 猜对了的玩儿家
        ...
        self.service.set_message_handle(self.service.codes.MESSAGE_UP_TEXT, self.handle_message_up_text)
    ...
    async def handle_notice_user_joined(self, service, data, ws, path):
        await gs.handle_notice_user_joined(service, data, ws, path)
        if self.drawguess_status == self.STATUS_GUESSING:
            await service.reply_command_image(data, ws, self.drawguess_image)
            await service.reply_command_text(data, ws, '来来来，我们在玩儿你画我猜，你也来猜！')
    
    async def handle_message_up_text(self, service, data, ws, path):
        answer = data['extra']['msg_body'].strip()
        user = data['extra']['from_user_id']
        channel_id = data['extra']['channel_id']

        if self.drawguess_status == self.STATUS_GUESSING:
            if answer == self.drawguess_answer:
                # 如果回答是对的
                # 看看是谁
                if user == self.drawguess_drawer:
                    # 如果是出题人
                    await service.send_command_text(ws, channel_id, [user], '田人吧，你怎么说答案？\n')
                else:
                    # 如果是其他人
                    if user in self.drawguess_finished:
                        # 这孩子已经猜对了
                        await service.send_command_text(ws, channel_id, [user], '你之前已经猜对了，没必要重复！\n')
                    else:
                        self.drawguess_finished.append(user)
                        # 对猜对回答者
                        await service.send_command_text(ws, channel_id, [user], f'你猜对了！答案就是“{self.drawguess_answer}”！你是第{len(self.drawguess_finished)+1}个聪明逼！\n')
                        # 对出题人
                        # TO DO: 这个接口需要封装
                        await service._send_command_down(
                            ws, 
                            ...
                            channel_id = channel_id, 
                            to_user_ids = [self.drawguess_drawer], 
                            args = {'text': f'{user}猜对了你召唤的画儿！已有{len(self.drawguess_finished)+1}个聪明逼猜对！\n', 'clear': False}
                        )
                        # 对其他回答者
                        await service._send_command_down(
                            ws, 
                            ...
                            channel_id = channel_id, 
                            to_user_ids = list(set(service.agent.get_channel_user_list(channel_id)) - {self.drawguess_drawer, user}), 
                            args = {'text': f'{user}已经猜对了！答案是{self.drawguess_answer}！已有{len(self.drawguess_finished)+1}个聪明逼猜对！\n', 'clear': False}
                        )
                # 不论如何，只要回答对了，就得把文本变成**
                # 获取群里所有人
                channel_id = data['extra']['channel_id']
                user_list = service.agent.get_channel_user_list(channel_id)
                # 排除自己
                to_user_list = list(set(user_list) - {user})
                # 发送命令
                await service.send_message_text(
                    ws,
                    channel_id = channel_id,
                    from_user_id = user,
                    to_user_ids = to_user_list,
                    text = '**'
                )
            else:
                # 获取群里所有人
...
service = GoddessService(
    port = 10005,
    uri = "wss://frog.4fun.chat/foreskin_goddess",
    token = "c6041ecb5e7269a1c4ce403d4fd7b64d44825a36bb52fe18b5c1656638f5636e",
    dbfile = 'drawguess2.json',
    name = 'Yellow 502 Goddess Drawguess Service'
)

drawguess = Drawguess(service)

asyncio.get_event_loop_policy().get_event_loop().run_until_complete(service.get_server_coroutine())
print('----------CH8889 Service Starts------------')
