...
def Factor_Calculator(pricebyday,minbar,conn,todaydate,notst,factors_to_cal):
    ...
    if 'qua20m' in factors_to_cal or factors_to_cal == 'all':
        ...
        qua20m = prv_factor.append(q).unstack().rolling(20).mean().iloc[-1:].stack().rename(columns = {'qua':'qua20m'})
    ...
    if 'skew20m' in factors_to_cal or factors_to_cal == 'all':
        ...
        skew20m = prv_factor.append(sk).unstack().rolling(20).mean().iloc[-1:].stack().rename(columns = {'skew':'skew20m'})
    ...
    if 's_reverse_10_sum' in factors_to_cal or factors_to_cal == 'all':
        ...
        s_reverse_10_sum = prv_factor.append(sr).unstack().rolling(10).sum().iloc[-1:].stack().rename(columns = {'s_reverse':'s_reverse_10_sum'})
    ...
    if 'ideal_reverse' in factors_to_cal or factors_to_cal == 'all':
        ...
        daily_sta_cal = prv_factor.append(dsta90)
