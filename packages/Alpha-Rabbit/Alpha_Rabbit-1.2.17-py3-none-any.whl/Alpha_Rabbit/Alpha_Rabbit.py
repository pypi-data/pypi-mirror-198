...
class single_signal_test(object):
    ...
    def one_factor_return(self,df,factorname,ndays,return_col): # 计算分组收益
        qreturn = df.groupby(level = 'date', group_keys = False).apply(lambda x: x.groupby(factorname+'_quantile')[[return_col]].mean()/ndays).unstack()
...
class Barra_factor_ana(object):
    ...
    def rise_barra_factors(self,rank_normalize:bool):
        ...
        # leverage
        print('rise leverage')
        self.leverage = self.filedict['MLEV']['du_equity_multiplier_ttm']*0.38+self.filedict['MLEV']['debt_to_asset_ratio_ttm']*0.35+self.filedict['MLEV']['book_leverage_ttm']*0.27
