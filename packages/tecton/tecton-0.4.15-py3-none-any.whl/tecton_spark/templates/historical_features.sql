...
{%- macro join_window(name, spine, spine_timestamp_key, feature_timestamp_key, feature_columns, join_keys, time_aggregation, last_join) %}
    ...
    {%- for feature in time_aggregation.features %}
        {{ aggregate_cte_name(name, spine, time_aggregation.features, loop.index0) }} AS (
            ...
            GROUP BY {{ all_spine_join_keys }}, {{ name|upper }}{{ separator }}{{ feature.output_feature_name|upper }}
...
SELECT
    _TT_SPINE_TABLE.*{%- if feature_set_items|length > 0 %},{%- endif -%}
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                {# Use the closest recent tile start time as the feature timestamp for aggregation features #}
                DATEADD('SECOND', -MOD(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }}), DATE_TRUNC('SECOND', _TT_SPINE_TABLE.{{ spine_timestamp_key }})) AS {{ final_select_name(item.name + separator + item.timestamp_key, false, true) }}
            {%- else %}
    ...
    {%- for item in feature_set_items -%}
        ...
        {%- if item.aggregation -%}
            {%- for feature in item.aggregation.features %}
                {{- final_select_name(item.name + separator + feature.output_feature_name, feature.function|snowflake_function == "count", not (loop.last and last_item)) | indent }}
            {%- endfor -%}
        {%- else %}
