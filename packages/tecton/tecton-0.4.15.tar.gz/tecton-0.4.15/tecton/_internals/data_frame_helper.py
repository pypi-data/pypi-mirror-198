...
class _FeatureSetQueryBuilder(object):
    ...
    def build(self) -> DataFrame:
        ...
        feature_df = _get_feature_dataframe_with_limits(
            ...
            data_skew_reduction=self.data_skew_reduction,
        ).to_spark()
        ...
        if spine_df is not None:
            if self.fd.is_temporal_aggregate:
                ...
                if self.join_type == "left":
                    time_aggregation = self.fd.trailing_time_window_aggregation
                    if self.data_skew_reduction:
                        # Adds the most recently scheduled anchor column as TEMPORAL_JOIN_COLUMN, accounts for data delay
                        # and batch schedule.
                        spine_df = add_retrieval_anchor_time(
                            input_df=spine_df,
                            timestamp_field=time_aggregation.time_key,
                            anchor_time_name=TEMPORAL_JOIN_COLUMN,
                            batch_schedule=self.fd.get_batch_schedule_for_version,
                            tile_interval=self.fd.get_tile_interval_for_version,
                            data_delay_seconds=self.fd.online_store_data_delay_seconds,
                            is_stream=self.fd.is_stream,
                            feature_store_format_version=self.fd.get_feature_store_format_version,
                        )
                        feature_endtime = convert_epoch_to_timestamp_column(
                            functions.col(TEMPORAL_JOIN_COLUMN), self.fd.get_feature_store_format_version
                        )
                    else:
...
def _sort_key_for_features(fs_config: "tecton_spark.feature_set_config.FeatureSetConfig"):  # type: ignore
...
def get_features_for_spine(
    ...
    for fd_and_config in sorted(
        ...
        df = _FeatureSetQueryBuilder(
            ...
            data_skew_reduction=conf.get_bool("DATA_SKEW_REDUCTION"),
        ).build()
...
def get_features_for_preview(
    spark,
    feature_set_config: "tecton_spark.feature_set_config.FeatureSetConfig",
