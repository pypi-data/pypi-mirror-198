...
from tecton_spark.query_utils import add_duration
from tecton_spark.query_utils import add_effective_timestamp
from tecton_spark.query_utils import add_retrieval_anchor_time
from tecton_spark.query_utils import asof_join
from tecton_spark.query_utils import EFFECTIVE_TIMESTAMP
from tecton_spark.query_utils import EXPIRATION_TIMESTAMP
from tecton_spark.query_utils import filter_expired_features
from tecton_spark.query_utils import TIMESTAMP_PLUS_TTL
...
def construct_full_tafv_df(
    ...
    data_skew_reduction: bool = False,
):
    ...
    output_df = construct_full_tafv_df_with_anchor_time(
        ...
        data_skew_reduction=data_skew_reduction,
    )
...
def construct_full_tafv_df_with_anchor_time(
    ...
    if not spine_df:
        ...
        materialization_params = MaterializationParams.from_feature_definition(fd)
        if data_skew_reduction:
            spine_df = add_retrieval_anchor_time(
                input_df=spine_df,
                timestamp_field=time_aggregation.time_key,
                anchor_time_name=TEMPORAL_ANCHOR_COLUMN_NAME,
                batch_schedule=fd.get_batch_schedule_for_version,
                tile_interval=fd.get_tile_interval_for_version,
            ...
            spine_df = spine_df.withColumn(TEMPORAL_ANCHOR_COLUMN_NAME, anchor_time)
...
def construct_spine_with_all_join_keys_for_TFV(
    ...
    # When data_skew_reduction = True, effective expiry is used for filtering expired features
    if data_skew_reduction:
        join_condition = join_key_conditions + [tile_df[EFFECTIVE_TIMESTAMP] <= spine_df[timestamp_column]]
        join_condition.append(spine_df[timestamp_column] < tile_df[EXPIRATION_TIMESTAMP])
    else:
        join_condition = join_key_conditions + [tile_df[timestamp_column] <= spine_df[timestamp_column]]
...
def construct_full_tfv_or_ft_df(
    ...
    if wildcard_key_not_in_spine and wildcard_join_key:
        if data_skew_reduction:
            tiles_df = _add_effective_timestamp_and_effective_expiry(fd, tiles_df, timestamp_key)
        ...
        spine_with_all_join_keys = construct_spine_with_all_join_keys_for_TFV(
            spine_df, tiles_df, timestamp_column, serving_ttl, spine_join_keys, fd.join_keys, data_skew_reduction
        )
    else:
        spine_with_all_join_keys = spine_df.select(all_join_keys).drop_duplicates()

    if data_skew_reduction:
        return _construct_full_tfv_or_ft_df_data_skew_reduction(
            fd, spine_with_all_join_keys, all_tiles_df, timestamp_key, timestamp_key
        )
...
def _add_effective_timestamp_and_effective_expiry(fd, features_df, features_timestamp_field):
    batch_schedule_seconds = int(fd.fv.materialization_params.schedule_interval.ToTimedelta().total_seconds())
    data_delay_seconds = fd.online_store_data_delay_seconds
    features_df = add_effective_timestamp(
        features_df,
    ...
    features_df = add_duration(
        input_df=features_df,
        new_column_name=TIMESTAMP_PLUS_TTL,
        timestamp_field=features_timestamp_field,
        duration=fd.serving_ttl,
    )
    features_df = add_effective_timestamp(
        features_df,
...
def _construct_full_tfv_or_ft_df_data_skew_reduction(
    fd, spine_df, features_df, spine_timestamp_field, features_timestamp_field
):
    features_df = _add_effective_timestamp_and_effective_expiry(fd, features_df, features_timestamp_field)
    rightside_join_prefix = "_tecton_right"
    joined_df = asof_join(
        spine_df=spine_df,
        features_df=features_df,
        join_cols=fd.join_keys,
        spine_timestamp_field=spine_timestamp_field,
        effective_timestamp_field=EFFECTIVE_TIMESTAMP,
        feature_timestamp_field=fd.timestamp_key,
        right_join_prefix=rightside_join_prefix,
    )
    join_prefixed_feature_names = [f"{rightside_join_prefix}_{f}" for f in fd.features]
    joined_df = filter_expired_features(
        joined_df, join_prefixed_feature_names, fd.timestamp_key, f"{rightside_join_prefix}_{EXPIRATION_TIMESTAMP}"
    )
    for f in fd.features:
        joined_df = joined_df.withColumnRenamed(f"{rightside_join_prefix}_{f}", f)

    cols_to_drop = []
    cols_to_drop.append(f"{rightside_join_prefix}_{fd.timestamp_key}")
    cols_to_drop.append(f"{rightside_join_prefix}_{TEMPORAL_ANCHOR_COLUMN_NAME}")
    ...
    cols_to_drop.append(f"{rightside_join_prefix}_{EXPIRATION_TIMESTAMP}")
