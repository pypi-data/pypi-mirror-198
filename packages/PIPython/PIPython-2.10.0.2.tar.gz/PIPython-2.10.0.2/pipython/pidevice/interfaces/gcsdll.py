...
__signature__ = 0x5bdbf03cef3f70c56d0d11225b73e72d
...
PI_DLL_CODEPAGE = PI_CONTROLLER_CODEPAGE
...
def read_registry_gcstranslator(gcsdir):
    ...
    """
    # Import outside toplevel (winreg) pylint: disable=C0415
    ...
    try:
        ...
        PIWarning('no GCSTranslator path in Windows registry (HKLM\\%s)', regkey)
    else:
        PIDebug('read HKLM\\%s: %r', regkey, gcsdir)
    finally:
...
class GCSDll(PIGateway):
    ...
    def __init__(self, devname='', dllname=''):
        ...
        """
        PIDebug('create an instance of GCSDll(devname=%r, dllname=%r)', devname, dllname)
    ...
    def settimeout(self, value):
        ...
        try:
            ...
            PIDebug('GCSDLL.settimeout: cannot set timeout in GCS DLL')
    ...
    def warning(self, msg):
        ...
        self._warnmsg = msg
        if self._warnmsg:
            PIDebug('GCSDll.warning = %s', self._warnmsg)
    ...
    def send(self, msg):
        ...
        """
        PIDebug('GCSDll.send(id%d): %r', self._id, msg)
        msg = ctypes.c_char_p(str(msg).encode(PI_DLL_CODEPAGE))
    ...
    def _getanswer(self, bufsize):
        ...
        """
        bufstr = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
    ...
    def read(self):
        ...
        if self._answersize:
            ...
            PIDebug('GCSDll.read: %r', received)
            return received.decode(encoding=PI_DLL_CODEPAGE, errors='ignore')
        return u''

    def close(self):
        """Close connection to device and daisy chain."""
        PIDebug('GCSDll.close(id%d)', self._id)
        if self._id >= 0:
    ...
    def unload(self):
        ...
        if self._dllhandle is not None:
            self.close()
            ...
            self._dllhandle = None
            PIDebug('GCSDll.unload %r', self._dllpath)
    ...
    def _handle(self):
        ...
        if self._dllhandle is None:
            ...
            PIDebug('GCSDll.load: %r', self._dllpath)
            if sys.platform in ('win32', 'cygwin'):
    ...
    def _prefix(self):
        ...
        if self._dllprefix is None:
            ...
            elif dllname.find('libpi_pi_gcs2') > -1:
                self._dllprefix = 'PI_'
            else:
    ...
    def GetAsyncBuffer(self, firstline=1, lastline=0, numtables=1):
        ...
        """
        PIDebug('DEPRECATED -- GcsDll.GetAsyncBuffer(id%d, firstline=%r, lastline=%r, numtables=%r)', self._id,
        ...
        PIDebug('DEPRECATED -- GCSDll.GetAsyncBuffer(id%d): %r', self._id, data)
        return data
    ...
    def GetAsyncBufferIndex(self):
        ...
        PIDebug('DEPRECATED -- GCSDll.GetAsyncBufferIndex(id%d): %r', self._id, bufindex)
    ...
    def InterfaceSetupDlg(self, key=''):
        ...
        """
        PIDebug('GCSDll.InterfaceSetupDlg(key=%r)', key)
        key = ctypes.c_char_p(str(key).encode(PI_DLL_CODEPAGE))
    ...
    def ConnectRS232(self, comport, baudrate):
        ...
        try:
            ...
            PIDebug('GCSDll.ConnectRS232ByDevName(devname=%r, baudrate=%s)', comport, baudrate)
            cdevname = ctypes.c_char_p(str(comport).encode(PI_DLL_CODEPAGE))
            self._id = getattr(self._handle, self._prefix + 'ConnectRS232ByDevName')(cdevname, cbaudrate)
        else:
            PIDebug('GCSDll.ConnectRS232(comport=%s, baudrate=%s)', comport, baudrate)
    ...
    def ConnectTCPIP(self, ipaddress, ipport=50000):
        ...
        """
        PIDebug('GCSDll.ConnectTCPIP(ipaddress=%s, ipport=%s)', ipaddress, ipport)
        cipaddress = ctypes.c_char_p(str(ipaddress).encode(PI_DLL_CODEPAGE))
    ...
    def ConnectTCPIPByDescription(self, description):
        ...
        """
        PIDebug('GCSDll.ConnectTCPIPByDescription(description=%r)', description)
        cdescription = ctypes.c_char_p(str(description).encode(PI_DLL_CODEPAGE))
    ...
    def ConnectUSB(self, serialnum):
        ...
        """
        PIDebug('GCSDll.ConnectUSB(serialnum=%r)', serialnum)
        cserialnum = ctypes.c_char_p(str(serialnum).encode(PI_DLL_CODEPAGE))
    ...
    def ConnectNIgpib(self, board, device):
        ...
        """
        PIDebug('GCSDll.ConnectNIgpib(board=%s, device=%s)', board, device)
    ...
    def ConnectPciBoard(self, board):
        ...
        """
        PIDebug('GCSDll.ConnectPciBoard(board=%s)', board)
        cboard = ctypes.c_int(int(board))
    ...
    def EnumerateUSB(self, mask=''):
        ...
        """
        PIDebug('GCSDll.EnumerateUSB(mask=%r)', mask)
        mask = ctypes.c_char_p(str(mask).encode(PI_DLL_CODEPAGE))
        bufsize = 100000
        bufstr = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
        ...
        PIDebug('GCSDll.EnumerateUSB: %r', devlist)
        return devlist
    ...
    def EnumerateTCPIPDevices(self, mask=''):
        ...
        @return: Found devices as list of strings.
        """
        PIDebug('GCSDll.EnumerateTCPIPDevices(mask=%r)', mask)
        mask = ctypes.c_char_p(str(mask).encode(PI_DLL_CODEPAGE))
        bufsize = 100000
        bufstr = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
        ...
        devlist = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore').split('\n')[:-1]
        devlist = [item.strip() for item in devlist]
        PIDebug('GCSDll.EnumerateTCPIPDevices: %r', devlist)
        return devlist
    ...
    def OpenRS232DaisyChain(self, comport, baudrate):
        ...
        @return: Found devices as list of strings.
        """
        PIDebug('GCSDll.OpenRS232DaisyChain(comport=%s, baudrate=%s)', comport, baudrate)
        ccomport = ctypes.c_int(int(comport))
        cbaudrate = ctypes.c_int(int(baudrate))
        ...
        devlist = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore').split('\n')[:-1]
        devlist = [item.strip() for item in devlist]
        PIDebug('GCSDll.OpenRS232DaisyChain: %r', devlist)
    ...
    def OpenUSBDaisyChain(self, description):
        ...
        @return: Found devices as list of strings.
        """
        PIDebug('GCSDll.OpenUSBDaisyChain(description=%r)', description)
        cdescription = ctypes.c_char_p(str(description).encode(PI_DLL_CODEPAGE))
        numdev = ctypes.byref(ctypes.c_int())
        bufsize = 10000
        bufstr = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
        ...
        devlist = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore').split('\n')[:-1]
        devlist = [item.strip() for item in devlist]
        PIDebug('GCSDll.OpenUSBDaisyChain: %r', devlist)
    ...
    def OpenTCPIPDaisyChain(self, ipaddress, ipport=50000):
        ...
        @return: Found devices as list of strings.
        """
        PIDebug('GCSDll.OpenTCPIPDaisyChain(ipaddress=%r, ipport=%s)', ipaddress, ipport)
        cipaddress = ctypes.c_char_p(str(ipaddress).encode(PI_DLL_CODEPAGE))
        cipport = ctypes.c_int(int(ipport))
        numdev = ctypes.byref(ctypes.c_int())
        bufsize = 10000
        ...
        devlist = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore').split('\n')[:-1]
        devlist = [item.strip() for item in devlist]
        PIDebug('GCSDll.OpenTCPIPDaisyChain: %r', devlist)
    ...
    def ConnectDaisyChainDevice(self, deviceid, daisychainid=None):
        ...
        """
        PIDebug('GCSDll.ConnectDaisyChainDevice(deviceid=%s, daisychainid=%s)', deviceid,
                daisychainid)
    ...
    def CloseConnection(self):
        ...
        PIDebug('GCSDll.CloseConnection(id=%d)', self._id)
    ...
    def CloseDaisyChain(self):
        ...
        PIDebug('GCSDll.CloseDaisyChain(dcid=%d)', self._dcid)
    ...
    def AddStage(self, axis):
        ...
        """
        PIDebug('GCSDll.AddStage(axis=%r)', axis)
        axis = ctypes.c_char_p(str(axis).encode(PI_DLL_CODEPAGE))
    ...
    def RemoveStage(self, axis):
        ...
        """
        PIDebug('GCSDll.RemoveStage(axis=%r)', axis)
        axis = ctypes.c_char_p(str(axis).encode(PI_DLL_CODEPAGE))
    ...
    def SetConnectTimeout(self, timeout):
        ...
        :type timeout: int
        """
        PIDebug('GCSDll.SetConnectTimeout(timeout=%s)', timeout)
    ...
    def EnableBaudRateScan(self, enable_bautrate_scan):
        ...
        """
        PIDebug('GCSDll.EnableBaudRateScan(timeout=%s)', enable_bautrate_scan)
    ...
    def applyconfig(self, items, config, andsave=False):
        ...
        """
        PIDebug('GCSDll.applyconfig(items=%r, config=%r)', items, config)
        items = ctypes.c_char_p(str(items).encode(PI_DLL_CODEPAGE))
        config = ctypes.c_char_p(str(config).encode(PI_DLL_CODEPAGE))
        bufsize = 20000
        warnings = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
        ...
        if not getattr(self._handle, funcname)(self._id, items, config, warnings, bufsize + 1):
            self.warning = warnings.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore')
            raise GCSError(self._error)
    ...
    def saveconfig(self, items, config):
        ...
        """
        PIDebug('GCSDll.saveconfig(items=%r, config=%r)', items, config)
        items = ctypes.c_char_p(str(items).encode(PI_DLL_CODEPAGE))
        config = ctypes.c_char_p(str(config).encode(PI_DLL_CODEPAGE))
        bufsize = 20000
        warnings = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
    ...
    def getconfigs(self):
        ...
        @return : Answer as string.
        """
        PIDebug('GCSDll.getconfigs()')
        bufsize = 20000
        configs = ctypes.create_string_buffer('\000'.encode(PI_DLL_CODEPAGE), bufsize + 2)
        ...
        if not getattr(self._handle, funcname)(self._id, configs, bufsize + 1):
            raise GCSError(self._error)
        configs = configs.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore')
        PIDebug('GCSDll.getconfigs(id%d): %r', self._id, configs)
        return configs
    ...
    def GetDllVersionInformation(self):
        ...
        """
        PIDebug('GCSDll.GetDllVersionInformation()')
        ...
        answer = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore')
        PIDebug('GCSDll.GetDllVersionInformation = %r', answer)
        return answer
    ...
    def GetPIStages3VersionInformation(self):
        ...
        """
        PIDebug('GCSDll.GetPIStages3VersionInformation()')
        ...
        answer = bufstr.value.decode(encoding=PI_DLL_CODEPAGE, errors='ignore')
        PIDebug('GCSDll.GetPIStages3VersionInformation = %r', answer)
        return answer
