...
__signature__ = 0xdba336e0d054872b559d0737a210f392
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_3arg_req_req_[opt].test_set_scalar_args(%s)', cmd)
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_[opt].test_wrong_argtype(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(['1'], [2, 3, 4], ['3', 'a', 5])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(['1', 2.1, 3], [2], ['3', 'a', 5])
    with pytest.raises(TypeError):
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_[opt].test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2'))
    getattr(gcs, cmd)(['1'], [2], [])
    gcs.svr.queue(cmdstr(cmd, '1 2 3'))
    ...
    gcs.svr.queue(cmdstr(cmd, 'a 2 3 4 5.6'))
    getattr(gcs, cmd)('a', 2, ['3', 4, 5.6])
    checksvr(gcs)


def test_different_list_sizes(gcs, cmd):
    """Arguments as lists with different sizes."""
    PIDebug('enter test_set_3arg_req_req_[opt].test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_[opt].test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)({}, 2)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)([], [])
