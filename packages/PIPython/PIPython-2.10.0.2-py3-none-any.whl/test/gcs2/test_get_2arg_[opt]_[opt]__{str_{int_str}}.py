#!/usr/bin/python
# -*- coding: utf-8 -*-
"""Test getter commands returning a dict of str/int/str with this signature: CMD [opt] [opt]."""

# Redefining name 'cmd' from outer scope pylint: disable=W0621
# Invalid module name pylint: disable=C0103
__signature__ = 0x7dd722e8949ce37df53bc5f7be291f65
...
def test_get_noargs(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_get_noargs(%s)', cmd)
...
def test_wrong_args(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_wrong_args(%s)', cmd)
...
def test_get_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_get_scalar_args(%s)', cmd)
...
def test_get_list_args(gcs, cmd):
    """Arguments as list."""
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_get_list_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2'), '1 2=foo\n')
    gcs.svr.queue(cmdstr(cmd, '1 2'), '1 2=foo\n')
    assert OrderedDict([('1', OrderedDict([(2, 'foo')]))]) == getattr(gcs, cmd)(['1'], [2])
    assert OrderedDict([(1, OrderedDict([('2', 'foo')]))]) == getattr(gcs, cmd)([1], ['2'])
...
def test_get_answer_with_spaces(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_get_answer_with_spaces(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd), '  1\t2 =  foo   \n 4   5\t=    bar    \n  7    8  =         egg\n')
    assert OrderedDict([('1', OrderedDict([(2, 'foo')])), ('4', OrderedDict([(5, 'bar')])),
                        ('7', OrderedDict([(8, 'egg')]))]) == getattr(gcs, cmd)()
    checksvr(gcs)
...
def test_wrong_answer(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_str}}.test_wrong_answer(%s)', cmd)
