...
__signature__ = 0x4e7c81e37dd79cd65de92fb32dc138f9
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_wav_sweep_wav.test_set_scalar_args(%s)', cmd)
    wavetype = cmd.split('_')[1]
    gcs.svr.queue('WAV 1 & %s 40 50 60 2.3 4.5\n' % wavetype)
    getattr(gcs, cmd)('1', '&', 40, 50, 60, 2.3, 4.5)
    checksvr(gcs)


def test_wrong_argtype(gcs, cmd):
    """Wrong argument sizes and/or types."""
    PIDebug('enter test_wav_sweep_wav.test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_wav_sweep_wav.test_set_list_scalar_args(%s)', cmd)
    wavetype = cmd.split('_')[1]
    gcs.svr.queue('WAV 1 & %s 40 50 60 2.3 4.5\n' % wavetype)
    gcs.svr.queue('WAV 1 & %s 40 50 60 2.3 4.5\n' % wavetype)
    getattr(gcs, cmd)('1', '&', 40, 50, 60, 2.3, [4.5])
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_wav_sweep_wav.test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_wav_sweep_wav.test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [], [], [], [], [], [])
