...
__signature__ = 0xcfb29126e49ab6fb098c19b0b82b716b
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_4arg_req_[req]_[req]_[req].test_set_scalar_args(%s)', cmd)
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_[req]_[req]_[req].test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_[req]_[req]_[req].test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3 4'))
    ...
    gcs.svr.queue(cmdstr(cmd, '1 2 3 4'))
    getattr(gcs, cmd)([1], [2], [3], [4])
    gcs.svr.queue(cmdstr(cmd, '1 2 3 -4 4 2 3 3 3 4'))
    getattr(gcs, cmd)('1', [2.0, 4, 3], [3.0, 2, 3], [-4.0, 3, 4])
    gcs.svr.queue(cmdstr(cmd, 'a b 3 0'))
    getattr(gcs, cmd)('a', 'b', 3.0, [0])
    gcs.svr.queue(cmdstr(cmd, '1 -2 1 2 3 4 -2'))
    getattr(gcs, cmd)('1', ['-2', 3], [True, 4], [2, -2])
    gcs.svr.queue(cmdstr(cmd, '1 4 12 5 3 5 5'))
    getattr(gcs, cmd)([1], [4, 3], ['12', 5], ['5', 5])
    checksvr(gcs)


def test_different_list_sizes(gcs, cmd):
    """Arguments as lists with different sizes."""
    PIDebug('enter test_set_4arg_req_[req]_[req]_[req].test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_[req]_[req]_[req].test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [], [])
