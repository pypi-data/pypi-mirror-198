...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_wav_lin_wav.test_set_scalar_args(%s)', cmd)
    gcs.svr.queue('WAV 1 & %s 40 2 4 30 3 5\n' % 'SIN_P')
    getattr(gcs, cmd)('1', 3, 30, '&', 5.0, 2.0, 4, 40)
    checksvr(gcs)


def test_wrong_argtype(gcs, cmd):
    """Wrong argument sizes and/or types."""
    PIDebug('enter test_wav_lin_wav.test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_wav_lin_wav.test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue('WAV 1 & %s 40 2 4 30 3 5\n' % 'SIN_P')
    gcs.svr.queue('WAV 1 & %s 40 2 4 30 3 5\n' % 'SIN_P')
    getattr(gcs, cmd)('1', 3, 30, '&', 5.0, 2.0, 4, [40])
    getattr(gcs, cmd)('1', [3], 30, ['&'], 5.0, 2.0, 4, 40)
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_wav_lin_wav.test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_wav_lin_wav.test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [], [], [], [], [], [])
