...
__signature__ = 0x1057596033983b147f4d2b1d6261105f
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_keys_aap.test_set_missing_args(%s)', cmd)
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_set_keys_aap.test_set_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_keys_aap.test_wrong_argtype(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_keys_aap.test_set_scalar_args(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_keys_aap.test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3 4'))
...
def test_set_all_args(gcs, cmd):
    ...
    PIDebug('enter test_set_keys_aap.test_set_all_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 abc 4.56 SA 1 N 8 A 10'))
    getattr(gcs, cmd)('1', 2.0, 'abc', 4.56, 1.0, 8, 10)
    checksvr(gcs)


def test_set_keyword_args(gcs, cmd):
    """Some keyword arguments."""
    PIDebug('enter test_set_keys_aap.test_set_keyword_arg(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 2 4.56 SA 1'))
    getattr(gcs, cmd)('1', 2.0, '2', 4.56, 1.0)
    gcs.svr.queue(cmdstr(cmd, '1 2 abc 4.56 SA 1 N 8 A 5'))
    getattr(gcs, cmd)('1', 2.0, 'abc', 4.56, 1.0, 8, 5)
    checksvr(gcs)
