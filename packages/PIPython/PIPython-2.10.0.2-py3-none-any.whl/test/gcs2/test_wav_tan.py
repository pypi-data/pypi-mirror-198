...
__signature__ = 0x8c650bac091de1a1abe5ff97d94fc5b6
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_wav_tan_wav.test_set_scalar_args(%s)', cmd)
    wavetype = cmd.split('_')[1]
    gcs.svr.queue('WAV 1 & %s 1 2 3 4 5 6 7\n' % wavetype)
    getattr(gcs, cmd)('1', '&', 1, 2, 3.0, 4, 5, 6.0, 7.0)
    checksvr(gcs)


def test_wrong_argtype(gcs, cmd):
    """Wrong argument sizes and/or types."""
    PIDebug('enter test_wav_tan_wav.test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_wav_tan_wav.test_set_list_scalar_args(%s)', cmd)
    wavetype = cmd.split('_')[1]
    gcs.svr.queue('WAV 1 & %s 1 2 3 4 5 6 7\n' % wavetype)
    gcs.svr.queue('WAV 1 & %s 1 2 3 4 5 6 7\n' % wavetype)
    getattr(gcs, cmd)('1', '&', 1, 2, 3.0, 4, 5, 6.0, [7.0])
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_wav_tan_wav.test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_mistang_args(gcs, cmd):
    """No arguments, not enough arguments, empty strings, lists and dicts."""
    PIDebug('enter test_wav_tan_wav.test_mistang_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [], [], [], [], [], [])
