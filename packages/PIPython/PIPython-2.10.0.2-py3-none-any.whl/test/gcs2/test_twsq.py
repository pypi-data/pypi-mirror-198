...
__signature__ = 0x514a23fefe9e41f75c5d4e856668332d
...
def test_wrong_args(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_wrong_args(%s)', cmd)
    ...
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(['1', 2])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(['1', 2], ['1', 2])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(1, ['1', 2])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(count=16)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(start=1, trigout=1)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(count=16, trigout=1)
    checksvr(gcs)


def test_get_scalar_args(gcs, cmd):
    """No rectables given."""
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_get_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 16'), ANSWER)
    assert HEADER == getattr(gcs, cmd)(start=1, count=16)
    while gcs.bufstate is not True:
        pass
    assert DATA == gcs.bufdata
    gcs.svr.queue(cmdstr(cmd, '1 -1'), ANSWER)
    assert HEADER == getattr(gcs, cmd)(start=1, count=-1)
    while gcs.bufstate is not True:
        pass
    assert DATA == gcs.bufdata
    checksvr(gcs)
...
def test_get_list_args(gcs, cmd):
    """With rectables."""
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_get_list_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 16 1 2 3 4'), ANSWER)
    assert HEADER == getattr(gcs, cmd)(start=1, count=16, trigout=[1, 2, 3, 4])
    while gcs.bufstate is not True:
        pass
    assert DATA == gcs.bufdata
    checksvr(gcs)
...
def test_too_many_datasets(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_too_many_datasets(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 15'), ANSWER)
    with GCSRaise(gcserror.E_1089_PI_TOO_MANY_GCS_DATA):
        assert HEADER == getattr(gcs, cmd)(start=1, count=15)
        while gcs.bufstate is not True:
            pass
    assert DATA == gcs.bufdata
    checksvr(gcs)
...
def test_too_few_datasets(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_too_few_datasets(%s)', cmd)
...
def test_invalid_number(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_invalid_number(%s)', cmd)
...
def test_missing_number(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_missing_number(%s)', cmd)
...
def test_no_answer(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_no_answer(%s)', cmd)
...
def test_header_lf(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_header_lf(%s)', cmd)
...
def test_header_only(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_header_only(%s)', cmd)
...
def test_no_endofmessage(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_no_endofmessage(%s)', cmd)
...
def _test_timeout(gcs, cmd):
    ...
    PIDebug('enter test_get_3arg_opt_opt_[opt]__gcs.test_timeout(%s)', cmd)
    gcs.svr.delay = 10
    gcs.svr.queue(cmdstr(cmd, '1 16'), ANSWER)
    assert HEADER == getattr(gcs, cmd)(start=1, count=16)
    timeout = gcs.timeout
