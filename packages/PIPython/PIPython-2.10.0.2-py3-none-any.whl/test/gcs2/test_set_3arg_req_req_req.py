...
__signature__ = 0x4cdc8ea227c9887776320d78bee1822d
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_3arg_req_req_req.test_set_scalar_args(%s)', cmd)
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_req.test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_req.test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3'))
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_req.test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_req_req.test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', 2, [])
