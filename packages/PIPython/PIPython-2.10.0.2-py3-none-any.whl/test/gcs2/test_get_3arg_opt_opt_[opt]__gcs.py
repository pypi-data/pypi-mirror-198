...
__signature__ = 0x4880b461d5c8ff0e524879ef7601ae90
...
def test_wrong_args(gcs, cmd):
    ...
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(1, ['1', 2])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(offset=1)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(numvalues=16)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(offset=1, tables=1)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(numvalues=16, tables=1)
    checksvr(gcs)
...
def test_get_list_args(gcs, cmd):
    ...
    assert HEADER == getattr(gcs, cmd)(offset=1, numvalues=16, tables=[1, 2, 3, 4])
    while gcs.bufstate is not True:
        pass
    assert DATA == gcs.bufdata
    checksvr(gcs)
