...
__signature__ = 0x263fd813ca194b322fd5ae21102f2309
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_4arg_req_req_req_[req].test_set_scalar_args(%s)', cmd)
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_req_req_[req].test_wrong_argtype(%s)', cmd)
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_req_req_[req].test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3 4'))
    ...
    gcs.svr.queue(cmdstr(cmd, '1 2 3 4'))
    getattr(gcs, cmd)([1], [2], [3], [4])
    gcs.svr.queue(cmdstr(cmd, '1 2 3 -4 3'))
    getattr(gcs, cmd)('1', 2.0, 3.0, [-4.0, 3])
    gcs.svr.queue(cmdstr(cmd, 'a b 3 0 1'))
    getattr(gcs, cmd)('a', 'b', 3.0, [False, True])
    gcs.svr.queue(cmdstr(cmd, '1 -2 1 2 -2'))
    getattr(gcs, cmd)('1', '-2', True, [2, -2])
    checksvr(gcs)


def test_different_list_sizes(gcs, cmd):
    """Arguments as lists with different sizes."""
    PIDebug('enter test_set_4arg_req_req_req_[req].test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_4arg_req_req_req_[req].test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', [], [], [])
