#!/usr/bin/python
# -*- coding: utf-8 -*-
"""Test getter commands returning a dict of int/float with this signature: CMD [opt]."""

# Redefining name 'cmd' from outer scope pylint: disable=W0621
# Invalid module name pylint: disable=C0103
__signature__ = 0xabd9a64022c334eca6d16104b60ca397
...
def test_get_noargs(gcs, cmd):
    ...
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_get_noargs(%s)', cmd)
...
def test_get_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_get_scalar_args(%s)', cmd)
...
def test_get_list_args(gcs, cmd):
    ...
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_get_list_args(%s)', cmd)
...
def test_get_answer_with_spaces(gcs, cmd):
    """Arguments as lists."""
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_get_answer_with_spaces(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3'), '1 =1.23 \n2=  2.34 \n3  =   3.45\n')
    assert OrderedDict([('1', 1.23), (2, 2.34), ('3', 3.45)]) == getattr(gcs, cmd)(['1', 2, '3'])
    checksvr(gcs)
...
def test_wrong_args(gcs, cmd):
    ...
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_set_wrong_arg(%s)', cmd)
...
def test_wrong_answer(gcs, cmd):
    ...
    PIDebug('enter test_get_1arg_[opt]__{int_float}.test_wrong_answer(%s)', cmd)
