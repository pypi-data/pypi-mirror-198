#!/usr/bin/python
# -*- coding: utf-8 -*-
"""Test getter commands returning a dict of str/int/int with this signature: CMD [opt] [opt]."""

# Redefining name 'cmd' from outer scope pylint: disable=W0621
# Invalid module name pylint: disable=C0103
__signature__ = 0x5b802541adf15ddb776be22654c54421
...
def test_get_noargs(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_get_noargs(%s)', cmd)
...
def test_wrong_args(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_wrong_args(%s)', cmd)
...
def test_get_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_get_scalar_args(%s)', cmd)
...
def test_get_list_args(gcs, cmd):
    """Arguments as list."""
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_get_list_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2'), '1 2=3\n')
    gcs.svr.queue(cmdstr(cmd, '1 2'), '1 2=3\n')
    assert OrderedDict([('1', OrderedDict([(2, 3)]))]) == getattr(gcs, cmd)(['1'], [2])
    assert OrderedDict([(1, OrderedDict([('2', 3)]))]) == getattr(gcs, cmd)([1], ['2'])
...
def test_get_answer_with_spaces(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_get_answer_with_spaces(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd), '  1\t2 =  3   \n 4   5\t=    6    \n  7    8  =         9\n')
    assert OrderedDict([('1', OrderedDict([(2, 3)])), ('4', OrderedDict([(5, 6)])),
                        ('7', OrderedDict([(8, 9)]))]) == getattr(gcs, cmd)()
    checksvr(gcs)
...
def test_wrong_answer(gcs, cmd):
    ...
    PIDebug('enter test_get_2arg_[opt]_[opt]__{str_{int_int}}.test_wrong_answer(%s)', cmd)
