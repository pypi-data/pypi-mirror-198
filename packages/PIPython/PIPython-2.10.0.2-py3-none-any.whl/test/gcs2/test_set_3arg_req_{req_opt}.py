...
__signature__ = 0xa76a8ede8c25f1fc623fd1440bce7d8f
...
def test_set_scalar_args(gcs, cmd):
    """Scalar arguments."""
    PIDebug('enter test_set_3arg_req_{req:opt}.test_set_scalar_args(%s)', cmd)
...
def test_wrong_argtype(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_{req:opt}.test_wrong_argtype(%s)', cmd)
...
def test_set_dict_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_{req:opt}.test_set_dict_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3'))
...
def test_set_list_scalar_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_{req:opt}.test_set_list_scalar_args(%s)', cmd)
    gcs.svr.queue(cmdstr(cmd, '1 2 3'))
    getattr(gcs, cmd)(['1'], [2], '3')
    gcs.svr.queue(cmdstr(cmd, '1 2 3'))
    getattr(gcs, cmd)(['1'], 2, ['3'])
...
def test_different_list_sizes(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_{req:opt}.test_different_list_sizes(%s)', cmd)
    with pytest.raises(TypeError):
...
def test_missing_args(gcs, cmd):
    ...
    PIDebug('enter test_set_3arg_req_{req:opt}.test_missing_args(%s)', cmd)
    with pytest.raises(TypeError):
        getattr(gcs, cmd)('1', 2, [])
    with pytest.raises(TypeError):
        getattr(gcs, cmd)(2, {})
