from test7_powerhouse_helper.AddressBook.addressbook import *
...
class Load(Command):
    def processing(self, data_container: AddressBot):
        path = prompt('Enter the path to folder (it should be an binary file): ', validator=ValidateLoad())
...
class ValidateLoad(Validator):
    def validate(self, document) -> None:
        path = document.text
        if not len(path):
            return path 
        elif not os.path.exists(path) or not os.path.splitext(path)[-1] == '.bin':
            raise ValidationError(message='File is not exist or is not binary', cursor_position = 0)
        return path
