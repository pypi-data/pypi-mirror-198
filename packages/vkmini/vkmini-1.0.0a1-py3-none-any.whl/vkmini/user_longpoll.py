from typing import AsyncGenerator, Optional, Union, List, Any

from aiohttp.client import ClientSession

from vkmini.api import VkApi
from vkmini.utils import AbstractLogger
from vkmini.request import longpoll_get, check_longpoll_session
    ...
    async def _set_longpoll_data(self, new_ts: bool) -> None:
        ...
        if not self._vk.excepts:
            if data.get('error', {}).get('error_code') == 5:
                raise TokenInvalid(data['error'])  # type: ignore
        self.server = data['server']  # type: ignore
        self.key = data['key']  # type: ignore
        if new_ts:
            self.ts = data['ts']  # type: ignore
