...
class _StreamAudioFFMPEG(StreamAudio):
    ...
    def _snapshot(self, timestamp: np.ndarray[numbers.Real]) -> np.ndarray[numbers.Real]:
        ...
        # reparation shift
        cum_shifts = np.cumsum( # cumulative algebraic shifts, > 0 is jump forward
            [
                ti_next - tf_prec # the algebraic shifts
        ...
        if any(np.abs(cum_shifts) > self._av_container.streams[self.index].time_base):
            message = (
                ...
                f"the maximum allowed gap is {self._av_container.streams[self.index].time_base}"
            )
            logging.warning(message)
            array = _shift_audio_samples(
                t_min,
                self.samplerate,
                *zip(*((samples, t_ref) for  _, (t_ref, _), _, samples in references)),
            )
            references = [(False, (t_min, t_max), self.samplerate, array)]

        # assembling references and get the nerest reference
        t_ref = float(references[0][1][0])
        array = np.concatenate([s for _, _, _, s in references], axis=1)
        ...
        np.minimum(indexs, array.shape[1]-1, out=indexs) # prevention agains IndexError
        samples = np.where(timestamp<t_ref, np.nan, array[:, indexs])
        return samples
...
def _shift_audio_samples(
    ref_time: numbers.Real,
    samplerate: int,
    frames: typing.Iterable[np.ndarray[numbers.Real]],
    times: typing.Iterable[numbers.Real]
) -> np.ndarray[numbers.Real]:
    """
    ** Realign audio frames offset. **

    Parameters
    ----------
    ref_time : numbers.Real
        The reference time for the first frame (in seconds).
    samplerate : int
        The number of frames per seconds.
    frames : typing.Iterable[np.ndarray[numbers.Real]]
        The frames contraining the audio samples.
    times : typing.Iterable[numbers.Real]
        The real position of the start of each frames (in seconds).

    Returns
    -------
    samples : np.ndarray[numbers.Real]
        The set of samples represented by a single frame.
        Each slice is reset to its position. The holes are filled by nan.

    Examples
    --------
    >>> import numpy as np
    >>> from movia.core.io.read import _shift_audio_samples
    >>> _shift_audio_samples(
    ...     ref_time = 0.5,
    ...     samplerate = 10,
    ...     frames = [np.full((1, 5), .0), np.full((1, 6), 1.)],
    ...     times = [0.3, 1],
    ... )
    array([[ 0.,  0.,  0., nan, nan,  1.,  1.,  1.,  1.,  1.,  1.]])
    >>>
    """
    assert isinstance(ref_time, numbers.Real), ref_time.__class__.__name__
    assert isinstance(samplerate, int), samplerate.__class__.__name__
    assert hasattr(frames, "__iter__"), frames.__class__.__name__
    frames = list(frames)
    assert all(isinstance(f, np.ndarray) for f in frames), frames
    assert all(f.ndim == 2 for f in frames), frames
    assert hasattr(times, "__iter__"), times.__class__.__name__
    times = list(times)
    assert len(frames) == len(times), (len(frames), len(times))
    assert all(isinstance(t, numbers.Real) for t in times), times
    channels = {f.shape[0] for f in frames}
    assert len(channels) == 1, channels
    channels = channels.pop()
    dtype = {f.dtype.type for f in frames}
    assert len(dtype) == 1, dtype
    dtype = dtype.pop()

    # convertion second to index
    ref_time = round(ref_time * samplerate)
    times = [round(t * samplerate) for t in times]

    # create empty array
    max_time = round(max(t + f.shape[1] for t, f in zip(times, frames)))
    array = np.full(
        (channels, max_time-ref_time), np.nan, dtype=dtype
    )

    # positionning of each slices
    for frame, time in zip(frames, times):
        pos = time - ref_time # absolute position
        if pos < 0:
            frame = frame[:, -pos:]
            pos = 0
        if frame.shape[1]:
            array[:, pos:pos+frame.shape[1]] = frame
    return array
