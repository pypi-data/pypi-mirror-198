...
class DataCollectClient:
    ...
    def _gen_image_save_path(self, correlation_id: str, designation: str = '', metadata=None) -> str:
        ...
        if metadata.get('extension'):
            file_path = f"{dir_name}/{file_name}.{metadata.get('extension')}"
        else:
            file_path = f"{dir_name}/{file_name}.png"
        return file_path
    ...
    def _gen_blob_save_path(self, correlation_id: str, metadata: Dict = None):
        ...
        extension = metadata.get("extension") if metadata is not None and metadata.get(
            "extension") is not None else "bin"
        file_name = correlation_id
        if metadata.get('sub_name'):
            file_name = f"{file_name}_{metadata.get('sub_name')}"
