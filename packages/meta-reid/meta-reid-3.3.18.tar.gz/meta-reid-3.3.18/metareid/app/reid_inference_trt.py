...
def list_files(target_dir, pattern):
    file_pattern = target_dir + "/" + pattern
...
class ReidInferenceTrt:
    ...
    def load_features(self, ):
        query_files, query_names, query_features = None, None, None
        ...
        return query_files, query_names, query_features

    def update_features(self, query_file: str, query_name: str, query_feature: np.ndarray):
    ...
    def save_folder(self, img_dir):
        ...
        for img_file in tqdm.tqdm(img_files):
            ...
            query_feature = self.predict(img)
            self.update_features(os.path.basename(img_file), query_name, query_feature)
    ...
    def index(self, features, recall_num=1):
        if self.use_gpu:
            ...
            maximum = np.max(index_cossim, axis=1)
