...
from typing import TYPE_CHECKING, Annotated, Iterator, Literal, Sequence
...
Vec2 = Annotated[npt.NDArray[np.float_] | Sequence[float], (2,)]

Point = tuple[float, float]

RoundPoint = tuple[int, int]
RoundEdge = tuple[tuple[int, int], tuple[int, int]]
...
def do_segs_cross(a: Point, b: Point, c: Point, d: Point) -> bool:
...
def do_seg_and_bbox_cross(
    ...
    return any(do_segs_cross(seg_a, seg_b, *side) for side in sides)


def maybe_get_seg_seg_intersection(
...
def map_segments_to_segment_intersections(
    ...
    for seg1, seg2 in potential_intersections:
        intersection = maybe_get_seg_seg_intersection(*seg1, *seg2)
...
def map_segments_to_pixel_intersections(
    ...
    for seg1, seg2 in potential_intersections:
        ...
        if the_pnt[0] in the_seg:
            continue
        if do_seg_and_bbox_cross(*the_seg, get_pixel_bbox(the_pnt)):
