...
_BBox = Annotated[tuple[float, ...], "min_x, min_y, ..., max_x, max_y, ..."]
_T = TypeVar("_T")

_GetBBox = Callable[[Any], _BBox]
_SequenceT = TypeVar("_SequenceT", bound=Sequence[Any])


def _default_get_bbox(obj: Any) -> _BBox:
    """Return a bbox attribute if no GetBBox function is provided."""
    return tuple(obj.bbox)


def _split_sequence(seq: _SequenceT) -> tuple[_SequenceT, _SequenceT]:
    """Split a tuple into two relately equal length tuple.
...
def _get_bbox_union(*bboxes: _BBox) -> _BBox:
    ...
    if len(bboxes) == 1:
        return bboxes[0]
    all_mins, all_maxs = zip(*(_split_sequence(b) for b in bboxes))
    return tuple([min(x) for x in zip(*all_mins)] + [max(x) for x in zip(*all_maxs)])
...
def _get_bbox_measurements(bbox: _BBox) -> list[float]:
    ...
    return [y - x for x, y in zip(mins, maxs)]
...
class BoundObj(Generic[_T]):
    ...
    def __post_init__(self) -> None:
        ...
        self.centroid = [(x + y) / 2 for x, y in zip(mins, maxs)]
...
def _build_bvh_from_bound_objects(bound_objects: list[BoundObj[_T]]) -> BVHNode[_T]:
    ...
    axis = np.argmax(_get_bbox_measurements(bbox))
...
def build_bvh(
    objs: Iterable[_T], get_bbox: _GetBBox = _default_get_bbox
) -> BVHNode[_T]:
...
def _do_bboxes_intersect(bbox_a: _BBox, bbox_b: _BBox) -> bool:
    ...
    if any(a > b for a, b in zip(min_a, max_b)):
        return False
    if any(a < b for a, b in zip(max_a, min_b)):
        return False
    return True
