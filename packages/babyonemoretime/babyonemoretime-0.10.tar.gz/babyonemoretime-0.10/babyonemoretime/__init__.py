...
def restart_everything(pyfile, sysarv, restart=True):

    mypid = os.getpid()
    allpi = []
    for chi in psutil.Process(mypid).children():
        try:
            allpi.append(chi.pid)
        except Exception as fe:
            continue
    allpi.append(mypid)
    argis = sysarv[1:]
    fi = os.path.normpath(pyfile)
    fipa = os.path.normpath(os.path.dirname(sys.executable))
    inter = os.path.normpath(sys.executable)
    coenv = os.environ.get("CONDA_DEFAULT_ENV")
    copre = os.environ.get("CONDA_PREFIX")
    batfile = ""
    for pid in allpi:
        batfile += f"CALL taskkill /F /PID {pid} /T\n"
    if not inter.endswith("python.exe"):
        batfile += f"CD {fipa}\n"
        if restart:
            batfile += f'START "" "{inter}" {" ".join(argis[1:])}\n'
    else:
        if not coenv and not copre:
            batfile += f"CD {fipa}\n"
            if restart:
                batfile += f'START "" "{inter}" {fi} {" ".join(argis)}\n'
        else:
            batfile += f"CD {copre}\n"
            if restart:
                batfile += f'START "" "{inter}" {fi} {" ".join(argis)}\n'

    tmpfi = get_tmpfile(suffix=".bat")
    with open(tmpfi, mode="w", encoding="utf-8") as f:
        f.write(batfile)

    linkfile = tmpfi[:-4] + ".lnk"
    notcli(target=tmpfi, name=linkfile, workdir=os.path.dirname(tmpfi))
    mainprocess.mainprocess(linkfile)
...
_ENTRY_TYPES = {
    ...
    0x35: "FOLDER (UNICODE)",
    0x36: "FILE (UNICODE)",
    0x802E: "ROOT_KNOWN_FOLDER",  # founded in doc, not tested
    0x1F: "ROOT_FOLDER",
    0x61: "URI",
    0x71: "CONTROL_PANEL",
}
...
class RootEntry(object):
    def __init__(self, root):
        if root is not None:
            ...
            self.root = _ROOT_LOCATIONS.get(
                self.guid, f"UNKNOWN {self.guid}"
            )  # if self.root == "UNKNOWN":  #     self.root = _ROOT_INDEX.get(index, "UNKNOWN")

    @property
    def bytes(self):
        guid = self.guid[1:-1].replace("-", "")
        chars = [
            bytes([int(x, 16)]) for x in [guid[i : i + 2] for i in range(0, 32, 2)]
        ]
        return (
            b"\x1F\x50"
            + chars[3]
...
class DriveEntry(object):
    ...
    def bytes(self):
        ...
        return (
            b"\x2F" + padded_str
        )  # drive = self.drive  # if isinstance(drive, str):  #     drive = drive.encode()  # return b'/' + drive + b'\\' + b'\x00' * 19
...
class PathSegmentEntry(object):
    ...
    def _validate(self):
        ...
        if self.file_size is None:
            if self.type.startswith("FOLDER") or self.type in [
                "KNOWN_FOLDER",
                "ROOT_KNOWN_FOLDER",
            ]:
                self.file_size = 0
...
class UwpSubBlock:
    block_names = {
        ...
        0x0D: "Wide310x150Logo",  # 0x04: '',
        # 0x05: '',
        0x13: "Square310x310Logo",  # 0x0e: '',
        0x0B: "DisplayName",
        0x14: "Square71x71Logo",
        0x64: "RandomByte",
        0x0A: "DisplayName",  # 0x07: '',
    }
...
class UwpSegmentEntry:
    ...
    def bytes(self):
        blocks_bytes = [block.bytes for block in self._blocks]
        blocks_size = (
            sum([len(block) + 4 for block in blocks_bytes]) + 4
        )  # with terminator
        size = (
            2  # size
            + len(self.magic)
    ...
    def create(cls, package_family_name, target, location=None, logo44x44=None):
        ...
        if location:
            blocks.append(
                UwpSubBlock(type=0x0F, value=location)
    ...
    def _validate(self):
        ...
        if type(self.items[0]) == RootEntry and self.items[0].root == ROOT_MY_COMPUTER:
            if type(self.items[1]) == DriveEntry:
                return
            if type(self.items[1]) == PathSegmentEntry and self.items[
                1
...
class LinkInfo(object):
    ...
    def _parse_path_elements(self, lnk):
        ...
        if self.local:
            ...
            self.local_base_path = read_cstring(lnk)  # TODO: unicode
        self.make_path()
    ...
    def _calculate_sizes_and_offsets(self):
        ...
        if self.remote:
            self.size_network_volume_table = (
                20 + len(self.network_share_name) + len(self.base_name) + 1
            )
            ...
            self.offs_base_name = (
                self.offs_network_volume_table + self.size_network_volume_table
            )
        else:
            ...
            self.offs_local_base_path = (
                self.offs_local_volume_table + self.size_local_volume_table
            )
            self.offs_network_volume_table = 0
...
class ExtraData_Unparsed(object):
    def __init__(self, bytes=None, signature=None, data=None):
        ...
        if bytes:
            ...
            self.data = bytes  # self.read(bytes)
...
class Lnk(object):
    ...
    def _parse_lnk_file(self, lnk):
        ...
        self._show_command = (
            _SHOW_COMMANDS[show_command]
            if show_command in _SHOW_COMMANDS
            else _SHOW_COMMANDS[1]
        )
    ...
    def path(self):
        ...
        link_info_path = (
            self._link_info.path if self._link_info and self._link_info.path else None
        )
        id_list_path = (
            self._shell_item_id_list.get_path()
            ...
            for block in self.extra_data.blocks:
                if type(block) == ExtraData_EnvironmentVariableDataBlock:
                    env_var_path = block.target_unicode.strip(
                        "\x00"
                    ) or block.target_ansi.strip("\x00")
                    break
    ...
    def specify_local_location(
        self, path, drive_type=None, drive_serial=None, volume_label=None
    ):
...
def for_file(
    ...
    if target_file.startswith("\\\\"):
        ...
        elements = [RootEntry(ROOT_MY_COMPUTER), DriveEntry(levels[0])]
...
def notcli(
    target, name, workdir, arguments=None, description="", window_mode="Minimized"
):
    action = "c"
