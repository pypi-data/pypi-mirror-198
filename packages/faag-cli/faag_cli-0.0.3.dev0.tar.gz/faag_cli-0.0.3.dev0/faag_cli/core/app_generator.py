    ...
    def __add_folders_files(app_name: str, app_type: str) -> None:
        ...
        for folder, files in FOLDERS_FILES.items():
            if folder in ["response", "request"]:
                ...
                for file in files:
                    with open(
                        f"{app_name}/schemas/{folder}/{file}.py", "w", encoding="UTF-8"
                    ) as gen_file:
                        if file.startswith("__"):
                            template = templates_environment.get_template(
                                f"/schemas/{folder}/{folder + file}.jinja"
                            )
                        else:
                            template = templates_environment.get_template(
                                f"/schemas/{folder}/{file}.jinja"
                            )
                ...
                for file in files:
                    with open(
                        f"{app_name}/{folder}/{file}.py", "w", encoding="UTF-8"
                    ) as gen_file:
                        if file.startswith("__"):
                            template = templates_environment.get_template(
                                f"/{folder}/{folder + file}.jinja"
                            )
                        else:
                            template = templates_environment.get_template(
                                f"/{folder}/{file}.jinja"
