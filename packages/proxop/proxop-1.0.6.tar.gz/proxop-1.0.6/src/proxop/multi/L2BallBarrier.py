...
class L2BallBarrier:
    ...
    def prox(
            self,
            x: np.ndarray,
            gamma: Union[float, np.ndarray] = 1) -> np.ndarray:
        ...
        def polynom_phi(t):
            return t**3 - l2_norm * t**2 - (r + 2 * gamma) * t + r * l2_norm

        def derive_polynom_phi(t):
            return 3 * t**2 - 2 * l2_norm * t - (r + 2 * gamma)
        ...
        root = newton_(
            polynom_phi, fp=derive_polynom_phi, x0=root, low=0, high=np.sqrt(r)
        )

        return center + (r - root**2) / (r - root**2 + 2 * gamma) * (x - center)
