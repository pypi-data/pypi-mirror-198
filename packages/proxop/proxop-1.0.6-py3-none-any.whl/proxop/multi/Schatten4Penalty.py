...
class Schatten4Penalty:
    r"""Compute the proximity operator and the evaluation of gamma*f.

    Where f is the function defined as:


         f(x) = (1/2)*||X||_F^2 + mu*R_4^4(X) = (1/2)||s||_2^2 + mu*||s||_4^4

    where
           * R_p(X) is the p-Shatten norm for matrices.
    ...
    def prox(self, x: np.ndarray, gamma: float = 1) -> np.ndarray:
        ...
        def prox_phi(s, gam_):
            a = ((gam_ + 1) ** 3) / (27 * gam_ * mu)
            return (
                (s + (s**2 + a) ** 0.5) ** (1 / 3)
                - (-s + (s**2 + a) ** 0.5) ** (1 / 3)
            ) * ((8 * gam_ * mu) ** (-1 / 3))

        return prox_svd(x, gamma, prox_phi)

    def __call__(self, x: np.ndarray) -> float:
        def fun_phi(s):
            return 0.5 * np.sum(s**2) + self.mu * np.sum(s**4)
