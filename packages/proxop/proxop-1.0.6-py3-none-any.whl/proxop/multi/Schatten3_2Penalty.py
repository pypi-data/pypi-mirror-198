...
class Schatten3_2Penalty:
    ...
    def prox(self, x: np.ndarray, gamma: float = 1) -> np.ndarray:
        ...
        def prox_phi(s, gam):
            d = (
                ...
                * (((gam * mu) ** 2) / (8 * (1 + gam)))
                * np.sign(s)
                * (1 - (1 + 16 * (1 + gam) * np.abs(s) / (9 * (gam * mu) ** 2)) ** 0.5)
            ) / (1 + gam)
            return d
    ...
    def __call__(self, x: np.ndarray) -> float:
        def fun_phi(s):
            return 0.5 * np.sum(s**2) + self.mu * np.sum(np.abs(s) ** (3 / 2))
