...
from PyxlSql.pyxlEngine import Cmd,\
    ...
    SaveCmd, DeleteCmd, UpdateCmd, SelectIntoCmd, ImportCmd, RenameCmd, LoadCmd, PivotCmd, \
    JoinClause, RightJoinClause, LeftJoinClause, \
...
class SheetToken(Token):
    ...
    def complies(self, workbook: PyxlWorkbook, command: Cmd, sheet_name: str):
        ...
        sheet: PyxlSheet = workbook.get_sheet(sheet_name, raise_exception=False)
...
class Parser:
    ...
    def __init__(self, runner: AbstractRunner):
        ...
        self.add(CmdToken(self.runner, RenameCmd,
                          sub_tokens=[SheetToken(self.runner, "sheet"), KeywordToken(self.runner, "AS"), StringToken(self.runner, "new_name")]))
