...
from typing import Any, List, Dict, Iterable, Optional, Literal, Union, ClassVar
...
from .rdata import to_table, de_dup
...
class RConnect(object):
    ...
    def engine(self, database: Optional[str] = None, **query: str) -> Engine:
        ...
        drivernames = de_dup(drivernames)
        ...
        if limit != None:
            if type(limit) in [str, int]:
                ...
                if len(limit) in [1, 2]:
                    ...
                    raise ValueError("The length of the limit parameter value must be 1 or 2")
