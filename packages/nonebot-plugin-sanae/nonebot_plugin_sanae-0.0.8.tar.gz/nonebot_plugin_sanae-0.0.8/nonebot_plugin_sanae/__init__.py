...
async def _(bot: Bot, event: Event):
    #把收到的信息整一整发过去
    asyncio.create_task(send_to_ws(bot.self_id,to_json(event),bot))
...
# 发送消息到 WebSocket 服务端
async def send_to_ws(self_id, message,bot):
    ...
    if ws_conn:
        try:
            ...
            # 发生连接重置错误，重新连接WebSocket
            while True:
                try:
                    # 关闭旧的连接
                    await ws_conn.close()
                    # 重新连接WebSocket
                    await asyncio.create_task(_setup(self_id, bot.config))
                    logger.info(f"重建连接到早苗服务器ing请稍等,维护结束后会自动连接成功")
                    ws_conn = ws_dict.get(self_id)
                    # 发送消息到新的WebSocket连接
                    await ws_conn.send_str(message)
                    break
