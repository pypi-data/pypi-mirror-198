...
from EVMVerifier.certoraType import TypeLocation
...
class FinderGenerator(object):
    ...
    def normalize_arg(self, idx: int, ty: CT.TypeInstance, arg_name: str) -> Optional[str]:
        ...
        if ty.get_source_str() is None:
            ...
            return None
        self.alpha_renamings.append(f"{ty.get_source_str()} {renamed} = {arg_name};")
        return renamed
...
def generate_modifier_finder(f: Func, internal_id: int, sym: int, compiler_collector: CompilerCollectorSol) -> \
    ...
    if last_loggable_arg is None:
        ...
        modifier_body += f"({logged_ty.get_source_str()} {logged_name}) {{ "
        ...
        if finder_gen.is_decomposed(logged_ty):
            ...
            to_log = \
                compiler_collector.normalize_storage(getattr(logged_ty, "location", None) == CT.TypeLocation.STORAGE,
                                                     alpha_renamed)
