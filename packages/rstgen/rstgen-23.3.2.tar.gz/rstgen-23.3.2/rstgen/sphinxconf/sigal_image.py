...
class Format(object):
    ...
    def update_context(self, caption, tplkw):
        ...
        tplkw.update(style="; ".join(styles))
...
class Thumb(Format):
    ...
    def update_context(self, caption, tplkw):
        ...
        if len(chunks) == 1:
            pass
            # self.float = "right"
            # tplkw['style'] = "padding:4px; float:right; height:{};".format(self.height)
        elif len(chunks) == 2:
        ...
        super().update_context(caption, tplkw)
...
def parse_image_spec(name, **kwargs):
    """
                ...
                if fmt is None:
                    raise Exception(
                        "Invalid format name '{}' (allowed names are {}).".format(
                            fmtname, tuple(FORMATS.keys())))
    fmt.update_context(caption, kwargs)
    return name, kwargs
...
def line2html(name, buildurl=buildurl):
    ...
    name, kw = parse_image_spec(name, **kw)
    if ' ' in name:
