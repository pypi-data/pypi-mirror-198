...
class CheckModel:
    ...
    def __init__(self, model_name, model_path):  # pragma: no cover
    ...
    def check_model_arguments(self):
        ...
        try:
            ...
            self.message = "Model file not provided as per docs: MyModel function receives no arguments"
            raise
    ...
    def is_model_supported(self):
        ...
        if supported:  # pragma: no cover
            ...
            if not hasattr(model, "input_shape"):
                self.message = "Model file not provided as per docs: unsupported API used for Model"  # pragma: no cover
    ...
    def layer_instance_check(self):
        ...
        for layer in self.model.layers:
            if not isinstance(layer, tf.keras.layers.Layer):
                self.message = "Layers in Model are not supported by Tensorflow"  # pragma: no cover
                raise Exception("invalid layer")  # pragma: no cover

    def small_training_loop(self, custom_loss=False):
        try:
            ...
            if custom_loss:
                ...
                serialized_loss = self.request.data["function_data"]
                decoded_loss = base64.b64decode(serialized_loss.encode("utf-8"))
                # Deserialize the data using dill
                deserialize_loss = dill.loads(decoded_loss)
                loss = deserialize_loss()
            ...
            self.message = "Model not support training on image classification dataset."
            raise
    ...
    def model_func_checks(self):
        ...
        if not eligible:
            return eligible, message, None  # pragma: no cover
        return True, self.message, self.model_name
