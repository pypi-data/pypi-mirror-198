...
class LinkModelDataSet:
    ...
    def trainingDatasetSize(self, training_dataset: dict):
        """
        ...
        trainingObject.trainingDatasetSize({'car': 30, 'person': 30})
        """
        checkeligible = True
    ...
    def learningRate(self, learningRate: dict):
        ...
        trainingObject.learningRateCustom({'type': 'constant', 'value': custom_LearningRate_schedular})
                ...
                if callable(lossFunction[VALUE]):
                    try:
                        ...
                        re = requests.post(
                            ...
                            headers=header,
                            data={
                                "type": "custom_function_test",
                                "function_type": "loss",
                                "function_data": encoded_data,
                            },
                        )
                        body_unicode = re.content.decode("utf-8")
                        content = json.loads(body_unicode)
                        if re.status_code == 202:
                            ...
                            error_msg = f"custom loss provided give error as {content['text']}\n"
                            self.__print_error(error_msg)
                            return
                    except:
                        error_msg = f"Please provide supported loss functions or custom loss build with tensorflow supported default losses: {lossFunction}\n"
                        self.__print_error(error_msg)
            else:
