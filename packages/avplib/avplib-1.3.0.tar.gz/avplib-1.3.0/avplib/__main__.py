...
def cav(video_path: str, res: Tuple[int, int], fps: int, threading: bool):
    if sum(res) > 0:
...
def convert2avf(from_video_path: str, to_video_path: str, res: Tuple[int, int], fps: int):
...
def play_avf(ascii_video_path: str) -> None:
    st = time.time()
    with Progress(transient=True) as pr:
