...
class ElementWithoutIntermediateFrequencyError(_InvalidConfigChangeError):
    @staticmethod
    def _build_from_response(request, response) -> "ElementWithoutIntermediateFrequencyError":
...
def _handle_job_manager_error(
    request: Union[InsertInputStreamRequest, SetElementCorrectionRequest, GetElementCorrectionRequest],
    response: Union[InsertInputStreamResponse, SetElementCorrectionResponse, GetElementCorrectionResponse],
...
def _get_handle_config_query_error(request, response):
    ...
    if error_type == ConfigQueryErrorTypes.MissingElementError:
        ...
        exception_to_raise = MissingDigitalInputError._build_from_response(request, response)
    else:
...
def _get_handle_job_operation_error(request, response):
    ...
    if error_type == JobOperationSpecificErrorTypes.SingleInputElementError:
        exception_to_raise = ElementWithSingleInputError._build_from_response(request, response)
    elif error_type == JobOperationSpecificErrorTypes.InvalidCorrectionMatrixError:
        exception_to_raise = InvalidElementCorrectionError._build_from_response(request, response)
    elif error_type == JobOperationSpecificErrorTypes.ElementWithoutIntermediateFrequencyError:
        exception_to_raise = ElementWithoutIntermediateFrequencyError._build_from_response(request, response)
    elif error_type == JobOperationSpecificErrorTypes.InvalidDigitalInputThresholdError:
        exception_to_raise = InvalidDigitalInputThresholdError._build_from_response(request, response)
    elif error_type == JobOperationSpecificErrorTypes.InvalidDigitalInputDeadtimeError:
        exception_to_raise = InvalidDigitalInputDeadtimeError._build_from_response(request, response)
    elif error_type == JobOperationSpecificErrorTypes.InvalidDigitalInputPolarityError:
        exception_to_raise = InvalidDigitalInputPolarityError._build_from_response(request, response)
    else:
