...
def generate_html(body,dirname,max_num=100,span=None):
    ...
    def write_element_icon(urlt,namet):
        ...
        return  ts.format(nn = namet,infojpg=f'builtin_icon_{typname}.png')
...
<body>
    <form>
        ...
        <div class="actions">
            <div>
                <button type="button"  id="submitBtn1" onclick="submit_form()" data-url="/submit_func?action=rename">重命名</button>
                ...
                <button type="button"  id="submitBtn4" onclick="submit_form()" data-url="/submit_func?action=delete">删除</button>
            </div>
        </div>
    </form>
    <div class="button-wrapper">
        ...
        <div class="list-wrapper">
            <ul class="list">
                <li class="show"><input type="checkbox" class="checkbox_settings" name="__settings__hide_file">隐藏文件</li>
                <li class="hide"><input type="checkbox" class="checkbox_settings" name="__settings__sort_name">名称排序</li>
                <li class="sort"><input type="checkbox" class="checkbox_settings" name="__settings__sort_type">类型排序</li>
            </ul>
        </div>
    </div>
    <script>
        ...
        li.innerHTML = `
            <input type="checkbox" class="checkbox" name="${{file.name}}">
            <img src="${{file.icon}}" alt="${{file.name}}" class="icon">
...
'''
    ...
    if global_setting['icon']:
        ...
        t = '\n'.join(a)
        html_bytes.write(file_list_html_str.format(flist=t).encode('utf8'))
    else:
...
class FileSystem:
    def GET(self,*d):
        ...
        for i in x:
            filename=p+i
            if not user_icon and yxsfile.yxsFile(i).decode_filename().name.startswith('__'):
                continue
