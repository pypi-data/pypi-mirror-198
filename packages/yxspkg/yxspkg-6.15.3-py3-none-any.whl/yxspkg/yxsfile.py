...
class yxsFile:
    ...
    def play_video(self):
        os.chdir(self.filename.absolute().parent)
        url = 'http://0.0.0.0:9090/file_downloader/x./'+quote(self.filename.name)
        ...
        if stat == 0:
            ...
            td.join()
    def view_image(self):
        songziviewer.main(self.filename)
    ...
    def file_size(self):
        if self.is_pureFile:
            ...
            stem = self.filename.stem
            if stem.endswith('.3'):
                t = open(self.filename,'rb').read(self.title_size)
                head_info = parse_header(t,self.passwd)
                size = head_info['len']
            else:
                size = self.filename.stat().st_size - self.title_size
            return size
    ...
    def get_head_info(self):
        if self._head_info is None:
            tts = open(self.filename,'rb').read(self.title_size)
            if self.filename.stem[-2] != '.':
                self.passwd = self.passwd_v1
            self._head_info = parse_header(tts,self.passwd)
