        ...
        if recommendations == True:
            ...
            for i in range(len(recommendations_table)):
                ...
                recommendations_table.no_recommend_flag_usage.iloc[i] == 0) == True:
                    ...
                    output = str('There is no recommendation for the device ' + recommendations_table.friendly_name.iloc[i] + ' .')
                    print(output)
                    return(output)

            return(explaination_activity)

            if self.diagnostics == False:
                output2 = 'For detailed information switch on the diagnostics parameter.'
                print(output2)
                return(output2)

        else:
            return('There are no recommendations for today.')
