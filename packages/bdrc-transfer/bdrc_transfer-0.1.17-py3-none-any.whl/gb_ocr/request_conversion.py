...
def request_conversion(volumes: [str]):
    ...
    processed_volumes: [] = [ x for x in gb.process_volumes(volumes) if not x.startswith('Barcode')]

    # Filter the banners - process_volumes
    # Drop the first row of output- it's just a banner
    if not processed_volumes:
        logging.error(f"Could not retrieve gb_process_volumes result from f{volumes}")
        return
