...
@singleton
class DrillholeModel(WrapperBase):
  """Provides access to functions available from the drillholemodel.dll"""
  def __init__(self):
    super().__init__("drillholemodel", "mapteksdk.capi.drillholemodel")
  ...
  def capi_functions(self):
    return [
      ...
      # Functions changed in version 1.
      {
        ...
        "DrillholeModelCloseDrillholeInformation" : (ctypes.c_void_p, [S_DrillholeInformation, ]),
  ...
  def SetDatabaseDesurveyMethod(self, lock, desurvey_method, length, tolerance):
    ...
    length: float
      The length to use for the "Tangent with length" desurvey method.
    tolerance: float
      The tolerance to use for the "Tangent with length" desurvey method.
    """
