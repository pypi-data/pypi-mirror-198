...
class Raster(DataObject):
  ...
  def __init__(
      ...
      height: int=None,
      *,
      image: str | Image.Image | pathlib.Path=None):
    ...
    self.__pixels_2d: np.ndarray | None = None
    initial_pixels = None
    if self.__is_new:
      ...
      try:
        ...
        object_id = ObjectID(Visualisation().NewRaster2D(
    ...
    def read_pixel_count(lock):
      ...
      width, height = Visualisation().ReadRaster2DDimensions(lock)
      return width * height

    # This doesn't set save_function because
    # Visualisation().SetRaster2DPixels does not return a RW array, so
    ...
    self.__pixels: DataProperty = DataProperty(
      ...
      DataPropertyConfiguration(
        ...
        default=0,
        column_count=4,
        load_function=Visualisation().GetRaster2DPixels,
        save_function=None,
  ...
  @pixels.setter
  def pixels(self, new_pixels: np.ndarray):
    self.__pixels.values = new_pixels

  @property
  def pixels_2d(self) -> np.ndarray:
  ...
  @pixels_2d.setter
  def pixels_2d(self, new_pixels_2d: np.ndarray):
  ...
  def _save_registration_two_point(
      self,
      image_points: np.ndarray,
      world_points: np.ndarray,
      orientation: np.ndarray):
    """Saves two point registration information for the raster.
  ...
  def _save_registration_multi_point(
      self,
      image_points: np.ndarray,
      world_points: np.ndarray):
    if image_points.shape[0] != world_points.shape[0]:
  ...
  def _load_registration_information(self) -> RasterRegistration:
    ...
    if registration_type == 8:
      # :TODO: 2021-04-22 SDK-484 Implement scan association similar to the
      # PointStudio transaction.
...
class PointPairRegistrationBase(RasterRegistration):
  ...
  def image_points(self) -> np.ndarray:
    ...
    bottom left hand corner of the image and [width, height] is the top right
  ...
  @image_points.setter
  def image_points(self, value: np.ndarray | None):
    if value is None:
  ...
  @world_points.setter
  def world_points(self, value: np.ndarray | None):
    if value is None:
