...
class GridSurface(Topology, PointProperties, CellProperties):
  ...
  faster on them, however they are less versatile. They cannot represent surfaces
  with disconnected parts or holes in the surface (unless the grid surface
  self-intersects).
  ...
  @staticmethod
  def _generate_point_grid(major_dimension_count, minor_dimension_count,
  ...
  @property
  def id(self) -> ObjectID["GridSurface"]:
    return super().id
