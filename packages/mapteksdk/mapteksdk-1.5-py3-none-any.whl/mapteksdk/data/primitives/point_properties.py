...
if typing.TYPE_CHECKING:
  from ...common.typing import (
    PointArray, BooleanArray, ColourArray, FloatArray, IndexArray)
...
class PointProperties:
  ...
  """
  __points: DataProperty
  __point_colours: DataProperty
  __point_selection: DataProperty
  __point_visibility: DataProperty
  ...
  def _initialise_point_properties(self, known_point_count: bool):
    ...
    if known_point_count:
      ...
      set_point_count = Modelling().SetPointCount
    ...
    self.__points = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        save_function=Modelling().PointCoordinatesBeginRW,
    ...
    self.__point_colours = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        load_function=Modelling().PointColourBeginR,
        save_function=Modelling().PointColourBeginRW,
        is_colour_property=True,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )

    self.__point_visibility = DataProperty(
      lock=self._lock,
      configuration=DataPropertyConfiguration(
        ...
        primitive_count_function=Modelling().ReadPointCount,
        cached_primitive_count_function=lambda: self.point_count,
        load_function=Modelling().PointVisibilityBeginR,
        save_function=Modelling().PointVisibilityBeginRW,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )
    ...
    self.__point_selection = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        save_function=Modelling().PointSelectionBeginRW,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )
  ...
  def point_colours(self, point_colours: npt.ArrayLike):
    self.__point_colours.values = point_colours
  ...
  def point_visibility(self) -> BooleanArray:
    ...
    """
    return self.__point_visibility.values

  @point_visibility.setter
  def point_visibility(self, point_visibility: npt.ArrayLike):
    self.__point_visibility.values = point_visibility
  ...
  def point_selection(self, point_selection: npt.ArrayLike):
    self.__point_selection.values = point_selection
  ...
  def point_count(self) -> int:
    ...
    if not self.__points.are_values_cached:
      return Modelling().ReadPointCount(self._lock.lock)
    return self.points.shape[0]

  @property
  def _are_points_cached(self) -> bool:
    """Allow subclasses to query if the points are cached."""
    return self.__points.are_values_cached
  ...
  def _invalidate_point_properties(self):
    ...
    self.__point_visibility.invalidate()
    self.__point_attributes = None
    self.__point_z = None
  ...
  def _save_point_properties(self):
    ...
    if not self.__points.read_only:
      if self.point_count == 0:
        message = "Object must contain at least one point."
        raise DegenerateTopologyError(message)
      self.__points.save()
    self.__point_colours.save()
    self.__point_visibility.save()
    self.__point_selection.save()
  ...
  def save_point_attribute(self, attribute_name: str, data: npt.ArrayLike):
  ...
  def delete_point_attribute(self, attribute_name: str):
...
class PointDeletionProperties:
  ...
  _invalidate_properties: typing.Callable[[], None]

  def remove_points(self, point_indices: npt.ArrayLike):
    ...
    if isinstance(point_indices, int):
      ...
      if not isinstance(point_indices, np.ndarray):
        point_indices = np.array(point_indices, dtype=np.uint32)
  ...
  def _remove_points(self, point_indices: IndexArray):
