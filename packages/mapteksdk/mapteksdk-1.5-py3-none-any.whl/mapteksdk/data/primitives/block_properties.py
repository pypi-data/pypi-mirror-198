...
class BlockProperties:
  ...
  """
  __block_visibility: DataProperty
  __block_colours: DataProperty
  __block_centroids: DataProperty
  __block_dimensions: tuple[int, int, int] | None
  __block_selection: DataProperty
  ...
  def _initialise_block_properties(self, has_immutable_blocks: bool):
    ...
    if has_immutable_blocks:
      ...
      set_block_count = Modelling().SetBlockCount
    ...
    self.__block_centroids = DataProperty(
      lock=self._lock,
      configuration=DataPropertyConfiguration(
        ...
        default=np.nan,
        column_count=3,
        primitive_count_function=Modelling().ReadBlockCount,
        load_function=Modelling().BlockCentroidsBeginR,
        save_function=Modelling().BlockCentroidsBeginRW,
    ...
    self.__block_sizes = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        column_count=3,
        primitive_count_function=Modelling().ReadBlockCount,
        load_function=Modelling().BlockSizesBeginR,
        save_function=Modelling().BlockSizesBeginRW,
    ...
    self.__block_visibility = DataProperty(
      lock=self._lock,
      configuration=DataPropertyConfiguration(
        ...
        primitive_count_function=Modelling().ReadBlockCount,
        cached_primitive_count_function=lambda: self.block_count,
        load_function=Modelling().BlockVisibilityBeginR,
        save_function=Modelling().BlockVisibilityBeginRW,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )
    ...
    self.__block_colours = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        primitive_count_function=Modelling().ReadBlockCount,
        cached_primitive_count_function=lambda: self.block_count,
        load_function=Modelling().BlockColourBeginR,
        save_function=Modelling().BlockColourBeginRW,
        is_colour_property=True,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )

    self.__block_selection = DataProperty(
      lock=self._lock,
      configuration=DataPropertyConfiguration(
        ...
        primitive_count_function=Modelling().ReadBlockCount,
        cached_primitive_count_function=lambda: self.block_count,
        load_function=Modelling().BlockSelectionBeginR,
        save_function=Modelling().BlockSelectionBeginRW,
        raise_on_error_code=Modelling().RaiseOnErrorCode
      )
    )
  ...
  def block_count(self) -> int:
    ...
    if self.__block_centroids.are_values_cached:
  ...
  def block_centroids(self) -> PointArray:
    ...
    """
    return self.__block_centroids.values
  ...
  def _set_block_centroids(self, new_centroids: npt.ArrayLike):
    ...
    """
    self.__block_centroids.values = new_centroids
  ...
  def block_selection(self) -> BooleanArray:
    ...
    block models.
    """
    return self.__block_selection.values

  @block_selection.setter
  def block_selection(self, block_selection: npt.ArrayLike):
    self.__block_selection.values = block_selection
  ...
  def block_visibility(self) -> BooleanArray:
    ...
    block models.
    """
    return self.__block_visibility.values

  @block_visibility.setter
  def block_visibility(self, block_visibility: npt.ArrayLike):
    self.__block_visibility.values = block_visibility
  ...
  def origin(self, new_origin: npt.ArrayLike):
    ...
    if new_origin is None:
      ...
      if (not self.__block_centroids.read_only
  ...
  def _invalidate_block_properties(self):
    ...
    the project.
    """
    self.__block_visibility.invalidate()
    self.__block_colours.invalidate()
    self.__block_centroids.invalidate()
    self.__block_dimensions = None
    self.__block_selection.invalidate()
  ...
  def _save_block_properties(self):
    ...
    if not self.__block_centroids.read_only:
      ...
      self.__block_centroids.save()
      self.__block_sizes.save()

    self.__block_selection.save()
    self.__block_visibility.save()
  ...
  def save_block_attribute(self, attribute_name: str, data: npt.ArrayLike):
  ...
  def delete_block_attribute(self, attribute_name: str):
...
class BlockDeletionProperties(BlockProperties):
  ...
  def remove_block(self, index: int):
    ...
    Modelling().RemoveBlock(self._lock.lock, index)
