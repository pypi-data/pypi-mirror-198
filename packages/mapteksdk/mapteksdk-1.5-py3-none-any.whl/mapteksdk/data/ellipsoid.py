...
class Ellipsoid(Topology, RotationMixin):
  ...
  * The dip is -30 degrees.
  ...
  def __init__(self, object_id=None, lock_type=LockType.READWRITE):
    if not object_id:
      object_id = ObjectID(Modelling().NewEllipsoid())
  ...
  def size(self) -> tuple[float, float, float]:
    ...
    if self.__size is None:
      self.__size = Modelling().GetEllipsoidSize(self._lock.lock)
    return self.__size
  ...
  def centre(self) -> np.ndarray:
    ...
    if self.__centre is None:
      ...
      centre[:] = Modelling().GetEllipsoidCentre(self._lock.lock)
  ...
  @property
  def id(self) -> ObjectID[Ellipsoid]:
    return super().id

  def _get_rotation(self):
    return Rotation(*Modelling().GetEllipsoidRotation(self._lock.lock))
  ...
  def _save(self):
    if self.__size is not None:
      ...
      Modelling().SetEllipsoidSize(self._lock.lock, major, semi_major, minor)
    if self.__centre is not None:
      x, y, z = self.centre
      Modelling().SetEllipsoidCentre(self._lock.lock, x, y, z)
    if self._rotation_cached:
      q0, q1, q2, q3 = self._rotation.quaternion
      Modelling().SetEllipsoidRotation(self._lock.lock, q0, q1, q2, q3)
