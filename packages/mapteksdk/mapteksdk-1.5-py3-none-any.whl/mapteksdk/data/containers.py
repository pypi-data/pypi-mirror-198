...
class Container(DataObject):
  ...
  def save(self):
    ...
    CannotSaveInReadOnlyModeError
      If the object was open for read only (i.e not for editing).
      It not necessary to call this for a read only object as there will be no
    ...
    # Ideally, this would call Checkpoint() on the object however that
    # function is not exposed. For now, that behaviour can be emulated by
    # closing the object and reopening it.
  ...
  @property
  def id(self) -> ObjectID["Container"]:
    return super().id
  ...
  def append(self, child: tuple[str, ObjectID]):
    ...
    DataEngine().ContainerAppend(self._lock.lock, name.encode('utf-8'),
  ...
  def insert(self, index: int, child: tuple[str, ObjectID]):
    ...
    DataEngine().ContainerInsert(self._lock.lock, insertion_iterator,
                                 name.encode('utf-8'),
                                 oid.handle, force_primary_parenting)
  ...
  def remove(self, name: str) -> ObjectID[DataObject]:
    ...
    # Make the change - this won't be visible until the object is closed.
    DataEngine().ContainerRemove(self._lock.lock, name.encode('utf-8'))
  ...
  def replace(self,
    ...
    DataEngine().ContainerReplaceElement(
  ...
  def rename(self, old_name: str, new_name: str):
    ...
    if (not self.allow_standard_containers and
      ...
      DataEngine().ContainerAppend(
        self._lock.lock, new_name.encode('utf-8'), new_container.id.handle,
        force_primary_parenting)
    else:
      self._current_children[index] = (new_name, object_id)

      # Make the change - this won't be visible until the object is closed.
      element = DataEngine().ContainerFindElement(
        self._lock.lock, old_name.encode('utf-8'))

      delete_if_orphaned = False
      insertion_point = DataEngine().ContainerRemoveElement(
        self._lock.lock, element, delete_if_orphaned)

      force_primary_parenting = False
      DataEngine().ContainerInsert(
  ...
  def clear(self):
    ...
    if self.allow_hidden_objects and self.allow_standard_containers:
      ...
      DataEngine().ContainerPurge(lock)
    else:
      ...
      for name, oid in self._current_children:
        DataEngine().ContainerAppend(
          lock, name.encode('utf-8'), oid.handle, True)
  ...
  def __contains__(self, item: str | ObjectID | DataObject) -> bool:
  ...
  def _children(self, include_hidden: bool):
    ...
    iterator = DataEngine().ContainerBegin(lock)
    end = DataEngine().ContainerEnd(lock)
    while iterator.value != end.value:
      ...
      DataEngine().ContainerElementName(lock,
                                        iterator,
                                        name_buffer,
                                        buf_size)
      name = name_buffer.value.decode("utf-8")

      # This check could be deferred to ChildView class to handle, so all the
      # children are queried and cached before being filtered.
      if _should_include_child(name):
        object_id = ObjectID(
          DataEngine().ContainerElementObject(lock, iterator))

        children.append((name, object_id))

      iterator = DataEngine().ContainerNextElement(lock, iterator)
  ...
  def _iterator_from_index(self, index):
    ...
    """
    lock = self._lock.lock
    begin = DataEngine().ContainerBegin(lock)
    end = DataEngine().ContainerEnd(lock)
...
class VisualContainer(Container):
  ...
  @property
  def id(self) -> ObjectID["VisualContainer"]:
    return super().id
...
class StandardContainer(VisualContainer):
  ...
  @property
  def id(self) -> ObjectID["StandardContainer"]:
    return super().id
