...
class Edge(EdgeProperties, PointProperties, PointDeletionProperties, Topology):
...
class EdgeNetwork(Edge):
  ...
  Examples
  --------
  Creating an edge network with an edge between points 0 and point 1 and a second
  edge edge between points 2 and 3.
  ...
  @property
  def id(self) -> ObjectID["EdgeNetwork"]:
    return super().id
  ...
  def remove_edges(self, edge_indices):
...
class Polyline(Edge):
  ...
  @property
  def id(self) -> ObjectID["Polyline"]:
    return super().id

  @property
  def edges(self):
    # Polyines and Polygons generate their edges in Python, so the __edges
