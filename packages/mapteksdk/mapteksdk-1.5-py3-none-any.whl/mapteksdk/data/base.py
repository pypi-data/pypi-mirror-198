...
from ..internal.colour_map_information import (
  ColourMapInformation, NO_COLOUR_MAP)
...
class DataObject:
  ...
  def set_attribute(
      ...
      name: str,
      dtype: type[
        ObjectAttributeTypes | datetime.datetime | datetime.date] | None,
      data: typing.Any):
    ...
    Warnings
    --------
    Object attributes are saved separately from the object itself - any changes
    made by this function (assuming it does not raise an error) will be saved
    even if save() is not called (for example, due to an error being raised by
    another function).
...
class Topology(DataObject):
  ...
  def __init__(self, object_id: ObjectID, lock_type: LockType, *,
    ...
    self.__colour_map_information: (
      ColourMapInformation | typing.Literal["NO_COLOUR_MAP"] | None) = None
  ...
  def get_colour_map(self) -> ObjectID[ColourMap]:
    ...
    ObjectID
      ...
      no colour map.
    """
    if self.__colour_map_information is None:
      self.__colour_map_information = self.__get_colour_map_information()

    if self.__colour_map_information == NO_COLOUR_MAP:
      return ObjectID()
    return self.__colour_map_information.colour_map_id
  ...
  def _set_colour_map(
    ...
    such checks.
    """
    self.__colour_map_information = ColourMapInformation(
      attribute_name, primitive_type, colour_map
    )

  def _colour_map_information(self) -> ColourMapInformation | None:
    """Get the colour map information for this object.

    Returns
    -------
    ColourMapInformation
      If there is a colour map associated with this object.
    None
    ...
    return self.__colour_map_information
  ...
  def _invalidate_colour_map(self):
    ...
    """
    self.__colour_map_information = None
  ...
  def coordinate_system(self) -> CoordinateSystem | None:
    ...
    """
    wkt, c_local_transform = Modelling().GetCoordinateSystem(self._lock.lock)
    if wkt != "":
  ...
  def coordinate_system(self, value: CoordinateSystem):
    self._raise_if_read_only("set coordinate system")
  ...
  def __get_colour_map_information(self
      ) -> ColourMapInformation | typing.Literal["NO_COLOUR_MAP"]:
    """Get the colour map information for this object.

    Returns
    -------
    ColourMapInformation
      If a colour map is associated with this object.
    NO_COLOUR_MAP
      If no colour map is associated with this object.
    """
    read_type = Modelling().GetDisplayedAttributeType(
      self._lock.lock)
    ...
    colour_map_handle = Modelling().GetDisplayedColourMap(self._lock.lock)
    colour_map_id = ObjectID(colour_map_handle)

    if not colour_map_id:
      return NO_COLOUR_MAP

    return ColourMapInformation(
      attribute_name=name,
      primitive_type=actual_type,
      colour_map_id=colour_map_id
    )

  def __save_colour_map_information(self):
    """Save the colour map information.

    This does nothing if the colour map is not cached or if no
    colour map is associated with the object.
    ...
    colour_map_info = self.__colour_map_information
    save_function = save_functions[colour_map_info.primitive_type]
    save_function(
      self._lock.lock,
      colour_map_info.attribute_name.encode("utf-8"),
