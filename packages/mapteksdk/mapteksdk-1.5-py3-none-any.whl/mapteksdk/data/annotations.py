...
class Text(Topology):
  ...
  def __init__(
    ...
    self.__points = DataProperty(
      ...
      configuration=DataPropertyConfiguration(
        ...
        set_primitive_count_function=Modelling().SetPointCount,
  ...
  def size(self) -> float:
    ...
    if self.__size is None:
      self.__size = self._get_text_size()
    return self.__size
  ...
  def colour(self) -> Colour:
    ...
    if self.__colour is None:
      self.__colour = np.array(self._get_text_colour(), dtype=ctypes.c_uint8)
...
class Marker(Topology, RotationMixin):
  ...
  def shape(self) -> Shape:
    ...
    if self.__shape is None:
      self.__shape = Marker.Shape(Modelling().GetMarkerStyle(self._lock.lock))
    return self.__shape
  ...
  def size(self) -> float:
    ...
    """
    if self.__size is None:
      self.__size = Modelling().GetAnnotationSize(self._lock.lock)
    return self.__size
  ...
  def height(self) -> float:
    ...
    if self.__height is None:
      self.__height = Modelling().GetMarkerHeight(self._lock.lock)
  ...
  def text_colour(self) -> Colour:
    ...
    if self.__text_colour is None:
      self.__text_colour = np.array(self._get_text_colour(), ctypes.c_uint8)
  ...
  def _save(self):
    ...
    if self.__height is not None:
      Modelling().SetMarkerHeight(self._lock.lock, self.__height)
