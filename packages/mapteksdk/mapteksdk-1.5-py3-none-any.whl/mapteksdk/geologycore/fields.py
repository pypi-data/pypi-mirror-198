...
from ..data.units import DistanceUnit, AngleUnit, UnsupportedUnit, NO_UNIT
...
class DrillholeFieldType(enum.Enum):
  ...
  def supports_data_type(self, data_type: np.dtype) -> bool:
  ...
  @property
  def default_unit(self) -> DistanceUnit | AngleUnit | NO_UNIT:
...
def _dtype_to_string(dtype: np.dtype) -> str:
  ...
  """
  dictionary = {
    np.dtype(np.bool_) : FIELD_TYPE_CONSTANTS.BOOLEAN,
    np.dtype(np.int32) : FIELD_TYPE_CONSTANTS.INTEGER_32_S,
    np.dtype(np.float32) : FIELD_TYPE_CONSTANTS.FLOAT,
    np.dtype(np.float64) : FIELD_TYPE_CONSTANTS.DOUBLE,
    np.dtype(np.str_) : FIELD_TYPE_CONSTANTS.STRING,
    np.dtype(np.datetime64) : FIELD_TYPE_CONSTANTS.DATETIME
  }

  try:
    return dictionary[dtype]
  except KeyError:
    LOG.warning("Encountered unknown data type: %s", dtype)
    return UNKNOWN_FIELD_TYPE
...
class BaseField:
  ...
  @property
  def unit(self) -> DistanceUnit | AngleUnit | UnsupportedUnit | NO_UNIT:
  ...
  def _determine_unit_built_in_field(
      self,
      unit: str | DistanceUnit | AngleUnit | NO_UNIT | UnsupportedUnit
      ) -> DistanceUnit | AngleUnit | NO_UNIT:
  ...
  def _determine_unit(
      self,
      unit: str | DistanceUnit | AngleUnit | NO_UNIT | UnsupportedUnit | None
      ) -> DistanceUnit | AngleUnit | NO_UNIT | UnsupportedUnit:
...
class FieldInformation(BaseField):
  ...
  def unit(self, value: DistanceUnit | AngleUnit | NO_UNIT):
    if self.__deleted:
      raise DeletedFieldError(self)
...
class DrillholeDatabaseField(BaseField):
  ...
  def _values_cached(self) -> bool:
    return not self.__values is None
  ...
  @values.setter
  def values(self, new_values : np.ma.MaskedArray):
