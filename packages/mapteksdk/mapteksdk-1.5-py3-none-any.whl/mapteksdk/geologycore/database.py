...
class DrillholeDatabase(VisualContainer, TablesMixin):
  ...
  def _load_desurvey_method(self):
    ...
    try:
      desurvey_method_id, tangent_length, tangent_tolerance = \
        DrillholeModel().GetDatabaseDesurveyMethod(self._lock.lock)
  ...
  def save(self):
    ...
    if (self.__desurvey_method is not None
      ...
      DrillholeModel().SetDatabaseDesurveyMethod(
        self._lock.lock,
  ...
  def tangent_length(self) -> float:
    ...
    """
    if self.desurvey_method != DesurveyMethod.TANGENT_WITH_LENGTH:
      return math.nan
    if self.__tangent_length is None:
      self._load_desurvey_method()
    return self.__tangent_length

  @tangent_length.setter
  def tangent_length(self, new_length: float):
    if self.desurvey_method != DesurveyMethod.TANGENT_WITH_LENGTH:
      raise RuntimeError(
        "Can only set tangent length for "
        f"'{DesurveyMethod.TANGENT_WITH_LENGTH}', not '{self.desurvey_method}'."
      )
  ...
  def _tangent_tolerance(self) -> float:
    ...
    if self.__tangent_tolerance is None:
      self._load_desurvey_method()
    return self.__tangent_tolerance
