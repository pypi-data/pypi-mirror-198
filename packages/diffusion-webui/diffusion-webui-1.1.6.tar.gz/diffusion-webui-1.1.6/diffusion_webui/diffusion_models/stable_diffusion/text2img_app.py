...
class StableDiffusionText2ImageGenerator:
    ...
    def load_model(
        ...
        if self.pipe is None:
            ...
            if scheduler == "DDIM":
                ...
                self.pipe.scheduler = HeunDiscreteScheduler.from_config(
                    self.pipe.scheduler.config
                )
    ...
    def generate_image(
        ...
        if seed_generator == 0:
            ...
            generator = torch.Generator("cuda").manual_seed(seed_generator)
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    text2image_prompt = gr.Textbox(
                        lines=1, placeholder="Prompt", label="Prompt"
                    )
                        ...
                        with gr.Row():
                            with gr.Column():
                                ...
                                text2image_seed_generator = gr.Number(
                                    label="Seed Generator(0 for random)",
                                    placeholder="Seed Generator",
                                    min=0,
                                    max=1000000,
                                )
