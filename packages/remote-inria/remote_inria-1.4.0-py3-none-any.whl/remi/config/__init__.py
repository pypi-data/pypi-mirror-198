...
class Desktop:
    ...
    def from_dict(cls, args_dict: dict[str, Any]) -> 'Desktop':
        ...
        background: Background = Background.from_dict(
            args_dict=background_dict
        )
        assert background.backend in ('screen', 'tmux'), \
...
class Config(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        if not isfile(config_path):
            print_error(f"Configuration file (`{config_path}`) was not found ! Exiting."
        ...
        with open(config_path, 'r') as file_stream:
            config_dict: dict[str, Any] = yaml.load(
                file_stream.read(),
                yaml.FullLoader
            )
    ...
    def initialize_from_dict(self, config_dict: dict[str, Any]) -> None:
        ...
        self.exclude_file: str = config_dict.get(
            'exclude_file',
            join(_DOT_REMI_PATH, 'exclude.txt')
        )

        # Desktop
        self.desktop: Desktop = Desktop.from_dict(
            args_dict=config_dict.get('desktop')
        )
        ...
        # Bastion
        self.bastion: Bastion = Bastion.from_dict(args_dict=config_dict.get('bastion'))
        ...
        # Jupyter:
        jupyter: Jupyter = Jupyter.from_dict(
            args_dict=remote_servers_dict.get('jupyter', {})
        )
        ...
        # Aim:
        aim: Aim = Aim.from_dict(
            args_dict=remote_servers_dict.get('aim', {})
        )
