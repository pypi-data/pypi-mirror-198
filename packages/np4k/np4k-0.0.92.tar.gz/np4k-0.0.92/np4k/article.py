...
from .lib import is_paywalled
from .outputformatters import OutputFormatter
from np4k.utils import (URLHelper, RawHelper, extend_config,
...
class Article(object):
    ...
    """
    def __init__(self, *, url: str, title: str = '', source_url: str = '', config: Configuration = None, **kwargs):
        ...
        if not source_url or source_url == '':
            ...
            if scheme is None:
                scheme = 'http'
            source_url = f"{scheme}://{urls.get_domain(url)}"
    ...
    def build(self, *, legacy_nlp: bool = False):
        """Build a lone article from a URL independent of the source (np4k).
        Don't normally call this method b/c it's good to multithread articles
        on a source (np4k) level.
        """
        self.download()
        self.parse()
        self.check_paywall()
    ...
    def _parse_scheme_file(self, *, path: str) -> Optional[str]:
        try:
    ...
    def _parse_scheme_http(self) -> Optional[str]:
        """
        Downloads the HTML content of the article's URL using HTTP(S) and returns it as a string.
        :return: The HTML content of the article's URL, or None if the download fails.
        """
        try:
    ...
    def download(self, input_html: Optional[str] = None, title: Optional[str] = None, recursion_counter: int = 0):
        """
        Downloads the article's HTML content from its URL, or uses the provided input HTML if it is provided.
        don't use if you are batch async.
        recursion_counter (currently 1) stops refreshes that are potentially infinite
        :param input_html: The HTML content of the article, if available.
        :param title: The title of the article, if available.
        :param recursion_counter: The number of times this method has been called recursively due to a meta-refresh tag.
        :return: None
        """
        if input_html is None:
            # If no input HTML is provided, download the HTML content from the URL.
            parsed_url = urlparse(self.url)
            if parsed_url.scheme == "file":
                html = self._parse_scheme_file(path=parsed_url.path)
            else:
                # If input HTML is provided, use that instead of downloading from the URL.
        ...
        if self.config.follow_meta_refresh:
            # If the config setting follow_meta_refresh is True, check for a meta-refresh tag in the HTML content.
            meta_refresh_url = extract_meta_refresh(html)
            if meta_refresh_url and recursion_counter < 1:
                # If a meta-refresh tag is found and the recursion counter is less than 1, recursively download the
                # HTML content from the meta-refresh URL.
    ...
    def parse(self) -> None:
        """
        Parses the article's HTML content and extracts various metadata.
        :return: None
        """
    ...
    def check_paywall(self) -> bool:
        """
        Attempts to check if the article is behind a paywall.
        :return: True if the article is behind a paywall, False if unknown.
        """
        paywalled = is_paywalled(text=self.title) or is_paywalled(text=self.text)
    ...
    def is_valid_url(self, source_domain: Optional[str] = None):
        """Performs a check on the url of this link to determine if article
        is a real news article or not
        """
        return urls.valid_url(self.url, source_domain=source_domain)
