...
class Source(object):
    ...
    def purge_articles(self, reason: str, articles):
    ...
    # pretty broken / not comprehensive. probably want to use a sitemap crawler
    ...
    def download_categories(self):
        ...
        for index, _ in enumerate(self.categories):
            ...
            if req.resp is not None:
                ...
                log.warning(f"Deleting category {self.categories[index].url} from source {self.url} due to download error")
    ...
    def parse(self):
        ...
        if self.doc is None:
            log.warning(f'Source {self.url} parse error.')
            return
