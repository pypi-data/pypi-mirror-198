...
class HiveHadoopConnConf(RelationalConnConf):
    ...
    def _equals(self, other: 'ConnConf') -> bool:
        if not isinstance(other, HiveHadoopConnConf):
            return False
        other = cast(HiveHadoopConnConf, other)
        return self.db_host == other.db_host \
               and self.db_name == other .db_name \
               and self.db_port == other.db_port \
               and self.user == other.user \
    ...
    def _apply_credentials_to(self, other: 'HiveHadoopConnConf'):
        assert isinstance(other, HiveHadoopConnConf), "apply credentials failed. other must be HiveHadoopConnConf"
        other = cast(HiveHadoopConnConf, other)
        other.db_host = self.db_host
        other.db_name = self.db_name
        other.db_port = self.db_port
...
class AWSElasticSearchConnConf(RelationalConnConf):
    ...
    def _equals(self, other: 'ConnConf') -> bool:
        if not isinstance(other, AWSElasticSearchConnConf):
            return False
        other = cast(AWSElasticSearchConnConf, other)
        return self.db_host == other.db_host \
               and self.db_port == other.db_port \
               and self.user == other.user \
               and self.password == other.password
    ...
    def _apply_credentials_to(self, other: 'AWSElasticSearchConnConf'):
        assert isinstance(other, AWSElasticSearchConnConf), "apply credentials failed. other must be AWSElasticSearchConnConf"
        other = cast(AWSElasticSearchConnConf, other)
        other.db_host = self.db_host
        other.db_port = self.db_port
        other.user = self.user
    ...
    def create_or_get(conn_conf_json_dict_or_obj) -> ConnConf:
        ...
        if isinstance(conn_conf_json_dict_or_obj, dict):
            ...
            if conn_type_str == PRTConn.LOCAL_FILE:
                ...
                conn_conf = HiveHadoopConnConf.from_dict(conn_conf_json_dict_or_obj)
                ...
                conn_conf = AWSElasticSearchConnConf.from_dict(conn_conf_json_dict_or_obj)
...
class ConnConfClassFactory:
    ...
    def get_conn_conf_class(conn_type: PRTConn) -> Union[
        ...
        Type[MSSQLConnConf],
        Type[OracleConnConf],
        Type[HiveHadoopConnConf],
        Type[AthenaConnConf],
        Type[ElasticSearchConnConf],
        Type[AWSElasticSearchConnConf],
        ...
        elif conn_type == PRTConn.HIVE_HADOOP:
            return HiveHadoopConnConf
        elif conn_type == PRTConn.ATHENA:
            return AthenaConnConf
        elif conn_type == PRTConn.ELASTIC_SEARCH:
            return ElasticSearchConnConf
        elif conn_type == PRTConn.AWS_ELASTIC_SEARCH:
            return AWSElasticSearchConnConf
