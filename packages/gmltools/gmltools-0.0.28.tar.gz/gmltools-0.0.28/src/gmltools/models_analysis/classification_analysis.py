...
class Ranalysis:
    ...
    def residual_analysis(self, train_or_test_or_val:str, remove_vars=None, figsize=(20, 10), bins=30, smooth_order=5):
        ...
        """
        sns.set()
        assert train_or_test_or_val.lower() in ['train', 'test'], "train_or_test must be either 'train' or 'test'"
        assert isinstance(figsize, tuple) and len(figsize) == 2, "figsize must be a tuple and figsize must be a tuple of length 2"
        ...
        nplots = df.shape[1]
        # Create subplots
        fig, axs = plt.subplots(np.floor(np.sqrt(nplots))+1, np.ceil(np.sqrt(nplots)), figsize=figsize)
