...
class ExploreManager:
    ...
    def plot_correlation(self,figsize=(1000,1000),colorscale='agsunset',title=None):
        """
    ...
    def plot_distribution(self, bins:int=50, figsize:tuple=(1000, 600), category_column=None, x_range=None, shared_yaxes:bool=False, shared_xaxes:bool=False, histnorm=None):
        """
        ...
        if len(numeric_columns) > 2: #For more than 2 columns the subplots are used with 3 columns
            n_cols = 3
        else: #For 2 columns ore less the subplots are used with 2 columns
            n_cols = len(numeric_columns)
        n_rows = math.ceil(len(numeric_columns) / n_cols)
        fig = make_subplots(rows=n_rows, cols=n_cols, shared_yaxes=shared_yaxes, shared_xaxes=shared_xaxes,subplot_titles=[ str(col) for col in numeric_columns])
        ...
        for i, col_ in enumerate(numeric_columns):
            ...
            if category_column is not None: #For categorical selection
                ...
                hist = px.histogram(self.df, x=col_, nbins=bins,histnorm=histnorm)
            fig.add_trace(hist.data[0], row=row, col=col)
            fig.update_xaxes(title_text=col, row=row, col=col, range=x_range)
            fig.update_yaxes(title_text="Probability Density", row=row, col=col)
            #add the subplot title

        fig.update_layout(height=figsize[1]*n_rows, width=figsize[0], title_text="Distribution of the data")
        fig.show()
    ...
    def plot_pairplot(self, y_var:list, hues=None,alpha=0.4,size=10,aspect=8/10):
        """
        ...
        if hues is None:
            hues=self.df.select_dtypes(include=['category']).columns.tolist()
        assert isinstance(hues, list), "hues must be a list, Puede que no haya variables categoricas"
        if y_var is str:
            y_var=[y_var]
        for hue in hues:
            sns.pairplot(data=self.df, y_vars=y_var, hue=hue,diag_kws=dict(alpha=alpha), plot_kws=dict(alpha=alpha),size=size,aspect=aspect)
            plt.show()
