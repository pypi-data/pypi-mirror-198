...
class StandingPutBody(BaseModel):
    ...
    def grouping_base_validate_enum(cls, v):
        ...
        if v not in ("OVERALL", "ROUND"):
            raise ValueError("must validate the enum values ('OVERALL', 'ROUND')")
        return v
    ...
    def grouping_conference_division_validate_enum(cls, v):
        ...
        if v not in ("OVERALL", "CONFERENCE", "DIVISION"):
            raise ValueError("must validate the enum values ('OVERALL', 'CONFERENCE', 'DIVISION')")
        return v
    ...
    def grouping_stage_pool_validate_enum(cls, v):
        ...
        if v not in ("OVERALL", "STAGE", "STAGEPOOL"):
            raise ValueError("must validate the enum values ('OVERALL', 'STAGE', 'STAGEPOOL')")
        return v
        ...
        _obj = StandingPutBody.parse_obj(
            {
                ...
                "points": dict(
                    (_k, Dict[str, StandingPostBodyPointsValue].from_dict(_v)) for _k, _v in obj.get("points").items()
                ),
                "calculated": dict(
                    (_k, Dict[str, StandingPostBodyCalculatedValue].from_dict(_v))
                    for _k, _v in obj.get("calculated").items()
                ),
