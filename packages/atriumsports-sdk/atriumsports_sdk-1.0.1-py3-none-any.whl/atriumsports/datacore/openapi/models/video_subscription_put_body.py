...
class VideoSubscriptionPutBody(BaseModel):
    ...
    def output_format_validate_enum(cls, v):
        ...
        if v not in ("RTMP", "RTMP_PULL", "HLS", "SRT"):
            raise ValueError("must validate the enum values ('RTMP', 'RTMP_PULL', 'HLS', 'SRT')")
        return v
    ...
    def audio_validate_enum(cls, v):
        ...
        if v not in ("COMMENTARY", "AMBIENCE", "BOTH", "BOTH_SPLIT_LR"):
            raise ValueError("must validate the enum values ('COMMENTARY', 'AMBIENCE', 'BOTH', 'BOTH_SPLIT_LR')")
        return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VideoSubscriptionPutBody:
        """Create an instance of VideoSubscriptionPutBody from a JSON string"""
