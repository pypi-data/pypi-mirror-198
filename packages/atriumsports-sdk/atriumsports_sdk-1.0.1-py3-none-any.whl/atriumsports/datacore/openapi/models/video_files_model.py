...
class VideoFilesModel(BaseModel):
    ...
    def origin_validate_enum(cls, v):
        ...
        if v not in ("STREAM", "VENUE", "UPLOAD"):
            raise ValueError("must validate the enum values ('STREAM', 'VENUE', 'UPLOAD')")
        return v
    ...
    def format_validate_enum(cls, v):
        ...
        if v not in ("HLS", "MP4"):
            raise ValueError("must validate the enum values ('HLS', 'MP4')")
        return v
    ...
    def storage_provider_validate_enum(cls, v):
        ...
        if v not in ("5STREAM", "KEEMOTION", "SYNERGY"):
            raise ValueError("must validate the enum values ('5STREAM', 'KEEMOTION', 'SYNERGY')")
        return v
    ...
    def status_validate_enum(cls, v):
        ...
        if v not in ("AVAILABLE", "BUILDABLE", "PENDING"):
            raise ValueError("must validate the enum values ('AVAILABLE', 'BUILDABLE', 'PENDING')")
        return v
