...
class PersonsModel(BaseModel):
    ...
    def status_validate_enum(cls, v):
        ...
        if v not in ("ACTIVE", "UNREGISTERED", "PENDING", "DECEASED", "INACTIVE"):
            raise ValueError(
                "must validate the enum values ('ACTIVE', 'UNREGISTERED', 'PENDING', 'DECEASED', 'INACTIVE')"
            )
        return v
    ...
    def gender_validate_enum(cls, v):
        ...
        if v not in ("MALE", "FEMALE", "UNKNOWN"):
            raise ValueError("must validate the enum values ('MALE', 'FEMALE', 'UNKNOWN')")
        return v
    ...
    def to_dict(self):
        ...
        if self.additional_names is None:
        ...
        _obj = PersonsModel.parse_obj(
            {
                ...
                "additional_names": dict(
                    (_k, Dict[str, PersonPostBodyAdditionalNamesValue].from_dict(_v))
