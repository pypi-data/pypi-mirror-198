...
class SoPBigThing(SoPThing):
    ...
    def _print_packet(self, topic: str, payload: str, mode: SoPPrintMode = SoPPrintMode.FULL, pretty: bool = False) -> str:

        def prune_payload(payload: str, mode: SoPPrintMode):
            if mode == SoPPrintMode.SKIP:
                return colored(f'skip... (print_packet mode={mode})', 'yellow')
        ...
        payload = prune_payload(
            payload=dict_to_json_string(dict_object=payload, pretty=pretty), mode=mode)

        SOPLOG_DEBUG(
            f'[{topic_indicator:20}] topic : {topic} payload : {payload}')
    ...
    def _publish(self, topic: str, payload, qos: int = 0):
        ...
        self._print_packet(topic=topic, payload=payload, mode=self._log_mode)
