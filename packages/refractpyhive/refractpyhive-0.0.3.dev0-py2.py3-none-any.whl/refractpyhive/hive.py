...
from refractpyhive import common
from refractpyhive.common import DBAPITypeObject
# Make all exceptions visible in this module per DB-API
from refractpyhive.exc import *  # noqa
...
def get_auth_header():
    import kerberos
    __, krb_context = kerberos.authGSSClientInit(service="hive@{0}".format(os.getenv('HIVE_HOSTNAME')))
...
class Connection(object):
    ...
    def __init__(
        ...
        try:
            ...
            with contextlib.closing(self.cursor()) as cursor:
                self.kerberos_connection(cursor, database)
    ...
    def kerberos_connection(self, cursor, database):
        try:
            cursor.execute('USE `{}`'.format(database))
        except:
            cursor._connection._transport.setCustomHeaders(
                headers={"Authorization": "Negotiate {auth_header}".format(auth_header=get_auth_header())})
...
class Cursor(common.DBAPICursor):
    ...
    def description(self):
        ...
        try:
            ...
            self._connection._transport.setCustomHeaders(
                headers={"Authorization": "Negotiate {auth_header}".format(auth_header=get_auth_header())})
...
def get_hive_connection(hive_config, source, database_name):
    ...
    if source == "hive":
        ...
        return connect(host=hive_config.get("KEREROS_SERVICE_ALIAS"),
