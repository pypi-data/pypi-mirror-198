...
def applyPhi(im, Phi=None, PhiCentre=None, interpolationOrder=1):
    ...
    if len(im.shape) == 2 or (numpy.array(im.shape) == 1).any():
        print(
            "DIC.deformationFunction.applyPhi(): looks like a 2D image which cannot be handled. Please use DIC.deformationFunction.applyPhiPython"
        )
        return
    ...
    if interpolationOrder > 1:
        print(
            "DIC.deformationFunction.applyPhi(): interpolation Order > 1 not implemented"
        )
        return
...
def applyPhiPython(im, Phi=None, PhiCentre=None, interpolationOrder=3):
    ...
    coordinatesInitial = numpy.ones(
        (4, im.shape[0] * im.shape[1] * im.shape[2]), dtype="<f4"
    )
    ...
    imDef += (
        scipy.ndimage.map_coordinates(im, coordinatesDef[0:3], order=interpolationOrder)
        .reshape(imDef.shape)
        .astype("<f4")
    )
    return imDef
...
def applyPhiField(
    ...
    if displacementMode == "interpolate":
        ...
        def computeDisplacementForSeriesOfPixels(seriesNumber):
            ...
            for localPixelNumber, globalPixelNumber in enumerate(pixelNumbers):
                ...
                pixCoordsRefSeries[localPixelNumber] = (
                    pixCoordDef + displacement / weights.sum()
                )
        ...
        splitPixNumbers = numpy.array_split(
            numpy.arange(pixCoordsDef.shape[0]), nProcesses
        )
        ...
        with multiprocessing.Pool(processes=nProcesses) as pool:
            for returns in pool.imap_unordered(
                computeDisplacementForSeriesOfPixels, range(nProcesses)
            ):
    ...
    if imMaskDef is not None:
        ...
        imDef[imMaskDef] = scipy.ndimage.map_coordinates(
            im, pixCoordsRef.T, mode="constant", order=interpolationOrder
        )

    else:  # no pixel mask, image comes out directly
        imDef = scipy.ndimage.map_coordinates(
            im, pixCoordsRef.T, mode="constant", order=interpolationOrder
        ).reshape(im.shape)
...
###############################################################
def applyMeshTransformation(
    im, points, connectivity, displacements, imTetLabel=None, nThreads=1
):
    """
    ...
    if imTetLabel is None:
        ...
        print(
            "spam.DIC.applyMeshTransformation(): imTetLabel not passed, recomputing it."
        )
        imTetLabel = spam.label.labelTetrahedra(
            im.shape, pointsDef, connectivity, nThreads=nThreads
        )
