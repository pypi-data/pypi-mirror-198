...
from pawnlib.__version__ import __version__ as pawn_version
...
def handle_exception(exc_type, exc_value, exc_traceback):
    if issubclass(exc_type, KeyboardInterrupt):
        pawn.console.log(f"[red][Exception] {exc_type}")
...
def run_module(module_name=None):
    module = importlib.import_module(f"toolchains.cli.{module_name}")
    module.main()
...
def get_args():
    ...
    for module_name in get_submodule_names():
        ...
        _module = importlib.import_module(f"toolchains.cli.{module_name}")
        _module.get_arguments(_parser)
...
def main():
    pawn.console.debug(f"Starting main_cli wrapper")
    try:
    ...
    if command:
        ...
        try:
            ...
            pawn.console.log(f"[red]KeyboardInterrupt")
        except Exception as e:
