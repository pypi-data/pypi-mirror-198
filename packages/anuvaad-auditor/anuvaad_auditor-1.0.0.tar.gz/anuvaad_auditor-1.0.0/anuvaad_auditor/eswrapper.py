...
from .config import anuvaad_log_topic
...
# Method to push errors to the kafka topic.
def push_error_to_kafka(index_obj):
    try:
        ...
        log.info(f'logging ... {index_obj}')
        push_to_queue(index_obj,anuvaad_log_topic)
    except Exception as e:
        log.exception("Indexing FAILED for errorID: " + index_obj["errorID"], e)


# Method to push audit details to the kafka topic
def push_audit_to_kafka(index_obj):
    try:
        # es = instantiate_es_client()
