...
def get_layer_to_remove(
    layer: tf.keras.layers.Layer, remove_layer: List[Type]
) -> Tuple[tf.keras.layers.Layer, bool]:
    """
    converts the layer if necessary

    Args:
        layer:layer to study
        remove_layer: list of types of layers to remove
    """
    for type_layer in remove_layer:
        if isinstance(layer, type_layer):
            return True
    return False


def remove_layers_by_types_in_model(
    model: tf.keras.Model,
    remove_layer: List[Type] = None,
    verbose: bool = False,
) -> tf.keras.Model:
    """
    ...
    for layer_cpt, layer in enumerate(model_layers):
        if verbose:
            print(
                f"\r[\033[96mRemoveLayersByType\033[0m] ["
        ...
        if len(network_dict["input"][layer.name]) == 1:
            replace = get_layer_to_remove(layer=layer, remove_layer=remove_layer)
            if replace:
                removed_layers += 1
                x = layer_input
            else:
    ...
    unremoved_layers = len(model.layers) - removed_layers

    if verbose:
        print(
            f"\r[\033[96mRemoveLayersByType\033[0m] [" + "\033[96m=\033[0m" * 15 + "]"
        )
        print("\n\r+" + "-" * 39 + "+")
        print(f'| {f"{model.name}".center(37)} |')
        print("\r+" + "-" * 39 + "+")
        print(f'| removed layers              | {f"{removed_layers}":<7} |')
        print(f'| kept layers                 | {f"{unremoved_layers}":<7} |')
        print("\r+" + "-" * 39 + "+")
    return new_model
