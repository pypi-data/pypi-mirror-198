...
def get_layer_to_call(
    layer: tf.keras.layers.Layer,
    replace_layers: List[Tuple[Type, Type]],
    ...
    Args:
        ...
        replace_layers: list of pairs (old layer type, new layer type)
        extra_args: extra arguments for new layers in replace by types.
    """
    for old_type, new_type in replace_layers:
        if isinstance(layer, old_type):
            if extra_args is not None:
                return new_type.from_config({**extra_args, **layer.get_config()}), True
            else:
                return new_type.from_config(layer.get_config()), True
    return layer, False


def replace_layers_by_types_in_model(
    ...
    for layer_cpt, layer in enumerate(model_layers):
        if verbose:
            print(
                f"\r[\033[96mReplaceLayersByType\033[0m] ["
        ...
        if len(network_dict["input"][layer.name]) == 1:
            new_layer, replace = get_layer_to_call(
                layer=layer, replace_layers=replace_layers, extra_args=extra_args
            )
            if replace:
                modified_layers += 1
    ...
    if verbose:
        print(
            f"\r[\033[96mReplaceLayersByType\033[0m] [" + "\033[96m=\033[0m" * 15 + "]"
        )
        print("\n\r+" + "-" * 39 + "+")
