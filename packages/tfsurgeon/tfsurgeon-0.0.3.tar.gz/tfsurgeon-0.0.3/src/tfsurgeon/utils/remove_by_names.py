...
def remove_layers_by_names_or_indices_in_model(
    model: tf.keras.Model,
    remove_layer: List[Union[str, int]] = None,
    verbose: bool = False,
) -> tf.keras.Model:
    """
    ...
    Args:
        model: model to edit
        remove_layer: list of names or indices of layers to remove
        verbose: verbatim level
    """
    tmp = []
    for e in remove_layer:
        if isinstance(e, str):
            tmp.append(e)
        else:
            tmp.append(model.layers[e].name)
    remove_layer = tmp
    removed_layers = 0
    ...
    for layer_cpt, layer in enumerate(model_layers):
        if verbose:
            print(
                f"\r[\033[96mRemoveLayersByName\033[0m] ["
    ...
    if verbose:
        print(
            f"\r[\033[96mRemoveLayersByName\033[0m] [" + "\033[96m=\033[0m" * 15 + "]"
        )
        print("\n\r+" + "-" * 39 + "+")
