...
class LogicOfflineJobFakeBroker(LogicJobBroker, PriceInterface):
    ...
    def createOrder(self, side: OrderSide, symbol: BrokerSymbol, quantity, type: OrderType =OrderType.MARKET,
                    price=None, leverage=None, clientOrderId: string = None,
                    reduceOnly: bool = False, group=None):
    ...
    def buy(self, symbol: BrokerSymbol=None, amount: float=None,
            amountQuote: float=None, price=None, leverage=None, group=None):
        if symbol is None:
        ...
        if price is None:
            return self.createOrder(OrderSide.BUY, symbol, amount,
                                    OrderType.MARKET, price=price, leverage=leverage, group=group)
        else:
            return self.createOrder(OrderSide.BUY, symbol, amount,
                                    OrderType.LIMIT, price=price, leverage=leverage, group=group)
        return None
    ...
    def sell(self, symbol: BrokerSymbol=None, amount: float=None, amountQuote: float=None, price=None, leverage=None, group=None):
        if symbol is None:
            symbol = self.manager.getSymbolForChart()
        symbol = self.fillSymbolInfo(symbol)
        super().sell(symbol, price=price, leverage=leverage, group=group,
                     amount=amount, amountQuote=amountQuote)
        if amount is None:
            amount = self.calculate_amount(amountQuote, symbol)

        if price is None:
            return self.createOrder(OrderSide.SELL, symbol, amount,
                                    OrderType.MARKET, price=price, leverage=leverage, group=group)
        else:
            return self.createOrder(OrderSide.SELL, symbol, amount,
                                    OrderType.LIMIT, price=price, leverage=leverage, group=group)
    ...
    def buyAll(self, symbol: BrokerSymbol=None, price=None, leverage=None, group=None):
        ...
        portfolio = self.manager.getPortfolio()
        quoteMoney = portfolio.getQuoteAsset().free * 0.99 ## we remove 1 %
        ...
        if price is not None:
            return self.createOrder(OrderSide.BUY, symbol, quantity=quantity,
    ...
    def sellAll(self, symbol: BrokerSymbol=None, price=None, leverage=None, group=None):
        ...
        if price is not None:
            return self.createOrder(OrderSide.SELL, symbol,
                                    quantity=asset.free, leverage=leverage,
                                    type=OrderType.LIMIT, price=price)

        if asset is None:
            raise Exception("We have not found your symbol: "+symbol.symbol)

        return self.createOrder(OrderSide.SELL, symbol, quantity=asset.free,
                                leverage=leverage, type=OrderType.MARKET)
