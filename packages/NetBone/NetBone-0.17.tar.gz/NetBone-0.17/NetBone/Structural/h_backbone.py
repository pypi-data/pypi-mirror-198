...
#         G[u][v]['bridge'] = round(betweenness_values[(u,v)]/len(G.nodes()),3)
    ...
    if is_graph:
        return Backbone(G, name="H-Backbone Filter", column="h_backbone") #h_bridge, h_weight, G
    # return nx.to_pandas_edgelist(G.to_directed()), "h_backbone"
    return Backbone(nx.to_pandas_edgelist(G), name="H-Backbone Filter", column="h_backbone")
