...
# algo: marginal_likelihood_filter.py
'''
Created on May 6, 2021

@author: Navid Dianati

Implements the Marginal Likelihood Filter which computes a significance p_value for each
...
class MLF:
    ...
    def fit_transform(self, graph):
        '''
         ...
         representation, only with a significance p_value calculated
         for each edge. 
         ...
         of 4-tuples and the 4th tuple is the significance p_value.
         ...
         '''
        #print(isinstance(graph,Graph))
        if isinstance(graph, Graph):
            graph = to_pandas_edgelist(graph)
    ...
    def _cast(self, graph, dtype):
        ...
        if dtype == ig.Graph:
            ...
            edgelist = [(graph.vs[e.source]['name'], graph.vs[e.target]
                         ['name'], e['weight'], e['p_value']) for e in graph.es]
            df = pd.DataFrame(edgelist, columns=[
            ...
            raise(TypeError(
                'Can only recast the graph into one of: igraph.Graph, a DataFrame or a list of 3-tuples'))
    ...
    def _compute_significance_directed(self, G):
        ...
        for e in G.es:
            ...
            try:
                ...
                logger.debug("ValueError weight: {} ks[i0]:{} ks[i1]:{} total_degree:{} p:{}".format(
                    e['weight'], ks[i0], ks[i1], total_degree, p))
                e['p_value'] = None
        ...
        max_sig = max([s for s in G.es['p_value'] if s is not None])
        for e in G.es:
    ...
    def _compute_significance_undirected(self, G):
        ...
        for e in G.es:
            ...
            try:
                ...
                # to MAX_NEG_LOG
                e['p_value'] = max(
                    MAX_NEG_LOG, MAX_NEG_LOG if p <= 0 else p
                )
            except ValueError as error:
                logger.warning("warning: ValueError {}".format(str(error)))
                logger.debug("ValueError weight: {} ks[i0]:{} ks[i1]:{} total_degree:{} p:{}".format(
        ...
        max_sig = min([s for s in G.es['p_value'] if s is not None])
        for e in G.es:
            if e['p_value'] is None:
                e['p_value'] = max_sig
        return G
    ...
    def _convert_to_graph(self, graph, directed=False):
        ...
        if isinstance(graph, ig.Graph):
            if not graph.is_simple():
                raise ValueError(
                    'Expected a simple graph: no loops and no multiple edges')
            if graph.is_directed() and not directed:
                raise ValueError(
                    'Provided graph is directed but the "directed" parameter is False')
            if not graph.is_directed() and directed:
                raise ValueError(
                    'Provided graph is undirected but the "directed" parameter is True')
            return graph
        ...
        if isinstance(graph, list):
            ...
            print("heeere")
            g = ig.Graph.TupleList(
                to_edgelist(graph)[['source', 'target', 'weight']].itertuples(index=False),
                directed=directed,
                weights=True)
            if not g.is_simple():
                raise ValueError(
        ...
        if type(graph) == pd.DataFrame:
            for column in ["source", "target", "weight"]:
                if column not in graph.columns:
                    raise ValueError(
                        '{} must be in the DataFrame\'s columns'.format(column))
            ...
            if set([len(x) for x in graph]) != {3}:
                raise ValueError(
                    "Each element of the edgelist must have length 3")

            if set([type(x[2]) for x in graph]) != {int}:
                raise TypeError(
                    "The 3rd element of each edge tuple must be an int")
            ...
            if "weight" not in graph.es.attributes():
                raise ValueError(
                    "The EdgeSequence must have a 'weight' attribute")
            types = set([type(e['weight']) for e in graph.es])
            if types not in [{int}, {int, np.int64}, {np.int64}]:
                raise TypeError(
                    'The weight attribute of the edges must be an int')
            if min([e['weight'] for e in graph.es]) < 0:
                raise ValueError('All weights must be non-negative')

        else:
            raise(ValueError(
                'The graph must be given as a, igraph.Graph, a DataFrame or a list of 3-tuples'))
