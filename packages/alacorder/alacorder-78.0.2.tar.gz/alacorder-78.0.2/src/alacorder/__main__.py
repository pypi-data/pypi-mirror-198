...
def cli(ctx):
    ...
    if ctx.invoked_subcommand is None:
        ...
        fetch_layout = [
            ...
            [sg.Text("Password: "), sg.InputText(key="SQ-PASSWORD-",password_char='*', do_not_clear=False)],
    ...
    Alacorder retrieves and processes case detail PDFs\ninto data tables suitable for research purposes.""",font="Courier", pad=(10,10))],
        ...
        while True:
            ...
            if event == "SQ":
                ...
                window["SQ-PASSWORD-"].update("")
                a = alac.fetch(window['SQ-INPUTPATH-'].get(),window['SQ-OUTPUTPATH-'].get(),cID=window['SQ-CUSTOMERID-'].get(),uID=window['SQ-USERID-'].get(),pwd=pwd,qmax=int(window['SQ-MAX-'].get()),speed=int(window['SQ-SPEED-'].get()),qskip=int(window['SQ-SKIP-'].get()))
            if event == "MQ":
