...
class OwlModel:
    ...
    def validateFormat(self, format, ontology_url):
        ...
        register(constants.PUGLIN_DEFINITION[format], Parser, constants.FORMATS_DEFINITION[format], constants.PARSER_DEFINITION[format])
        try:
            g = rdflib.Graph()
    ...
    def get_metadata_as_iri_list(self) -> list:
        ...
        if self.__ontology.base_iri.endswith("#"):
            base_iri = self.__ontology.base_iri[:-1]
        query = constants.SPARQL_METADATA_EXTRACT
        query = query.replace("##PH1##", base_iri)
        for metadata in self.__ontology.metadata:
    ...
    def get_classes(self) -> list:
        ...
        for classes in self.__ontology.classes():
            if not classes.iri.__str__().startswith(constants.NS_OWL) and classes.namespace.base_iri.__eq__(self.__ontology.base_iri):
    ...
    def get_object_properties(self) -> list:
        ...
        for obj_property in self.__ontology.object_properties():
            if not obj_property.iri.__str__().startswith(constants.NS_OWL) and obj_property.namespace.base_iri.__eq__(self.__ontology.base_iri):
    ...
    def get_data_properties(self) -> list:
        ...
        for data_property in self.__ontology.data_properties():
            if not data_property.iri.__str__().startswith(constants.NS_OWL) and data_property.namespace.base_iri.__eq__(self.__ontology.base_iri):
                data_property_list.append(data_property)
        return data_property_list

    def get_classes_with_label(self):
        list_classes_with_label = []
        for term in self.get_classes():
            if term.label is not None and len(term.label) > 0:
                list_classes_with_label.append(term)
        list_metadata = list(
            self.__ontology.world.sparql(constants.SPARQL_LABEL_EXTRACT))
        for metadata in list_metadata:
            for m in metadata:
                list_classes_with_label.append(str(m))
        return len(list_classes_with_label)

    def get_object_properties_with_label(self):
        list_obj_with_label = []
        for term in self.get_object_properties():
            if term.label is not None and len(term.label) > 0:
    ...
    def get_data_properties_with_label(self):
        ...
        for term in self.get_data_properties():
            if term.label is not None and len(term.label) > 0:
