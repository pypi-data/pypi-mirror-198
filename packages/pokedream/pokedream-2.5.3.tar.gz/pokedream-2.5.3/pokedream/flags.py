...
The pokedream cog base, which contains most of the actual functionality of pokedream.
...
class Flag:
    """
    Dataclass that represents a pokedream flag state. Only for internal use.
    """
    ...
    def resolve_raw(self, flags: 'FlagMeta'):  # pylint: disable=too-many-return-statements
        ...
        env_value = os.getenv(f"pokedream_{self.name}", "").strip()
...
class Flags(metaclass=FlagMeta):  # pylint: disable=too-few-public-methods
    """
    The flags for pokedream.

    You can override these either through your environment, e.g.:
        export pokedream_HIDE=1
    Or you can override them programmatically:
        pokedream.Flags.HIDE = True
    """

    # Flag to indicate the pokedream base command group should be hidden
    HIDE: bool
    ...
    # The scope prefix, i.e. the prefix that appears before pokedream's builtin variables in REPL sessions.
