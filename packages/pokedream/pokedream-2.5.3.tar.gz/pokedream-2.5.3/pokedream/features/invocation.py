...
"""
pokedream.features.invocation
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The pokedream command invocation related commands.
...
from pokedream.models import copy_context_with
from pokedream.paginators import PaginatorInterface, WrappedPaginator, use_file_check
from pokedream.types import ContextA, ContextT
...
class InvocationFeature(Feature):
    ...
    @Feature.Command(parent="psk", name="override", aliases=["execute", "exec", "override!", "execute!", "exec!"])
    async def psk_override(
        self,
        ctx: ContextT,
        ...
        for override in overrides:
            if isinstance(override, discord.User):
                ...
                if ctx.guild:
                    ...
                    #  the command more compatible with chaining, e.g. `psk in .. psk su ..`
                    target_member = None
    ...
    @Feature.Command(parent="psk", name="repeat")
    async def psk_repeat(self, ctx: ContextT, times: int, *, command_string: str):
        """
        ...
        You can use this in conjunction with `psk sudo` to bypass this.
        """
    ...
    @Feature.Command(parent="psk", name="debug", aliases=["dbg"])
    async def psk_debug(self, ctx: ContextT, *, command_string: str):
        """
    ...
    @Feature.Command(parent="psk", name="source", aliases=["src"])
    async def psk_source(self, ctx: ContextA, *, command_name: str):
        """
