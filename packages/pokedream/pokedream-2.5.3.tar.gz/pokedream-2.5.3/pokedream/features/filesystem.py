...
"""
pokedream.features.filesystem
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The pokedream filesystem-related commands.
...
from pokedream.hljs import get_language, guess_file_traits
from pokedream.paginators import PaginatorInterface, WrappedFilePaginator, use_file_check
...
class FilesystemFeature(Feature):
    ...
    @Feature.Command(parent="psk", name="cat")
    async def psk_cat(self, ctx: ContextA, argument: str):
        """
    ...
    @Feature.Command(parent="psk", name="curl")
    async def psk_curl(self, ctx: ContextA, url: str):
        """
        Download and display a text file from the internet.

        This command is similar to psk cat, but accepts a URL.
        """
