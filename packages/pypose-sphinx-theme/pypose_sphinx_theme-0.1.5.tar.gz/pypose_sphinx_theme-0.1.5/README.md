...
```
git clone https://github.com/pypose/pypose_sphinx_theme
pip install -e pypose_sphinx_theme
```
...
2. In your local docs or tutorials repo, remove any existing `pypose_sphinx_theme` packages in the `src` folder (there should be a `pip-delete-this-directory.txt` file there)
3. Clone the repo locally `git clone https://github.com/pytorch/pypose_sphinx_theme`
4. Install `pypose_sphinx_theme` by running `pip install -e pypose_sphinx_theme`
...
In `conf.py` change the html theme to `pypose_sphinx_theme` and point the html theme path to this repo's local folder, which will end up looking something like:

```
html_theme = 'pypose_sphinx_theme'
html_theme_path = ["../../../pypose_sphinx_theme"]
```
