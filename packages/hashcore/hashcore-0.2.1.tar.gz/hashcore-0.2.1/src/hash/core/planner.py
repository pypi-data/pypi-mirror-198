...
class Planner(object):
    ...
    @staticmethod
    def re_action(action: str, resource, state: utils.ImDict, _hash: str, deps=utils.ImDict({}), environment=None, base_path="") -> str:
        """
        ...
        action_obj = Action(action, resource, state,
    ...
    def planAction(self, action: str, resource: Resource, env: EnvResource, state: utils.ImDict, deps):
        ...
        try:
            if env is None:
                self.__space.cal_spec(
                    resource, None, self.__store.get_globals())
            else:
                self.__space.cal_spec(resource, env.getName(),
                                      self.__store.get_globals())
                try:
                    self.__space.cal_spec(
                        env, env.getName(), self.__store.get_globals())
        ...
        try:
            ...
            if env is None:
                r = Renderer(resource, self.__space, self.__store,
                             self.__store.get_globals(), env, self.__states)
                r.render(action=action)
            else:
                r = Renderer(resource, self.__space, self.__store,
                             self.__store.get_globals(), env.getName(), self.__states)
