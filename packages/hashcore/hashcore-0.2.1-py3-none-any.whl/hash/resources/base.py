...
class TerraformResource(Resource):
    ...
    def terraform_init(self, workspace):
        init_command = f"{self.__terraform} init -upgrade -input=false -force-copy"
    ...
    def re_action(self, action, state, env):
        ...
        if self.__get_version() != state.get("envs", {}).get(env.getName(), {}).get(action, {}).get("version"):
            return True
        ...
        build_command = f"{self.__terraform} plan -detailed-exitcode -no-color -input=false"
    ...
    def __install_terraform(self, version):
        ...
        try:
            ...
            sha256_sum = self.execute(
                f"sha256sum terraform_{version}_linux_amd64.zip", hash_dir).stdout.decode("UTF-8").split(" ")[0]
            if sha256_sum != terraform_check_sum:
                raise errors.ResourceError(
                    f"downloaded file checksum is {sha256_sum}, does not match {terraform_check_sum}")
            self.execute(
                f"unzip -u terraform_{version}_linux_amd64.zip", hash_dir)
            self.execute(
                f"mv terraform terraform_{version}_linux_amd64", hash_dir)
    ...
    def check_terraform(self):
        ...
        if version:
            try:
                ...
                current_version = current_version_output.stdout.decode(
                    "UTF-8").split(" ")[1][1:].split("o")[0].strip()
    ...
    def build(self, state: dict, env: EnvResource):
        ...
        return {"status": "plan generated", "artifacts": artifacts, "globals": self.get_outputs(self.getPath()), "version": self.__get_version()}
    ...
    def deploy(self, state: dict, env: EnvResource):
        ...
        try:
            ...
            raise errors.ResourceBuildError(
                f"Timeout initializing terraform, {e}")
        deploy_command = f"{self.__terraform} apply -input=false -auto-approve -no-color"
        variables = self.getSpec("variables")
        if variables:
            vars = ""
            for (var_name, var_value) in variables.items():
        ...
        try:
            ...
            return {"status": "plan applied", "globals": self.get_outputs(self.getPath()), "version": self.__get_version()}
        except FileNotFoundError as e:
            raise errors.ResourceBuildError("cannot find terraform in PATH")
        except subprocess.CalledProcessError as e:
            raise errors.ResourceBuildError(e.stderr.decode("UTF-8"))
...
class KustomizeResource(Resource):
    ...
    @classmethod
    def generate(cls, path, file, env: EnvResource, overlay=None, missing_overlay="use_base"):
        if overlay is None:
    ...
    def build(self, state: dict, env: EnvResource):
        ...
        self.generate(self.getPath(), f"artifact-manifests-{self.name}.yaml", env, self.getSpec(
            "overlay"), self.getSpec("missing_overlay", "use_base"))
        ...
        with open(path, "r") as f:
            ...
            for manifest in manifests:
                if manifest.get("kind") == "Namespace" or manifest.get("kind", "").startswith("Cluster"):
                    new_manifests.append(manifest)
                    continue
                if manifest.get("metadata", {}).get("namespace") is not None and force_namespace:
    ...
    @classmethod
    def runTests(cls, target: Target, path: str, artifacts: list, env: EnvResource, overlay=None, missing_overlay="use_base"):
        if overlay is None:
            path_overlay = os.path.join(path, "overlays", env.getName())
        else:
            path_overlay = os.path.join(path, "overlays", overlay)
    ...
    def test(self, state: dict, env: EnvResource):
        ...
        try:
            self.execute(
                f"kubectl create namespace {namespace}", self.getPath())
        except subprocess.CalledProcessError:
            pass
        self.runTests(target, self.getPath(), artifacts, env, self.getSpec(
            "overlay"), self.getSpec("missing_overlay", "use_base"))
    ...
    def test(cls, res, target, artifacts, env: EnvResource):
        ...
        for artifact in artifacts:
            if artifact.getId() == "k8s":
                ...
                KustomizeResource.runTests(target, k8s_path, [artifact], env)
                break
...
class GoMicroServiceResource(Resource):
    ...
    def deploy(self, state: dict, env):
        ...
        if deploy_target == "kubernetes":
            ...
            url = target.action("deploy", {
                "project_dir": self.getPath(),
                "package_name": self.getSpec("do_function", {}).get("package_name", self.getName()),
                "function_name": self.getSpec("do_function", {}).get("function_name", self.getName())
            })
            return {
                "globals": {
                    "do_function_url": url
    ...
    def re_action(self, action, state, env: EnvResource):
        ...
        h = core.Hash().hash(go_mod_path, ["go.mod", "go.sum"])
        ...
        if action == "deploy" and env is not None:
            target = env.getTarget("K8STarget")
            if target:
                artifacts = state.get("envs", {}).get(
                    env.getName(), {}).get("build", {}).get("artifacts", [])
                for artifact in artifacts:
                    if artifact.getId() != "k8s":
                        continue
                    config = {
