...
def sync(
    *,
    ...
    to its step data, then these files or datasets will be overwritten when this endpoint is called. If
    appending files/datasets, first call the (#/Analyses/getAnalysisStep) to get the current list of
    files/datasets, append to it and pass it back to this endpoint.

    When this endpoint is called, the status of the analysis is updated based on the status of the step
    data. If a piece of data is `IN_PROGRESS` the step status will be uptated automatically once it
    completes.
    ...
    1. Create the datasets from existing files using the [Create a dataset](#/Analyses/createDataset)
       endpoint
