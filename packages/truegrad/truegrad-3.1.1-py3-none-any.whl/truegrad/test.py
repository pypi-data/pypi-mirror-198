...
lr = 1e-3
batch_size = 2 ** 18
iterations = 2 ** 24 * 256 // batch_size
noise_scale_range = list(range(-12, -13, -1))
offset = 2048 * 3 * 256 // batch_size
printervall = 2048 * 4 * 256 // batch_size
...
for sc_idx, scale in enumerate(noise_scale_range):
    ...
    oo = [torch.optim.AdamW(ps[0], lr=lr, weight_decay=wd),
          optim.Graft(ps[1], torch.optim.Adam(ps[1], lr=lr), Lion(ps[1], lr=1), weight_decay=wd),
          optim.Graft(ps[2], torch.optim.Adam(ps[2], lr=lr), torch.optim.SGD(ps[2], lr=1), weight_decay=wd),
          optim.Graft(ps[3], torch.optim.SGD(ps[3], lr=lr), torch.optim.Adam(ps[3], lr=1), weight_decay=wd),
          optim.Sign(ps[4], torch.optim.Adam(ps[4], lr=lr), lr, graft_to_self=False, weight_decay=wd),
          optim.Sign(ps[5], torch.optim.Adam(ps[5], lr=lr, weight_decay=wd))]
