#
# External Code Sets (created 1 Dec 2021 - 3Q2021 v2)
# https://www.iso20022.org/external_code_list.page
#
# This file is part of the Python FinTech Package
# https://www.joonis.de/fintech/
#
# Copyright (c) 2021 joonis new media
...
CANCELLATION_REASON = {
    ...
    'UAPA': 'The Debtor is requesting a return of the payment because the payment was not properly authorized._x000D_ Usage: This code can be used in the case where a Credit Transfer was made without proper authorization from the Debtor. This could be due to compromised end user credentials',
...
CATEGORY_PURPOSE = {
    ...
    'SWEP': 'Classification: Cash Management. Transaction relates to a cash management instruction, requesting a sweep of the account of the Debtor above an agreed floor amount, up to a target or zero balance._x000D_ The purpose is to move the funds from multiple accounts to a single bank account. Funds can move domestically or across border and more than one bank can be used',
    'TOPG': 'Classification: Cash Management. Transaction relates to a cash management instruction, requesting to top the account of the Creditor above a certain floor amount, up to a target or zero balance. _x000D_ The floor amount, if not pre-agreed by the parties involved, may be specified',
    'ZABA': 'Transaction relates to a cash management instruction, requesting to zero balance the account of the Debtor._x000D_ Zero Balance Accounts empty or fill the balances in accounts at the same bank, in the same country into or out of a main account each day',
}
...
CREDITOR_ENROLMENT_AMENDMENT_REASON = {
    'AM05': 'Duplication',
...
CREDITOR_ENROLMENT_CANCELLATION_REASON = {
    'AM05': 'Duplication',
    'RF01': 'Reference is not unique',
    'RR04': 'Regulatory Reason',
    'TRJT': 'Technical rejection',
...
CREDITOR_ENROLMENT_STATUS_REASON = {
...
DEBTOR_ACTIVATION_AMENDMENT_REASON = {
    'AM05': 'Duplication',
    'RF01': 'Reference is not unique',
    'RR04': 'Regulatory Reason',
    'TRJT': 'Technical rejection',
...
DEBTOR_ACTIVATION_CANCELLATION_REASON = {
    'AM05': 'Duplication',
    'RF01': 'Reference is not unique',
    'RR04': 'Regulatory Reason',
    'TRJT': 'Technical rejection',
...
DEBTOR_ACTIVATION_STATUS_REASON = {
    'ACRD': 'Already existing creditor',
    'AM05': 'Duplication',
    'FF01': 'Invalid file format',
    'RF01': 'Reference is not unique',
...
ENCRYPTED_ELEMENT_IDENTIFICATION = {
...
ENTRY_STATUS = {
    'BOOK': 'Booked means that the transfer of money has been completed between account servicer and account owner. Usage: Status Booked does not necessarily imply finality of money as this depends on other factors such as the payment system used, the completion of the end-to-end transaction and the terms agreed between account servicer and owner. Status Booked is the only status that can be reversed',
...
LOCAL_INSTRUMENT = {
    ...
    'CRP': 'Request-to-pay preferred payment via Credit Transfer but Instant Credit Transfer is also possible',
}
...
PERSON_IDENTIFICATION = {
    ...
    'POID': 'Other identification of the person',
}
...
PURPOSE = {
    'BKDF': 'Delayed draw funding. Certain issuers may utilize delayed draw loans whereby the lender is committed to fund cash within a specified period once a call is made by the issuer. The lender receives a fee for entering into such a commitment',
    ...
    'BKFM': 'Bank loan funding memo. Net cash movement for the loan contract final notification when sent separately from the loan contract final notification instruction',
    ...
    'MARG': 'Daily margin on listed derivatives – not segregated as collateral associated with an FCM agreement. Examples include listed futures and options margin payments; premiums for listed options not covered in the MT54X message',
    ...
    'SWCC': 'Cash collateral payment for swaps associated with an ISDA agreement. Where such payment is not segregated and is available for use by the client upon return. Includes any cash collateral payments made under the terms of a CSA agreement for instruments such as swaps and FX forwards',
    'SWPT': 'Cash collateral related to an option on interest rate swap',
    'SWSB': 'Swaps Broker Owned Segregated Cash Collateral - Any cash payment related to the collateral for Swap margin , which is segregated, and not available for use by the client. This includes any collateral identified in a CSA agreement such as Swap or FX Forward collateral',
    ...
    'SPLT': 'Split payments. To be used when cash and security movements for a security trade settlement are instructed separately',
...
RETURN_REASON = {
    ...
    'RC08': 'ClearingSystemMemberidentifier is invalid or missing._x000D_ Generic usage if cannot specify between debit or credit account',
    ...
    'FF03': 'Payment Type Information is missing or invalid._x000D_ Generic usage if cannot specify Service Level or Local Instrument code',
    ...
    'AG07': 'Debtor account cannot be debited for a generic reason._x000D_ Usage: Code value may be used in general purposes and as a replacement for AM04 if debtor bank does not reveal its customer\'s insufficient funds for privacy reasons',
...
SYSTEM_BALANCE_TYPE = {
    ...
    'PIPO': 'Balance representing the fictive forecast of automated direct debits or payment based on standing arrangements between a central securities depository and the user.  Usage: Pay-Ins and Pay-Outs can be different based on individual payment instructions or available funds',
    'PRAV': 'Average of the daily balances on the account used to fulfil the reserve requirements calculated from the beginning of the maintenance period',
...
UNABLE_TO_APPLY_INCORRECT_DATA = {
