...
LastEditTime: 2023-03-13 17:50:34
LastEditors: Hexu
Description: 数据集切分工具
...
def get_set_spliter(data, not_spliter=False):
    ...
    msg = "Dataset Split Reader: \033[33m{}\033[0m"
...
class DataSetSpliter(metaclass=ABCMeta):
    def __init__(self, data, train_ratio, valid_ratio, test_ratio):
        ...
        if config.SOURCE_TYPE == DataSourceType.CV_TYPE:  # 图片数据源:
            self.alldata = data
        else:
...
class SequentialSpliter(DataSetSpliter):
    ...
    def _set_data(self):
        ...
        if config.SOURCE_TYPE == DataSourceType.CV_TYPE:
...
class TargetRandomSpliter(DataSetSpliter):
    ...
    def __init__(self, data, train_ratio, valid_ratio, test_ratio, target_col):
        super().__init__(data, train_ratio,
                         valid_ratio, test_ratio)
    ...
    def _set_data(self):
        ...
        # 边界处理
        for tag in tags:
            if tag is None or engine.pd.isna(tag):
                tags.remove(tag)
                break
        tag_count = len(tags)
        ...
        for tag in tags:
            ...
            # 边界处理
            line_count = all_data.shape[0]
            if line_count == 1:
                raise DatasetException(f"目标列 {tag} 只有一条数据")
            ...
            # train
            all_data = all_data.sample(frac=float(1), random_state=self.seed)
            train_data = all_data[:train_num]
            if test_num == 0:
                valid_data = all_data[train_num:]
            else:
                valid_data = all_data[train_num: train_num + valid_num]
                test_data = all_data[train_num + valid_num:]
