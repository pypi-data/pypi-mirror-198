        ...
        if function.__name__ == 'save_checkpoint':
            ...
            if kwargs.get("save_best_only") is True:
                ...
                if max_to_keep < 0:
                    ...
                    save_index = (save_index + 1) % max_to_keep % config.CHECKPOINT_SAVE_MAX
        else:
            save_index = "model"
        # 执行用户的保存函数
...
def __push_model_to_cloud(reporter, model_path, index, is_checkpoint, kwargs):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"上传模型文件到{config.FILE_UP_TYPE}成功：{curkey}, checkpoint模式：{is_checkpoint}")
            if is_checkpoint:
            ...
            logger.info(f"上传模型文件到{config.FILE_UP_TYPE}失败: {err_info}, checkpoint模式：{is_checkpoint}")
