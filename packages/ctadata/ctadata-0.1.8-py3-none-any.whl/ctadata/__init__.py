...
dsbase = urljoin(hubbase, "services/downloadservice/fetch/")


def list_dir(url):
    fullurl = urljoin(urljoin(dsbase, "pnfs/cta.cscs.ch/"), url)
    print(fullurl)
    ...
    try:
        return [u.replace(dsbase, "") for u in r.json()['urls']]
...
def fetch_and_save_file(url, fn=None):
    ...
    with open(fn, "wb") as outf:
        with requests.get(
            urljoin(dsbase, "pnfs/cta.cscs.ch", url),
            params = {'token': os.getenv("JUPYTERHUB_API_TOKEN")},
            stream=True
        ) as f:          
            f.raise_for_status()
