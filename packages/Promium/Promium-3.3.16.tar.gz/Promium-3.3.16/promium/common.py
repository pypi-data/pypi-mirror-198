...
def download_and_check_file(download_url, file_path):
    try:
        ...
        return bool(os.path.isfile(file_path) and os.path.exists(file_path))
    finally:
...
def find_network_log(driver, find_mask, find_status=200, timeout=5):
    """ find xrh response and requset data from browser network logs
    :param driver: selenium driver
    :param find_mask: find word in request url
    :param find_status: find response with this status
    :param timeout: tries
    :return: {response, data, body} or None
    """
    found_logs = []
    ...
    while timeout >= 0:
        ...
        if xhr_log:
            requestId = xhr_log['params']['requestId']  # noqa: N806
            response = xhr_log['params']['response']
            break
        time.sleep(0.5)
        timeout -= 1
    ...
    log.info(
        ...
        f"request_data: {request_data}, response_body: {response_body},"
    )
