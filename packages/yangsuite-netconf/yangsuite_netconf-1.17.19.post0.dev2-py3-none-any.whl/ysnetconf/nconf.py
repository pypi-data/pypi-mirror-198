...
def gen_rpc_api(request):
    ...
    def rem_dup_xpaths(modules):
        """Remove duplicate XPaths from cfgd's modules in-place"""
        ...
        for module_name in result:
            ...
            tmp_configs = []
            for index in range(len(module['configs'])):
                config = module['configs'][index]
                xpath = config['xpath']
                if xpath not in xpaths_seen:
                    # If XPath not seen, add to seen list
                    xpaths_seen.append(xpath)
                    # Add to non-duplicate XPath configs
                    tmp_configs.append(config)
            # Replace old configs with new configs
            result[module_name]['configs'] = tmp_configs
        return result

    # Remove duplicate XPaths from cfgd's modules
    if isinstance(cfgd, dict) and 'modules' in cfgd:
        cfgd['modules'] = rem_dup_xpaths(cfgd['modules'])
    elif isinstance(cfgd, list):
        for index in range(len(cfgd)):
            elem = cfgd[index]
            if ('cfgd' in elem and 'modules' in elem['cfgd']):
                modules = elem['cfgd']['modules']
                cfgd[index]['cfgd']['modules'] = rem_dup_xpaths(modules)
