...
class TestNetconfRPCBuilder(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Quiet down the loggers from other modules
...
""")
        for fp in zip(foobar.iter(), payload.iter()):
...
""")
        for fp in zip(foobar_min.iter(), payload_min.iter()):
            self.assertEqual(fp[0].tag, fp[1].tag)
            self.assertEqual(fp[0].nsmap, fp[1].nsmap)
