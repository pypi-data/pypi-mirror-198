...
class AlphaTest:
    ...
    def init_tables(
        ...
        database_lib.init_databases(
            binds=binds, tables=tables, truncate=truncate, drop=drop, create=create
        )
    ...
    def assert_api_answer(
        ...
        if not self.partial_output:
            LOG.info(
                f"{self.current_test_name} {msg} - Assert: {raw_params=} > {params=} does not provide an {expected_output=} but {real_output}"
            )
