...
class RustBackendGenerated:
    ...
    def reposition_defaults_raw(self, message: bytes) -> bytes:
        return self._run_command(0, 25, message)

    def reposition_defaults(self) -> anki.scheduler_pb2.RepositionDefaultsResponse:
        message = anki.generic_pb2.Empty()
        raw_bytes = self._run_command(0, 25, message.SerializeToString())
    ...
    def set_current_deck_raw(self, message: bytes) -> bytes:
        return self._run_command(1, 22, message)

    def set_current_deck(self, did: int) -> anki.collection_pb2.OpChanges:
        message = anki.decks_pb2.DeckId(did=did)
        raw_bytes = self._run_command(1, 22, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output
    ...
    def get_current_deck_raw(self, message: bytes) -> bytes:
        return self._run_command(1, 23, message)

    def get_current_deck(self) -> anki.decks_pb2.Deck:
        message = anki.generic_pb2.Empty()
        raw_bytes = self._run_command(1, 23, message.SerializeToString())
        output = anki.decks_pb2.Deck()
        output.ParseFromString(raw_bytes)
        return output
    ...
    def compare_answer_raw(self, message: bytes) -> bytes:
        return self._run_command(6, 11, message)
    ...
    def compare_answer(self, *, expected: str, provided: str) -> str:
        ...
        raw_bytes = self._run_command(6, 11, message.SerializeToString())
        output = anki.generic_pb2.String()
        output.ParseFromString(raw_bytes)
        return output.val
    ...
    def extract_cloze_for_typing_raw(self, message: bytes) -> bytes:
        return self._run_command(6, 12, message)
    ...
    def extract_cloze_for_typing(self, *, text: str, ordinal: int) -> str:
        ...
        raw_bytes = self._run_command(6, 12, message.SerializeToString())
        output = anki.generic_pb2.String()
        output.ParseFromString(raw_bytes)
        return output.val
    ...
    def all_tts_voices_raw(self, message: bytes) -> bytes:
        return self._run_command(6, 13, message)
    ...
    def all_tts_voices(self, validate: bool) -> Sequence[anki.card_rendering_pb2.AllTtsVoicesResponse.TtsVoice]:
        ...
        raw_bytes = self._run_command(6, 13, message.SerializeToString())
    ...
    def write_tts_stream_raw(self, message: bytes) -> bytes:
        return self._run_command(6, 14, message)
    ...
    def write_tts_stream(self, *, path: str, voice_id: str, speed: float, text: str) -> anki.generic_pb2.Empty:
        ...
        raw_bytes = self._run_command(6, 14, message.SerializeToString())
        output = anki.generic_pb2.Empty()
        output.ParseFromString(raw_bytes)
        return output
    ...
    def update_deck_configs(self, *, target_deck_id: int, configs: Sequence[anki.deckconfig_pb2.DeckConfig], removed_config_ids: Sequence[int], apply_to_children: bool, card_state_customizer: str, limits: anki.deckconfig_pb2.DeckConfigsForUpdate.CurrentDeck.Limits, new_cards_ignore_review_limit: bool) -> anki.collection_pb2.OpChanges:
        message = anki.deckconfig_pb2.UpdateDeckConfigsRequest(target_deck_id=target_deck_id, configs=configs, removed_config_ids=removed_config_ids, apply_to_children=apply_to_children, card_state_customizer=card_state_customizer, limits=limits, new_cards_ignore_review_limit=new_cards_ignore_review_limit)
    ...
    def open_collection(self, *, collection_path: str, media_folder_path: str, media_db_path: str, force_schema11: bool) -> anki.generic_pb2.Empty:
        message = anki.collection_pb2.OpenCollectionRequest(collection_path=collection_path, media_folder_path=media_folder_path, media_db_path=media_db_path, force_schema11=force_schema11)
