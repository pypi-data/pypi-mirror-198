...
def main():
    ...
    if ".knu" in arg:
        ...
        if modul == "kenu":
            ...
            os.makedirs(new_folder, exist_ok=True)
            print("Loading...")
            filepath = os.path.join(os.environ['APPDATA'], "kenucorp", "modules") + f"\\{modul}.zip"
            kenan = downloadModule(modul, filepath)
            if kenan:
                print("\nThe module downloaded successfully.")
            else:
...
def downloadModule(name, filepath):
    try:
        if("knu" in name):
            ...
            urllib.request.urlretrieve("https://kenucorp.com/modules/" + name + ".zip", filepath)
            with filepath.ZipFile(filepath, 'r') as zip_ref:
