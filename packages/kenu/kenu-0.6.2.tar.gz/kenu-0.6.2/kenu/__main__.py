...
def main():
    ...
    if ".knu" in arg:
        ...
        if modul == "kenu":
            ...
            os.makedirs(new_folder, exist_ok=True)
            print("Loading...")
            filepath = os.path.join(os.environ['APPDATA'], "kenucorp", "modules") + f"\\{modul}"
            kenan = downloadModule(modul, filepath)
            if kenan:
                print("\nThe module downloaded successfully.")
            else:
                print("\nNo such module found.")
@loadwave.process 
def downloadModule(name, filepath):
    try:
        if("knu" in name):
            urllib.request.urlretrieve("https://kenucorp.com/modules/" + name + ".knu", filepath)
        else:
            name = os.path.join(os.environ['APPDATA'], "kenucorp", "modules") + f"\\{name}"
            os.makedirs(name, exist_ok=True)
            urllib.request.urlretrieve(f"https://kenucorp.com/modules/" + name + ".py", filepath + "\\{name}\\{name}.py")
            urllib.request.urlretrieve(f"https://kenucorp.com/modules/{name}requirements.txt", filepath + "\\{name}\\requirements.txt")
            
            os.remove(filepath)
            try:
                with open(filepath + "\\{name}\\requirements.txt", 'r') as file:
                    line = file.readline()
                    while line:
                        os.systs
                        line = file.readline()
