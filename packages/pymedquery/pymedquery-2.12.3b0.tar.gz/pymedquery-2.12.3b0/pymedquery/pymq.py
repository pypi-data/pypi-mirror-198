...
class PyMedQuery:
    ...
    def __init__(self) -> NoReturn:
        ...
        self.mh: Any = MinioHandler(access_key=config.USER, secret_key=config.PASSWORD)
        self.log: Any = Logger(__name__)
        self._check_version()  # check version of the client
    ...
    def __set_extract_params(
        ...
        if get_all:
            if not project_id:
                raise ValueError(
                    "please specify the project id that corresponds to the requested data"
                )
            if include_mask is None:
                raise ValueError("please specify `include_mask` to true or false")
            sub: str = "subq3" if include_mask else "subq2"
            sql_file_path: os.PathLike = (
                config.SERIES_MASK_QUERY_DEFAULT
                if get_affines
                else config.SERIES_MASK_AFF_QUERY
            )
    ...
    def __pg_extract(
        ...
        pg_payload: Dict[Tuple[Any]] = self.crud.read(
            sql=sql_file_path, format_params=format_params, verbose=verbose
        )
    ...
    def __fetch_columns(
        self, table_name: str, verbose: bool, post_close_connection: bool
    ) -> List[str]:
        ...
        if colnames:
            ...
            self.log.error(
                "I tried to fetch column names but got an empty result. Lordy lord!"
            )
    ...
    def extract(
        ...
        try:
            ...
            # merge the two dicts
            payload = blobs | data_info if image_extraction else (blobs, data_info)
            if post_close_connection:
                self.crud.close_connection()
            ...
            self.log.error(
                f"failed to extract data with uids {all_uids} and error: {e}"
            )
    ...
    def batch_extract(
        ...
        try:
            ...
            sample: Generator[List[str], None, None] = batch_maker(
                iterable=all_uids[0], batch_size=batch_size
            )
            ...
            self.log.error(
                f"failed to extract the following uids {all_uids} with the error: {e}"
            )
    ...
    def _check_version(self):
        # Fetch the latest version number of the package from the PyPI API
        try:
            ...
            if version != latest_version:
                self.log.warning(
                    f"A new version is available! Current version: {version} and the latest version is {latest_version}"
                )
        except requests.exceptions.HTTPError as e:
            self.log.error(
                f"Failed to fetch the latest version number from PyPI with the error: {e}"
            )
