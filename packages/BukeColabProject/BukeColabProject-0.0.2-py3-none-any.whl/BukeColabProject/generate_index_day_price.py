...
def get_trading_days(loc, start_date: date, end_date: date):

    df = pd.read_csv(f"{loc}/data_file/korea_trading_days.csv", index_col=0)
...
def data_refactor(start_date, end_date, df):
...
def generate_index_day_price(loc, day):
    ...
    df = data_refactor(start_date, end_date, df)

    MSCI_Korea_ETF = pd.DataFrame(df)
    MSCI_Korea_ETF.to_csv(f"{loc}/data_file/VIX_지수.csv")
    print("VIX 지수 수집완료")
...
# MSCI 한국 ETF
    ...
    MSCI_Korea_ETF = pd.DataFrame(df)
    MSCI_Korea_ETF.to_csv(f"{loc}/data_file/MSCI_한국_ETF.csv")
    print("MSCI 한국 ETF data 수집완료\n")


    trading_days = get_trading_days(start_date, day)
