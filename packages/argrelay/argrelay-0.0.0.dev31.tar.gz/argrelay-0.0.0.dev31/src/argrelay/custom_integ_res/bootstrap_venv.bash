...
# This is a custom deploy script called by `bootstrap_venv.bash`.
# Python `venv` is already activated before it is called.
...
then
    # Overwrite itself:
    cp -p "${argrelay_path}"/custom_integ_res/bootstrap_venv.bash "."
...
module_path_file_tuples=(
    ...
    argrelay custom_integ_res bootstrap_venv.bash
...
# This is a custom build script called by `bootstrap_venv.bash`.
# Python `venv` is already activated before it is called.
