...
# Test files generated by `deploy_config_files.bash` script:
test -f ~/".argrelay.conf.d/argrelay.server.yaml" || return 1
test -f ~/".argrelay.conf.d/argrelay.client.json" || return 1
...
if [[ ! -f "./argrelay_rc_conf.bash" ]]
then
    echo "ERROR: \`$(pwd)/argrelay_rc_conf.bash\` does not exists" 1>&2
    echo "It is required to know which command names will have \`argrelay\` auto-completion." 1>&2
    echo "Provide \`$(pwd)/argrelay_rc_conf.bash\`, for example (copy and paste and modify):" 1>&2
    echo "" 1>&2
...
do
    ...
    # Create `${argrelay_command_name}` command (just symlink to `run_argrelay_client`):
    cd "${script_dir}" > /dev/null || return 1
    ln -snf run_argrelay_client "${argrelay_command_name}"
