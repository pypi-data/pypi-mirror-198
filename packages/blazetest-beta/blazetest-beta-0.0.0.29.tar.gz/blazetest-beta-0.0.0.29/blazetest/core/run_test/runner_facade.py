...
class TestRunner:
    ...
    def __init__(self, pytest_args: List[str], config: BlazetestConfig):
        """
    ...
    def run_tests(self, flaky_test_retry_enabled: bool = True) -> Optional[TestSessionResult]:
        """
        Collects the test items, creates the report paths,
        and invokes the Lambda functions in parallel.
        """
        node_ids = collect_tests(pytest_args=self.pytest_args)
    ...
    # TODO: separate arguments of pytest to: collection and execution
    def __process_pytest_arguments(self):
        pass
