...
class Module:
    ...
    def start_app (self, reloded = False):
        ...
        self.app_class = str (app.__class__)
        if "atila." in self.app_class:
            self.app_class = 'atila'
        elif "django." in self.app_class:
            from django.conf import settings
            self.reloader = DjangoReloader (str (settings.BASE_DIR), self.wasc.logger)
    ...
    def set_devel_env (self, app):
        ...
        if self.use_reloader:
            lifetime.maintern.sched (1.0, self.maybe_reload)
    ...
    def maybe_reload (self, now = None):
        reload = False
        if self.reloader:
            if self.reloader.reloaded ():
                reload = True
        elif self.file_info:
            stat = os.stat (self.abspath)
        ...
        if reload:
            self.wasc.logger ("server", "reloading app, %s" % self.abspath, "debug")
        ...
        for apph in apphs:
            ...
            if not hasattr (apph.get_callable (), 'ATILA_THE_HUN') or apph.get_callable ().find_method (path_info, '__proto__') [-1] != 404:
                ...
                if script_name == '/':
                    ...
                    if dirname.startswith ("/api"): # /api is almost second root
                        if dirname [1:].find ("/") != -1:
                            basepath = "/" + '/'.join (script_name [1:].split ('/') [:2]) # 2 depth caching
                            self.modules [basepath] = [apph]
                    else:
                        basepath = "/" + script_name [1:].split ('/') [0] # 1 depth caching
                        self.modules [basepath] = [apph]
                return apph
        return None # 404
