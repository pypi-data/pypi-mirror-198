...
def update_config_local(role: str, protocol: str = "gwas") -> None:
    ...
    """
    doc_ref_dict: dict = get_doc_ref_dict()
    if "usingblocks-" in doc_ref_dict["description"]:
        if role != "0":
            geno_file_prefix, data_path = get_file_paths()
            print("Using blocks with config files")
            source = f"{data_path}/" + geno_file_prefix.split("blocks-")[1]
            ...
            config = doc_ref_dict["description"].split("usingblocks-")[1]
            # mv files from sfgwas/config/blocks/{config} to sfgwas/config/gwas
...
def update_config_global(protocol: str = "gwas") -> None:
    ...
    geno_file_prefix, _ = get_file_paths()
    if "blocks-" not in geno_file_prefix:
