...
from bamt.external.pyBN.utils.independence_tests import mutual_information
from bamt.external.pyBN.utils.graph import would_cause_cycle


def hc_rr(data, M=5, R=8, metric='AIC', max_iter=300, debug=False, init_nodes=None, restriction=None):
    """
    Arguments
    ---------
    ...
    *bn* : a BayesNet object
    ...
    while improvement:
        ...
        for u in bn.nodes():
            for v in bn.nodes():
                if v in c_dict[u] and not would_cause_cycle(c_dict,v,u, reverse=True) and (init_nodes is None or not (u in init_nodes)) and (restriction is None or (v,u) in restriction): 
            ...
            if _restarts < R:
                ...
                for _ in range(M):
                    ...
                    if operation == 0:
                        ...
                        if u not in p_dict[v] and u!=v and not would_cause_cycle(c_dict,u,v) and (init_nodes is None or not (v in init_nodes)) and (restriction is None or (u,v) in restriction):
                            if debug:
                        ...
                        if u in p_dict[v] and not would_cause_cycle(c_dict,v,u, reverse=True) and (init_nodes is None or not (u in init_nodes)) and (restriction is None or (v,u) in restriction):
                            if debug:
