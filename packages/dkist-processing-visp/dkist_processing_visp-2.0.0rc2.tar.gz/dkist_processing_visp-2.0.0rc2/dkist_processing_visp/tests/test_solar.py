...
def test_solar_gain_task(solar_gain_calibration_task_that_completes, mocker):
    ...
    for beam in range(1, task.constants.num_beams + 1):
        equalization_flux = np.sum(
            np.ones((10, 10)) * (1 + beam + 1) * (10 * beam * 1) * np.mean(np.arange(1, 11) / 5)
        )
        for modstate in range(1, task.constants.num_modstates + 1):
            # Gains aren't normalized. This expression comes from the math applied above. Sorry.
            raw = (
                np.ones((10, 10))
                * (1 + beam + modstate)
                * (10 * beam * modstate)
            ...
            expected = raw * equalization_flux / np.sum(raw)
