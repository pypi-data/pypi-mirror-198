...
class SolarCalibration(
    ...
    def pre_run(self) -> None:
        """Initialize the equalization flux dict."""
        super().pre_run()

        self._equalization_flux_dict = {1: np.nan, 2: np.nan}

    def get_equalization_flux(self, beam: int) -> float:
        """Return the equalization flux for a given beam.

        Raise and error if it hasn't been set.
        """
        value = self._equalization_flux_dict[beam]
        if np.isnan(value):
            raise ValueError(
                f"Equalization flux is not set for {beam = }. This should never happen."
            )
        return value

    def set_equalization_flux(self, beam: int, value: float) -> None:
        """Set the equalization flux for a given beam."""
        self._equalization_flux_dict[beam] = value
    ...
    def run(self) -> None:
        ...
        for beam in range(1, self.constants.num_beams + 1):
            for modstate in range(1, self.constants.num_modstates + 1):
                ...
                with self.apm_processing_step(f"Equalizaing flux for {apm_str}"):
                    gain = self.equalize_flux(gain, beam=beam, modstate=modstate)

                with self.apm_writing_step(f"Writing solar gain for {apm_str}"):
                    self.write_solar_gain_calibration(gain_array=gain, beam=beam, modstate=modstate)
    ...
    def equalize_flux(self, array: np.ndarray, beam: int, modstate: int) -> np.ndarray:
        """Correct an array so that each modstate has the same total flux value (for each beam)."""
        total_flux = np.nansum(array)
        if modstate == 1:
            self.set_equalization_flux(beam=beam, value=total_flux)
            logger.info(f"Setting equalization flux to {total_flux:.4f}")
            return array

        correction = self.get_equalization_flux(beam) / total_flux

        logger.info(f"Equalizing {beam = } and {modstate = } by {correction:.4f}")
