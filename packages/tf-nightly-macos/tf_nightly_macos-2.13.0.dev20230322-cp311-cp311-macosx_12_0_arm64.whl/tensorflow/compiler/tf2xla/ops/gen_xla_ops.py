...
def xla_call_module(args, version, module, Sout, Tout, dim_args_spec=[], platforms=[], name=None):
  ...
  if tld.is_eager:
    ...
    try:
      ...
      return xla_call_module_eager_fallback(
          ...
          dim_args_spec=dim_args_spec, platforms=platforms, name=name,
          ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
    ...
    _result = _dispatcher_for_xla_call_module(
        (args, version, module, Sout, Tout, dim_args_spec, platforms, name,),
        None)
    if _result is not NotImplemented:
      return _result
  # Add nodes to the TensorFlow graph.
...
def xla_call_module_eager_fallback(args, version, module, Sout, Tout, dim_args_spec, platforms, name, ctx):
  version = _execute.make_int(version, "version")
  module = _execute.make_str(module, "module")
  if not isinstance(Sout, (list, tuple)):
    raise TypeError(
