...
class CurrentClamp():
    def __init__(self, template_name, post_init_function=None, record_sec="soma", record_loc="0.5", tstart=50, tstop=1000,
...
class Passive(CurrentClamp):
    def __init__(self, template_name, post_init_function=None, record_sec="soma", record_loc="0.5", tstart=50, tstop=1000,
        ...
        super(Passive, self).__init__(template_name=template_name,post_init_function=post_init_function,
                                      ...
                                      tstart=tstart, tstop=tstop, inj_sec=inj_sec, inj_loc=inj_loc,
    ...
    def execute(self):
        ...
        index_v_rest = int(((1000/h.dt)/1000 * self.tstart))
        index_v_final = int(((1000/h.dt)/1000 * (self.tstart+self.inj_delay)))
        ...
        time_tau = (index_v_tau / ((1000/h.dt)/1000)) - self.tstart
        self.tau = time_tau #/ 1000 (in ms)
        self.r_in = (v_diff)/(0-self.inj_amp) * 1e6 #MegaOhms -> Ohms
        print()
        print(f"V Rest: {self.v_rest} (mV)")
        print(f"Resistance: {self.r_in/1e6} (MOhms)")
        print(f"tau: {self.tau} (ms)")
        print()
...
class Profiler():
    ...
    def current_injection(self, template_name:str, post_init_function=None, plot=True, 
                          record_sec="soma", record_loc="0.5", tstart=50, tstop=1000,
