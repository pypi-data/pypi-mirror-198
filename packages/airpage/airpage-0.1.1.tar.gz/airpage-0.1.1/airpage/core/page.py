...
class Page:
    ...
    * BFS页面跳转寻路
    ...
    def login(self, name, function, check=True):
        """
        注册任务到指定页面
        :param name:
        :param action:
        :param check: 执行任务前是否需要检测当前screen是否为当前页面. 在dotask时你仍然有机会临时修改是否需要check
        :return:
        """
        if self.tasks.get(name):
            warn(f"You are replacing exist task:{name}")
        self.tasks[name] = function
        self.task_checks[name] = check
    ...
    def LoginTask(page_name, fn):
        ...
        :return:
        """
        return GetEnviron().GetPage(page_name).login(fn.__name__, fn)
...
class BootLoader(Page):
    """
    目标程序启动器
    * 用于启动目标程序
    * 用于检测目标程序是否存在
    """
    def __init__(self, running_identity, bootloader=None):
        super().__init__("unboot", running_identity)
    ...
    def check(self, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.5, snapshot=True):
        """
        bootloader的identity是正常运行状态下的，但是unboot对应未启动状态，所以要加一个not
        :return:
        """
        return not super().check(timeout, threshold, interval, snapshot)
...
def transfer(page_a: typing.Union[str, Page], page_b: typing.Union[str, Page]) -> typing.Callable:
    ...
    def _wrapper_(fn):
        page_a = page_a if isinstance(page_a, Page) else GetEnviron().GetPage(page_a)
        page_b = page_b if isinstance(page_b, Page) else GetEnviron().GetPage(page_b)
