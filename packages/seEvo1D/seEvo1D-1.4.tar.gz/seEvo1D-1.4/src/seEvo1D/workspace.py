...
def stoalfafit(path, mi, ma, step):
    ...
    for i in range(1,26,1):
        columns = np.append(columns, np.repeat("%.4f" % ((i-1)*step + mi), 3))
        files = np.array(os.listdir(path + str(i)))
        files = files[_filter(files)]
        a = np.zeros((len(files), 3))
        for j in files:
            idx = int(float(re.search(r'\d+.\d+', j[::1]).group())/25)
...
if __name__ == "__main__":
    path = 'E:/Simulations/Stationary Wave/stationary_1d_for_s/'
    stoalfafit(path, 0.0001, 0.0049, 0.0002)
