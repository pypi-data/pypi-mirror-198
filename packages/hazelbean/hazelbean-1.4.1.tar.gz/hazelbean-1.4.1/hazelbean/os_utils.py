...
def compile_exam_from_md(exam_template_path, question_bank_path, output_path, number_of_variations=4, questions_to_include=None):
    import numpy as np
    ...
    for variation_id in range(number_of_variations):    
        for is_key in (False, True):
            ...
            included_questions_md_string, current_answers = hb.make_exam_md_from_dicts(header_dict, question_bank_path, random_seed, is_key=is_key)
            ...
            variation_output_path = hb.suri(output_path, 'variation_' + str(variation_id+1))

            if is_key:
                variation_output_path = hb.suri(variation_output_path, 'KEY')

            output_md_path = hb.replace_ext(variation_output_path, '.md')
            hb.write_to_file(output_string, output_md_path)
            hb.convert_file_via_quarto(output_md_path, variation_output_path, verbose=True)
    ...
    hb.python_object_to_csv(all_variation_answers_t, hb.replace_ext(os.path.join(os.path.split(output_md_path)[0], hb.suri(os.path.split(output_path)[1], 'COMBINED_KEY')), '.csv'), csv_type='2d_list')
...
def make_exam_md_from_dicts(header_dict, question_bank_path, random_seed, is_key=False):
    import random
...
def print_dict(input_dict, level = 0):
    level += 1
