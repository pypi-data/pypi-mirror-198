...
class Drive(NetBoxModel):
    ...
    def free_space_on_drive(self):
        # print(f"Free Space: {Partition.objects.filter(drive=self).aggregate(Sum('size'))['size__sum']}")
        return self.size - Partition.objects.filter(drive=self).aggregate(Sum('size'))['size__sum'] or 0.0
