...
class DynamodbClient(LunarClient):
    ...
    def get_target_table_count(
        ...
        ```python
        data = client.get_target_table_count_async(
            table_name="my_target_table"
        )
        ```
        """
    ...
    async def delete_target_table_async(self, table_name: str, force: bool = False) -> Dict[str, Any]:
        """
        ...
        body = await self._request(
            method="DELETE", url=f"{self.config.API_PATH}/target_tables/{table_name}", params={"force": force}
        )
        return body["data"]

    def delete_target_table(self, table_name: str, force: bool = False) -> Dict[str, Any]:
        """
