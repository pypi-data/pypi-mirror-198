...
def upload_files(dat, unfold=True, image_lap_thr = "None", remove_blurred = False, tile_shape = (512,512)):
...
def get_focused_image_data(dat, unfold, image_lap_thr = "None", remove_blurred = False, tile_shape = (512,512)):
...
def get_upload_data(file_data):
    ...
    for file_name,data in file_data.items():
        ...
        if data["time_index"] == 0:
            ...
            file_data[file_name]["usermeta1"] = "Direct From Plate GASKET"
...
files = glob(r"D:\ScanR Data\AluGasket\**\*ome.xml")  

file_data = [read_xml([file]) for file in files]
upload_data = pd.concat([get_upload_data(dat) for dat in file_data])


# upload_data = upload_data[["well_position","well_position_name","well_position_name_transpose", "antibiotic","abxconcentration", "folder","usermeta2","usermeta3"]]
