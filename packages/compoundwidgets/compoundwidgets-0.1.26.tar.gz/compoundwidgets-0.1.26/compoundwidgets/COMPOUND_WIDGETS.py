        ...
        if isinstance(self.unit_combo, LabelEntryUnit.NoUnitCombo):
            ...
            metric_unit_list = \
                ('mm', 'kPa', 'MPa', 'GPa', 'mm²', 'cm²',
                 'kgf/cm²', 'bar', 'N', 'kgf', 'N.m', 'kgf.m', 'MPa.√m')
            imperial_unit_list = \
                ('in', 'psi', 'ksi', 'x10³ ksi', 'in²', 'in²',
                 'psi', 'psi', 'lbf', 'lbf', 'lbf.ft', 'lbf.ft', 'ksi.√in')
            conversion = \
                (25.4, 6.894757, 6.894757, 6.894757, 645.16, 6.4516, 0.07030696, 0.06894757, 4.448222,
                 0.4535924, 1.35582, 0.1382552, 1.0988)
            value, unit = self.get()
            if unit not in metric_unit_list:
                index = imperial_unit_list.index(unit)
                new_value = round(float(value)*conversion[index], 2)
                self.set(new_value, metric_unit_list[index])
    ...
    def convert_data_to_metric(value, unit):
        ...
        if unit == '-':
            ...
            if unit not in metric_unit_list:
                index = imperial_unit_list.index(unit)
                new_value = round(float(value)*conversion[index], 2)
                return new_value, metric_unit_list[index]
        ...
        if isinstance(self.unit_combo, LabelEntryUnit.NoUnitCombo):
            ...
            conversion = \
                (25.4, 6.894757, 6.894757, 6.894757, 645.16, 6.4516, 0.07030696, 0.06894757, 4.448222,
                 0.4535924, 1.35582, 0.1382552, 1.0988)
            value, unit = self.get()
            if unit not in imperial_unit_list:
                index = metric_unit_list.index(unit)
                new_value = round(float(value) / conversion[index], 2)
                self.set(new_value, imperial_unit_list[index])
    ...
    def convert_data_to_imperial(value, unit):
        ...
        if unit == '-':
            ...
            if unit not in imperial_unit_list:
                index = metric_unit_list.index(unit)
                new_value = round(float(value) / conversion[index], 2)
                return new_value, imperial_unit_list[index]
