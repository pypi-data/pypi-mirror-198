...
class SftpConnection:
    ...
    def upload_folder(self, local_source: Path, remote_parent: Path, recursive: bool = True, make_tree: bool = False) -> None:
        if not local_source.is_dir():
            raise FileNotFoundError(f"{local_source} does not exist.")
        if not self.connection.isdir(str(remote_parent)):
            if make_tree:
        ...
        files, folders = list_files_and_folders(local_source)
