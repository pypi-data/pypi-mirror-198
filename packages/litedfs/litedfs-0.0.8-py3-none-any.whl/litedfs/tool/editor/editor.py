...
from litedfs.tool.editor.utils import common
from litedfs.tool.editor.config import CONFIG, load_config
from litedfs.tool.editor import logger

LOG = logging.getLogger(__name__)


def main():
    parser = argparse.ArgumentParser(prog = 'ldfseditor')
    parser.add_argument("-g", "--generate-config", help = "generate configuration file & scripts into given path")
    parser.add_argument("-c", "--config", help = "run litedfs editor with configuration file")
    ...
    if args.generate_config:
        ...
        if success:
            common.init_storage()
            logger.config_logging(file_name = "editor.log",
            ...
            from litedfs.tool.editor.handlers import info, fsimage, editlog, filesystem
            ...
            class Application(tornado.web.Application):
                def __init__(self):
                    handlers = [
                        ...
                        (r"/filesystem", filesystem.FileSystemHandler),
                        (r"/filesystem/logs", filesystem.FileSystemLogsHandler),
                        (r"/fsimage", fsimage.FSImageHandler),
                        (r"/editlog", editlog.EditLogHandler),
                    ]
                    settings = dict(
                        debug = False,
