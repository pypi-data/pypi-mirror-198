...
class InitModel(CliApiBase):
    ...
    _model_name = plumbum.cli.SwitchAttr(
        names=['-n', '--name'],
        argtype=str,
    ...
    def _model_path(self):
        return os.path.join(self._path, self._model_name)
    ...
    def _generate_template_repo(self):
        cookiecutter(
            ...
            extra_context={
                'model_name':  self._model_name.replace('-', ' ').title(),
    ...
    def _create_repo(self, github):
        ...
        self._pass(
            f'Created the GitHub repository "{self._org}/{self._model_name}"')
        return response
