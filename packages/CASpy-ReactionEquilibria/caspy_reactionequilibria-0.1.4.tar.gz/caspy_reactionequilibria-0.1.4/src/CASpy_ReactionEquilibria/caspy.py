...
def main(input_file):
    ...
    # The input file should be in the same folder with the solver
    input_parameters = read_input(input_file)
    input_parameters['V'] = 1e-3  # Volume in m3 (default=1e-3 m3)
    ...
    if input_parameters['impose_ptotal']:
        input_parameters['init_amounts'] = np.zeros((input_parameters['Nbalances'] + input_parameters['Ngas'], len(input_parameters['Ptotal'])))
    else:
        input_parameters['init_amounts'] = np.zeros((input_parameters['Nbalances'], len(input_parameters['cgas'][0, :])))

    input_parameters['gas_indices'] = []
    for i in input_parameters['gas_names']:
        input_parameters['gas_indices'].append(input_parameters['names'].index(i))

    if input_parameters['impose_ptotal']:
        for i in range(input_parameters['Nbalances']):
            for j in range(len(input_parameters['Ptotal'])):
                for k in range(len(input_parameters['balances'][i])):
                    init_amounts[i][j] = init_amounts[i][j] + input_parameters['x0'][input_parameters['balances'][i][k]]
        for i in range(input_parameters['Nbalances'], input_parameters['Nbalances'] + input_parameters['Ngas']):
            for j in range(len(input_parameters['Ptotal'])):
                init_amounts[i][j] = (
                    (input_parameters['Ptotal'][j] * input_parameters['gas_composition'][i - input_parameters['Nbalances']])
                    * input_parameters['V']
                    * np.exp(-input_parameters['muex_gas'][i - input_parameters['Nbalances']] / (R * input_parameters['T']))
                    / (R * input_parameters['T'])
                )

    else:
        for i in range(input_parameters['Nbalances']):
            for j in range(len(input_parameters['cgas'][0, :])):
                for k in range(len(input_parameters['balances'][i])):
                    if input_parameters['balances'][i][k] in input_parameters['gas_indices']:
                        ind = input_parameters['gas_names'].index(input_parameters['names'][input_parameters['balances'][i][k]])
                        input_parameters['init_amounts'][i][j] = input_parameters['init_amounts'][i][j] + input_parameters['cgas'][ind, j]
                    else:
                        input_parameters['init_amounts'][i][j] = input_parameters['init_amounts'][i][j] + input_parameters['x0'][input_parameters['balances'][i][k]]

    # The function calc_K0 computes the desired equilibrium constants of the reactions in the system
    K0 = calc_k0(**input_parameters) # compute desired equilibrium constants
    lnK0 = np.log(K0)
    # The function calc_speciation computes the speciation as a function of gas concentration
    if input_parameters['impose_ptotal']:
        spec = calc_speciation_impose_ptotal(**input_parameters)
    else:
        spec = calc_speciation(**input_parameters)
...
def read_input(file_name):
    ...
    for line in input_data:
        if "Temperature" in line:
            ...
            input_parameters['T'] = float(input_data[index + 1])
        if "Number of Species" in line:
            index = input_data.index(line)
            Nspecies = int(input_data[index + 1])  # Number of species
        if "C0" in line:
            index = input_data.index(line)
            input_parameters['x0'] = np.zeros(Nspecies)
            for i in range(Nspecies):
                if float(input_data[index + 1].split()[i]) <= 0:
                    input_parameters['x0'][i] = 1e-10
                else:
                    input_parameters['x0'][i] = float(input_data[index + 1].split()[i])
        if "Names of species" in line:
            index = input_data.index(line)
            # Names of the species
            input_parameters['names'] = []
            for i in range(Nspecies):
                input_parameters['names'].append(input_data[index + 1].split()[i])
        if "Charges" in line:
            index = input_data.index(line)
            # Net charges of the species, follows the indices in the the list of names
            input_parameters['charges'] = np.zeros(Nspecies)
            for i in range(Nspecies):
                input_parameters['charges'][i] = int(input_data[index + 1].split()[i])
        if "mu^0 species" in line:
            index = input_data.index(line)
            # The ideal gas standard chemical potentials of the species in J/mol
            input_parameters['mu_0'] = np.zeros(Nspecies)
            for i in range(Nspecies):
                # convert kJ/mol to J/mol and save the value
                input_parameters['mu_0'][i] = float(input_data[index + 1].split()[i]) * 1e3
        if "mu^ex species" in line:
            index = input_data.index(line)
            # The excess chemical potentials of the species in J/mol
            input_parameters['mu_ex'] = np.zeros(Nspecies)
            for i in range(Nspecies):
                # convert kJ/mol to J/mol and save the value
                input_parameters['mu_ex'][i] = float(input_data[index + 1].split()[i]) * 1e3
        if "Impose Ptotal and gas composition?" in line:
            index = input_data.index(line)
            true_answers = ["true", "t"]
            input_parameters['impose_ptotal'] = input_data[index + 1].split()[0].lower() in true_answers
        ...
        if "Ptotal / [kPa]" in line:
            index = input_data.index(line)
            input_parameters['Ptotal'] = float(input_data[index + 1].split()[i]) * 1e3
        if "Gas phase species" in line:
            ...
            input_parameters['Ngas'] = len(input_data[index + 1].split())  
            # Names of the species in the gas phase
            input_parameters['gas_names'] = []
            for i in range(Ngas):
                input_parameters['gas_names'].append(input_data[index + 1].split()[i])
        if "Gas phase composition" in line:
            index = input_data.index(line)
            input_parameters['gas_composition'] = np.zeros(Ngas)
            for i in range(Ngas):
                input_parameters['gas_composition'][i] = float(input_data[index + 1].split()[i])
            # Normalize gas phase composition so the total mole fraction is 1.
            sum_gas_molefraction = sum(input_parameters['gas_composition'])
            input_parameters['gas_composition'] = np.divide(input_parameters['gas_composition'], sum_gas_molefraction)
        if "mu^ex gases" in line:
            index = input_data.index(line)
            # The excess chemical potentials of the gas species in solvent at T in J/mol.
            input_parameters['muex_gas'] = np.zeros(Ngas)
            for i in range(Ngas):
                # convert kJ/mol to J/mol
                input_parameters['muex_gas'][i] = float(input_data[index + 1].split()[i]) * 1e3
        if "Ctotal,gas" in line:
            index = input_data.index(line)
            # The concentrations of the gas species. This is a list.
            input_parameters['cgas'] = np.zeros((Ngas, len(input_data[index + 1].split())))
            ...
            for i in range(Ngas):
                for j in range(len(input_parameters['cgas'][i, :])):
                    input_parameters['cgas'][i, j] = float(input_data[index + i + 1].split()[j])
        if "Number of Reactions" in line:
            index = input_data.index(line)
            # Number of reactions
            input_parameters['Nreactions'] = int(input_data[index + 1])  
        if "Stoichiometry" in line:
            index = input_data.index(line)
            # The matrix that contains the stoichiometry.
            input_parameters['stoichiometry'] = np.zeros((input_parameters['Nreactions'],Nspecies))
            for i in range(input_parameters['Nreactions']):
                for j in range(Nspecies):
                    input_parameters['stoichiometry'][i, j] = int(input_data[index + 1 + i].split()[j])
        if "Number of mass balance" in line:
            index = input_data.index(line)
            # Number of mass balances in the system
            input_parameters['Nbalances'] = int(input_data[index + 1])
        if "Balances" in line:
            index = input_data.index(line)
            # This list contains lists of the species included in each balance equation.
            input_parameters['balances'] = [
                []
            ] * input_parameters['Nbalances']
            for i in range(input_parameters['Nbalances']):
                input_parameters['balances'][i] = []
                for j in range(len(input_data[index + 1 + i].split())):
                    input_parameters['balances'][i].append(int(input_data[index + 1 + i].split()[j]) - 1)
        if "ln(K)" in line:
            index = input_data.index(line)
            # this is list contains the ln(K) of each reaction.
            input_parameters['lnK'] = np.zeros(input_parameters['Nreactions'])
            input_parameters['compute_Kdes'] = np.full(input_parameters['Nreactions'],False)
            ...
            for i in range(input_parameters['Nreactions']):
                if input_data[index + 1].split()[i] == key:
                    input_parameters['lnK'][i] = int(0.0)
                    input_parameters['compute_Kdes'][i] = True
                else:
                    input_parameters['lnK'][i] = float(input_data[index + 1].split()[i])
        if "Name of the solvent" in line:
            index = input_data.index(line)
            input_parameters['solvent_index'] = input_parameters['names'].index(input_data[index + 1].split("\n")[0])
        if "Pure Density" in line:
            index = input_data.index(line)
            # convert from mol/dm3 to molecules per cubic angstrom
            input_parameters['pure_density'] = float(input_data[index + 1]) * NA / 1e27
