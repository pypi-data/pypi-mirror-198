...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    def __init__(self,
        ...
        for k in self.btn_icons.keys():
            self.btn_icons[k]['img'] = PhotoImage(file=os.path.join(os.path.dirname(__file__), self.btn_icons[k]['icon_path']))
...
class App(object):
    def __init__(self):
        ...
        icon_path = os.path.join(os.path.dirname(__file__), Paths.LOGO_ICON_PATH.value)
        ...
        for k in self.menu_icons.keys():
            self.menu_icons[k]['img'] = PhotoImage(file=os.path.join(os.path.dirname(__file__), self.menu_icons[k]['icon_path']))
        ...
        video_process_menu.add_command(label='Down-sample videos',command=DownsampleVideoPopUp)
        ...
        video_process_menu.add_command(label='Create path plot', command=MakePathPlotPopUp)
        video_process_menu.add_cascade(label='Change fps...',menu =fps_menu)
        video_process_menu.add_cascade(label='Concatenate two videos', command=ConcatenatingVideosPopUp)
        video_process_menu.add_cascade(label='Concatenate multiple videos', command=lambda:ConcatenatorPopUp(config_path=None))
        ...
        video_process_menu.add_cascade(label='Reorganize Tracking Data', command= PoseReorganizerPopUp)
        video_process_menu.add_cascade(label='Drop body-parts from tracking data', command=DropTrackingDataPopUp)
        ...
        video_process_menu.add_command(label='CLAHE enhance video',command=CLAHEPopUp)
        video_process_menu.add_command(label='Superimpose frame numbers on video',command=lambda:superimpose_frame_count(file_path=askopenfilename()))
        video_process_menu.add_command(label='Convert to grayscale',command=lambda:video_to_greyscale(file_path=askopenfilename()))
        video_process_menu.add_command(label='Merge frames to video',command=MergeFrames2VideoPopUp)
        video_process_menu.add_command(label='Generate gifs', command=CreateGIFPopUP)
        video_process_menu.add_command(label='Print classifier info...', command=PrintModelInfoPopUp)
        ...
        video_process_menu.add_cascade(label='Extract frames...',menu=extract_frames_menu)
        ...
        video_process_menu.add_cascade(label='Convert working file type...', menu=convert_data_menu)
