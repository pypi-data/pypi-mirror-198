...
def reads2bam(args):
    ...
    if (r1_seq is not None) and (r2_seq is not None) and (unpaired_seq is None):
        cmd_bowtie2 = 'bowtie2 -x %s -1 %s -2 %s -S %s_raw.sam %s'          % (ref_basename, r1_seq, r2_seq, ref_basename, bowtie2_parameter)
    elif (r1_seq is not None) and (r2_seq is not None) and (unpaired_seq is not None):
        cmd_bowtie2 = 'bowtie2 -x %s -1 %s -2 %s -U %s -S %s_raw.sam %s'    % (ref_basename, r1_seq, r2_seq, unpaired_seq, ref_basename, bowtie2_parameter)
    elif (r1_seq is None) and (r2_seq is None) and (unpaired_seq is not None):
        cmd_bowtie2 = 'bowtie2 -x %s -U %s -S %s_raw.sam %s'                % (ref_basename, unpaired_seq, ref_basename, bowtie2_parameter)
    else:
