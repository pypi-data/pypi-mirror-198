...
from .chai_tools import contains_any
...
def fetch_args(argv):
    ...
    parser = ArgumentParser(
        prog="chai",
        description=f"""\
      )  (              ChaiGPT - its like ChatGPT but better!
     (   ) )
   (     (    )         ChaiGPT is an extensible open source framework and CLI
 ...
 |              |    ||
 |              |___//  Chai can already use DuckDuckGo, URL summarization and
  \            /----'   Python programming to better assist the user, with Voice
   \          /         and WebUI coming soon. Your chat history is saved locally,
    \________/          and you can use your own OpenAI API key. Happy chatting!""",
        epilog="""\
You can contribute new injections and extensions for Chai!
Visit https://github.com/VulcanicAI/ChaiGPT to learn more.
""",
    ...
    if (
        ...
        parser.add_argument(  # Only require prompt if necessary
            "prompt", type=str, default="", help="the prompt to submit",
        )
    else:
        parser.set_defaults(prompt="")

    parser.add_argument(
        "-l", "--list", action="store_true", help="list past conversations",
    )
    # parser.add_argument(
    #     "-n",
    #     "--num",
    ...
    # )
    parser.add_argument(
        "-c", "--clean", action="store_true", help="force a fresh instance",
    )
    parser.add_argument(
        "-s",
        "--search",
    ...
    parser_resume.add_argument(
        "-r", "--resumelast", action="store_true", help="resume previous chat session",
    )
    parser_resume.add_argument(
        "--time",
        type=str,
    ...
    parser.add_argument(
        "-x", "--python", action="store_true", help="enable python interpreter",
    )
    parser.add_argument(
        "-P", "--professor", action="store_true", help="summon the professor",
    )
    parser.add_argument(
        "-f", "--fileprompt", action="store_true", help="prompt will be read from file",
    )
    parser.add_argument(
        "-D",
        "--datadir",
        type=str,
        help="specify a directory to save and load Chai history",
    )
    # parser.add_argument(
    #     "-a",
    #     "--audio",
    ...
    # if TESTS_ENABLED:
    #     parser.add_argument(
    #         "--test", action="store_true", help="drop everything and test",
    #     )
    # else:
