...
from .chai_tools import clamp, plural, msg, contains_any, contains_all
...
class ChaiChat(object):
    ...
    def make_filename(self):
        filename = slugify.slugify(self.title, separator="_")
    ...
    def parse_cmds(self, reprompt=True, cmd_chain=0):
        ...
        if cmd_chain > MAX_COMMANDS:
            print(
                f"""\
{MAX_COMMANDS} is too many bot commands in a row, but Chai would like to keep going. To allow \
{MAX_COMMANDS} more bot {plural('command', MAX_COMMANDS)}, use the -b flag."""
            )
            return
