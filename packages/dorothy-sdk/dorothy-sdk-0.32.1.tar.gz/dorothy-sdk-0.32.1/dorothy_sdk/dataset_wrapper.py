...
class Dataset:
    ...
    def load_images(self):
        """Returns a tensor containing all images and the target vector"""
        if not self._cache_exists(self.data_file_name):
            self._assemble_dataset()
        return np.load(os.path.join(self.cache_location, self.data_file_name))

    def load_dataset(self):
        """Returns the images tensor and the corresponding metadata (img_mat, metadata_df)"""
        return self.load_images(), self.load_metadata()
    ...
    def _assemble_dataset(self):
        ...
        for img in ds.list_images():
            ...
            images_list.append(Dataset.pre_process_image(img))
        ...
        #Saving images
        img_file_name = os.path.join(self.cache_location, self.data_file_name)
        tensor_images = np.concatenate(images_list, axis=0)
        np.save(img_file_name, tensor_images)
    ...
    def join_datasets(*datasets) -> pd.DataFrame:
        ...
        for ds_name in datasets:
            ...
            cv_splitter = cv_splitter.merge(meta, how='inner', on='image')
            
            ds_list.append(cv_splitter)
    ...
    @staticmethod
    def load_from_file(df: pd.DataFrame, path_col_name: str='image_local_path'):
