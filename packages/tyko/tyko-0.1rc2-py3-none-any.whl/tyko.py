...
class _TykoAPI:
    ...
    def __init__(
        ...
        if key is None and _TYKO_KEY in os.environ:
            key = os.environ[_TYKO_KEY].encode("utf-8")
        ...
        if key is None:
            try:
                ...
                if host in netrc_.hosts:
                    key = netrc_.hosts[self._hostname][2].encode("utf-8")
            except FileNotFoundError:
                pass

        # Finally, just try to create a new one.
        if key is None:
            key = self._get_new_key().encode("utf-8")
    ...
    def _set_key(self, key: bytes):
        ...
        self._session.headers['X-API-KEY'] = self._key
...
class _LiveReporterThread(threading.Thread):
    ...
    def __init__(self, q, key_shm: SharedMemory = None):
        super().__init__()
        self._q = q
        self._keep_running = True
        self._api = _TykoAPI(key_shm=key_shm)
...
class _LiveReporter(_Reporter):
    ...
    _key_shm: SharedMemory

    def __init__(self, key_shm: SharedMemory = None):
        ...
        # Start a thread.
        self._thread = _LiveReporterThread(self._queue, key_shm=self._key_shm)
        self._thread.start()
...
class Tyko:
    ...
    def __init__(self, project_name: str, offline=False, **kwargs):
        ...
        if not offline:
            self._reporters.append(_LiveReporter(key_shm=self._key_shm))
