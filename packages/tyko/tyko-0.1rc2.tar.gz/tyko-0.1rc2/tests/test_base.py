...
#     def setUp(self):
#         self.monotonic_ns = patch('time.monotonic_ns')
#         self.monotonic_ns_val = 0
#         self.monotonic_ns_mock = self.monotonic_ns.start()

#         def monotonic_ns():
#             self.monotonic_ns_val += 1
#             return self.monotonic_ns_val - 1

#         self.monotonic_ns_mock.side_effect = monotonic_ns

#     def tearDown(self):
#         self.monotonic_ns.stop()
...
# @pytest.fixture
# def fs():
#      with Patcher(allow_root_user=False, additional_skip_names=['multiprocessing.connection', 'requests.adapters']) as patcher:
#          yield patcher.fs
...
def test_tyko_object_api(monotonic, api):
    t = Tyko('my-project')
    for i in range(3):
        t.log({'i': i})
    sleep(0.5)
