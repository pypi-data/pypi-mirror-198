...
def get_auction_info(order_book_ids, start_date=None, end_date=None, auction_type='close', market="cn"):
    ...
    try:
        data = get_client().execute("get_{}_auction_info_daybar".format(auction_type), order_book_ids,
                                    start_date, end_date, AUCTION_FIELDS + ["datetime", "date"], market=market)
    except PermissionDenied:
        data = []
        history_permission_denied = True
    ...
    if data:
        ...
        df = convert_bar_to_multi_df(data, 'datetime', AUCTION_FIELDS + extra_fields, int17_to_datetime_v, default=0.0)
        if df is None:
    ...
    try:
        live_df = get_today_auction(order_book_ids, auction_type, today, market=market)
    except PermissionDenied:
        live_df = None
        realtime_permission_denied = True
    except MarketNotSupportError:
...
def get_open_auction_info(order_book_ids, start_date=None, end_date=None, market="cn"):
    ...
    """
    return get_auction_info(order_book_ids, start_date, end_date, 'open', market)


@export_as_api
def get_close_auction_info(order_book_ids, start_date=None, end_date=None, market="cn"):
    """获取尾盘集合竞价数据
    :param order_book_ids: 股票代码
    :param start_date: 起始日期，默认为今天
    :param end_date: 截止日期，默认为今天
    ...
    :returns: pd.DataFrame or None
    """
    return get_auction_info(order_book_ids, start_date, end_date, 'close', market)
