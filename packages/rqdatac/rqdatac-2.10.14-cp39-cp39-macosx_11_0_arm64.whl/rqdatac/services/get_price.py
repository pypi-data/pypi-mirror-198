...
def convert_history_tick_to_multi_df(data, dt_name, fields, convert_dt):
    ...
    for i, (obid, d) in enumerate(data):
        ...
        if daybar is not None:
            ...
            if instruments(obid).type in ("CS", "ETF", "LOF", "FUND", "Future", "Spot", "Option"):
    ...
    for ins in ins_list:
        try:
            if history_df is not None and \
                    date_to_int8(get_current_trading_date(history_df.loc[ins.order_book_id].index.max())) == live_date:
                continue
        except KeyError:
            pass
        try:
