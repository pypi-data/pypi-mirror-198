...
def init(platform: str):
    if os.path.isfile('./script/config/env_config.ini'):
        LoadConfig()
    else:
        SetConfig(platform)

    lib_dir = platform_configs.get(configs.get('PLATFORM'))
    ...
    if not os.path.isfile(os.path.join(out_path, 'config.cfg')):
        utils.copy(os.path.join(src_path, '../data/config/config.cfg'), os.path.join(out_path))
