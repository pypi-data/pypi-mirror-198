...
class SnowflakeSourceConnector(C8Connector):
    ...
    def validate(self, integration: dict) -> None:
        ...
        try:
            # TODO: CHECK ABOUT do_discover() and do_discovery and make changes
            do_discover(config)
        except Exception as e:
            raise e

    def samples(self, integration: dict) -> list[Sample]: # TODO: Need to add handling of reserved keys
        """Fetch sample data using the provided configurations."""
        config = self.get_config(integration)
        try:
            streams = do_discover(config)
            results = []
    ...
    def schemas(self, integration: dict) -> list[Schema]: # TODO: Need to add handling of reserved keys
    ...
    def config(self) -> list[ConfigProperty]:
        ...
        return [
            ConfigProperty('acccount', 'Account', ConfigAttributeType.STRING, True, False,
    ...
    def get_config(integration: dict) -> dict:
        try:
            return {
                ...
                'role': integration.get('role', None),
                'private_key_path': integration.get('private_key_path', None),
                'private_key_passphrase': integration.get('private_key_passphrase', None),
