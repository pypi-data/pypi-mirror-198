...
def test_execution_instance(*args):
    ...
    expected_merged_notebook_path = os.path.join(system.get_repo_root_dir(), test_notebook_path, scheduled_file_folder,
                                                 f'.spy.pull.executor.{test_sched_index}.{label}.ipynb')
    assert system.cleanse_path(execution_instance.merged_notebook_path) == \
           system.cleanse_path(expected_merged_notebook_path)

    expected_job_result_path = os.path.join(system.get_repo_root_dir(), test_notebook_path,
                                            _execute.RESULTS_FOLDER,
                                            f'spy.pull.executor.{test_sched_index}.{label}.html')
    assert system.cleanse_path(execution_instance.job_result_path) == system.cleanse_path(expected_job_result_path)
