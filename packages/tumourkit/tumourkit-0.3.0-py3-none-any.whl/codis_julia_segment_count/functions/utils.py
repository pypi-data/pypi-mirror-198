...
def create_subfolders_output(root_path, subfolders_names):
    
    if not os.path.exists(root_path):
        os.makedirs(root_path)
    
    for subfolder_name in subfolders_names:
        if not os.path.exists(root_path + subfolder_name):
            os.mkdir(root_path + subfolder_name)
...
def split_dataset(ori_path, gt_path, sufix):
    ...
    train_FileNames, val_FileNames= np.split(np.array(allFileNames),[int(len(allFileNames)*0.95)])
