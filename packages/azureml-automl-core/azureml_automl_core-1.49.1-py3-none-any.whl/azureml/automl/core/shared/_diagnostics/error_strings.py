...
class AutoMLErrorStrings:
    ...
    DATASET_CONTAINS_NAN = "The input dataset {data_object_name} contains NaN (np.NaN) values. " \
                           "Please drop these rows before submitting the experiment again or turn on featurization."
    ...
    FORECAST_DISTRIBUTED_WRONG_INDICATOR_TYPE = "Wrong data type in the indicator type dictionary was encountered."
    ...
    INSUFFICIENT_GPU_MEMORY = "There is not enough GPU memory on the machine to do the operation. " \
                              "Please try running the experiment on a VM with higher GPU memory, decrease the batch " \
                              "size or image size."
    INSUFFICIENT_MEMORY = "There is not enough memory on the machine to do the operation. " \
    ...
    INSUFFICIENT_MEMORY_LIKELY = "'Subprocess (pid {pid}) killed by unhandled signal {errorcode} ({errorname}). " \
                                 ...
                                 "Please try running the experiment on a VM with higher memory."
    INSUFFICIENT_MEMORY_WITH_HEURISTICS = "There is not enough memory on the machine to do the operation. " \
    ...
    INSUFFICIENT_SAMPLE_SIZE = "The training dataset '{data_object_name}' has {sample_count} usable records (i.e. " \
                               "those which are not NaN or None). That is less than the minimum required training " \
                               "sample size of {minimum_count}. Consider adding more data points to ensure better " \
                               "model accuracy."
    INSUFFICIENT_SHM_MEMORY = "There is not enough shared memory on the machine to do the operation. " \
    ...
    LARGE_DATA_ALGORITHMS_UNSUPPORTED = "AveragedPerceptronClassifier, FastLinearRegressor, " \
                                        "OnlineGradientDescentRegressor are supported only in AutoML SDK " \
                                        "versions 1.48.0 or below. Consider upgrading to AutoML SDK V2 to evaluate " \
                                        "distributed training algorithms that can handle large data."
    ...
    NETWORK_VNET_MISCONFIG = "A virtual network '{vnet}' appears to have been set on '{cluster_name}' compute. " \
                             "Please verify that the required inbound and outbound network traffic permissions are " \
                             "configured in the NSG and firewall of the VNet. For more details, refer to " \
                             "https://learn.microsoft.com/en-us/azure/machine-learning/how-to-access-azureml" \
                             "-behind-firewall?tabs=ipaddress%2Cpublic ."
    NLP_ARGUMENT_PARSING_ERROR = "Encountered exception while parsing command-line arguments: {details}"
    NLP_CLASSIFICATION_INSUFFICIENT_COLUMNS = "Multi-class and multi-label classification training requires at" \
                                              " least one non-label column."
    ...
    NLP_NER_INSUFFICIENT_EXAMPLES = "NER training requires at least {exp_cnt:log_safe} samples for" \
                                    " {split_type:log_safe} set, but only {act_cnt:log_safe} were found."
    NLP_NETWORK_VNET_MISCONFIG = "A virtual network '{vnet}' appears to have been set on '{cluster_name}' compute. " \
                                 "Please verify that the required inbound and outbound network traffic permissions " \
                                 "are configured in the NSG and firewall of the VNet. For more details, refer to " \
                                 "{info_link:log_safe}."
    ...
    TIMESERIES_INVALID_DATE_OFFSET = \
        "The provided date offset has vary high granularity relative to provided data set. " \
        "This error can happen if the data set frequency is yearly, but the freq parameter is set to " \
        "second. Please decrease the granularity of a freq parameter and run the experiment again."
    ...
    TIMESERIES_UNEXPECTED_ORIGIN = "Input dataframe is malformed for this transform (origin times are present)." \
                                   ...
                                   "support team."
    TIMESERIES_UNSUPPORTED_TIME_SEQUENCE_STATIONARY_FEATURIZER = "Unsupported time sequence for stationary featurizer."
