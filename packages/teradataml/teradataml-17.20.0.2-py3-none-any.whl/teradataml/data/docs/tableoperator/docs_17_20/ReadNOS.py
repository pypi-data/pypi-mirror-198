def ReadNOS(data=None, location=None, authorization=None, return_type="NOSREAD_RECORD", sample_perc=1.0,
    ...
    PARAMETERS:
        ...
        authorization:
            ...
            platform, user can specify an authorization object
            ([DatabaseName.]AuthorizationObjectName). User must have the EXECUTE
            privilege on AuthorizationObjectName. On Amazon S3 and Azure Blob
            storage and Azure Data Lake Storage Gen2, user can specify either an
            ...
            If S3 user account requires the use of physical or virtual security, session token can be used 
            with Access_ID and Access_Key in this syntax:
                authorization = '{"Access_ID":"access_id", "Access_Key":"secret_key",
                                  "Session_Token":"session_token"}'
            In which, the session token can be obtained using the AWS CLI. 
                For example: aws sts get-session-token
            When accessing GCS, Analytics Database uses either the S3-compatible connector or the native Google 
            connector, depending on the user credentials.
            Types: str
        ...
        row_format:
            ...
            JSON format must be used to specify the format. It can include only the three
            ...
            Default Value: '{"character_set":"UTF8"}'
            Types: str
    ...
    EXAMPLES:
        ...
        # an authorization object, and a function mapping (for READ_NOS Analytics Database function).
