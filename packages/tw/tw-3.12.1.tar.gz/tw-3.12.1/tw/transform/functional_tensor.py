...
def affine_theta(inputs: torch.Tensor, theta, interpolation=T.RESIZE_MODE.BILINEAR, **kwargs) -> torch.Tensor:
  return tvf.F_t.affine(inputs, matrix=inputs, interpolation=T.RESIZE_MODE_TO_PIL[interpolation])
...
def ten_crop(inputs: torch.Tensor, **kwargs) -> torch.Tensor:
  raise NotImplementedError


def five_crop(inputs: torch.Tensor, **kwargs) -> torch.Tensor:
  raise NotImplementedError
