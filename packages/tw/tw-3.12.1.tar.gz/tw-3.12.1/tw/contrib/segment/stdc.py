...
class STDCNet(nn.Module):
  ...
  """classes
    ...
    0 未定义 不属于以下任何一个类别的； #ffffff
    1 面部 #b71c1c
    2 面部外的皮肤 #ffccbc
    3 头发 #010101
    4 服饰 包括衣服、裤子、帽子、头巾、口罩等； #4a148c
    5 玩偶 #969696
    6 话筒 麦克风的主体部分，不包含支架、电线等；#0d47a1
    7 头戴式耳机 #2196f3
    8 坐具 各种类型的椅子、凳子等；#006064
    9 弦乐器 例如吉他、提琴、古筝等; #f57f17
    10 管乐器 例如萨克斯、长笛、黑管等; #fbc02d
    11 键盘乐器 例如电子琴、钢琴等的琴键部分; #ffeb3b
    12 DJ控制器 #e65100
  ...
  def __init__(self,
    ...
    if ckpt_path is not None:
      tw.checkpoint.load_state_dict_from_url(self.seg, ckpt_path)

    self.mean = torch.tensor((0.5081, 0.4480, 0.4340)).reshape(1, 3, 1, 1)
    self.std = torch.tensor((0.2822, 0.2757, 0.2734)).reshape(1, 3, 1, 1)
  ...
  def process(self, frame, is_bgr=True):
    ...
    seg_out, sod_out = self.seg(input)
