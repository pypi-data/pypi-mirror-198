...
def docx_report(y_bl,y_report,df,df_report,conformite,conformite_report,table_incertitude,table_incertitude_report,df_savings,report_prediction,bl_prediction,start_baseline_period,end_baseline_period,start_reporting_period,end_reporting_period):
    ...
    table = doc.add_table(rows=y_report.head().shape[0]+1, cols=y_report.head().shape[1]+1)
    # Ajouter l'en-tête de colonne
    for j in range(y_report.head().shape[-1]):
        table.cell(0,j+1).text = y_report.head().columns[j]
    # Ajouter l'en-tête de ligne
    table.cell(0,0).text = 'Index'
    # Ajouter les données du dataframe
    for i in range(y_report.head().shape[0]):
        table.cell(i+1,0).text = str(y_report.head().index[i])
        for j in range(y_report.head().shape[-1]):
            table.cell(i+1,j+1).text = str(y_report.head().iloc[i,j])
