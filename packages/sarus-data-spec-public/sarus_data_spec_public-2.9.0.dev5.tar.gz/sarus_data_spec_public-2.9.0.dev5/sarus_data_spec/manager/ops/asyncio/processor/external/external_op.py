...
from sarus_data_spec.query_manager.typing import PEPKind
...
class ExternalScalarOp(ScalarImplementation):
    async def value(self) -> t.Any:
        ...
        return await self.value_from_inputs(transform, *ds_args, **ds_kwargs)

    @staticmethod
    async def value_from_inputs(
        transform: st.Transform,
        *ds_args: st.DataSpec,
        **ds_kwargs: st.DataSpec,
    ) -> t.Any:
        """Compute the a Scalar value without manipulating the Scalar itself.

        Useful for computing a Mock value and inferring if the result is
...
def to_pyarrow_table(x: t.Any) -> pa.Table:
    """Convert the result of an external transform to a Pyarrow Table."""
    if not isinstance(x, pd.DataFrame):
        raise TypeError(f"Cannot convert {type(x)} to Arrow batches.")

    df = t.cast(pd.DataFrame, x)
