...
import platform
import re
from typing import Any, Dict, Generator, List, MutableMapping, Optional, cast
...
def db_session_small(
    ...
    df = create_dataset(dims=(500, 4))
    df2 = create_dataset(multiimage=True, file_image=True, path=tmp_path, dims=(500, 4))
...
class TestPyTorchModel:
    ...
    def test_weight_is_constraint_for_secure_agg(
        ...
        neural_network._model._modules["bn_cont"].weight.data *= 2**60
        neural_network._model._modules["bn_cont"].running_var.data *= 2**60
        neural_network._model._modules["bn_cont"].running_mean.data *= 2**60

        neural_network.fit(datasource)
        assert torch.max(
            neural_network._model._modules["bn_cont"].weight.data
        ) <= LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
        assert torch.max(
            neural_network._model._modules["bn_cont"].weight.data
        ) >= -LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
        assert torch.max(
            neural_network._model._modules["bn_cont"].running_var.data
        ) <= LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
        assert torch.max(
            neural_network._model._modules["bn_cont"].running_var.data
        ) >= -LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
        assert torch.max(
            neural_network._model._modules["bn_cont"].running_mean.data
        ) <= LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
        assert torch.max(
            neural_network._model._modules["bn_cont"].running_mean.data
        ) >= -LARGE_PRIME_NUMBER / (FLOAT_32_BIT_PRECISION * 20)
