...
FLOW_TYPE_OPTIONS = ["okta", "approval-only", "aws-iam"]
...
def generate(options: GlobalOptions, type_: str, flow_name: Optional[str] = None) -> None:
    ...
    generator = generator_cls(flow_name=flow_name)
    generator.generate()
    ...
    if generator_cls.REQUIRES_AWS:
        cli_output.info(
            "\nWhen you are ready to apply your configuration, make sure you're logged into your AWS account and run the following:"
        )
    else:
