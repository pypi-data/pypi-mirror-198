import pymongo
class Mixin:
    def search_txs_on_list_of_hashes(self, list_of_hashses, skip, limit, sort_on='amount', sort_direction=-1):
        '''
        ...
        sort_condition = { '$sort':      { 'amount_ccd': sort_direction} } if sort_on =='amount' else { '$sort':      { 'blockHeight': sort_direction} }
        pipeline = [
    ...
    def search_transfers_based_on_indices(self, transfer_type, gte, lte, start_block, end_block, 
        # skip, limit, 
    ...
    def search_transfers_based_on_indices_v2(self, transfer_type, gte, lte, start_block, end_block, 
        # skip, limit, 
        list_of_tx_hashes_with_memo_predicate,
        # sort_on='amount', sort_direction=-1, 
        filter_on_memo_txs=False):
    ...
    def search_transfers_mongo_v2(self, transfer_type, gte, lte, start_date, end_date, 
        skip, limit, 
        list_of_tx_hashes,
        sort_on='amount', sort_direction=-1, filter_on_tx_hashes=False):
        '''
        This pipeline now includes all transfers, so regular, scheduled and with memo.
        '''
        
        sort_direction = pymongo.ASCENDING if sort_direction == 1 else pymongo.DESCENDING
        sort_condition = { '$sort':      { 'amount_ccd': sort_direction} } if sort_on =='amount' else { '$sort':      { 'block_info.slot_time': sort_direction} }
        ...
        if filter_on_tx_hashes:
            pipeline.extend ([{
                '$match': { '_id': { '$in': list_of_tx_hashes } }
            }])
        ...
        if transfer_type == 'transferred_with_schedule':
            ...
            pipeline.extend([
                { '$set': {
                    "amount_ccd": {
                        '$divide': ["$account_transaction.effects.account_transfer.amount" , 1000000]
                                    }
                    }
                }
            ])
        ...
        pipeline.extend([    
            { '$match':     { 'amount_ccd': { '$gte': gte } } },
            { '$match':     { 'amount_ccd': { '$lte': lte } } },
            { "$match":     {"block_info.slot_time": {"$gte": start_date, "$lt": end_date } } },
            sort_condition,
            {'$facet': {
            'metadata': [ { '$count': 'total' } ],
            'data': [ { '$skip': int(skip) }, { '$limit': int(limit) } ]
    ...
    def search_scheduled_transfers_mongo_v2(self, transfer_type, gte, lte, start_date, end_date, 
        skip, limit, 
        list_of_tx_hashes,
        sort_on='amount', sort_direction=-1, filter_on_tx_hashes=False):
        '''
    ...
    def search_exchange_txs_as_receiver(
        ...
        pipeline = [
            ...
            { '$match': { 
                'receiver_canonical': { '$in': exchanges_canonical } 
                } 
            },
            { "$match":     {"blockHeight": {"$gt": start_block, "$lte": end_block } } },
