...
def generate_lines(elements,set):
    ...
    for element in elements:
        lines.append(obp.Line(element[0], element[1], set.scan_speed.value, set.get_beam_parameters()))
    return lines
...
def generate_curves(elements, set):
    curves = []
    for element in elements:
        curves.append(obp.Curve(element[0],element[1],element[3],element[0], set.scan_speed.value, set.get_beam_parameters()))
    return curves