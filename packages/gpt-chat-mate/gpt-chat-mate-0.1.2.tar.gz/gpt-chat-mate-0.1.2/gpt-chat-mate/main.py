...
class ChatShell(QueryShell):
    ...
    def get_output_messages(self):
        ...
        """
        # a token has on average 4 characters, so we are kind of just estimating tokens here.
        char_limit = self.config.token_limit * 4
        # 0 is always the system prompt, and we always want that in the output
        char_count = len(self.messages[0]["content"])
        ...
        while char_count <= char_limit:
            ...
            char_count += len(self.messages[i]["content"])
            i -= 1
...
class CliGPT(QueryShell):
    ...
    def generate_chat_name(self, system_prompt):
        ...
        resp = openai.ChatCompletion.create(
            model=self.config.gpt_model, messages=messages
        )
    ...
    def do_chat(self, _id):
        ...
        elif not _id.isnumeric():
            print("Given ID must be a number.")
            return
        elif int(_id) not in existing_chats:
    ...
    def preloop(self):
        print(Figlet().renderText("cliGPT"))
