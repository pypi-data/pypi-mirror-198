...
class cdnActions:
    ...
    def cdn_monitoring(self):
        ...
        url = "https://api.e2enetworks.com/myaccount/api/v1/cdn/monitoring-data/?&apikey="+API_key+"&location=Delhi"
        req="GET"
    ...
    def cdn_bandwidth_usage(self):
        ...
        API_key=self.API_key
        Auth_Token=self.Auth_Token
        url = "https://api.e2enetworks.com/myaccount/api/v1/cdn/domain-usage/?apikey="+API_key+"&location=Delhi"
        req="POST"
        status=Request(url, Auth_Token, my_payload, req, query=query).response.json()
