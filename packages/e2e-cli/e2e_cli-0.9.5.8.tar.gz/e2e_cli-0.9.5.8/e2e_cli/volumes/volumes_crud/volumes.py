...
class volumes_Crud:
    ...
    def create_volumes(self):
        ...
        size=Py_version_manager.py_input("input name of space desired (only multiple of 250 GB): ")
        my_payload= json.dumps({
            "name": Py_version_manager.py_input("input name of your new volumes : "),
            "size": size,
            "iops": iops_list[size]
        })
        API_key=self.API_key
        Auth_Token=self.Auth_Token
        url = "https://api.e2enetworks.com/myaccount/api/v1/block_storage/?apikey="+API_key+"&location=Delhi"
        req="POST"
    ...
    def delete_volumes(self):
        my_payload={}
        blockstorage_id=Py_version_manager.py_input("input blockstorage_id of the volumes you want to delete : ")
        
        API_key=self.API_key
        Auth_Token=self.Auth_Token
        url = "https://api.e2enetworks.com/myaccount/api/v1/block_storage/"+blockstorage_id+"/?apikey="+API_key+"&location=Delhi"
        req="DELETE"
        status=Request(url, Auth_Token, my_payload, req).response.json()
