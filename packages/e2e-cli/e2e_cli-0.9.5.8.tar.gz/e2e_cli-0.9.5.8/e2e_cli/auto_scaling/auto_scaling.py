...
class autoscaling_Crud:
    ...
    def create_autoscaling(self):
        ...
        # url = "https://api.e2enetworks.com/myaccount/api/v1/storage/autoscalings/"+ autoscaling_name +"/?apikey="+API_key+"&location=Delhi"
        # req="POST"
    ...
    def delete_autoscaling(self):
        ...
        API_key=self.API_key
        Auth_Token=self.Auth_Token
        url = "https://api.e2enetworks.com/myaccount/api/v1/scaler/scalegroups/"+ autoscaling_id +"?apikey="+API_key
        req="DELETE"
        status=Request(url, Auth_Token, my_payload, req).response.json()
