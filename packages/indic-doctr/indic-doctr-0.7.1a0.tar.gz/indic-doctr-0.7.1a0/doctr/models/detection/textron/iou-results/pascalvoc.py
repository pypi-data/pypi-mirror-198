...
def getBoundingBoxes(
    ...
    if len(files) != 0:
    ...
    elif len(glob.glob("*.txt")) == 0 and len(glob.glob("*.json")) != 0:
        # the format is doctr detection
        # so the coordinate will be in x, y, x2, y2 format
        # the coordinates will be absolute
        print("The ground truth is json file.")
        assert (
            len(glob.glob("*json")) == 1
        ), "There should be only one json file for doctr format."
        label_path = glob.glob("*.json")[0]
        dataset = DetectionDataset(
            img_folder=directory, label_path=label_path, use_polygon=False
        )
        if allClasses is None:
            allClasses = []
        allBoundingBoxes = BoundingBoxes()
        for nameOfImage, (arrs, classes) in dataset.data:
            for [x, y, x2, y2], idClass in zip(arrs, classes):
                bb = BoundingBox(
                    nameOfImage, idClass, x, y, x2, y2, format=BBFormat.XYX2Y2
                )
                if idClass not in allClasses:
                    allClasses.append(idClass)
                allBoundingBoxes.addBoundingBox(bb)
        ...
        raise IOError("The ground truth files can be either .txt or .json.")


def getBoundingBoxesDoctr():
    dataset = DetectionDataset(
        img_folder=imgs_path, label_path=gt_path, use_polygons=False
    )
    # elif len(glob.glob('*.json')) != 0:
    #     assert len(glob.glob('*.json')) == 1, "There are multiple json files. There should be just one."
    #     fname = glob.glob('*.json')[0]
    #     labels = json.load(open(fname, 'r'))
    #     for imgname in labels:
    #         imgname[]
    # else:
    #     raise FileNotFoundError('No relevant files containing ground truth found in %s ' % directory)
...
allBoundingBoxes, allClasses = getBoundingBoxes(
    gtFolder, True, gtFormat, gtCoordType, imgSize=imgSize, isExpt2=isExpt2
)
# Get detected boxes
allBoundingBoxes, allClasses = getBoundingBoxes(
    detFolder,
