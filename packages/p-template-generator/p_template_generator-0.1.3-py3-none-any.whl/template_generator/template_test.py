...
def test():
    ...
    config = {
        ...
        "template":f"{rootDir}/GreenAlbum_9-16",
        "params":{},
        "output":f"{rootDir}/out.mp4"
        }
    with open(os.path.join(rootDir, "param.config"), 'w') as f:
        json.dump(config, f)
    ...
    command = f'{binary} {os.path.join(rootDir, "param.config")}'
