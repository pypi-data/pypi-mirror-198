...
- `__getattr__(self, key)`: Returns a `MongoDataWrapper` instance for the given `key`. See below for the capabalities of `MongoDataWrapper`
    ...
    Here, we initialize MyClass with `id`, `collection` and `filter_query` as mandatory attributes for MongoGetterSetter to function properly. These 3 arrtibutes are used internally to do further manipulations to MongoDB

    ```
    class MyClass(metaclass=MongoGetterSetter):
        def init(self, _id):
        self.id = _id # this ID is used to query
