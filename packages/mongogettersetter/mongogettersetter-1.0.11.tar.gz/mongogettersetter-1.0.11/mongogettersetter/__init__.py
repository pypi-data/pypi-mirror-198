...
class MongoDictWrapper(dict):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(nested_dict[key], dict):
            dictwrapper = MongoDictWrapper(nested_dict[key])
...
class MongoDataWrapper:    
    ...
    def __int__(self):
        return int(self.collection.find_one(self.filter_query)[self.key])
    
    def __float__(self):
        return float(self.collection.find_one(self.filter_query)[self.key])
    
    def __bool__(self):
        return bool(self.collection.find_one(self.filter_query)[self.key])
    ...
    def __add__(self, other):
        ...
        # if the current value is not a number, raise an error
        if not isinstance(current_value, (int, float)):
            raise TypeError("unsupported operand type(s) for +: '{}' and '{}'".format(type(current_value), type(other)))
        
        # if the other value is not a number, raise an error
