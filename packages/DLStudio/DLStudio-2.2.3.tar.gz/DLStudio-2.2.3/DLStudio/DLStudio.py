...
__date__      = '2023-March-18'                   
__url__       = 'https://engineering.purdue.edu/kak/distDLS/DLStudio-2.2.3.html'
...
CHANGE LOG:
  ...
  Version 2.2.3:
    ...
    variants of the basic IoU (Intersection over Union) loss function.  These
    IoU variants are explained in the slides 37-42 of my Week 7 Lecture on
    "Object Detection and Localization."  Your best entry point to become
    familiar with these loss functions is the script
    object_detection_and_localization_iou.py in the Examples directory of
    DLStudio.

  Version 2.2.2:

    This version of DLStudio presents my implementation of transformers in
    deep learning. You will find two transformer implementations in the
    Transformers co-class of DLStudio in the distribution directory:
    TransformerFG and TransformerPreLN.  "FG" in TransformerFG stands for
    "Transformer First Generation"; it is my implementation of the
    architecture presented originally in the seminal paper "Attention is All
    You Need" by Vaswani et el.  And the second, TransformerPreLN ("PreLN"
    stands for "Pre Layer Norm") is a small but important modification of the
    original idea that is based on the paper "On Layer Normalization in the
    Transformer Architecture" by Xiong et al.  I could have easily combined
    the two implementations with a small number of conditional statements to
    account for the differences, however I have chosen to keep them separate
    in order to make it easier for the two to evolve separately and to be used
    differently for educational purposes.

  Versions 2.1.7 through 2.2.1:

    These version numbers are for the stepping-stones in my journey into the
    world of transformers --- my experiments with how to best implement the
    different components of a transformer for educational purposes.  As things
    stand, these versions contain features that did not make into the public
    release version 2.2.2 on account of inadequate testing.  I may include
    those features in versions of DLStudio after 2.2.2.
...
INTRODUCTION:
    ...
    The idea of DLStudio is to offer an integrated software platform for
    teaching (and learning) a wide range of basic architectural features of
    deep-learning neural networks.  Most instructors who teach deep learning
    ask their students to download the so-called famous networks from, say,
    GitHub and become familiar with them by running them on the datasets used
    by the authors of those networks.  This approach is akin to teaching
    automobile engineering by asking the students to take the high-powered
    cars of the day out for a test drive.  In my opinion, this rather commonly
    used approach does not work for instilling in the students a deep
    understanding of the issues related to network architectures.

    DLStudio offers its own implementations for a variety of key features of
    neural network architectures.  These implementations, along with their
    explanations through detailed slide presentations at our Deep Learning
    class website at Purdue, result in an educational framework that is much
    more efficient in what it can deliver within the time constraints of a
    single semester.

    DLStudio facilitates learning through a combination of inner classes of
    the main module class --- called DLStudio naturally --- and several
    co-classes of the main class that deal with adversarial learning,
    sequence-to-sequence learning, data prediction, text analysis, and
    transformers.

    For the most part, the common code that you'd need in different scenarios
    for using neural networks has been placed inside the definition of the
    main DLStudio class in a file named DLStudio.py in the distribution.  That
    makes more compact the definition of the other inner classes within
    DLStudio. And, to a certain extent, that also results in a bit more
    compact code in the co-classes of DLStudio.

@tag2_skip
    SKIP CONNECTIONS:

    Starting with Version 1.0.6, you can now experiment with skip connections
    in a CNN to see how a deep network with this feature might yield improved
    classification results.  Deep networks suffer from the problem of
    vanishing gradients that degrades their performance.  Vanishing gradients
    means that the gradients of the loss calculated in the early layers of a
    network become increasingly muted as the network becomes deeper.  An
    important mitigation strategy for addressing this problem consists of
    ...
    using the constructor option "depth" for BMEnet. The basic block of the
    ...
    If you want to use DLStudio for learning how to create your own versions
    of SkipBlock-like shortcuts in a CNN, your starting point should be the
    ...
    for experimenting with skip connections in a CNN. As the script shows, the
    constructor of the BMEnet class comes with two options: skip_connections
    and depth.  By turning the first on and off, you can directly illustrate
    in a classroom setting the improvement you can get with skip connections.
    And by giving an appropriate value to the "depth" option, you can show
    results for networks of different depths.

@tag2_detect
    OBJECT DETECTION AND LOCALIZATION:

    The code for how to solve the problem of object detection and localization
    with a CNN is in the inner classes DetectAndLocalize and
    CustomDataLoading.  This code was developed for version 1.0.7 of the
    module.  In general, object detection and localization problems are more
    challenging than pure classification problems because solving the
    localization part requires regression for the coordinates of the bounding
    box that localize the object.  If at all possible, you would want the same
    CNN to provide answers to both the classification and the regression
    questions and do so at the same time.  This calls for a CNN to possess two
    different output layers, one for classification and the other for
    regression.  A deep network that does exactly that is illustrated by the
    LOADnet classes that are defined in the inner class DetectAndLocalize of
    the DLStudio module.  [By the way, the acronym "LOAD" in "LOADnet" stands
    for "LOcalization And Detection".] Although you will find three versions
    of the LOADnet class inside DetectAndLocalize, for now only pay attention
    to the LOADnet2 class since that is the one I have worked with the most
    for creating the 1.0.7 distribution.

    As you would expect, training a CNN for object detection and localization
    requires a dataset that, in addition to the class labels for the images,
    also provides bounding-box annotations for the objects in the images. Out
    of my great admiration for the CIFAR-10 dataset as an educational tool for
    solving classification problems, I have created small-image-format
    training and testing datasets for illustrating the code devoted to object
    detection and localization in this module.  The training dataset is named
    PurdueShapes5-10000-train.gz and it consists of 10,000 images, with each
    image of size 32x32 containing one of five possible shapes --- rectangle,
    triangle, disk, oval, and star. The shape objects in the images are
    randomized with respect to size, orientation, and color.  The testing
    dataset is named PurdueShapes5-1000-test.gz and it contains 1000 images
    generated by the same randomization process as used for the training
    ...
    code for object detection and localization, your starting point should be
    the following script in the Examples directory of the distro:
...
@tag2_noisy
    ...
    noise levels of 20%, 50%, and 80%: At 20% noise, both the labeling and the
    regression accuracies become worse compared to the noiseless case, but
    they would still be usable depending on the application.  For example,
    with two epochs of training, the overall classification accuracy decreases
    from 91% to 83% and the regression error increases from under a pixel (on
    the average) to around 3 pixels.  However, when the level of noise is
    increased to 50%, the regression output is often a NaN (Not a Number), as
    presented by 'numpy.nan' or 'torch.nan'.  To deal with this problem,
    Version 1.1.0 of the DLStudio module checks the output of the bounding-box
    regression before drawing the rectangles on the images.
    ...
    in the Examples directory of the distribution.  Note that you would need
    to download the datasets for such experiments directly from the link
    provided near the top of this documentation page.

@tag2_diou
    OBJECT DETECTION AND LOCALIZATION WITH IoU REGRESSION:

    Starting with version 2.2.3, DLStudio illustrates how you can use modern
    variants of the IoU (Intersection over Union) loss function for the
    regression needed for object localization.  These loss functions are
    provided by the class DIoULoss class that is a part of DLStudio's inner
    class DetectAndLocalize. If you wish to experiment with these loss
    functions, you best entry point would be the script
    ...
    testing datasets as the object_detection_and_localization.py script
    mentioned earlier.

@tag2_semantic
    SEMANTIC SEGMENTATION:

    The code for how to carry out semantic segmentation is in the inner class
    that is appropriately named SemanticSegmentation.  At its simplest, the
    ...
    segmentation should also output a symbolic expression that reflects an
    understanding of the scene in the image that is based on the objects found
    in the image and their spatial relationships with one another.  The code
    in the new inner class is based on only the simplest possible definition
    of what is meant by semantic segmentation.
    
    The convolutional network that carries out semantic segmentation DLStudio
    is named mUnet, where the letter "m" is short for "multi", which, in turn,
    stands for the fact that mUnet is capable of segmenting out multiple
    object simultaneously from an image.  The mUnet network is based on the
    now famous Unet network that was first proposed by Ronneberger, Fischer
    and Brox in the paper "U-Net: Convolutional Networks for Biomedical Image
    Segmentation".  Their UNET extracts binary masks for the cell pixel blobs
    of interest in biomedical images.  The output of UNET can therefore be
    treated as a pixel-wise binary classifier at each pixel position.  The
    mUnet class, on the other hand, is intended for segmenting out multiple
    objects simultaneously form an image. [A weaker reason for "m" in the name
    of the class is that it uses skip connections in multiple ways --- such
    connections are used not only across the two arms of the "U", but also
    also along the arms.  The skip connections in the original Unet are only
    between the two arms of the U.

    mUnet works by assigning a separate channel in the output of the network
    to each different object type.  After the network is trained, for a given
    input image, all you have to do is examine the different channels of the
    output for the presence or the absence of the objects corresponding to the
    channel index.
    ...
    segmenting images should be the following script in the Examples directory
    of the distro:

                semantic_segmentation.py

    Execute the script and understand how it uses the functionality packed in
    the inner class SemanticSegmentation for segmenting out the objects in an
    image.
...
@tag2_text
    ...
    networks with feedback (that are also called Recurrent Neural Networks).
    With an RNN, textual data of arbitrary length can be characterized with a
    hidden state vector of a fixed size.  To facilitate text based
    experiments, this module also comes with text datasets derived from an old
    Amazon archive of product reviews.  Further information regarding the
    datasets is in the comment block associated with the class
    SentimentAnalysisDataset. If you want to use DLStudio for experimenting
    with text, your starting points should be the following three scripts in
    ...
    The first of these is meant to be used with the TEXTnet network that does
    not include any protection against the vanishing gradients problem that a
    poorly designed RNN can suffer from.  The second script mentioned above is
    based on the TEXTnetOrder2 network and it includes rudimentary protection,
    but not enough to suffice for any practical application.  The purpose of
    TEXTnetOrder2 is to serve as an educational stepping stone to a GRU (Gated
    Recurrent Unit) network that is used in the third script listed above.

    Starting with Version 2.0.8, the Examples directory of DLStudio also
    includes the following three scripts that use the same learning networks
    as the corresponding scripts mentioned above but with word representations
    based on word2vec embeddings:
...
@tag2_adversarial
    ...
    approaches for data modeling.  Adversarial Learning consists of
    simultaneously training a Generator and a Discriminator (or, a Generator
    and a Critic) with the goal of getting the Generator to produce from pure
    noise images that look like those in the training dataset.  When
    Generator-Discriminator pairs are used, the Discriminator's job is to
    become an expert at recognizing the training images so it can let us know
    when the generator produces an image that does not look like what is in
    the training dataset.  The output of the Discriminator consists of the
    probability that the input to the discriminator is like one of the
    training images.
    ...
    The AdversarialLearning class includes two kinds of adversarial networks
    for data modeling: DCGAN and WGAN.
    ...
    instance of nn.Linear) at the topmost level.  For the topmost layer in the
    Generator network, DCGAN uses another convolution layer that produces the
    final output image.  And for the topmost layer of the Discriminator, DCGAN
    flattens the output and feeds that into a sigmoid function for producing
    scalar value.  Additionally, DCGAN also gets rid of max-pooling for
    downsampling and instead uses convolutions with strides.  Yet another
    feature of a DCGAN is the use of batch normalization in all layers, except
    in the output layer of the Generator and the input layer of the
    ...
    Critic seems to update its parameters that maximize the Wasserstein
    distance between the true and the fake distributions. Subsequently, as the
    Critic parameters are kept fixed, the Generator updates its learnable
...
@tag2_transformers
    ...
    described earlier in this Introduction except that now you completely
    forgo recurrence. That is, you only use the mechanism of attention to
    translate sentences from a source language into sentences in the target
    ...
    intra-sentence relationships between the words and cross-attention refers
    to the inter-sentence relationships between the words in a pair of
    sentences, one in the source language and the other in the target
    language. I have explained these concepts in great detail in the doc
    sections of the inner classes in the Transformers class.  In particular, I
    have explained the concept of the "dot-product" attention in which each
    word puts out three things: a Query Vector Q, a Key Vector K, and a Value
    Vector V. By taking the dot-product of the Query Vector Q of a word with
    the Key Vector K for all the words in a sentence, the neural network gets
    a measure of the extent to which each word in a sentence is important to
    every other word.  These dot-product values are then used as weights on
    the Value Vectors, V, for the individual words.  Cross attention works in
    a similar manner, except that now you take the dot-products of the Q
    vectors in the target-language sentence with the K vectors in the
    corresponding source-language sentence for producing the weight vectors
    that tell us how to weight the source-language Value Vectors vis-a-vis the
    words in the target language.

    You will see two different implementations of the transformer architecture
    in the Transformers co-class of DLStudio:

          TransformerFG
    and
          TransformerPreLN

    with the "FG" suffix standing for "First Generation" and the "PreLN"
    suffix for "Pre LayerNorm". TransformerFG is my implementation of the
    transformer architecture proposed in the famous paper by Vaswani et al.
    and TransformerPreLN my implementation of the same architecture but with
    the modification suggested by Xiong et al. for more stable learning.
    Since, the modification is small from an architectural standpoint, I could
    have combined both transformer types in the same implementation with some
    conditional logic to account for the differences.  However, I have chosen
    to keep them separate mostly for educational purposes.  Further details on
    these implementations are in the documentation blocks in the Transformers
    co-class.
 
    If you want to use my code for learning the main ideas related to how to
    create purely attention based networks, your starting point for that
    should be the following scripts in the ExamplesTransformers directory of
    the DLStudio distribution:
    ...
    number of words in each sentence limited to 8 words.  For processing by
    the attention networks, each sentence is enclosed in <SOS> and <EOS>
    tokens, with the former standing for "Start of Sentence" and the latter
    for "End of Sentence".
...
THE MAIN INNER CLASSES OF THE MODULE:
    ...
    (9)  class TextClassificationWithEmbeddings
         ...
         are learned on the basis of the contextual similarity of the
...
CO-CLASSES IN THE DLStudio MODULE:
    ...
    As stated at the beginning of the previous section, a Co-Class resides at
    the same level of abstraction in the distribution directory as the main
    DLStudio class. Each Co-Class is defined in a separate subdirectory at the
    top level of the distribution directory.  While the main DLStudio class is
    defined in a subdirectory of the same name, the other subdirectories that
    contain definitions for the co-classes are named AdversarialLearning,
    Seq2SeqLearning, DataPrediction, and Transformers.  What follows in this
...
@tag_coclass1
    ...
    consists of 20,000 images containing randomly shaped, randomly colored,
    and randomly positioned objects in 64x64 arrays.  The dataset comes in
...
@tag_coclass2
    ...
    languages. The seq2seq code demonstrated in this co-class uses the
...
@tag_coclass4
    ...
    TransformerPreLN.  TransformerFG is my implementation of the architecture
    as conceptualized in the famous paper "Attention is All You Need" by
    Vaswani et el.  And TransformerPreLN is my implementation of the original
    idea along with the modifications suggested by Xiong et al. in their paper
    "On Layer Normalization in the Transformer Architecture" for more stable
    learning.  The two versions of transformers differ in only one respect:
    The placement of the LayerNorm in relation to the architectural components
    related to attention and the feedforward network.  Literally, the
    difference is small, yet its consequences on the stability of learning
    significant.

    The fundamentals of how the attention works in both TransformerFG and
    TransformerPreLN are exactly the same.  For self-attention, you associate
    a Query Vector Q_i and a Key Vector K_i with each word w_i in a sentence.
    For a given w_i, the dot product of its Q_i with the K_j vectors for all
    the other words w_j is a measure of how related w_i is to each w_j with
    regard to what's needed for the translation of a source sentence into the
    target sentence.  One more vector you associate with each word w_i is the
    Value Vector V_i.  The value vectors for the words in a sentence are
    weighted by the output of the activation nn.LogSoftmax applied to the
    dot-products.

    The self-attention mechanism described above is half of what goes into
    each base encoder of a transformer, the other half is a feedforward
    network (FFN). The overall encoder consists of a cascade of these base
    encoders.  In my implementation, I have referred to the overall encoder as
    the MasterEncoder.  The MasterDecoder also consists of a cascade of base
    decoders.  A base decoder is similar to a base encoder except for there
    being a layer of cross-attention interposed between the self-attention
    layer and the feedforward network.
    ...
    gradients that would otherwise be caused by the depth of the overall
    network is mitigated by using LayerNorm and residual connections.  In
    TransformerFG, on the encoder side, LayerNorm is applied to the output of
    the self-attention layer and the residual connection wraps around both.
    Along the same lines, LayerNorm is applied to the output of FFN and the
    residual connection wraps around both.  

    In TransformerPreLN, on the other hand, LayerNorm is applied to the input
    to the self-attention layer and residual connection wraps around both.
    Similarly, LayerNorm is applied to the input to FFN and the residual
    connection wraps around both.  Similar considerations applied to the
    decoder side, except we now also have a layer of cross-attention
    interposed between the self-attention and FFN.
   
    As I mentioned in the Introduction, your main entry point for
    experimenting with the transformer code in DLStudio are the following two
    ...
    training dataset used in these examples.  See the "For Transformers" part
    of the "The Datasets Included" section of this doc page for that.
...
Examples DIRECTORY:
    ...
    (2)  playing_with_sequential.py
         ...
         module that is meant to experiment with your own network.  The name
         of the inner class in this example script is
         ExperimentsWithSequential

    (3)  playing_with_cifar10.py

         This is very similar to the previous example script but is based on
         the inner class ExperimentsWithCIFAR which uses more common examples
         of networks for playing with the CIFAR-10 dataset.
    ...
    (6)  custom_data_loading.py
         ...
         meant specifically for the PurdueShapes5 dataset that is used in
         object detection and localization experiments in DLStudio.

    (7)  object_detection_and_localization.py

         This script shows how you can use the functionality provided by the
         inner class DetectAndLocalize of the DLStudio module for
         experimenting with object detection and localization.  Detecting and
         localizing (D&L) objects in images is a more difficult problem than
         just classifying the objects.  D&L requires that your CNN make two
         different types of inferences simultaneously, one for classification
         and the other for localization.  For the localization part, the CNN
         must carry out what is known as regression. What that means is that
         the CNN must output the numerical values for the bounding box that
         encloses the object that was detected.  Generating these two types of
         inferences requires two different loss functions, one for
    ...
    (8)  noisy_object_detection_and_localization.py
         ...
         would be best to create a separate script for studying the effects of
         noise, just to allow for the possibility that the noise-related
    ...
    (9)  object_detection_and_localization_iou.py
         ...
         variants of the IoU (Intersection over Union) loss functions provided
         by the class DIoULoss class that is a part of DLStudio's inner class
...
ExamplesTransformers DIRECTORY:
    ...
    The ExamplesTransformers directory of the distribution contains the
    following two scripts for experimenting with transformers:

        seq2seq_with_transformerFG.py 
        seq2seq_with_transformerPreLN.py         

    Both these scripts deal with English-to-Spanish translation in a manner
    similar to what's demonstrated by the code in the Seq2SeqLearning co-class
    and the example scripts associated with that co-class.
...
@tag2_dataset
    FOR Seq2Seq LEARNING:
        ...
        This data archive is a lightly curated version of the main dataset
        ...
        Download the above archive in the ExamplesSeq2Seq2Learning directory
        and execute the following command in that directory:
        ...
        With that, you should be able to execute the Seq2SeqLearning based
        scripts in the 'ExamplesSeq2SeqLearning' directory.
...
@tag2_dataset
    FOR TRANSFORMERS:
        ...
        These are both derived from the same data source as in the dataset for
        the examples associated with the Seq2SeqLearning co-class.  The first
        has only 10,000 pars of English-Spanish sentences and meant primarily
        for debugging purposes.  The second contains 90000 pairs of such
        sentences.  The number '8' in the dataset names means that no sentence
        contains more than 8 real words.  With the "SOS" and "EOS" tokens
        used as sentence delimiters, the maximum number of words in each
        sentence in either language is 10.
...
ACKNOWLEDGMENTS:
    ...
    Version 2.2.2: My laboratory's (RVL) journey into the world of
    transformers began with a series of lab seminars by Constantine Roros and
    Rahul Deshmukh.  Several subsequent conversations with them were
    instrumental in helping me improve the understanding I had gained from the
    seminars.  Additional conversations with Rahul about the issue of masking
    were important to how I eventually implemented those ideas in my code.
...
#                x2 = x.view(-1, 128 * (32 // 2**self.pool_count)**2 )
        ...
        class DIoULoss(nn.Module):
            ...
            All the formulas presented on those slides require calculating a quantity
            represented by d^2, which is the square of the distance between the center of
            the ground-truth BB and the predicted BB.  In the forward() of the code shown
            below, in Line (1) I start by calculating the straightforward MSELoss that
            calculates according to the formula shown through Eq. (6) on Slide 35. 
            I have referred to this as 'displacement_loss' in Line (1).

            As it turns out, having nn.MSELoss return a single scalar value for the whole
            batch does NOT suit our purpose for calculating IoU-variant losses.  That is
            because these losses need to normalize the value of d^2 on an instance basis
            in every batch.  Therefore, we want nn.MSELoss to return the d^2 calculations
            separately for each instance. This is accomplished in Lines (2) and (3): In 
            Line (2) I invoke nn.MSELoss with the option 'reduction' to 'none'.  That 
            causes MSELoss to return a tensor of shape Bx4, with one row for each instance
            and the four entries in each row corresponding to the four squares shown in
            Eq. (6).  Line (3) calculates the mean values in each row separately and,
            subsequently, we end up with B of the MSE values, one for each instance.
            By the way, if we average these B values, we should get back the value 
            that was returned by MSELoss in line (1). This can be demonstrated by comparing
            d2_loss in Line (4) with the value of displacement_loss in Line (1). This
            verification can be carried by turning on the debug flag for the print
            statements that follow in the code.

            The rest of the code for calculating the set-theoretic operations of union
            and intersections of the ground-truth BB and the predicted BB, and, even
            more importantly, the convex hull of the two BBs.  The three variants of 
            the basic IoU Loss that are defined by Eqs. (8), (9), and (10) on Slides 
            (38) through (41) all require that we find the convex hull C of the two
            BBs. 

            Lines (5), (6) and (7) initialize three tensors (it is more convenient to
            think of them as arrays) for holding the input array, the target array,
            and the convex-hull array. All three are of shape (B,H,W), where B is the
            batch size, and (H,W) the height and the width of the images.  You will 
            notice that these statements do not mention the element data types for the
            tensors. So, my default, the elements types will be set to 32-bit floats.
            This is intentional because I'll be calculating the set intersection by
            multiplying the two argument arrays. Although I'll be using the logical-or
            for the union operation, I can generate the boolean versions of the tensors
            on the fly, as you will see later.

            The next four statements, in Lines (8) through (11) separate out the top-left
            and the bottom-right coordinates for the predicted and the ground-truth
            BBs. This is just for making more compact some of the statements that follow.

            In Lines (13) and (14), I fill out each of the arrays, for the predicted and
            for the target BBs, from their BB coordinates.  Subsequently, Lines (15) 
            through (18) carry out the set operations of intersection and union, the
            former directly by multiplying the two argument arrays and the latter by
            using the logical-or over the boolean versions of the two arrays.

            The code in Lines (19) through (28) is for the purpose of, first, finding
            the convex hull of the two BBs in each instance of the batch and, next, 
            calculating the diagonal of the convex hull.  The call to "torch.where" in
            Line (22) plays a very important role in this logic:  this function returns
            two lists, one for the row-coordinates of all the pixels that satisfy the 
            argument condition supplied to "torch.where" and the other for the column
            coordinates.  Subsequently, it becomes trivial to find the convex hull of
            the two BBs, as shown in Lines (23) through (26).
            """
            ...
            def forward(self, input, target, loss_mode):
                debug = 0
                displacement_loss = nn.MSELoss()(input, target)                                                    ## (1)
                d2_loss_per_instance = nn.MSELoss(reduction='none')(input, target)                                 ## (2)
                d2_mean_loss_per_instance = torch.mean(d2_loss_per_instance, 1)                                    ## (3)
                d2_loss = torch.mean(d2_mean_loss_per_instance,0)                                                  ## (4)
                if debug:
                    ...
                    print("\n\nd2 loss: ", d2_loss)
                ## Need to create image sized arrays for computing IoU and its variants:
                img_size = self.dl_studio.image_size
                epsilon = 1e-6
                input_arr = torch.zeros(input.shape[0], img_size[0], img_size[1]).to(self.dl_studio.device)        ## (5)
                target_arr = torch.zeros(input.shape[0], img_size[0], img_size[1]).to(self.dl_studio.device)       ## (6)
                convex_hull_arr = torch.zeros(input.shape[0], img_size[0], img_size[1]).to(self.dl_studio.device)  ## (7)
                top_left_input = input[:,:2].type(torch.uint8)                                                     ## (8)
                bottom_right_input = input[:,2:].type(torch.uint8)                                                 ## (9)
                top_left_target = target[:,:2].type(torch.uint8)                                                   ## (10)
                bottom_right_target = target[:,2:].type(torch.uint8)                                               ## (11)
                ## I am embarrassed about the loop that follows. When calculating loss, one should not have to
                ## peer inside a batch and use instance-specific statements.  BUT I RAN OUT OF TIME.
                for k in range(input_arr.shape[0]):                                                                ## (12)
                    input_arr[ k, top_left_input[k,0]:bottom_right_input[k,0], top_left_input[k,1]:bottom_right_input[k,1] ] = 1       ## (13)
                    target_arr[ k, top_left_target[k,0]:bottom_right_target[k,0], top_left_target[k,1]:bottom_right_target[k,1] ] = 1  ## (14)
                intersection_arr = input_arr * target_arr                                                          ## (15)
                intersecs = torch.sum( intersection_arr, dim=(1,2) )                                               ## (16)
                union_arr = torch.logical_or( input_arr > 0, target_arr > 0 ).type(torch.uint8)                    ## (17)
                unions = torch.sum( union_arr, dim=(1,2) )                                                         ## (18)
                ## find the convex hull of input and target in each batch instance:            
                convex_hull_bb = torch.zeros( input_arr.shape[0], 4 , dtype=torch.uint8)                           ## (19)
                convex_hull_diagonal_squared = torch.zeros( input_arr.shape[0] ).to(self.dl_studio.device)         ## (20)
                ## I am again embarrassed about the loop that follows. When calculating loss, one should not have to
                ## peer inside a batch and use instance-specific statements. The "torch.where" doc page appears to
                ## say that this function does not support the batch axis:
                for k in range(input_arr.shape[0]):                                                                ## (21)
                    i,j = torch.where( union_arr[k] != 0 )                                                         ## (22)
                    convex_hull_bb[k,0] = torch.min(i)                                                             ## (23)
                    convex_hull_bb[k,1] = torch.min(j)                                                             ## (24)
                    convex_hull_bb[k,2] = torch.max(i)                                                             ## (25)
                    convex_hull_bb[k,3] = torch.max(j)                                                             ## (26)
                    convex_hull_arr[ k, convex_hull_bb[k,0]:convex_hull_bb[k,2], 
                                                            convex_hull_bb[k,1]:convex_hull_bb[k,3] ] = 1          ## (27)
                    convex_hull_diagonal_squared[k] = (convex_hull_bb[k,0].item()-convex_hull_bb[k,2].item())**2 \
                                                   +  (convex_hull_bb[k,1].item()-convex_hull_bb[k,3].item())**2   ## (28)
